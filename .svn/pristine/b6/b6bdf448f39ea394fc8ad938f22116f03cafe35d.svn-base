...
public class HoaDonNewSvl extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String action = request.getParameter("action");		
 		...
 		if(tenform.equals("newform")){
 			nextJSP = request.getContextPath() + "/pages/Center/HoaDonNew.jsp";
 		}else{
 			nextJSP = request.getContextPath() + "/pages/Center/HoaDonUpdate.jsp";
 		}
    ...
    ///	while(tongtienavat.matches(",")){
    	...
    	String tienhuongkm=util.antiSQLInspection(request.getParameter("tienhuongkm"));
    	try{
    		dhBean.setSoTienTraKM(Double.parseDouble(tienhuongkm));
    	}catch(Exception er){
    		dhBean.setSoTienTraKM(0);
    	}
    	String tienhuongtb=util.antiSQLInspection(request.getParameter("tienhuongtrungbay"));
    	try{
    ...
    //	System.out.println(sql_infonpp);
		...
		if(action.equals("save"))
		{
			...
			if(dhBean.EditHoaDon()){
				...
				session.setAttribute("obj", dhBean);
				 nextJSP = request.getContextPath() + "/pages/Center/DonMuaHangTTXuLy.jsp";
					response.sendRedirect(nextJSP);
			}
		}
