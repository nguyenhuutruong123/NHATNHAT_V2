...
public  class DonHangNPP implements IDonhangnpp, Serializable{
	...
	public boolean saveDonHangNPP() {
		...
		sql = "select IDENT_CURRENT('donhang_npp') as dhId";
		...
		while(count < sanphamlist.size())
		{
			...
			sanpham.setKho(this.getKho());
			 sql="insert into donhangnpp_sanpham (sanpham_fk,donhangnpp_fk,soluong,kho_fk,giamua,soluongnhan)" +
			 		" values ("+sanpham.getIdSanPham()+","+sanpham.getDonHangNPP()+","+sanpham.getSoLuong()+","+sanpham.getKho()+","+sanpham.getGiaMua()+","+sanpham.getSoLuongNhan()+" )";
			...
			if(!db.update(sql)){
			  ...
			  sql = "update nhapp_kho set booked = booked + '" + sanpham.getSoLuong() + "', " +
			  		" available = available - '" + sanpham.getSoLuong() + "' " +
			  				" where sanpham_fk='" + sanpham.getIdSanPham() +"' and npp_fk='" + this.getNppId_Ban() + "' and kho_fk='" + this.getKho() + "' and kbh_fk="+this.KenhBanhang;
	...
	public boolean editDonHangNPP() {
		...
		sql =   " select a.soluong,a.sanpham_fk,b.kbh_fk,b.kho_fk,b.npp_fk_ban " +
				" from donhangnpp_sanpham a inner join donhang_npp b on b.pk_seq=a.donhangnpp_fk " +
				" where donhangnpp_fk="+this.getId();
		...
		int soluong=rs_sp.getInt("soluong");
			...
			String query = " update nhapp_kho set booked = booked -'" + soluong + "',  " +
						   " available = available + '" + soluong + "' where sanpham_fk='" + idsp +"'" +
						   " and npp_fk='" +rs_sp.getString("npp_fk_ban") + "' and kho_fk='" +rs_sp.getString("kho_fk")  + "' and kbh_fk="+rs_sp.getString("kbh_fk") ;
		...
		while(count < sanphamlist.size())
		{
			...
			sanpham.setKho(this.getKho());
			sql="insert into donhangnpp_sanpham (sanpham_fk,donhangnpp_fk,soluong,kho_fk,giamua,soluongnhan)values ("+sanpham.getIdSanPham()+","+this._ID+","+sanpham.getSoLuong()+","+sanpham.getKho()+","+sanpham.getGiaMua()+","+sanpham.getSoLuongNhan()+" )";
			if( !db.update(sql)){
				  db.update("rollback");
				  this.setthongbao("Error line 674 :Classname:  sql_insert_chitiet :  SQL command " + sql);
				  return false;
			}
			
			 sql = "update nhapp_kho set booked = booked + '" + sanpham.getSoLuong() + "', available = available - '" + sanpham.getSoLuong() + "' " +
			 		" where sanpham_fk='" + sanpham.getIdSanPham() +"' and npp_fk='" + this.getNppId_Ban() + "' and kho_fk='" + this.getKho() + "' and kbh_fk="+this.KenhBanhang;
			 if(!db.update(sql)){
				 this.setthongbao("Error update KHO : line -672 : DonHangNPP : "+sql);
				 db.update("rollback");
				 return false;
			 }
	...
	public boolean DeleteDonHangNPP() {
	  ...
	  //Cap nhat lai xoa cho ==2
	   db.getConnection().setAutoCommit(false);
		...
		int soluong=rs_sp.getInt("soluong");
			String idsp=rs_sp.getString("sanpham_fk");
			sql = " update nhapp_kho set booked = booked -'" + soluong + "',  " +
						   " available = available + '" + soluong + "' where sanpham_fk='" + idsp +"'" +
						   " and npp_fk='" +rs_sp.getString("npp_fk_ban") + "' and kho_fk='" +rs_sp.getString("kho_fk")  + "' and kbh_fk="+rs_sp.getString("kbh_fk") ;
	...
	public boolean ChotDonHangNPP_Mua() {
		...
		while(count < sanphamlist.size())
		{
			...
			if( !db.update(sql_insert_chitiet)){
				this.thongbao=sql_insert_chitiet;
				db.update("rollback");
				return false;
			}
	...
	public void setListNhaPPMua(String sql) {
        ...
        " FROM         dbo.DONHANG_NPP AS A INNER JOIN  dbo.NHAPHANPHOI AS M ON A.NPP_FK_BAN = M.PK_SEQ INNER JOIN "+
                            "  dbo.NHANVIEN AS T ON T.PK_SEQ = A.NGUOITAO INNER JOIN "+
                             "  dbo.NHANVIEN AS S ON S.PK_SEQ = A.NGUOISUA "+
       " WHERE     (A.NPP_FK_MUA = " + this.getNppId_Mua()+")";
	...
	public void SetDonHangNPP(String sql) {
		String query="";
		if(sql.equals("")){
			query= "SELECT     A.PK_SEQ, M.TEN, T.TEN AS NGUOITAO, S.TEN AS NGUOISUA, A.NGAYNHAP, A.TRANGTHAI, A.NGAYTAO, A.NGAYSUA, A.VAT, A.TONGGIATRI,"+ 
        "A.DATHANHTOAN, A.NPP_FK_MUA, A.NPP_FK_BAN, A.KHO_FK, A.KBH_FK "+
        "FROM         dbo.DONHANG_NPP AS A INNER JOIN "+
                              "dbo.NHAPHANPHOI AS M ON A.NPP_FK_MUA = M.PK_SEQ INNER JOIN "+
                              "dbo.NHANVIEN AS T ON T.PK_SEQ = A.NGUOITAO INNER JOIN " +
