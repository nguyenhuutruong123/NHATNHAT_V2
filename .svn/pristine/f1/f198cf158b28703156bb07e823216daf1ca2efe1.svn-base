...
public class ErpHoadontaichinhNPP implements IErpHoadontaichinhNPP
{
	...
	{
		this.getNppInfo();
		String query = "select PK_SEQ, MAFAST + '-' + TEN as TEN from KHACHHANG where TRANGTHAI = '1' and npp_fk ='"+ this.nppDangnhap +"' " +
				       " and pk_seq in (select KHACHHANG_FK from ERP_DONDATHANGNPP where trangthai in (2,4) )";
		this.khRs = db.get(query);
		
		System.out.println("\n Khach hang : "+query+"\n");
		
		 query = "select PK_SEQ, MAFAST + '-' + TEN as TEN from NHAPHANPHOI where TRANGTHAI = '1' and loaiNPP = '4' and tructhuoc_fk ='"+ this.nppDangnhap +"' " +
	...
	// TỰ TẠO SỐ HÓA ĐƠN CỦA USER
	  ...
	  else // ID > 0
	  {
		// LAY SP TRONG HOADON
			...
			query = "select isnull(a.ghichu,' ') ghichu,b.PK_SEQ as SPID, b.MA, isnull(a.tensp,b.TEN) as ten, a.donvitinh, a.soluong, a.dongia, isnull(a.chietkhau, 0) as chietkhau, a.scheme, a.vat," +
		  	...
		  	String spID = "";
			    ...
			    {				    	
					while(rsLaySP.next())
					{
						...
						ghichusp+=rsLaySP.getString("ghichu") + "__";
					...
					if(spMA.trim().length() > 0)
					{
						...
						ghichusp = ghichusp.substring(0, ghichusp.length() - 2);
						this.spghichu = ghichusp.split("__");
	...
	public void init() 
	{
		...
		String query="select isnull(c.ChietKhau,0) as ptchietkhau,a.kho_fk,dondathangnpp_fk, a.kbh_fk as kbhId, a.npp_fk, ngayxuatHD, ISNULL(a.ghichu, '') as ghichu, "+
				"  a.khachhang_fk, a.npp_dat_fk, a.trangthai, kyhieu, sohoadon, hinhthuctt ,  isnull(a.chietkhau,0) as chietkhau,"+
				"  (case when nguoimua is null and a.khachhang_fk is not null then (select isnull(chucuahieu,'') from KHACHHANG where pk_seq= a.khachhang_fk ) " +   
	...
	public static String update_gia_tri_hoadonnpp(Idbutils db , String id)
	{
		String query = "\n update hd set tongtienavat = isnull(sp.avat,0) -  isnull(km.tienkm,0)  " +
			"\n 	,tongtienbvat =  isnull(sp.bvat,0)   " +
			"\n 	,	vat = isnull(sp.avat,0) -  isnull(sp.bvat,0)   " +
			"\n from ERP_HOADONNPP hd  " +
			"\n outer apply  " +
			"\n (  " +
			"\n 	select sum( round( soluong * DONGIA * ( 1 + VAT/100.0) ,0)) avat ,  sum( round( soluong * DONGIA  ,0)) bvat     " +
			"\n 	from ERP_HOADONNPP_SP where HOADON_FK = hd.PK_SEQ  " +
			"\n )sp  " +
			"\n outer apply  " +
			"\n (  " +
			"\n 	select sum( GIATRI) tienkm   " +
			"\n 	from ERP_HOADONNPP_CHIETKHAU where HOADON_FK = hd.PK_SEQ 	  " +
			"\n )km  " +
			"\n where hd.PK_SEQ  = " + id;
		if(db.updateReturnInt(query)<=0)
		{
			return "Lỗi cập nhật tiền HĐ";
		}
		return "";
	}
	...
	public boolean chot_new()
	{
			...
			if(!loaiNPP.equals("4"))
			{
				...
				if(this.nppDangnhap.equals("100002") || (this.nppDangnhap.equals("106210")&& mau.equals("2"))) // NẾU LÀ CN HÀ NỘI/ HỒ CHÍ MINH && KHÁCH HÀNG DÙNG MẪU HD TRREN TT
				{
					...
					query= " select  count(pk_seq) as kiemtra " +
					       " from HOADON " +
					       " where nguoisua= '"+ this.userId +"' and RTRIM(LTRIM(sohoadon)) = '"+ this.sohoadon.trim() +"' and RTRIM(LTRIM(kyhieu)) ='"+ this.kyhieuhoadon +"' " +
					       "       and trangthai != '3' and pk_seq != "+ this.id +"  and npp_fk = "+this.nppDangnhap+" and mauhoadon = "+mau+" ";
		...
		*/	
			...
			if(msg.length()>0)
			 ...
			 query = "update ERP_HOADONNPP set trangthai = '2',NguoiSua='"+this.userId+"',NgaySua='"+getDateTime()+"'  where pk_seq = '" + this.id + "' and TrangThai!=2 ";
			...
			query = 
					" insert ERP_HOADONNPP_SP_CHITIET(hoadon_fk, donhang_fk, KBH_FK, Kho_FK, MA, TEN, DONVI, DVCHUAN, DVDATHANG, SOLUONG, SOLO, NGAYHETHAN, CHIETKHAU, THUEVAT, DONGIA,SoLuong_Chuan,DonGia_Chuan,SoLuong_DatHang,ngaynhapkho)  " +
	...
	public boolean update() 
	{
		...
		try
		{
			...
			if(!loaiNPP.equals("4"))
			{
				...
				// Check So hoa don sua da co dung chua
				// OTC
				query=  "\n select  count(pk_seq) as kiemtra " +
						"\n from HOADON " +
						"\n where nguoisua= '"+ this.userId +"' and RTRIM(LTRIM(sohoadon)) = '"+ this.sohoadon.trim() +"' and RTRIM(LTRIM(kyhieu)) ='"+ this.kyhieuhoadon +"' " +
						"\n       and trangthai != 3   and npp_fk = "+this.nppDangnhap+" and mauhoadon = "+mau+" ";
				System.out.println("Cau kiem tra so hoa don "+query);
				ResultSet KtraSHD =  db.get(query);
				int ktra= 0;
			...
			if(!trangthai.equals("1") )  //NEU DA DUYET THI CHI DUOC SUA CAC THONG NAY
			{
				...
				for(int i = 0; i < spId.length; i++)
				{
					if(spId[i].trim().length() > 0 && Double.parseDouble(spSoluong[i].trim().replaceAll(",", "")) > 0 )
					{	
						query = " update ERP_HOADONNPP_SP set  tensp = N'" + spTen[i].replaceAll(",", "") + "' "+
								" where hoadon_fk = '"+ this.id +"' and sanpham_fk='"+spId[i]+"'" ;

						System.out.println("1.1.update ERP_HOADONNPP_SP: " + query);
						if(!db.update(query))
						{
							this.msg = "Khong the tao moi update ERP_HOADONNPP_SP: " + query;
							db.getConnection().rollback();
							return false;
						}
					}
			...
			else
			{
				...
				for(int i = 0; i < spId.length; i++)
				{
					if(spId[i].trim().length() > 0 && Double.parseDouble(spSoluong[i].trim().replaceAll(",", "")) > 0 )
					{
						...
						query = "insert ERP_HOADONNPP_SP( soluong_chuan,ghichu,hoadon_fk, sanpham_fk, donvitinh, soluong, dongia, thanhtien, chietkhau, scheme, vat, TIENVAT,tensp ) values " +
								" ("+ spSoluong[i].replaceAll(",", "")+" * dbo.LayQuyCach( "+spId[i]+", null, (select pk_seq from donvidoluong where  donvi=N'"+ spDonvi[i] +"') ) ,N'"+this.spghichu[i]+"','"+ this.id +"', '" + spId[i] + "', N'"+ spDonvi[i] +"', '" + spSoluong[i].replaceAll(",", "") + "', '" + spDongia[i].replaceAll(",", "") + "'," +
				...
				// chua cho sua nen ko can update
			/*	//Tăng kho ngược lại trước khi xóa - trường hợp đổi số lô
				query=  "  select c.NPP_FK, a.Kho_FK, case when d.DUNGCHUNGKENH = 1 then 100025 else a.KBH_FK end as kbh_fk, b.PK_SEQ as sanpham_fk, a.SOLO, a.NGAYHETHAN, a.NGAYNHAPKHO  " + 
						"  from ERP_HOADONNPP_SP_ChiTiet a inner join SANPHAM b on a.MA = b.MA  " + 
						"  		inner join ERP_HOADONNPP c on a.hoadon_fk = c.PK_SEQ " + 
						"  		inner join NHAPHANPHOI d on c.NPP_FK = d.PK_SEQ " + 
				...
				while(rs.next())
				{
					...
					String kbhId =rs.getString("Kho_FK");
					...
					String msg1= util_kho.Update_NPP_Kho_Sp_Chitiet(this.ngayxuatHD, "Sửa hóa đơn ETC - tang kho nguoc lai :ErpHoadontaichinhNPP.java 2584 ", db, 
							khoId, sanpham_fk, nppId, kbhId, SoLo, NgayHetHan, Ngaynhapkho, 0, -1 * SoLuong, SoLuong, SoLuong , 0);
					if(msg1.length() >0){
						msg = msg1;
						db.getConnection().rollback();
						return false;
							...
							while(keys.hasMoreElements())
							{
								...
								if(key.startsWith( spMa[i] + "__" ) )
								{
									...
									query = "insert ERP_DONDATHANGNPP_SANPHAM_CHITIET( dondathang_fk, SANPHAM_FK, dvdl_fk, solo, soluong, ngayhethan, ngaynhapkho )  " +
											"select '" + ddhId+ "', pk_seq, ( select dvdl_fk from ERP_DONDATHANGNPP_SANPHAM where dondathang_fk = '" + ddhId + "' and sanpham_fk = a.pk_seq ),   " +
				...
				rs = db.get(query);				
				{
					while(rs.next())
					{
						...
						String msg1= util_kho.Update_NPP_Kho_Sp_Chitiet(this.ngayxuatHD, "Sửa hóa đơn đối tác :ErpHoadontaichinhNPP.java 2660 ", db, khoID, spID, nppID, kbhID, solo, ngayhethan, ngaynhapkho, 0, soluong, soluong*(-1), soluong*(-1), 0);
						if(msg1.length() >0){
							msg = msg1;
							db.getConnection().rollback();
							return false;
...
*/
				...
				query = "  insert ERP_HOADONNPP_SP_CHITIET(sanpham_fk,ghichu,hoadon_fk, donhang_fk, KBH_FK, Kho_FK, MA, TEN, DONVI, DVCHUAN, DVDATHANG, SOLUONG, SOLO, NGAYHETHAN, NGAYNHAPKHO, CHIETKHAU, THUEVAT, DONGIA,SoLuong_Chuan,DonGia_Chuan,SoLuong_DatHang)   " + 
						"  select b.sanpham_fk,(select top(1) ghichu from erp_hoadonnpp_sp where hoadon_fk='"+this.id+"' and sanpham_fk=b.sanpham_fk),'" + this.id + "' as hoadon_fk, a.pk_seq as donhang_fk, a.KBH_FK, a.KHO_FK, c.MA, (select top(1) tensp from ERP_HOADONNPP_SP where HOADON_FK="+this.id+" and sanpham_fk=b.sanpham_fk) as TEN, (select donvi from DONVIDOLUONG where pk_seq = dhsp.dvdl_fk ) as donvi, d.pk_seq as dvCHUAN, dhsp.dvdl_fk  as dvDATHANG,    " + 
						"  	b.soluong,    " + 
	...
	public boolean chot_update()
	{
		...
		try
		{
			String query = "select npp_fk, NOIBO from  ERP_HOADONNPP  where pk_seq = '" + this.id + "' ";
			ResultSet rs=db.get(query);
			String npp="";
			int noibo = 0;
			if(rs.next())
			...
			if(noibo==1) // NẾU LÀ ĐƠN NỘI BỘ THÌ TỰ ĐỘNG TẠO HÓA ĐƠN NCC DƯỚI CHI NHÁNH
   ...
   public String[] getDhck_diengiai() {
	...
	public void loadContents()
	{
		...
		/*if(this.id.length()> 0){
			  ...
			  try {
				if(rskho.next()){
					  this.khoNhanId=rskho.getString("kho_fk");
				  }
				rskho.close();
			} catch (Exception e) {
				// TODO Auto-generated catch block
	...
	public ResultSet getSoloTheoSp(String spMa, String donvi, String tongluong)
	{
		...
		"then ( select soluong2 / soluong1 from QUYCACH where SANPHAM_FK = sp.PK_SEQ and DVDL1_FK = sp.DVDL_FK and DVDL2_FK  " +
		" = (select PK_SEQ from DONVIDOLUONG where DIENGIAI like N'"+donvi+"') ) * AVAILABLE else AVAILABLE end as AVAILABLE,"+ 
		"NGAYHETHAN, SOLO ,ngaynhapkho  "+
		...
		"	))) )"+ 
		" and ct.ngaynhapkho<='"+this.ngayxuatHD+"'  and NPP_FK = '"+this.nppDangnhap+"' and KBH_FK ='"+kbh_fk+"' "+
	...
	public ResultSet getKhuyenMaiRs()
	{
		...
		{
			String query =  "\n select isnull(b.MA, ' ') as MA, isnull(b.TEN, ' ') as TEN, isnull(c.DONVI, ' ') as donvi, a.ctkm SCHEME, isnull(a.soluong, 0) as soluong, 0 tonggiatri " +
							"\n from erp_hoadonnpp_ctkm_trakm a inner join SANPHAM b on a.sanpham_fk = b.PK_SEQ  " +
							"\n left join DONVIDOLUONG c on b.DVDL_FK = c.PK_SEQ  " +
							"\n where a.hoadon_fk = '" + this.id + "'  " +
							"\n union all " +
							"\n select N'', N'Trả tiền' TEN, '' as donvi, a.DIENGIAI , 1 , a.giatri " +
