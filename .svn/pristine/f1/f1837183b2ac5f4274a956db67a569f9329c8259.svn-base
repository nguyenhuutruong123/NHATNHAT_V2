...
public class Khaosat implements IKhaosat
{
	...
	{	
		try 
		{
			...
			String query = "insert KHAOSAT(doituong,loaict,tieude, diengiai, bophan, socauhoi, trangthai, ngaytao, nguoitao, ngaysua, nguoisua,tungay,denngay) " +
							"values('"+this.doituong+"',"+this.loaict+",N'" + this.tieude + "', N'" + this.diengiai + "', N'" + this.bophan + "', '" + this.socauhoi + "', '0', '" + this.getDateTime() + "', '" + this.userId + "', '" + this.getDateTime() + "', '" + this.userId + "','" + this.tungay + "','" + this.denngay + "')";
	...
	public boolean updateKhaosat() 
	{
		try 
		{
			...
			String query = "Update KHAOSAT set doituong ='"+this.doituong+"' , loaict= "+this.loaict+", tieude = N'" + this.tieude + "', diengiai = N'" + this.diengiai + "', bophan = N'" + this.bophan + "', " +
								"socauhoi = '" + this.socauhoi + "', ngaysua = '" + getDateTime() + "', nguoisua = '" + this.userId + "',tungay = '" + this.tungay + "',denngay = '" + this.denngay + "' " +
	...
	public void createRs() 
	{
		...
		query = "select a.PK_SEQ, a.MANHANVIEN, a.TEN, a.DIACHI, a.DIENTHOAI  " +
				"from DAIDIENKINHDOANH a inner join NHAPHANPHOI b on b.PK_SEQ in (select NPP_FK from DAIDIENKINHDOANH_NPP where DDKD_FK=  a.PK_SEQ)  " +
				"where a.TRANGTHAI = '1'";
		...
		if(this.ddkdId.trim().length() > 0)
			query += " union select PK_SEQ, MANHANVIEN, TEN, DIACHI, DIENTHOAI from DAIDIENKINHDOANH where pk_seq in ( " + this.ddkdId + " ) ";
