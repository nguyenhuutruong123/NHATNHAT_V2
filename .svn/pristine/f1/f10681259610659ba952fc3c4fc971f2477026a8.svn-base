...
function addToCombo(){
	...
	var dvkd= document.getElementsByName("dvkdid");
	for(var k=0;k<dvkd.length;k++){
	
	
	objNewOption.innerText ="Khoa";
	objNewOption.Value ="Ngan";
	}
	}
...
function TinhTien()
{
	...
	function saveform()
	 {	 
		...
		if(khott==""){
			document.forms['dhForm'].dataerror.value="Chọn Chi nhánh / NPP để lấy kho ";
			return;
		...
		//kiem tra neu gia =0 thi khong the thuc hien luu
		 ...
		 for(var k = 0; k < masp.length; k++)
		 {
			if(masp.item(k).value != "")
			{
				...
				return;
					...
					//Thuc Hien Kiem Tra Xem Chuong Trinh Khuyen Mai Nay Co Nam Trong Bo Chuong Trinh Khuyen Mai Da Duoc Chon Phia Tren khong
					var checkone=document.getElementsByName("checkvalue");
					var ctkm=document.getElementsByName("chuongtrinhkm");
					var i=0;
					for( i=0;i<checkone.length;i++ ){
						if(checkone.item(i).checked==true){
							
			               if(ctkm.item(i).value.trim()==ctkm_.item(k).value.trim()){	
			            	   i++;
			            		break;	
			               }
						}
					}					
					if(i==checkone.length){
	            		document.forms['dhForm'].dataerror.value="Chuong trình khuyến mại["+ctkm_.item(k).value+"] bạn chọn cho sản phẩm  : "+tensp.item(k).value +" không hợp lệ, chương trình này không được chọn";
						return;
					}
		 ...
		 //Kiem tra tong tien san pham cua 1 don hang co vuot qua tien duoc huong km khong
		 var checkone=document.getElementsByName("checkvalue");
			var ctkm=document.getElementsByName("chuongtrinhkm");
			...
			for( i=0;i<checkone.length;i++ ){
				if(checkone.item(i).checked==true){
					var tienduockm=document.getElementsByName("ctkmsudung").item(i).value;	
					//voi moi chuong trinh km duoc chon, ta thuc hien tinh tong tien km cua chuong trinh nay
					var tongtienkm=0;
					
					var ctkm_=document.getElementsByName("ctkmid");
					var thanhtien=document.getElementsByName("thanhtien");
					for(var j=0;j<ctkm_.length;j++){
					
							if(ctkm_.item(j).value.trim()==ctkm.item(i).value.trim()){
								 	var sumtienkm=thanhtien.item(j).value;
								   	while(sumtienkm.match(',')){
								    	sumtienkm=sumtienkm.replace(',','');
								   }
								   tongtienkm=parseFloat(tongtienkm)+parseFloat(sumtienkm);
							}	
					}
					
					if(parseFloat(tongtienkm) > parseFloat(tienduockm)){
			      		document.forms['dhForm'].dataerror.value="Đã xuất quá số tiền được hưởng khuyến mãi cho chương trình "+ctkm.item(i).value.trim();
						return;
					}
				}
			}		
	...
	function addRow(name)
		{
			...
			var mactkmAdd=document.createElement("TD");
			...
			var idctkm=document.getElementsByName("chuongtrinhkm");
			var tenctkm=document.getElementsByName("tenctkm");
			var mactkm=document.createElement("select");
			for(var k=0;k<idctkm.length;k++ ){
				var option=document.createElement("option");
				option.value=idctkm.item(k).value;
				option.text=tenctkm.item(k).value;
				mactkm.add(option,null);
			}
			k=0;//set lai k=0 de khi thuc hien lai vong lap khong bi tang k len
			mactkm.setAttribute("style","width:100%;border:1px;	border-style:solid;border-color: #808080;");
			mactkm.name = 'ctkmid';
			mactkmAdd.appendChild(mactkm);
...
<form name="dhForm" method="post" action="../../HoaDonKMNewSvl">
					...
					<TR height="22">
						<TD align="left" >
								...
								<TR>
									<TD align="left">
									   <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
										   <TR height="22">
			 								   <TD align="left"  class="tbnavigation">&nbsp;Quản lý bán hàng > Tạo đơn hàng khuyến mãi > tạo mới </TD>								   
							...
							<TABLE border="1" width="100%" cellpadding = "1" cellspacing = "0" style="border-color:gray;" >
								...
								<TR class="plainlabel">
									<TD align="left" class="khoatest">						
										...
										<TABLE class="khoatest" border="0" bordercolor="white" width="100%" cellpadding = "1" cellspacing = "2" style="padding-left:2px ;" >
											...
											<td > Chọn DVKD </td>
											<td>
											 <select name='dvkdid' style="width: 100%" onchange="submitform();">
												 <option value="" ></option>
													  <% if(rs_dvkd != null){
                                          ...
                                          <td><%=Utility.GLanguage("Kênh bán hàng",session,jedis) %></td>
									          <td>
									          <select name='kenhbanhang' style="width: 100%" onchange="submitform();">
												 <option value=""></option>
											...
											<TR class="plainlabel" >
												...
												<td >
													<select name='nhappid' style="width: 100%" onchange="loadcontent();">
													 <option value=""> </option>
													  <% if(rs_nhapp != null){						  
							   ...
							   <th width="15%">Chọn <input type="checkbox" name="chonhet" onchange="checkall(this.checked);"></th>
							   </tr>
							    <%int k=0; 
							   ...
							   <tr class="plainlabel">
								   ...
								   <%if(ctkm.getId().equals("1")){ %>
								    <input type="checkbox" checked="checked" name="checkvalue"  onchange="recheck();"><input name="valuecheck" type="hidden" value="<%=ctkm.getId()%>" >
									<%}else{ %>
									<input type="checkbox" name="checkvalue"  onchange="recheck();"><input name="valuecheck" type="hidden" value="<%=ctkm.getId()%>" >
									<%} %>								  
								   </td>			
							   	<td>
							   </td>
							   ...
							   <td style="text-align: center">
								   Chi tiết đơn hàng khuyến mãi (Vui lòng chọn Chi nhánh / NPP,đơn vị kinh doanh,kênh bán hàng  trước khi sản phẩm cho đơn đặt hàng)
							   </td>
							   </tr>
							   <TR>
							   		<TD>
