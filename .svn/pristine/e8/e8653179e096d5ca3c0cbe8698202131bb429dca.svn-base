...
//   PrintWriter out = response.getWriter();
		...
		String nextJSP = request.getContextPath() + "/pages/Center/Mucdocungung.jsp";
...
//			this.doGet(request, response);
	...
	private String getSearchQuery(HttpServletRequest request, 	IMucdocungunghang obj){
		...
		String 	query = "select distinct " + 
						...
						"(ddh_sp.soluong + th.soluong) as dadat, qc.soluong1 as quycach, kho.ten as kho, " +
						...
						" inner join erp_khott kho on kho.pk_seq=th.khott_fk  where th.loai='1'" ;
		...
		query = query + "  order by dvkd.donvikinhdoanh, ddh.ngaydat, sp.ma, kho.ten, v.ten, kv.ten, npp.ten";
		return query;
	}			
	...
	private void CreateContent(WritableSheet ws, String query) throws WriteException
	{
		...
		try{	
			...
			while(rs.next())
			{
				...
				float dagiao = Float.valueOf(rs.getString("dagiao")).floatValue()/Float.valueOf(rs.getString("quycach")).floatValue();
