...
<%@page import="geso.dms.center.db.sql.*"%>
...
<ul id="theMenu">
	...
	<%
		...
		{
			db = new dbutils();
		 	ResultSet rs = util.getMenuRs(userId,"0", db);
		 	jedis = ChuyenNgu.getJedis(20*ChuyenNgu.timeout);
			...
			while(rs.next() )
			{
				...
				String ten =  ChuyenNgu.get(rs.getString("TEN").trim(),session,jedis);
				//String ten =  ChuyenNgu.get(rs.getString("TEN"),session);
				...
				else  //UNG DUNG
				{
					/* myMENU += " <li> " +
							  "		<h5 class='head' > " +
							  "			<a href='../../" + servlet + "?userId=" + userId + ""+par+"' target='content'>" + ten + "</a> " +
							  "		</h5> " +
							  "	</li>"; */
