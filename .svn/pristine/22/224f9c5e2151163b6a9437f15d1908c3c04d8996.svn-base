...
public class XLkhuyenmaiDonhangDXK 
{
	...
	public Ctkhuyenmai getSoxuattheoScheme(Ctkhuyenmai ctkm, Hashtable<String,Double> hashA2)
	{
		...
		Hashtable<String, Double> copySp = copyHashtable(hashA2);
		...
		for(int i = 0; i < dkkmList.size(); i++)
		{
			...
			{
				String[] str = checkDKKM_TongTien(dkkm, HashB, hashA2).split("-");
			...
			else   
			{
				...
				{
					String[] str = checkDKKM_TongLuong(dkkm, HashB, hashA2).split("-");
					sl = Integer.parseInt(str[0]);
					ttTheodk = Long.parseLong(str[1]);
				}
				else												//check so luong cu the tung san pham trong dkkm
				{
					String[] str = checkDKKM_SP(dkkm, HashB, hashA2).split("-");
					sl = Integer.parseInt(str[0]);
					ttTheodk = Long.parseLong(str[1]);
				}
			}
			...
			if(sl > 0)
			{
				...
				this.dieuChinhKM((Dieukienkhuyenmai)ctkm.getDkkhuyenmai().get(i), hashA2, HashB, sl);
				...
				if(i > 0 && pt == "&&") //dieu kien and cac sanpham
				{
					...
					{							
						...
						this.dieuChinhKM2((Dieukienkhuyenmai)ctkm.getDkkhuyenmai().get(j), hashA2, soXuatKM);
					}
				}
				else //dieu kien or
				{
	...
	public Hashtable<String, Double> copyHashtable(Hashtable<String, Double> hashA2)
	{
		Hashtable<String, Double> copy = new Hashtable<String, Double>();		
		Enumeration<String> keys = hashA2.keys();
		while(keys.hasMoreElements())
		{
			String key = keys.nextElement();
			copy.put(key, hashA2.get(key));
		}
		return copy;
	}
	
	private String checkDKKM_SP(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Double> hashA2)
	{
		int soxuatKm = 0;
		...
		{			
			...
			while(keyList.hasMoreElements())
			{
				...
				else
				{
					...
					{
						soSpA = hashA2.get(key);
					}
					else
					{
						soSpA = hashA2.get(key) / this.Hash_QuyCach.get(key);
					}
					...
					else
					{
						...
						tongtientheoDK += hashA2.get(key) * sanpham_dongia.get(key);
						if(k < min)
							min = k;
					}
				}
	...
	private String checkDKKM_TongLuong(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Double> hashA2)
	{
		double soxuatKm = 0;
			...
			while(keyList.hasMoreElements())
			{
				...
				if(dkkm.getType() == 2)
				{
					...
					if(hashA2.get(key) != null)
					{
						...
						{
							sum += hashA2.get(key) / this.Hash_QuyCach.get(key);
						}
				...
				else
				{
					...
					if(hashA2.get(key) < minSP)
						minSP = hashA2.get(key);
				}				
			}
			System.out.print("dk khuyen mai " + dkkm.getId() + " ---- Tong thung " + sum + ", Phai dat: " + tongluong + "\n");
			if(sum < tongluong)
				return "0-0";
			soxuatKm = min((sum / tongluong), minSP);
		}
		...
		return Integer.toString((int) soxuatKm) + "-" + Long.toString(tongtiendh);
	}
		
	private String checkDKKM_TongTien(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Double> hashA2)
	{
		Enumeration<String> keys = hashA2.keys();
		while(keys.hasMoreElements())
		{
			String key = keys.nextElement();
			System.out.println("__________San pham: " + key + ", So luong: " + hashA2.get(key));
		}
		...
		if(sanpham.size() > 0)
		{
			...
			while(keyList.hasMoreElements())
			{
				...
				if(dkkm.getType() == 2)
				{
					...
					if(sanpham_dongia.get(key) != null)
						sum += sanpham_dongia.get(key) * hashA2.get(key);
				}
				else
				{
					...
					if(hashA2.get(key) <= 0)
						return "0-0";
					sum += sanpham_dongia.get(key) * hashA2.get(key); //tong so tien cua tat ca sp trong don hang
			...
			System.out.println("Tong tien theo dieu kien khuyen mai trong ham la: " + tongtientheodk + "\n");
	...
	private void dieuChinhKM(Dieukienkhuyenmai dkkm, Hashtable<String, Double> hashA2, Hashtable<String, Float> sp_dongia, double sl)
	{
		...
		if(dkkm.getTongtien() <= 0)
		{
			...
			{		
				Sanpham[] sanpham = getSanpham_slg(dkkm, hashA2, sl);
				...
				if(sanpham != null)
				{
					for(int i=0; i < sanpham.length; i++)
					{
						...
						if(hashA2.containsKey(key))
						{
							Double slgconlai = (double) 0;
							Double soluongsudung = (double) 0;
							
							if(dkkm.getIsThung().equals("0"))
							{
								slgconlai = ( hashA2.get(key) -  sanpham[i].getSoluongThungCan() );
							...
							{
								slgconlai =  (hashA2.get(key) - ( sanpham[i].getSoluongThungCan() * ( this.Hash_QuyCach.get(key)) ));
							...
							hashA2.put(key, slgconlai); //dieu chinh lai soluong tuong ung
							sanphamList.add(sp);
						}
					}
				}
			...
			else
			{
				...
				while(keyList.hasMoreElements())
				{
					...
					if(hashA2.containsKey(key))
					{
						...
						if(dkkm.getIsThung().equals("0"))
						{
							slgconlai = hashA2.get(key) - sanpham_sl.get(key) * sl;
							soluongsudung = sanpham_sl.get(key) * sl;
						}
						else
						{
							slgconlai = (int) (hashA2.get(key) - sanpham_sl.get(key) * this.Hash_QuyCach.get(key) * sl);
		...
		{
			Sanpham[] sanpham = getSanpham_soluong(dkkm, hashA2, sl);
			if(sanpham != null)
			{
				for(int i=0; i < sanpham.length; i++)
				{
					...
					if(hashA2.containsKey(key))
					{
						...
						if(dkkm.getIsThung().equals("0"))
						{
							...
							slgconlai = hashA2.get(key) - sanpham[i].getSoluongcan();
							soluongsudung = sanpham[i].getSoluongcan();
						}
						else
						{
							slgconlai = (int) (hashA2.get(key) - ( sanpham[i].getSoluongThungCan() * ( this.Hash_QuyCach.get(key)) ));
	...
	private void dieuChinhKM2(Dieukienkhuyenmai dkkm,  Hashtable<String, Double> hashA2, double soXuatKM)
	{
		...
		for(int i = 0; i < sanpham.size(); i++ )
		{
			...
			if(dkkm.getType() == 2 ) //mua san pham co slg bat ky trong nhom san pham...
			{
				if(dkkm.getTongluong() > 0)
				{
					...
					{						
						...
						hashA2.put(sp.getMasp(), hashA2.get(sp.getMasp()) + slgCan - sum);
						...
						for(int j = i + 1; j < sanpham.size(); j++)
						{
							...
							hashA2.put(sanpham.get(j).getMasp(), hashA2.get(sanpham.get(j).getMasp()) + slgCanOld);
			...
			else //mua sp co soluong bat buoc
			{
				...
				//dieu chinh lai hashSanpham
				hashA2.put(sp.getMasp(), hashA2.get(sp.getMasp()) + (old - soXuatKM) * slgCan);
			}
	...
	private Sanpham[] InitSanPham(Dieukienkhuyenmai dkkm, Hashtable<String, Double> hashA2, boolean flag)
	{
		Hashtable<String, Integer> sp_sl = dkkm.getSanpham_Soluong();
		...
		while(keyList.hasMoreElements())
		{
			...
			if(hashA2.get(key) != null)
				slAvaiable = hashA2.get(key);
			
			Double slThungAvai = (double) 0;
			if(hashA2.get(key) != null)
			{
				if(dkkm.getIsThung().equals("0"))
					slThungAvai = hashA2.get(key);
				else
				{
					slThungAvai = hashA2.get(key) / this.Hash_QuyCach.get(key);
				}
			}
	...
	private Sanpham[] getSanpham_soluong(Dieukienkhuyenmai dkkm, Hashtable<String, Double> hashA2, double sl) //truong hon tong tien > 0
	{
		Sanpham[] sanpham = InitSanPham(dkkm, hashA2, false); //Sort sanpham tang dan theo tongtien
		if(sanpham == null)
			return null;
		...
		for(int i = 0; i < sanpham.length; i++)
		{
			double sum = getTongtien(sanpham);
			sum = sum - (sanpham[i].getDongia() * sanpham[i].getSoluongcan());
			
			double soluongcan = min(sanpham[i].getSoluongAvaiable(), (double)((tongtienKM - sum) / sanpham[i].getDongia()));
	...
	public Sanpham[] getSanpham_slg(Dieukienkhuyenmai dkkm, Hashtable<String, Double> hashA2, double sl) //truong hop tongluong > 0
	{
		Sanpham[] sanpham = InitSanPham(dkkm, hashA2, true); //Sort sanpham tang dan theo soluong

		if(sanpham == null)
			return null;
		
		double tongluongKM = dkkm.getTongluong() * sl; //tong luong theo tat ca cac xuat khuyenmai duoc huong
		...
		for(int i = 0; i < sanpham.length; i++)
		{
			...
			double soluongcan = min(sanpham[i].getSoluongThungAvaiable(), tongluongKM - sum);
	...
	public void setHashA(Hashtable<String, Double> sanpham_soluong)
	{
		this.HashA = sanpham_soluong;
