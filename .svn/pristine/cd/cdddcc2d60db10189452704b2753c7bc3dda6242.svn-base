...
public class ErpPhieuChiNPPList  extends Phan_Trang  implements IErpPhieuChiNPPList
{
	
	String loaiMenu = "1"; // fix cứng thu tiền khac
	String view = "";
	...
	public void init(String search)
	{
		...
		{			
			query =  			
			 ...
			 "\n  d.ten as nguoitao, e.ten as nguoisua,  isnull(a.SOTIENTHU,0) AS THUCTHU, a.CONLAI AS SOTIENCONLAI, isnull(nguoinhan, '') as tennguoinop," +
			 "\n         '' as khachhangs  "+ 
			 "\n  from Erp_PhieuChi a "  +
			 "\n  inner join NHANVIEN d on a.nguoitao = d.pk_seq " + 
			 "\n  inner join NHANVIEN e on a.nguoisua = e.pk_seq " +
			 "\n  where a.trangthai != 2 and a.NPP_FK= "+ (this.nppId.trim().length() > 0 ? this.nppId :"1")  +"  and a.noidungtt_fk = 100002 ";		
		}
		else
			query = search + " and  a.NPP_FK= "+ (this.nppId.trim().length() > 0 ? this.nppId :"1")  +"  " ;
		query +=" and a.loaiMenu =  " + this.loaiMenu;
		...
		if(this.view.equals("TT"))
			query ="select pk_seq, isnull(maFAST,'') + '-' + ten as TEN from Nhaphanphoi where trangthai = '1'" ;
