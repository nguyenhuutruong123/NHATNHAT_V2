...
public class DangkykhuyenmaitichluyTT implements IDangkykhuyenmaitichluyTT, Serializable
{
		...
		if( this.Id.length() > 0 )
		{
			...
			if(rs != null)
			{
				try
				{
					...
					//
					String query = "select '' as tentinhthanh, '' as tendaidien, a.PK_SEQ, a.mafast , a.TEN, a.DIACHI, b.MUCDANGKY_FK, \n" +
	...
	public boolean Upload(String values)
	{
		String query = "";
		if(!this.edit())
			return false;
		try
		{
			...
			String error = "";
			query  = " select mafast from ("+values+") data where  mafast not in (select mafast from  khachhang where mafast is not null) ";
			rs = db.get(query);
			while(rs.next())
			{
				if(error.trim().length() > 0)
					error += ", "  + rs.getString("mafast");
				else
					error += rs.getString("mafast");
		...
		//	db.viewQuery(query, this.dataUpload);
			...
			query  = " select mafast from ("+values+") data" +
					 " group by mafast " +
					 " having count(*) > 1 ";
			rs = db.get(query);
			while(rs.next())
			...
			query  = " select mafast, muc from ("+values+") data where data.muc < 0 and data.muc >" +muc;
			rs = db.get(query);
			while(rs.next())
			{
				if(error.trim().length() > 0)
					error += ", "  + rs.getString("mafast") + "  , mức "+( rs.getInt("muc") + 1  )+"  ";
				else
					error += rs.getString("mafast") + "  , mức "+( rs.getInt("muc") + 1  )+"  ";
			...
			query  = " insert DANGKYKM_TICHLUY_khachhang(nguoitao,DKKMTICHLUY_FK,khachhang_fk,soxuat,dangky,dahuy,mucdangky_fk,thuongtl_fk) " +
					 ...
					 " inner join khachhang kh on kh.mafast =  a.mafast ";
		...
		{
			er.printStackTrace();
			this.Msg="Không thể thực hiện insert, vui lòng thử lại";
	...
	public boolean save() 
	{
		try
		{
			...
			String query= 	" insert into DANGKYKM_TICHLUY (ngaydangky,diengiai,thuongtl_fk,tieuchiTL_fk, trangthai, ngaytao, nguoitao, ngaysua, nguoisua) " +
							" select '"+this.ngaydangky+"',N'"+this.diengiai+"',"+this.ctkmId+","+this.ctkmId+", '0','"+this.getDateTime()+"',"+this.userId+",'"+this.getDateTime()+"',"+this.userId+" " +
							" where not exists ( select 1 from DANGKYKM_TICHLUY where tieuchiTL_fk = "+this.ctkmId+" and trangthai = 0  ) ";
			if(db.updateReturnInt(query)!= 1){					
				this.Msg="Lỗi đăng ký";
				db.getConnection().rollback();
				db.getConnection().setAutoCommit(true);
				return false;				    
			}
	...
	public boolean edit() 
	{
		try
		{
			...
			String query= " update DANGKYKM_TICHLUY set ngaydangky = '"+this.ngaydangky+"', diengiai= N'"+this.diengiai+"', ngaysua = '"+this.getDateTime()+"', nguoisua =   " + this.userId +
	...
	public boolean Chot() 
	{
		...
		try{
			...
			if(db.updateReturnInt(insert_)!=1){
				    this.Msg="Không thể thực hiện update, vui lòng thử lại";
				    Utility.rollback_and_shutdown(db);
				    return false;
			}
	...
	public boolean UnChot() 
	{
		...
		try{
			db.getConnection().setAutoCommit(false);
			String query = "select count(*) _c from  DUYETTRAKHUYENMAI x where CTKM_FK = (SELECT THUONGTL_FK FROM DANGKYKM_TICHLUY WHERE PK_SEQ = "+ this.Id+"		) ";
    		ResultSet rs = db.get(query);
    		rs.next();
    		if(rs.getInt("_c")> 0 )
    		{
    			   this.Msg="Đã phát sinh duyệt trả không được mở chốt";
				    Utility.rollback_and_shutdown(db);
				    return false;
    		}
			String insert_="update DANGKYKM_TICHLUY set trangthai=0, ngaysua = '"+this.getDateTime()+"', nguoisua="+this.userId+
	...
	public boolean Delete() {
		...
		try{
			...
			String insert_="update DANGKYKM_TICHLUY set trangthai=2 ,ngaysua='"+this.getDateTime()+"',nguoisua="+this.userId+
					" where trangthai=0 and PK_SEQ = "+ this.Id;		
			if(db.updateReturnInt(insert_)!=1){
				    this.Msg="Không thể thực hiện hủy, vui lòng thử lại";
				    Utility.rollback_and_shutdown(db);
				    return false;
			}
	...
	public void createRs() 
	{
		...
		if(this.ctkmId.trim().length() > 0)
		{
			...
			try 
			{
				...
				query = "select PK_SEQ, mafast, TEN, isnull(MANHANVIEN, '') as maNV from DAIDIENKINHDOANH where NPP_FK in " + utl.quyen_npp(this.userId) + " and TRANGTHAI = '1'";
				this.nvbhRS = db.get(query);
				
				query = "select * from (" +
						"select PK_SEQ, mafast, TEN, DIACHI, isnull(d.STT, 999999999) AS STT, " +
						" STUFF " +
						" (  " +
						"	(	select ' , ' + p.ten " +
						"		from NHAPHANPHOI p " +
				...
				query += ") as d order by STT, mafast asc ";
				System.out.println(query);
	...
	public void getId_Khachhang(String maKh, Hashtable<String, Integer> kh_muc, Hashtable<String, Integer> kh_st) {
		try{
			...
			while(keyList.hasMoreElements())
			{
				...
				queryInsert += "INSERT INTO #KHACHHANG(STT, mafast, MUCDK) VALUES("+kh_st.get(key)+", '"+key+"', '"+kh_muc.get(key)+"')\n";
				i++;
			}
			...
			String command = // test
				 ...
				 " (   " + 
				 "  	STT INT, mafast varchar(500), MUCDK INT " +
				 " )  " + queryInsert;
			
			System.out.println(command);
			this.db.update(command);
			
			String query= "select t.mafast from KHACHHANG k right join #KHACHHANG t on t.mafast = k.mafast where k.PK_SEQ IS NULL";
			ResultSet rs = this.db.get(query);
			String _msg = "";
			while(rs.next()){
				_msg += rs.getString("mafast") + "\n";
			}
			if(_msg.length() > 0){
			...
			query = "select '' as tentinhthanh,''as tendaidien, k.PK_SEQ, k.mafast mafast, k.TEN,k.DIACHI, " +
					" p.TEN AS nppTen, p.PK_SEQ as nppPK_SEQ, t.MUCDK " +
					"from KHACHHANG k LEFT join NHAPHANPHOI p on k.NPP_FK = p.PK_sEQ " +
					"inner join #KHACHHANG t on t.mafast = k.mafast " +
					"order by t.STT";
			...
			while(this.khRs.next()){
				String mafast = this.khRs.getString("mafast");
	...
	public String setQueryKhachHangDangKy ()
	{
		...
		String query =	 	 "\n select kh.mafast [MÃ KH(Dữ liệu upload)], npp.ten [NPP] , kh.ten [TÊN] " + 
