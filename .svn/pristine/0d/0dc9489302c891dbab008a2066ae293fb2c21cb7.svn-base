...
public class Router implements IDRouter{
	...
	public void init() 
	{
      ...
      this.npp = db.get(sql);
        ...
        sql ="select * from daidienkinhdoanh where pk_seq in (select ddkd_Fk from DaiDienKinhDoanh_Npp where npp_fk='"+nppId+"') and ISPG=0 ";
        else
        	sql ="select * from daidienkinhdoanh where 1=1 AND  ISPG=0 ";
      	if(this.phanloai.equals("2"))
  			{
