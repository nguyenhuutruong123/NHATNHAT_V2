...
public class NhomNhaPhanPhoiUpdateSvl extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nextJSP = request.getContextPath() + "/pages/Center/NhomNhaPhanPhoiUpdate.jsp";
		if(querystring.indexOf("display") > 0)
	    {
	    	nextJSP = request.getContextPath() + "/pages/Center/NhomNhaPhanPhoiDisplay.jsp";
	    }
			...
			if (action.equals("save"))
			{
				...
				if (id == null)
				{
					if (!nnppBean.save())
					{
						session.setAttribute("nnppBean", nnppBean);
						nnppBean.setUserId(userId);
						nextJSP = request.getContextPath() + "/pages/Center/NhomNhaPhanPhoiNew.jsp";
						response.sendRedirect(nextJSP);
					} else
					{
						...
						nextJSP = request.getContextPath() + "/pages/Center/NhomNhaPhanPhoi.jsp";
						response.sendRedirect(nextJSP);
					}
				} else
				{
					if (!nnppBean.edit())
					{
						session.setAttribute("nnppBean", nnppBean);
						nnppBean.setUserId(userId);
						nextJSP = request.getContextPath() + "/pages/Center/NhomNhaPhanPhoiUpdate.jsp";
						response.sendRedirect(nextJSP);
					} else
					{
						obj.setUserId(userId);
