...
public class BangKeHoaDonSpList    extends Phan_Trang implements IBangKeHoaDonSpList, Serializable
{
	...
	public void createRs()
	{
		...
		query="select pk_seq,isnull(mafast,'') +' ' + ten + ' ' + isnull(diachi,'') as Ten from nhaphanphoi where loainpp=4 and trangthai=1 ";
	...
	public void init(String action)
	{
		...
		if (action.length()>0)
		{
			...
			query+=
					"        "+   
							"   select   sp.ma as spMa,sp.ten as spTen,ROW_NUMBER() OVER(PARTITION BY hdETC.npp_fk ORDER BY hdETC.npp_fk,hdETC.HOADON_FK DESC) AS n_Row, 'ETC' as LoaiHD, v.TEN as vten,kv.TEN as kvTen,tt.TEN as ttTEN,qh.TEN as qhTen  , '1'as nppMa, 'Tổng công ty TraphacoDMS' as nppTen,'' as ddkdTen,         "+   
							...
							"   (              "+   
							"			select   "+
							"					ETC.sanpham_fk,	ETC.npp_fk,ETC.HOADON_FK , sum(ETC.SoLuong*ETC.DONGIA) as BVAT,  "+                 
							...
							"					)  ETC   "+
							"					where soluong>0  "+
							"					group by ETC.npp_fk,ETC.HOADON_FK ,ETC.sanpham_fk  "+
							"   )as hdETC              "+     
