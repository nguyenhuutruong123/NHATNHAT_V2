...
public class Duyettratrungbay implements IDuyettratrungbay, Serializable{
	...
	public boolean Chot( HttpServletRequest request)
	{
		try 
		{
			...
			for(int i = 0; i < this.khIds.length ; i++)
			{
				...
				{
					this.msg = "Số xuất duyệt không thể lớn hơn số xuất đăng ký";
					db.getConnection().rollback();
					return false;
				}
			}
			...
			"where trangthai=0 and cttrungbay_fk = '" + this.schemeId + "' and lanthanhtoan = '" + this.lantt + "'" ;
			System.out.println("Cap nhat DE NGHI: " + sql);

			if(this.db.updateReturnInt(sql) != 1)
			{
				this.msg="Không thể cập nhật duyệt trả trưng bày này";
	...
	public boolean Aptrungbay() 
	{
		...
		try 
		{
			...
			String update = "UPDATE DENGHITRATRUNGBAY SET NGAYSUA = '"+this.getDateTime()+"', NGUOISUA = "+this.userId+" WHERE trangthai=0 and PK_SEQ = " + this.id;
			...
			if(!db.update(query))
			{
				this.msg = "Không thể tạo mới đề nghị trả trưng bày khách hàng";
				db.getConnection().rollback();
				return false;
			}
			...
			int dong = 0;
			query =  "\n select a.diengiai,a.PK_SEQ,a.LOAI, isnull(a.TONGTIEN,0)TONGTIEN,isnull(a.TONGLUONG,0)TONGLUONG , b.pheptoan , cb.ngaytds,cb.ngaykttds " + 
