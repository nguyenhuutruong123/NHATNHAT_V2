...
<%	ResultSet rs_nhacc=(ResultSet)session.getAttribute("rs_nhacc");
	...
	ResultSet rs_gsbh=hdBean.getReS_GSBH();
	...
	function saveform()
	 {	 		
		...
		if(parseFloat(tongtien)<100000){
			document.forms['dhForm'].dataerror.value="Tong Thanh Toan Cua Don Hang Khong Duoc Duoi Muc 100 000 VND";
			return;
		}
	...
	function addRow(name)
		{
			...
			var soluongdatAdd = document.createElement("TD");
			...
			idsp.setAttribute("hidden", "textbox");
			...
			var soluongdat = document.createElement("input");
			soluongdat.setAttribute("type", "textbox");
			soluongdat.setAttribute("readonly", "readonly");
			soluongdat.value = "";
			//soluongdat.style.width = '100%';	
			soluongdat.setAttribute("style","width:100%;border:1px;	border-style:solid;border-color: #808080;");

			soluongdat.name = "soluongdat";
			soluongdatAdd.appendChild(soluongdat);
		...
		function KiemTraGioiHangCongNo(){
			var conno=document.forms['dhForm'].nocu.value;
			var gioihancongno=document.forms['dhForm'].gioihancongno.value;
			var phaitra=document.forms['dhForm'].tienphaitra.value;
			if((parseFloat(conno)+ parseFloat(phaitra) > parseFloat(gioihancongno))){
				document.forms['dhForm'].dataerror.value	="Chi nhánh / NPP NÀY ĐÃ VƯỢI MỨC GIỚ HẠN CÔNG NỢ ";
			}else{
				document.forms['dhForm'].dataerror.value	="";
			}
		}
</script>
...
<form name="dhForm" method="post" action="../../HoaDonNewSvl">
...
<input type ="hidden"name="nocu" value="<%=Math.round(hdBean.getTienNo()) %>" >
<input type ="hidden"name="gioihancongno" value="<%=Math.round(hdBean.getGioiHanCongNo())%>" >
						...
						<TD align="left" >
								...
								<TR>
									<TD align="left">
									   <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
										   <TR height="22">
			 								   <TD align="left"  class="tbnavigation">&nbsp;Quản lý bán hàng > Duyệt đơn hàng> Chốt </TD>								   
							...
							<TABLE width="100%" border="0" cellpadding="1" cellspacing="0">
								<TR ><TD >
									<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
										<TR class = "tbdarkrow">
											<TD width="30" align="left"><A href = "../../DonmuahangSvl?userId=<%=userId %>" ><img src="../images/Back30.png" alt="Quay ve"  title="Quay ve" border="1" longdesc="Quay ve" style="border-style:outset"></A></TD>
										    <TD width="2" align="left" >&nbsp;</TD>
										    <TD width="30" align="left" ><A href="javascript:saveform()" ><img src="../images/Chot.png" alt="Luu lai"  title="Luu lai" border="1" longdesc="Luu lai" style="border-style:outset"></A></TD>
										    <TD width="2" align="left" >&nbsp;</TD>
								    		...
								    		<TD align="left" >
								  			 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <label  id="warnning" style="color: red ;font-size: 14 " ></label>							  				 
								    		</TD>
							...
							<TABLE border="1" width="100%" cellpadding = "1" cellspacing = "0" style="border-color:gray;" >
								<tr>
								 ...
								 <TD align="left" class="legendtitle">
									...
									<legend> <%=Utility.GLanguage("Báo lỗi nhập liệu",session,jedis) %></legend>
				    				<textarea name="dataerror"  style="width: 100% ; color:#F00 ; font-weight:bold"  style="width:100%;margin:0px ;color: red"  rows="1"  ><%=hdBean.getMessage() %></textarea>
							  	</fieldset>
							  	 </TD>
								</tr>
								...
								<TR class="plainlabel">
									<TD align="left" class="khoatest">						
										...
										<TABLE class="khoatest" border="0" bordercolor="white" width="100%" cellpadding = "3" cellspacing = "0" style="padding-left:2px ;" >
											<tr class="plainlabel">
											<th width='15%' ></th>
											<th width='35%' ></th>
											<th width='15%' ></th>
											...
											<td>
											<input type="text" readonly="readonly" name="sodonhangid" style="width:100%" value="<%= hdBean.getId() %>" >
											</td>
											...
											<td >Nhà cung cấp  </td>
											<td >
												<select name='nhaccid' readonly style="width: 100%">
												 <option value="" ></option>
													  <% if(rs_nhacc != null){
											...
											<td > Chọn DVKD </td>
											<td>
											 <select name='dvkdid' readonly style="width: 100%">
												 <option value="" ></option>
													  <% if(rs_dvkd != null){
											...
											<TR class="plainlabel">
											  ...
											  <table border=0 cellpadding="0" cellspacing="0">
                                                <TR>
                                                <TD>
											    <input type="text" readonly="readonly" name="ngaygiaodich" style="width:100%" value="<%= hdBean.getNgaygiaodich() %>" >
											    </TD>
											     <TD>
											...
											<TR class="plainlabel" >
												...
												<td >
													<select name='nhappid' readonly style="width: 100%" onchange="loadcontent();">
												 <option value=""> </option>
													  <% if(rs_nhapp != null){						  
												...
												<td>
													<select name='gsbhid' style="width: 100%" >
												 
													  <% if(rs_gsbh != null){						  
														  try{ while(rs_gsbh.next()){ 
											    			if(rs_gsbh.getString("gsbh_fk").trim().equals(hdBean.getGiamSatBanHang())){ %>
											      				<option value='<%=rs_gsbh.getString("gsbh_fk")%>' selected><%=rs_gsbh.getString("ten") %></option>
											      			<%}else{ %>
											     				<option value='<%=rs_gsbh.getString("gsbh_fk")%>'><%=rs_gsbh.getString("ten") %></option>
											     			<%}}}catch(java.sql.SQLException e){}} %>	
												</select> 					
																						</td>
											</TR>
											...
											<td>
												...
												<td>
												<input type='text'    name='ChietKhau' style="width:100%" value="<%=Math.round(hdBean.getChietkhau()) %> ">
												</td>
											</tr>
											<tr class="plainlabel">
											<td>Điện thoại</td>
											<td><input type='text' readonly="readonly"  name="dienthoai" style="width:100%" value="<%=infonpp.getSodienthoai()%>" ></td>
											 <TD  class="plainlabel">VAT (%) </TD>
											  <TD  class="plainlabel"><input name="VAT" id="VAT" type="text" value="<%= hdBean.getVAT() %>" onkeypress="return keypress(event);">%</TD>
											  ...
											  <TD  class="plainlabel">Tổng số tiền(Chưa VAT) </TD>
											  	...
											  	VND </TD>
											  		<td class="plainlabel">Tiền hưởng trưng bày </td>
												<td class="plainlabel">
												<input type='text' style="width:35%" name="tienhuongtrungbay"  id="tienhuongtrungbay" value="<%= Math.round(hdBean.getSoTienTraTB())%>">
													Còn lại <input align="right" style="width :35%" readonly="readonly" name="tienhuongtrungbayconlai" type="text" value="<%=Math.round(hdBean.getTienHuongTrungBay()) %>" >
												</td>
											  ...
											  <TR class="plainlabel">
										        ...
										        <TD   class="plainlabel"><input name="SoTienCoVAT" id="SoTienCoVAT" type="text" readonly="readonly" 
										          ...
										          <td>
										          <input name="tienhuongkm" id='tienhuongkm' style="width:35%" type="text"  value="<%=Math.round(hdBean.getSoTienTraKM()) %>">
										          Còn lại <input name="tienhuongkmconlai" align="right" readonly="readonly" style="width :35%" type="text" value="<%=Math.round(hdBean.getTienHuongKhuyenMai()) %>" >
										          </td>
									       </TR>
											...
											<TR class="plainlabel">
											 ...
											 <TD  class="plainlabel">Tổng tiền chiết khấu </TD>
											  ...
											  <TD   class="plainlabel">
											  	<input name="tienphaitra" id='tienphaitra' type="text" readonly value="" onchange="KiemTraGioiHangCongNo();">
									          VND </TD>
										  ...
										  <td colspan="3"> 
										  <textarea rows="1" style="font-size:12pt ;width:100%"  readonly="readonly" id="doctien">  </textarea>
										  </td>
										  </tr>
							...
							while (m < size){
								...
								%>
									...
									<TD align="left" >
										...
										<input name="masp"  type="text"  autocomplete='off' onkeyup="ajax_showOptions(this,'abc',event)" value="<%=sanpham.getMaSanPham()%>">
									</TD>
									<TD align="left" >
										<input name="tensp" type="text" readonly value="<%=sanpham.getTenSanPham()%>" ></TD>
									<TD align = "left" >
								        <input name="soluongdat" type="text" readonly="readonly" value="<%= sanpham.getSoLuongDat() %>"  style="text-align:left;">
								        </TD>								
						        	<%
						        	if(spThieuList!=null){
						        	if (spThieuList.containsKey(sanpham.getMaSanPham())){ %>
									    <TD align = "left"  class="addspeech">
								        <input name="soluong" type="text" value="<%= sanpham.getSoLuong() %>"  onkeypress="return keypress(event);" style="cursor:pointer; background-color:99CCCC;text-align:left ;" title="San pham nay con toi da <%= spThieuList.get(sanpham.getMaSanPham()) %> san pham, vui long chon lai so luong">
								        </TD>
								    <%}else{ %>
							        	<TD align = "left" >
								        <input name="soluong" type="text" value="<%= sanpham.getSoLuong() %>" onkeypress="return keypress(event);" style="text-align:left;">
								        </TD>
								    <%}}else{ %>
								    <TD align = "left" >
								        <input name="soluong" type="text" value="<%= sanpham.getSoLuong() %>" onkeypress="return keypress(event);" style="text-align:left;">
								    ...
								    <%} %>
								    <TD align = "center" ><input name="donvitinh" type="text" value="<%= sanpham.getDonViTinh() %>" readonly ></TD>
								    <TD align = "center" >
								    	<input type="text" name="dongia" value="<%= formatter.format(sanpham.getDonGia()) %>" readonly style="text-align:right">
								    </TD>
								    <TD align = "center" ><input name="chietkhau" type="text"  readonly  style="text-align:right"></TD>
								    <TD align = "center" ><input name="thanhtien" type="text"  readonly  style="text-align:right"></TD>
								</TR>
								<% m++; }}%>		
								<%
									int soSp=0;
								...
								<TR class= 'tblightrow'>
									...
									<TD align="left" >
										<input name="tensp" type="text" readonly value="" ></TD>
										<TD align = "left" >
								        <input name="soluongdat" type="text" readonly="readonly" value=""  style="text-align:left;">
								        </TD>		
								    <TD align = "left" >
							        	<input name="soluong" type="text" value="" style="width:100%; text-align:left" onkeypress="return keypress(event);">
							        </TD>
								    <TD align = "center" ><input name="donvitinh" type="text" value="" readonly ></TD>
								    <TD align = "center" ><input name="dongia" type="text" value="" readonly style=" text-align:right"></TD>
