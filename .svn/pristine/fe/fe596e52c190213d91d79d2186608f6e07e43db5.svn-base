...
public class DontrahangTTUpdateSvl extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		} else
		{
			...
			if (!querystring.contains("display"))
				nextJSP = request.getContextPath() + "/pages/Center/DonTraHangUpdate.jsp";
			else
				nextJSP = request.getContextPath() + "/pages/Center/DonTraHangDisplay.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		} else
		{
			...
			String ischot = util.antiSQLInspection( request.getParameter("ischot")==null? "0":request.getParameter("ischot") );
			...
			if (action.equals("save"))
			{
				if (id == null)
				{
					...
					} else
					{
						...
						response.sendRedirect(request.getContextPath() +"/pages/Center/DonTraHang.jsp");
					}
				} else
				{
					...
					if (!lsxBean.updateNK(ischot))
					{
						...
						lsxBean.laykm();
						session.setAttribute("lsxBean", lsxBean);
						String nextJSP = request.getContextPath() +"/pages/Center/DonTraHangUpdate.jsp";
						response.sendRedirect(nextJSP);
					} else
					{
						IDontrahangList obj = new DontrahangList();
			...
			} else
			{
				...
				session.setAttribute("lsxBean", lsxBean);
				String nextJSP = "";
				nextJSP = request.getContextPath() +"/pages/Center/DonTraHangNew.jsp";
				if (id != null)
					nextJSP = request.getContextPath() +"/pages/Center/DonTraHangUpdate.jsp";
