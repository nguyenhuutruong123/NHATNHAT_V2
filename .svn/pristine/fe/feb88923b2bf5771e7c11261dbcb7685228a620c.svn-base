...
public class Erp_chinhanh  extends Phan_Trang implements IErp_chinhanh
{
	...
	public Erp_chinhanh(String id)
	{
		db = new dbutils();
		this.Msg = "";
		String query = " SELECT * FROM erp_chinhanh WHERE pk_seq = '" + id + "' ";
...
public void init(String sql) 
{
	...
	public boolean ThemMoiMa(String congtyId) 
	{
		if (!CheckUnique())
		{
			this.Msg = "Mã chi nhánh '" + this.MA + "' đã tồn tại, vui lòng nhập mã chi nhánh khác!";
			return false;
		}
	...
	public boolean CheckUnique()
	{
		String query = "";
		if (this.ID.length() > 0)
			query = "Select count(*) as count From ERP_CHINHANH Where MA=N'" + this.MA + "' AND PK_SEQ !='" + this.ID + " '";
		else
