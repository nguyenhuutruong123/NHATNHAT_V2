package geso.dms.distributor.beans.baocao.imp;

import java.sql.ResultSet;
import java.sql.SQLException;

import geso.dms.center.db.sql.dbutils;
import geso.dms.distributor.beans.baocao.IBaocao;
...
public class Baocao implements IBaocao 
{
	...
	public void createRs() 
	{
		...
		query = "select pk_seq,   isnull(ten, '') as khoTen from kho where trangthai = '1' ";
		this.khoRs = db.get(query);
		query="select pk_seq,diengiai from kenhbanhang";
		this.rskbh=db.get(query);
	}
