...
<%
	IErpHoadontaichinhNPP lsxBean = (IErpHoadontaichinhNPP) session
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="hctmhForm" method="post"
		action="../../ErpHoadontaichinhNPPUpdateSvl">
				<% redis.clients.jedis.Jedis jedis =  geso.dms.center.util.Utility.getJedis(); %><% geso.dms.center.util.Csrf csdr=new geso.dms.center.util.Csrf(request,response,true,false,true);%>
 <input type="hidden" name="<%=csdr.get_tokenName() %>" value='<%=csdr.get_tokenValue() %>'>
		
		<input type="hidden" name="userId" value='<%=userId%>'> <input
			type="hidden" name="nppDNId" id="nppDNId"
			value='<%=lsxBean.getnppDangnhap()%>'> <input type="hidden"
			name="action" value='1'> <input type="hidden" name="id"
			value='<%=lsxBean.getId()%>'>

		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left"
					class="tbnavigation">Quản lý bán hàng > Bán hàng ETC/Đối tác
					> Xuất HĐTC > Hiển thị</div>
			...
			<div align="left" id="button"
				...
				class="tbdarkrow">
				<%-- <A href="../../ErpHoadontaichinhNPPSvl?userId=<%=userId%>"> <img
        	...
        	<img src="../images/Back30.png" alt="Quay ve"  title="Quay ve" border="1px" longdesc="Quay ve" style="border-style:outset" onclick="goBack()">
				...
				<span id="btnSave"> <A href="javascript:saveform()"> <IMG
						src="../images/Save30.png" title="Chỉnh sửa thông tin YC"
						alt="Luu lai" border="1px" style="border-style: outset">
				</A> </span>
				...
				<fieldset>
					...
					<div style="float: none; width: 100%" align="left">
						<TABLE width="100%" cellpadding="4" cellspacing="0">
							<TR>
								<TD width="130px" class="plainlabel" valign="top">Ngày xuất
									hóa đơn</TD>
								<TD class="plainlabel" valign="top"><input type="text"
									class="days" readonly="readonly" name="ngayxuat"
									value="<%=lsxBean.getNgayxuatHD()%>" /></TD>

								<TD width="150px" class="plainlabel" valign="top">Ngày ghi
									nhận công nợ</TD>
								<TD class="plainlabel" valign="top"><input type="text"
									class="days" readonly="readonly" name="ngayghinhan"
									value="<%=lsxBean.getNgayghinhanCN()%>" /></TD>
							</TR>
							...
							<TR>
								...
								<TD class="plainlabel" valign="top"><input type="text"
									name="kyhieuhoadon" value="<%=lsxBean.getKyhieuhoadon()%>" />
								</TD>

								<TD class="plainlabel">Số hóa đơn</TD>
								<TD class="plainlabel" valign="top"><input type="text"
									name="sohoadon" value="<%=lsxBean.getSohoadon().trim()%>" />
								</TD>
							</TR>

							<TR>
								<TD class="plainlabel">Loại xuất HD</TD>
								<TD class="plainlabel"><select name="loaiXHD"
									class="select2" style="width: 200px" onchange="submitform();">
										<%
											if (lsxBean.getLoaiXHD().equals("0")) {
										%>
										<option value=""></option>
										<option value="1">Xuất cho khách hàng ETC</option>
										<option value="0" selected="selected">Xuất cho đối
											tác</option>
										<%
											} else if (lsxBean.getLoaiXHD().equals("1")) {
										%>
										<option value=""></option>
										<option value="1" selected="selected">Xuất cho khách
											hàng ETC</option>
								...
								<TD class="plainlabel"><select name="nppId"
									class="select2" style="width: 200px" onchange="submitform();">
										...
										<option value="<%=nppRs.getString("pk_seq")%>"
											selected="selected"><%=nppRs.getString("ten")%>
										</option>
										<%
											} else {
										%>
						...
						</select> --%> <input type="text" name="hinhthuctt"
									id="hinhthuctt" value="<%=lsxBean.getHinhthucTT()%>" /></TD>
								<TD class="plainlabel"><%=Utility.GLanguage("Kênh bán hàng",session,jedis) %></TD>
								<TD class="plainlabel"><select name="khId"
									class="select2" style="width: 200px" disabled="disabled">
										<option value=""></option>
										<%
											if (kbhRs != null) {
													try {
										...
										<option value="<%=kbhRs.getString("pk_seq")%>"
											selected="selected"><%=kbhRs.getString("ten")%></option>
										<%
											} else {
										%>
							...
							<TR>
								...
								<TD class="plainlabel">
									...
									%> <input
									type="checkbox" name="innguoimuahang" value="1"
									checked="checked" /> <%
 	} else {
 %> <input type="checkbox"
									name="innguoimuahang" value="0" /> <%
 	}
 %>
								</TD>
								<%
								...
								<TD class="plainlabel"><input type="hidden"
									name="innguoimuahang" value="1" /></TD>
								<%
									}
								%>
							...
							<TR>
								...
								<TD class="plainlabel" colspan="3"><input type="text"
									name="ghichu" value="<%=lsxBean.getGhichu()%>"
									style="width: 500px" /></TD>
							</TR>
							<TR>
								<TD class="plainlabel">Mã vụ việc</TD>
								<TD class="plainlabel" colspan="3"><input type="text"
									name="mavv" value="<%=lsxBean.getMavuviec()%>"
									style="width: 500px" /></TD>
							</TR>
							<TR>
										...
										<div style="max-height: 100px; overflow: auto;">
											<table width="100%" cellpadding="4" cellspacing="0">
												...
												<%
													if (ddhRs != null) {
															while (ddhRs.next()) {
																if (lsxBean.getDondathangId().contains(
																		ddhRs.getString("pk_seq").trim())) {
												%>
												<tr class='tblightrow'>
													<td><%=ddhRs.getString("pk_seq")%></td>
													<td><%=ddhRs.getString("NgayDonHang")%></td>
													<%
														if (lsxBean.getDondathangId().contains(
																				ddhRs.getString("pk_seq").trim())) {
													%>
													<TD align="center"><input type="radio"
														checked="checked" name="ddhid"
														value="<%=ddhRs.getString("pk_seq")%>"
								...
								<TD class="plainlabel" colspan="2" valign="bottom">
									<table class="plainlabel" width="100%"
										style="padding-top: 0; margin-top: 0">
										<tr>
											<td>Tổng tiền</td>
											<td><input type="text" style="text-align: right"
												id="tongtien" name="tongtien"
												value="<%=lsxBean.getTongtienBVAT()%>"></td>
										</tr>
										<tr>
											<td>Tiền chiết khấu</td>
											<td><input type="text" style="text-align: right"
												id="tongchietkhau" name="tongchietkhau"
												value="<%=lsxBean.getTongCK()%>"></td>
										</tr>
										<tr>
											<td>Phần trăm chiết khấu</td>
											<td><input type="text" style="text-align: right"
												id="ptchietkhau" name="tongchietkhau"
												value="<%=lsxBean.getPtchietkhau()%>"></td>
										</tr>

										<tr>
											<td>Tiền vat</td>
											<td><input type="text" style="text-align: right"
												id="tienvat" name="tienvat"
												value="<%=lsxBean.getTongVAT()%>"></td>
										</tr>

										<tr>
											<td>Tiền thực thu</td>
											<td><input type="text" style="text-align: right"
												id="tiensauvat" name="tiensauvat"
												value="<%=lsxBean.getTongtienAVAT()%>"></td>
										</tr>
									</table></TD>
							</TR>
					...
					<TD colspan="4">
						...
						<table cellpadding="0px" cellspacing="1px" width="100%">
							...
							<tr>
								<td><input type="hidden" name="spId" value="<%=spId[i]%>"
									style="width: 100%"> <%-- <input type="hidden" name="spLoai" value="<%= spLoai[i] %>" style="width: 100%"   >  --%>
									<input type="text" name="spMa" value="<%=spMa[i]%>"
								...
								.parseDouble(spSoluong[i]))%>"
									style="width: 100%; text-align: right;" readonly="readonly">
								</td>
								<td><input type="text" name="spDongia"
									value="<%=formatter2.format(Double
								.parseDouble(spDongia[i]))%>"
									style="width: 100%; text-align: right;"
									onkeyup="SuaDonGia(<%=i%>);"></td>
								<td><input type="text" name="spVat"
									value="<%=formatter.format(Double.parseDouble(spVat[i]))%>"
									style="width: 100%; text-align: right;" readonly="readonly">
								</td>
								<td>
									<%
									...
									%> <input
									type="text" name="spChietkhau"
									value="<%=formatter2.format(Double
									.parseDouble(spChietkhau[i]))%>"
									style="width: 100%; text-align: right;" readonly="readonly">
									<%
										} else {
									%> <input type="text" name="spChietkhau"
									value="<%=formatter.format(Double
									.parseDouble(spChietkhau[i]))%>"
									style="width: 100%; text-align: right;" readonly="readonly">
									<%
								...
								<td>
									<%
										if (lsxBean.getnppDangnhap().equals("106179")) {
									%> <input
									type="text" name="spTienthue"
									value="<%=formatter2.format(Double
									.parseDouble(spTienThue[i]))%>"
									style="width: 100%; text-align: right;" readonly="readonly">
									<%
										} else {
									%> <input type="text" name="spTienthue"
									value="<%=formatter.format(Double
									.parseDouble(spTienThue[i]))%>"
									style="width: 100%; text-align: right;" readonly="readonly">
									<%
								...
								<td>
									<%
										if (lsxBean.getnppDangnhap().equals("106179")) {
									%> <input
									type="text" name="thanhtien"
									value="<%=formatter.format(Double
									.parseDouble(spSoluong[i])
									* Double.parseDouble(spDongia[i]))%>"
									style="width: 100%; text-align: right;"
									onkeypress="return keypress(event); " readonly="readonly">
									<%
										} else {
									%> <input type="text" name="thanhtien"
									value="<%=formatter2.format(Double
									.parseDouble(spSoluong[i])
									* Double.parseDouble(spDongia[i]))%>"
									style="width: 100%; text-align: right;"
							...
							if(kmRs != null)
								...
								%>
									<tr>
										...
										<td>
											<input type="text"  value="<%= formatter.format( kmRs.getDouble("TONGGIATRI")) %>" style="width: 100%; color: red;text-align: right" readonly  > 
										</td>
										<td>
