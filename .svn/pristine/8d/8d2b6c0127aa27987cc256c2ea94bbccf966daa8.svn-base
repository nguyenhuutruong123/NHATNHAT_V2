...
<%
	...
	int[] quyen = new  int[6];
	quyen = util.Getquyen("BCAnhTrungBay", "", userId);
	System.out.println("Loi :" + obj.getMsg());
	String url = util.getChuyenNguUrl("BCAnhTrungBay", "&view=TT",session);
%>
...
<!-- session.getAttribute("errors") -->
			...
			<div align="left"
				...
				<fieldset>
					<legend class="legendtitle"><%=Utility.GLanguage("Báo cáo ảnh trưng bày",session,jedis) %></legend>
					<div style="width: 100%; float: none" align="left" class="plainlabel">
							<TABLE width="80%" cellpadding="6" cellspacing="0">
								...
								<%} %>
								<TR>
									<TD class="plainlabel"><%=Utility.GLanguage("Trình dược viên",session,jedis) %></TD>
									<TD class="plainlabel">
								...
								<TR>
									...
									<td colspan="3">
									<a class="button" href="javascript:xemtrenweb();"> 
									...
									<TD width="98%">
										<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
											<TR class="tbheader">
												...
												<TH width="7%" align="center"><%=Utility.GLanguage("CTTB",session,jedis) %></TH>
												<TH width="6%" align="center"><%=Utility.GLanguage("MÃ CN/ĐT",session,jedis) %></TH>
												<TH width="14%" align="center"><%=Utility.GLanguage("CN/ĐT",session,jedis) %></TH>
												<TH width="5%" align="center"><%=Utility.GLanguage("Mã TDV",session,jedis) %></TH>
												<TH width="8%" align="center"><%=Utility.GLanguage("TDV",session,jedis) %></TH>
												<TH width="5%" align="center"><%=Utility.GLanguage("TUYẾN",session,jedis) %></TH>
												<TH width="5%" align="center"><%=Utility.GLanguage("MÃ KH",session,jedis) %></TH>
												<TH width="9%" align="center"><%=Utility.GLanguage("KHÁCH HÀNG",session,jedis) %></TH>
												<TH width="11%" align="center"><%=Utility.GLanguage("ĐỊA CHỈ",session,jedis) %></TH>
												<TH width="5%" align="center"><%=Utility.GLanguage("GHI CHÚ",session,jedis) %></TH>
												<TH width="6%" align="center"><%=Utility.GLanguage("NGÀY CHỤP",session,jedis) %></TH>
												<TH width="11%" align="center"><%=Utility.GLanguage("HÌNH",session,jedis) %></TH>
		                               ...
		                               while (dataRs.next()){
		                                   ...
		                                   <%} else {%>
		                                   	...
		                                   	<%}%>
												...
												<TD align="center"><%= dataRs.getString("diachigiaohang")%></TD>
												<TD align="center"><%= dataRs.getString("ghichu")%></TD>
														...
														<div id="<%=popup_id%>" style='padding:0px 5px; background:#fff;'>
															<table  cellpadding="4px" cellspacing="2px" width="100%" align="center" >
																...
																<TR>
																	...
																	<td >
																			<%= dataRs.getString("khMa") %> - <%= dataRs.getString("NgayChup")%>																																						
																	</td>
												...
												<%-- <TD>
												<%if(obj.getLoaiMenu().equals("TT")  && quyen[Utility.XOA]!=0){ %>
													<A href = "../../BCAnhTrungBay?userId=<%=userId%>&view=<%=obj.getLoaiMenu()%>&delete=<%=dataRs.getString("ANHCHUP") %>"><img src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Ban Muon Xoa Anh Trung Bay Nay?')) return false;"></A>
												<%} %>	
												</TD> --%>
