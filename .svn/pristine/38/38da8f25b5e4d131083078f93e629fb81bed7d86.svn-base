...
public class DontrahangTTSvl extends HttpServlet {
	...
	private String UnChot(String lsxId, String userId)
	{
		...
		try
		{
			...
			String query = "update DonTraHang set  Modified_Date=getdate() where pk_seq = '" + lsxId + "' and trangthai=2  ";
			if(db.updateReturnInt(query)!=1)
			{
				msg = "Trạng thái đơn không hợp lệ vui lòng thử lại !!!";
			...
			query = "update DonTraHang set trangthai = '0',NgaySua='"+getDateTime()+"',Modified_Date=getdate() where pk_seq = '" + lsxId + "' and trangthai=2  ";
			if(db.updateReturnInt(query)!=1)
			{
	...
	private String Chot(String id, String userId)
	{
		...
		try
		{
			String msg = "";
			db.getConnection().setAutoCommit(false);
			String query = "update DonTraHang set trangthai = '2',Modified_Date=getdate() where pk_seq = '" + id + "' and trangthai in (1)  ";
			if(db.updateReturnInt(query)!=1 )
			{
				msg = "Phiếu trả hàng này đã chốt rồi !"+query;
				db.getConnection().rollback();
	...
	private String DeleteChuyenKho(String lsxId, String userId)
	{
		...
		try
		{
			...
			String query = "update DonTraHang set Modified_Date=getdate() where pk_seq = '" + lsxId + "' and trangthai in(0,1) ";
			if(db.updateReturnInt(query)!=1)
			{
				msg = "Trạng thái Đơn không hợp lệ, vui lòng xử lý lại !!!";
				db.getConnection().rollback();
				return msg;
			}			
			...
			String kbhId = "";
			query =  "select isnull(dungchungkenh, 0) as dungchungkenh from NHAPHANPHOI where pk_seq = (select x.npp_fk from dontrahang x where x.pk_seq="+lsxId+") ";
			...
			Utility util = new Utility();
			query= " 	select a.ngaytra,b.ngaynhapkho,a.kho_fk as kho_fk, a.npp_fk, "+(dungchungkenh.equals("1") ? kbhId : "a.kbh_fk")+" as kbh_fk, b.sanpham_fk, b.solo, SUM(b.soluong) as tongxuat,b.NgayHetHan  " +
					" 	from DonTraHang a inner join DONTRAHANG_SP b on a.pk_seq = b.dontrahang_fk " +
					" 	where dontrahang_fk = '" + lsxId + "' " +
					...
					while (ckRs.next())
					{
						...
						String _msg=util.Update_NPP_Kho_Sp_Chitiet(ngaytra, "Đơn trả hàng về NCC-XOA " + lsxId, db, kho_fk, sanpham_fk, npp_fk, kbh_fk, solo, NgayHetHan, ngaynhapkho, 0.0, (-1)*tongxuat, tongxuat, tongxuat, 0.0);
						if(_msg.length()>0)
						{
							db.getConnection().rollback();
							return _msg;
						}
						_msg=util.Update_NPP_Kho_Sp(ngaytra, "Đơn trả hàng về NCC-XOA " + lsxId, db, kho_fk, sanpham_fk, npp_fk, kbh_fk, 0.0,  (-1)*tongxuat, tongxuat, 0.0);
						if(_msg.length()>0)
						{
							db.getConnection().rollback();
							return _msg;
