...
//			Utility util = new Utility();
			...
			String tungaytk = request.getParameter("tungay");
			if(tungaytk == null)
				tungaytk = "";	    	   	    
			lsxBeanList.setTungay(tungaytk);

			String denngaytk = request.getParameter("denngay");
			if(denngaytk == null)
				denngaytk = "";	    	   	    
			lsxBeanList.setDenngay(denngaytk);

			String vungtk = request.getParameter("vungId");
			if(vungtk == null)
				vungtk = "";	    	   	    
			lsxBeanList.setVungId(vungtk);

			String khuvuctk = request.getParameter("khuvucId");
			if(khuvuctk == null)
				khuvuctk = "";	    	   	    
			lsxBeanList.setKhuvucId(khuvuctk);

			String npptk = request.getParameter("nppId");
			if(npptk == null)
				npptk = "";	    	   	    
			lsxBeanList.setNppTen(npptk);

			String kbhtk = request.getParameter("kenhId");
			if(kbhtk == null)
				kbhtk = "";	    	   	    
			lsxBeanList.setKbhId(kbhtk);		    		   

			String trangthaitk = request.getParameter("trangthai");		    
			if(trangthaitk == null)
				trangthaitk = "";	    	   	    
...
//			Utility util = new Utility();
			...
			}else
			{
				...
				ngayyeucau = util.antiSQLInspection(request.getParameter("ngaychuyen"));
				if(ngayyeucau == null || ngayyeucau.trim().length() <= 0)
					ngayyeucau = getDateTime();
				lsxBean.setNgayyeucau(ngayyeucau);
				session.setAttribute("ngaydonhang", lsxBean.getNgayyeucau());
				...
				String txtCongNo = util.antiSQLInspection(request.getParameter("txtCongNo"));
				if (txtCongNo == null)
					txtCongNo = "";				
				lsxBean.setCongNo(txtCongNo.replaceAll(",", ""));
				...
				tiengiam = util.antiSQLInspection(request.getParameter("txtTiengiam"));
				if (tiengiam == null)
					tiengiam = "";				
				lsxBean.setTiengiam(tiengiam.replaceAll(",", ""));
				...
				pokhuyenmai = util.antiSQLInspection(request.getParameter("pokhuyenmai"));
				if (pokhuyenmai == null)
					pokhuyenmai = "0";				
				lsxBean.setPOKhuyenMai(pokhuyenmai);
				...
				String vungtk = request.getParameter("vungtk");
				if(vungtk == null)
					vungtk = "";	    	   	    
				lsxBeanList.setVungId(vungtk);

				String khuvuctk = request.getParameter("khuvuctk");
				if(khuvuctk == null)
					khuvuctk = "";	    	   	    
				lsxBeanList.setKhuvucId(khuvuctk);

				String npptk = request.getParameter("npptk");
				if(npptk == null)
					npptk = "";	    	   	    
				lsxBeanList.setNppTen(npptk);

				String kbhtk = request.getParameter("kbhtk");
				if(kbhtk == null)
					kbhtk = "";	    	   	    
				lsxBeanList.setKbhId(kbhtk);		    		   

				String trangthaitk = request.getParameter("trangthaitk");		    
				if(trangthaitk == null)
					trangthaitk = "";	    	   	    
				lsxBeanList.setTrangthai(trangthaitk);
						...
						if(trakm.getHinhthuc() == 2)
						{
							...
							else
							{
								...
								if(ctkm.getPhanbotheoDH().equals("1"))
								{
									...
									if(msg.trim().length() > 0 && !msg.contains(":")) {	slg = msg; }
									else
									{
										db.getConnection().rollback();
										return msg;
