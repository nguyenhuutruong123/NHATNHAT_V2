...
public class MucChietKhauTTSvl extends HttpServlet {
	...
	private String getSearchQuery(HttpServletRequest request) 
	{
		...
		Utility util = new Utility();
		HttpSession session = request.getSession();		
		String TenNhaPP = util.antiSQLInspection(request.getParameter("TenNhaPP"));
		session.setAttribute("TenNhaPP",TenNhaPP);
    	if ( TenNhaPP == null){
    		TenNhaPP = "";
		}
    	String chietkhau = util.antiSQLInspection(request.getParameter("mucchietkhau"));
    	if ( chietkhau == null)
    		chietkhau = "0";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    if (action.equals("new"))
	    {
    		...
    		String nextJSP = request.getContextPath() + "/pages/Center/MucChietKhauTTNew.jsp";
    		response.sendRedirect(nextJSP);
	    ...
	    if (action.equals("search"))
	    {
    		...
    		response.sendRedirect(request.getContextPath() + "/pages/Center/MucChietKhauTT.jsp");	    		    	
	    }
	}
