	...
	public boolean createNK() 
	{
		...
		try
		{
			...
			String query = " insert ERP_CHUYENKHONPP(SoChungTu,ngaychuyen, ghichu, trangthai, khoxuat_fk, kbh_fk, npp_fk, npp_dat_fk, ngaytao, nguoitao, ngaysua, nguoisua," +
						   ...
						   " values('"+this.sochungtu+"','" + this.ngayyeucau + "', N'" + this.ghichu + "', 99, '" +  this.khoXuatId + "', '" + this.kbhId + "', " + this.nppId + ", '" + this.khId + "', '" + getDateTime() + "', '" + this.userId + "', '" + getDateTime() + "', '" + this.userId + "'," +
			...
			query = "select scope_identity() as id_curent";
			ResultSet	rsId = db.get(query);
			rsId.next();
				String id_curent = rsId.getString("id_curent");
				rsId.close();
			...
			for(int i = 0; i < spMa.length; i++)
			{
				if(spMa[i].trim().length() > 0 && spSoluong[i].trim().length() > 0 )
				{
					...
					query = "insert ERP_CHUYENKHONPP_SANPHAM( chuyenkho_fk, SANPHAM_FK, soluongchuyen, dongia, dvdl_fk ) " +
							"select '"+id_curent+"', pk_seq, '" + spSoluong[i].replaceAll(",", "") + "', '" + spGianhap[i].replaceAll(",", "") + "', ISNULL( ( select pk_Seq from DONVIDOLUONG where donvi = N'" + spDonvi[i].trim() + "' ), DVDL_FK ) " +
					...
					if(this.sanpham_soluong != null)
					{
						...
						while(keys.hasMoreElements())
						{
							...
							if(key.startsWith( spMa[i] + "__" ) )
							{
								...
								System.out.println("key ne " +(_sp[3]));
								String _soluongCT = "0"; 
								...
								if(!_soluongCT.equals("0"))
								{
									...
									query = "select AVAILABLE, NGAYHETHAN,NGAYNHAPKHO from NHAPP_KHO_CHITIET " +
											"where KHO_FK = '" + this.khoXuatId + "' and NPP_FK = '" + this.nppId + "' and KBH_FK = '" + this.kbhId + "' and SOLO = '" + _sp[1] + "' and NGAYHETHAN = '" + _sp[2] + "' and NGAYNHAPKHO ='"+_sp[3]+ "' " +
									...
									query = "insert ERP_CHUYENKHONPP_SANPHAM_CHITIET( chuyenkho_fk, SANPHAM_FK, solo, ngayhethan, soluong ,ngaynhapkho) " +
											"select '"+id_curent+"', pk_seq, N'" + _sp[1] + "', '" + ngayhethan + "', '" + _soluongCT.replaceAll(",", "") + "', '" + _sp[3] + "' " +
									...
									System.out.println("NGAY NHAP KHO LA " +_sp[3]);
									this.msg=util.Update_NPP_Kho_Sp_Chitiet(this.ngayyeucau, "Chuyển Kho NPP", db, this.khoXuatId, spId, this.nppId, this.kbhId, _sp[1], ngayhethan, _sp[3], 0.0, Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), 0.0);
...
//									
									...
									this.msg=util.Update_NPP_Kho_Sp(this.ngayyeucau, "Chuyển Kho NPP", db, this.khoXuatId, spId, this.nppId, this.kbhId, 0.0, Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), 0.0);
									if(this.msg.length()>0)
									{
										db.getConnection().rollback();
										return false;
...
//									
			...
			query = "Update ERP_CHUYENKHONPP set trangthai=0 " +
					"where pk_seq=IDENT_CURRENT('ERP_CHUYENKHONPP') ";
			if(!db.update(query))
			{
	...
	public boolean updateNK() 
	{
		...
		try
		{
			...
			String query =	" Update ERP_CHUYENKHONPP set trangthai=99,SoChungTu='"+this.sochungtu+"',ngaychuyen = '" + this.ngayyeucau + "', ghichu = N'" + this.ghichu + "', " +
...
//			}
//			
			query="select a.ngaychuyen,b.ngaynhapkho,a.khoxuat_fk as kho_fk, a.npp_fk, "+this.kbhId+" as kbh_fk, b.sanpham_fk, b.solo, SUM(b.soluong) as tongxuat,b.NgayHetHan  " +
			" 	from ERP_CHUYENKHONPP a inner join ERP_CHUYENKHONPP_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk " +
			" 	where chuyenkho_fk = '" + this.id + "' " +
			" 	group by a.khoxuat_fk, a.npp_fk, a.kbh_fk, b.solo, b.sanpham_fk,b.NgayHetHan,a.ngaychuyen,b.ngaynhapkho " ;
			System.out.println("QUERY SAI LÀ  " +query);
			...
			while(nkRs.next()) 
			{
				...
				msg=util.Update_NPP_Kho_Sp_Chitiet(ngaychuyen, "Chuyển kênh NPP", db, kho_fk, sanpham_fk, npp_fk, kbh_fk, solo, ngayhethan, ngaynhapkho, 0.0,(-1)*soluong, soluong, soluong, 0.0);
				System.out.println("loi o day ne:::::::::::::::::::::::::::::::::: " +msg);
				...
				msg= util.Update_NPP_Kho_Sp(ngaychuyen, "Chuyển kênh NPP", db, kho_fk, sanpham_fk, npp_fk, kbh_fk, 0.0, (-1)*soluong, soluong, 0.0);
				if(msg.length()>0)
				{
					db.getConnection().rollback();
					return false;
						...
						while(keys.hasMoreElements())
						{
							...
							if(key.startsWith( spMa[i] + "__" ) )
							{
								...
								if(!_soluongCT.equals("0"))
								{
									...
									query = "insert ERP_CHUYENKHONPP_SANPHAM_CHITIET( chuyenkho_fk, SANPHAM_FK, solo, ngayhethan, soluong,ngaynhapkho ) " +
											"select '" + this.id + "', pk_seq, N'" + _sp[1] + "', '" + ngayhethan + "', '" + _soluongCT.replaceAll(",", "") + "','"+ngaynhapkho+"' " +
											"from SANPHAM where MA = '" + spMa[i] + "' ";
...
//									}
									...
									spRs.close();
									this.msg=util.Update_NPP_Kho_Sp_Chitiet(this.ngayyeucau, "Xuất chuyển nội bộ", db, this.khoXuatId, SpID, this.nppId, this.kbhId, _sp[1], ngayhethan, ngaynhapkho, 0.0, Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), 0.0);
									if(this.msg.length()>0)
									{
										db.getConnection().rollback();
										return false;
									...
									String sqldh="select convert(char(10),Created_Date,126) NGAYGIO_TAO from ERP_CHUYENKHONPP where pk_Seq="+this.id;
									String ngaytaodh="";
									...
									rsdh.close();
								//	System.out.println("ngay gio tao la "+ngaytaodh);
	...
	public void createRs() 
	{
		...
		query = 
						...
						" select PK_SEQ,TEN from NHAPHANPHOI where TRANGTHAI=1 and PK_SEQ in (select TRUCTHUOC_FK from NHAPHANPHOI where PK_SEQ='"+this.nppId+"') "
								+ " union all  "
								+ "  select PK_SEQ,TEN from NHAPHANPHOI where TRANGTHAI=1 and capcn =  (select capcn from NHAPHANPHOI where PK_SEQ='"+this.nppId+"') ";
	...
	private void initSANPHAM() 
	{
		...
		String query =  
					...
					"                                   from NHAPP_KHO" +
					"                                   where KHO_FK = 100000 and sanpham_fk = b.pk_seq " +
					"                                          and NPP_FK = (select NPP_DAT_FK from ERP_CHUYENKHONPP where PK_SEQ= '" + this.id + "') " +
					"                                          and KBH_FK ="+kenhId+"   ), 0 ) as avaicn " +
		...
		if(spRs != null)
		{
			try 
			{
				...
				while(spRs.next())
				{
					...
					sptonkhocn+=formater.format(spRs.getDouble("avaicn")) + "__";
				}
				spRs.close();
				...
				if(spMA.trim().length() > 0)
				{
					...
					sptonkhocn = sptonkhocn.substring(0, sptonkhocn.length() - 2);
					this.tonkhocn = sptonkhocn.split("__");
	...
	public ResultSet getSoloTheoSp(String spMa, String tongluong)
	{
		if(this.nppId.trim().length() > 0 && this.kbhId.trim().length() > 0 )
		{
			...
			String query = " select ct.ngaynhapkho as ngaynhapkho , AVAILABLE + ISNULL( ( select sum(soluong) from ERP_CHUYENKHONPP_SANPHAM_CHITIET  " +
			   " where chuyenkho_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo and NgayHetHan=ct.NgayHetHan and  ngaynhapkho=ct.ngaynhapkho  ), 0 ) as AVAILABLE, NGAYHETHAN, SOLO " +
			   ...
			   " and ( AVAILABLE + ISNULL( ( select sum(soluong) from ERP_CHUYENKHONPP_SANPHAM_CHITIET where chuyenkho_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "'  " +
	...
	public void setDungchungkenh(String dungchungkenh)
  {
	...
	public String[] getTonkhocn() {
		return tonkhocn;
	}

	public void setTonkhocn(String[] tonkhocn) {
		this.tonkhocn = tonkhocn;
	}
