	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		} else
		{
			...
			if (action.equals("dathang"))
			{
				...
				ddhBean.init2();
				nextJSP = request.getContextPath() + "/pages/Distributor/DonDatHangNewfromPR.jsp";
			} else
			{
				ddhBean.setId(id);
				ddhBean.init1();
				nextJSP = request.getContextPath() + "/pages/Distributor/DonDatHangUpdate.jsp";
			}
			...
			if (action.equals("save"))
			{
				if (id.length() == 0)
				{
					if (!(ddhBean.CreateDdhnpp(request)))
					{
						...
						session.setAttribute("ddhBean", ddhBean);
						String nextJSP = request.getContextPath() + "/pages/Distributor/DonDatHangNewfromPR.jsp";
						if (dondathangpr.equals(""))
						{
							nextJSP = request.getContextPath() + "/pages/Distributor/DonDatHangNew.jsp";
						}
						response.sendRedirect(nextJSP);
					} else
					{
				...
				} else
				{
					if (!(ddhBean.UpdateDdhnpp(request)))
					{
						ddhBean.initSelectboxData();
						session.setAttribute("ddhBean", ddhBean);
						String nextJSP = request.getContextPath() + "/pages/Distributor/DonDatHangUpdate.jsp";
						response.sendRedirect(nextJSP);
					} else
					{
						IDondathangList obj = new DondathangList();
			...
			} else
			{
				...
				} else
				{
					...
					} else if (action.equals("freshorder"))
					{
						...
						{
							ddhBean.init0();
							nextJSP = request.getContextPath() + "/pages/Distributor/DonDatHangParam.jsp";
						}
						session.setAttribute("ddhBean", ddhBean);
