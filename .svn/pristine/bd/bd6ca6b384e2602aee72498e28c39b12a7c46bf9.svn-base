	...
	public void init(String search)
	{
		...
		else
		{
			query = " select ( select SUM(data.tongtien) as tongtien from  ( \n"+
					...
					"		on a1.PK_SEQ=b1.BGBANLENPP_FK where b1.SANPHAM_FK=aa.sanpham_fk  and a1.npp_fk='"+this.nppId+"' \n"+
					"		order by a1.NGAYBATDAU )*aa.soluongchuyen  as tongtien \n"+
					"		 from ERP_CHUYENKHONPP_SANPHAM  aa where aa.chuyenkho_fk=a.pk_seq) as data) as tongtien, a.sonetId,a.PK_SEQ, a.trangthai,isnull( a.sochungtu,'')as SOCHUNGTU ,isnull(a.ngaychuyen,'') as ngaychuyen, a.ghichu as lydo, NV.TEN as nguoitao, b.ten as khoxuat, isnull(c.ten,'') as nppTEN,isnull(c.PK_SEQ, 0) as nppId, a.NGAYSUA, a.NGAYTAO, NV2.TEN as nguoisua, isnull(a.DAIN,0) DAIN \n" +
