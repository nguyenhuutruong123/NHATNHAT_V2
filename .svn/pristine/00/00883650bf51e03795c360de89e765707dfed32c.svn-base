                     ...
                     <TABLE border="0" width="100%" cellspacing = 0 cellpadding = 0>
                        ...
                        <TR>
                            <TD width="100%" align="left">
                                ...
                                <TR>
                                	<TD width="19%"  class="plainlabel" ><%=Utility.GLanguage("Mã CTTB",session,jedis) %></TD>
                                ...
                                <TR>
                                    <TD>
                                        ...
                                        <TR class="tbheader">
                                            ...
                                            <TH ><%=Utility.GLanguage("Ngày bắt đầu",session,jedis) %></TH>
									...
									<%}%>
									 ...
									 <td>
									 	<%if(dktbRs.getString("trangthai").equals("1")){ %>
									 		<%=Utility.GLanguage("Đã chốt",session,jedis) %>
									 	<%}else if(dktbRs.getString("trangthai").equals("2")) { %>
									 		<%=Utility.GLanguage("Đã xóa",session,jedis) %>
									 	<%}else{ %>
									 		<%=Utility.GLanguage("Đang xử lý",session,jedis) %>
									 	<%}%>
									 </td>
								     <TD align="center">
								     <%if(dktbRs.getString("trangthai").equals("0")){ %>
								     		...
								     		<A href = "../../RouterSvl?task=<%= Util.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"DangkytrungbayTTUpdateSvl?userId="+userId+"&update="+dktbRs.getString("pk_seq")+"")%>"><img src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" longdesc="Cap nhat" border = 0></A>
									   		...
									   		<A href ="../../RouterSvl?task=<%= Util.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"DangkytrungbayTTUpdateSvl?userId="+userId+"&chot="+dktbRs.getString("pk_seq")+"")%>"><img src="../images/Chot.png" alt="Chot" width="20" height="20" longdesc="Chot" border = 0 onclick="if(!confirm('Bạn có muốn duyệt đăng ký trưng bày này?')) return false;"></A>
									     	<%} %>
									     	
									     	<%if(quyen[Utility.XOA]!=0){ %>
									     	<A href = "../../RouterSvl?task=<%= Util.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"DangkytrungbayTTUpdateSvl?userId="+userId+"&delete="+dktbRs.getString("pk_seq")+"")%>"><img src="../images/Delete.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border = 0 onclick="if(!confirm('Bạn có muốn xóa đăng ký này?')) return false;"></A> 
									   		<%} %>
									   
									   <%} else { %>
									   		<%-- <%if(quyen[Utility.CHOT]!=0){ %>
									   		<A href = "../../DangkytrungbayTTUpdateSvl?userId=<%=userId%>&unchot=<%=dktbRs.getString("pk_seq")%>"><img src="../images/unChot.png" alt="UnChot" width="20" height="20" longdesc="Chot" border = 0 onclick="if(!confirm('Bạn có muốn bỏ duyệt đăng ký trưng bày này?')) return false;"></A>
									     	<%} %> --%>
