...
public class DuyettratrungbaySvl extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (request.getParameter("action").equals("new"))
		{
			...
			session.setAttribute("userId", userId);
			nextJSP = request.getContextPath() + "/pages/Center/DuyetTraTrungBayNew.jsp";
			response.sendRedirect(nextJSP);
			return;
		}
		dttbBean.init();
	...
	private String Delete(String id) {
		...
		try
		{
			...
			if(!db.update(update)){
			    ...
			    return "Không thể xóa duyệt trả trưng bày này. Lỗi (1)";
			}
			
			update = "DELETE FROM DENGHITRATB_KHACHHANG_DUYETANH WHERE DENGHITRATB_FK = " + id;
			System.out.println("update 2 : "+ update);
			if(!db.update(update)){
				geso.dms.center.util.Utility.rollback_and_shutdown(db);
				return "Không thể xóa duyệt trả trưng bày này. Lỗi (2)";
			}
			...
			if(db.updateReturnInt(update)!=1){
				geso.dms.center.util.Utility.rollback_and_shutdown(db);
				return "Không thể xóa duyệt trả trưng bày này. Lỗi (3)";
			}
	...
	private String Duyet(String id) {
		...
		try
		{
			...
			query = "\n insert khachhang_thuongtrungbay(CTTB_FK,TraTrungBay_FK,KhachHang_fk,TongLuong,TongGiaTri,SoXuat,denghitratrungbay_fk,loai_tra,hinhthuc_tra)" + 
			 ...
			 query = "\n insert khachhang_thuongtrungbay_sanpham (CTTB_FK ,denghitratrungbay_fk,tratrungbay_fk,khachhang_fk,sanpham_fk,SoLuong)" + 
