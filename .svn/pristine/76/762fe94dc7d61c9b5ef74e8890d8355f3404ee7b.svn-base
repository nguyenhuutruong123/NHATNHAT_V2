...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý công nợ > Công nợ phải trả > Ủy nhiệm chi > Tạo mới
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
        ...
        <span id="btnSave">
         ...
         <%--  <A href="../../ErpUynhiemchiPdfSvl?userId=<%= tthdBean.getUserId() %>&id=<%= tthdBean.getId() %>&httt=<%= tthdBean.getHtttId() %>" >
                        	...
                        	<%
                        		if(khList != null)
                        		{
                        			...
                        			if( khList.getString("pk_seq").equals(tthdBean.getKhachhangId())){ %>
                        				<option value="<%= khList.getString("pk_seq") + " - " +khList.getString("isNPP") %>" selected="selected" ><%= khList.getString("ten")%></option>
                        			<%}else { %>
                        				<option value="<%= khList.getString("pk_seq") + " - " +khList.getString("isNPP") %>" ><%= khList.getString("ten") %></option>
	                   ...
	                   <DIV id="subcontentVATNT" style="position:absolute; visibility: hidden; border: 2px solid #80CB9B; background-color: white; width: 900px; padding: 4px; max-height: 300px; overflow: auto; ">
							<TABLE width="100%" align="center">
        						<TR class="tbheader">
									...
									<TH width="100px" align = "center"><%=Utility.GLanguage("Diễn giải",session,jedis) %> </TH>
			  ...
			  <!-- KÊNH BÁN HÀNG  -->
				...
				<TD>
					<%if(Tienthuehd[i]!=null && Tienthuehd[i].trim().length()>0) {%>	
						<input type="text" name= "tienthue_VAT_<%=i %>"  id= "tienthue_VAT_<%=i %>" value = "<%= formatter2.format(Double.parseDouble(Tienthuehd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" onChange = "javascript:thue_loaikhac_new();" >
					<%} else { %>
						<input type="text" name= "tienthue_VAT_<%=i %>" id= "tienthue_VAT_<%=i %>"  value = "<%= Tienthuehd[i] %>" style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" onChange = "javascript:thue_loaikhac_new();">
					<%} %>
				</TD>  		                    
			</TR>
     <% }
        ...
        %>
        	...
        	<%  } %> 
				...
				<TD>
					...
					<%} else { %> --%>
						<input type="text" name= "tienthue_VAT_<%=i %>" id= "tienthue_VAT_<%=i %>" value = ''  style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" onChange = "javascript:thue_loaikhac_new();">
					<%-- <%} %> --%>
				</TD>  		                         		
	</TR>
	<%
