...
public class Bando implements IBando 
{
	...
	public void init() 
	{
		...
		String sql = "select kh.pk_seq as khId, kh.ten as khTen, isnull(kh.dienthoai, 'NA') as dienthoai, isnull(kh.diachi, '') as diachi, kh.lat, kh.long, " +
						...
						" CASE WHEN isnull(kh.LOAICUAHANG, '0') = '100890' THEN '1' ELSE '0' END AS khachhangsi " +
					"from KhachHang kh  " +
					"left join  " +
					"(   " +
		...
		{
			sql += " and kh.pk_seq in (select khachhang_fk from khachhang_tuyenbh where tbh_fk in (" + this.tbhId + ")) ";
		} 
		...
		{
			query = " select distinct a.pk_seq as khId, a.ten as khTen, isnull(a.diachi, '') as diachi, isnull(a.dienthoai, 'NA') as dienthoai, b.thoidiem, c.ten as ddkdTen, b.lat, b.long, " +
					"	case when anhcuahang is null then 'hinh1.jpg' else anhcuahang end as anhcuahang, " +
					"   cast(isnull(doanhsoThangtruoc, 0) as numeric(18, 0)) / 3 as doanhsoThangtruoc ,   " +
					"   cast( isnull(doanhsoTudauthang, 0) as numeric(18, 0)) as doanhsoTrongthang, " +
			...
			if(this.tbhId.trim().length() > 0)
			{
				query += " and a.pk_seq in (select khachhang_fk from khachhang_tuyenbh where tbh_fk in (" + this.tbhId + ")) ";
			}
			else if(this.ddkdId.trim().length() > 0)
			{
	...
	public void DBclose() 
	{
		try 
		{
			...
			if(this.tbhList != null)
				this.tbhList.close();
			if(this.kvRs != null) this.kvRs.close();
			if(this.vungRs != null) this.vungRs.close();
			if(this.nppRs != null) {
				this.nppRs.close();
			}
			if(this.spRs != null) {
	...
	public void initBCDoPhu() 
	{
		...
		this.chungloaiRs = db.get("select pk_seq, ten from NGANHHANG where trangthai = '1' ");
		...
		if(this.clId.length() <= 0)
		{
			...
			String sql = "select khId, khTen, isnull(dienthoai,'') dienthoai, lat, long, sum(tbdoanhso) as tbdoanhso, sum(doanhsotheonganh) as doanhsotheonganh  " +
					"from  " +
					"(  " +
		...
		{
			String query = "select khTen,  isnull(dienthoai,'') dienthoai, lat, long, sum(tbdoanhso) as tbdoanhso, sum(doanhsotheonganh) as doanhsotheonganh " +
					"from " +
					"(	" +
					...
					"inner join ( " +
							...
							"from donhang a inner join donhang_sanpham b on a.pk_seq = b.donhang_fk inner join sanpham c on b.sanpham_fk = c.pk_seq " +
								"inner join khachhang d on a.khachhang_fk = d.pk_seq " +
							"where a.khachhang_fk in (select pk_seq from khachhang where npp_fk = '" + this.nppId + "' and lat is not null) and a.trangthai = '1' " +
								"and '" + this.ngay + "' <= a.ngaynhap and a.ngaynhap <= '" + this.denngay + "' and c.NGANHHANG_FK = '" + this.clId + "' " +
							"group by d.pk_seq) theonganh on dophu.khId = theonganh.khId";
			
				query += " union  " +
						 "select b.ten, b.dienthoai, b.lat, b.long, 1, 1   " +
						 "from lichsu_dophu a inner join khachhang b on a.khachhang_fk = b.pk_seq   " +
						 "where lat is not null and long is not null and sanpham_fk in ( select pk_seq from sanpham where NGANHHANG_FK = '" + this.clId + "' )  " +
	...
	public void initBCDoPhuTT()
	{
		...
		query+= " where pk_Seq in " + util.Quyen_Vung(userId)+"";
		System.out.println("chon vung:"+query);
		this.vungRs = db.get(query);
		
		query = "select PK_SEQ, TEN from KHUVUC where TRANGTHAI = '1' ";
		
		System.out.println("vung TT"+query);
			...
			if(this.clId.length() <= 0)
			{
				String sql = "select khId, khTen, isnull(dienthoai, '') as dienthoai, lat, long, sum(tbdoanhso) as tbdoanhso, sum(doanhsotheonganh) as doanhsotheonganh  " +
					...
					"		and khachhang_fk in (select pk_seq from khachhang where npp_fk in ( " + this.nppLocId + " ) ) " +
							(this.spId.trim().length() > 0 ? " and a.sanpham_fk in ( " + this.spId + " ) " : "") +
					")  " +
					"dophu group by khId, khTen, dienthoai, lat, long";
			...
			else
			{
				query = "select khTen, isnull(dienthoai, '') as dienthoai, lat, long, sum(tbdoanhso) as tbdoanhso, sum(doanhsotheonganh) as doanhsotheonganh " +
					...
					"left join ( " +
							...
							"from donhang a inner join donhang_sanpham b on a.pk_seq = b.donhang_fk inner join sanpham c on b.sanpham_fk = c.pk_seq " +
								"inner join khachhang d on a.khachhang_fk = d.pk_seq " +
							"where a.khachhang_fk in (select pk_seq from khachhang where lat is not null and khachhang.npp_fk in ( " + this.nppLocId + " ) ) and a.trangthai = '1' " +
								"and '" + this.ngay + "' <= a.ngaynhap and a.ngaynhap <= '" + this.denngay + "' and c.NganhHang_fk = '" + this.clId + "' " + condition + 
							"group by d.pk_seq) theonganh on dophu.khId = theonganh.khId";
