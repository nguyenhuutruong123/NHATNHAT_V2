...
public class Duyettradonhang implements IDuyettradonhang
{
	...
	public boolean duyetSptrave(String id, String userId)
	{
		...
		try 
		{
			...
			query = "select dktrungbay_fk, khachhang_fk, dat from DKTRUNGBAY_DONHANG where donhang_fk = '" + this.dhId + "'";
			...
			while(rsDelete.next())
			{
				...
				query = "delete DKTRUNGBAY_DONHANG where dktrungbay_fk = '" + dk_fk + "' and khachhang_fk = '" + kh_fk + "' and donhang_fk = '" + this.dhId + "'";
				if(!db.update(query))
				{
					this.msg = "1.Không thể cập nhật DKTRUNGBAY_DONHANG " + query;
					db.getConnection().rollback();
					return false;
				}
				...
				if(!db.update(query))
				{
					this.msg = "2.Không thể cập DKTRUNGBAY_KHACHHANG " + query;
					db.getConnection().rollback();
					return false;
				}
