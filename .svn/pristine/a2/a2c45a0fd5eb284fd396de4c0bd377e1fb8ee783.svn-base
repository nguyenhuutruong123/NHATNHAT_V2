		...
		session.setAttribute("rs_dondathang", rs_dondathang);
         ...
         String sql_getdvkd="select pk_seq,donvikinhdoanh as ten from donvikinhdoanh where trangthai='1' ";//lay nhung don dat hang da chot,va chua chuyen het hang sang kho khac
			...
			if(action.equals("update")){
				String 	nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTTUpdate.jsp";
				response.sendRedirect(nextJSP);
			}else if(action.equals("chot")){
				String 	nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTTChot.jsp";
				response.sendRedirect(nextJSP);
			}else if(action.equals("display")){
				String 	nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTTDisplay.jsp";
				response.sendRedirect(nextJSP);
				...
				IPhieuXuatKhoTT   obj = new PhieuXuatKhoTT();
				  ...
				  String  nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTT.jsp";
				   response.sendRedirect(nextJSP);
			}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String action = request.getParameter("action");		
 		...
 		if(tenform.equals("newform")){
 			nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTTNew.jsp";
 		}else{
 		   nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTTUpdate.jsp";
 		}
		...
		String ngayxuatkho = util.antiSQLInspection(request.getParameter("ngayxuatkho"));
	    ...
	    String SoSo=util.antiSQLInspection(request.getParameter("soso"));
			  ...
			  if(phieuxuat.SavePhieuXuatKho()){
				...
				if(phieuxuat.EditPhieuXuatKho()){
				  ...
				  if(phieuxuat.ChotPhieuXuatKho()){
			       ...
			       obj.init("");
				   session.setAttribute("obj", obj);
				   nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTT.jsp";
				   response.sendRedirect(nextJSP);
				   }else
				   {
					   nextJSP = request.getContextPath() + "/pages/Center/PhieuXuatKhoTTChot.jsp";
					   session.setAttribute("phieuxuat",phieuxuat );
					   session.setAttribute("obj",hoadon);
					  response.sendRedirect(nextJSP);
				   }	
