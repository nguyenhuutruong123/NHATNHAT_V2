...
<% ResultSet nppkstd = (ResultSet)mksBean.getNppkstd(); %>
...
function Mokhoaso()
{
	document.forms['mksForm'].action.value= 'tlks';
	document.forms['mksForm'].submit();
}
											...
											<TH width="20%">Ngày khóa sổ</TH>
												...
												<% try{
									    	  	if(npp != null){	
									    	  		while(npp.next()){ %>
									    	  			<TR  class="tblightrow">
									      					<Td width="80%" align="left"><%=npp.getString("TEN") %><input type="hidden" name="npp" value="<%=npp.getString("nppid")%>" />	</Td>
									      					 <Td width="20%" align="center">
									      					 <%if(nppkstd==null){ %>
									      				 		 <input type="text" class="days" name="nppkstd" value="">
									      				 		 <%  									      				 
									      					 }else{
									      						 int flag=0;
									      						 while(nppkstd.next())
									      						 { 
									      							 if(nppkstd.getString("nppid").equals(npp.getString("nppid")))
									      							 { %>
									      							   <input type="text" class="days" name="nppkstd" value="<%=nppkstd.getString("ngayks")%>">
									      						 <%  flag=1;break ;}
