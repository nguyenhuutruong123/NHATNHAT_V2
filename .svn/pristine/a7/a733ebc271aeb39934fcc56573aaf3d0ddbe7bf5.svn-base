...
public class DailySecondarySalesTT_focusedSKUSvl extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String nextJSP = request.getContextPath() + "/pages/Center/DailySecondarySalesReportCenter_focusedSKU.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String nspId = util.antiSQLInspection(request.getParameter("nspId"))!=null?util.antiSQLInspection(request.getParameter("nspId")):"";
	...
	private String setQuery(String sql, IStockintransit obj) {
		...
		Customer += "ISNULL(GSBH.TEN, 'Chua xac dinh') AS SALESUPER, ISNULL(ROUND((CAST((QC.SOLUONG2 * DH.SOLUONG) AS FLOAT) / CAST(QC.SOLUONG1 AS FLOAT)),3), 0) AS QUANTITY, " +
					...
					"INNER JOIN NHOMSANPHAM NSP ON NSP.PK_SEQ = NSPSP.NSP_FK and nsp.TRANGTHAI = '1' and nsp.loainhom = '0' and nsp.TYPE = '0' " + sql + " " +
	...
	private boolean CreatePivotTable(OutputStream out, IStockintransit obj, String query)throws Exception 
	{
		...
		fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT_Customer_focusedSKU.xlsm");
//		fstream = new FileInputStream("D:\\DMS\\Best\\WebContent\\pages\\Templates\\DoanhSoTT_Customer_focusedSKU.xlsm");
