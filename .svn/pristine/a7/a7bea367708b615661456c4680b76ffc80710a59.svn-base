...
public class sitecode_convSvl extends HttpServlet
{
	...
	private void ToExcel(HttpServletResponse response, Isitecode_conv obj, String query) throws IOException
	{
		...
		try
		{
			...
			sheet.addCell(new Label(0, 1, "NHÀ PHÂN PHỐI ERP: ", new WritableCellFormat(cellTitle)));
			...
			sheet.addCell(new Label(1, 4, "TÊN NPP ERP", cellFormat));
			...
			sheet.addCell(new Label(9, 4, "TRẠNG THÁI", cellFormat));
			...
			while (rs.next())
			{
				...
				label = new Label(9, j, rs.getInt("trangthai") == 0 ? "Chờ xử lý" : "Hoàn tất", cformat);sheet.addCell(label);
				j++;
			}
			w.write();
			w.close();
