...
public class NhomCTTrungBayUpdateSvl extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nextJSP = request.getContextPath() + "/pages/Center/NhomCTTrungBayUpdate.jsp";
		if(querystring.indexOf("display") > 0)
	    {
	    	nextJSP = request.getContextPath() + "/pages/Center/NhomCTTrungBayDisplay.jsp";
	    }
			...
			if (action.equals("save"))
			{
				...
				if (id == null)
				{
					if (!ncttbBean.save())
					{
						session.setAttribute("ncttbBean", ncttbBean);
						ncttbBean.setUserId(userId);
						nextJSP = request.getContextPath() + "/pages/Center/NhomCTTrungBayNew.jsp";
						response.sendRedirect(nextJSP);
					} else
					{
						...
						nextJSP = request.getContextPath() + "/pages/Center/NhomCTTrungBay.jsp";
						response.sendRedirect(nextJSP);
					}
				} else
				{
					if (!ncttbBean.edit())
					{
						session.setAttribute("ncttbBean", ncttbBean);
						ncttbBean.setUserId(userId);
						nextJSP = request.getContextPath() + "/pages/Center/NhomCTTrungBayUpdate.jsp";
						response.sendRedirect(nextJSP);
					} else
					{
						obj.setUserId(userId);
