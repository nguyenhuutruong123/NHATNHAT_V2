...
public class Dontrahang implements IDontrahang, Serializable
{
	...
	public boolean updateNK(String ischot) 
	{
		try
		{
			...
			{
				msg += "NPP chưa có ngày khóa sổ. Không chốt được đơn trả hàng! \n";
				return false; 
			}
			if(trangthai.equals("2"))
			{
				msg += "Đơn trả hàng đã chốt rồi ! \n";
				return false; 
			}
			...
			{
				msg += "Đơn trả hàng số "+this.id+" không được phép chốt trước hoặc bằng ngày khóa sổ.Vui lòng bỏ chọn \n";
				return false; 
			}
			...
			{
				msg = "Không thể cập nhật kho NPP: " + query;
				db.getConnection().rollback();
				return false;	
			}*/
			...
			query= " 	select a.ngaytra,b.ngaynhapkho,a.kho_fk as kho_fk, a.npp_fk, "+this.kbhId+" as kbh_fk, b.sanpham_fk, b.solo, SUM(b.soluong) as tongxuat,b.NgayHetHan  " +
					...
					while (ckRs.next())
					{
						...
						this.msg=util.Update_NPP_Kho_Sp_Chitiet(ngaytra, "Đơn trả hàng về NCC-CHOT " + this.id, db, kho_fk, sanpham_fk, npp_fk, kbh_fk, solo, NgayHetHan, ngaynhapkho, (-1)*tongxuat, (-1)*tongxuat, 0, tongxuat, 0.0);
						if(this.msg.length()>0)
						{
							db.getConnection().rollback();
							return false;
						}
						this.msg=util.Update_NPP_Kho_Sp(ngaytra, "Đơn trả hàng về NCC-CHOT " + this.id, db, kho_fk, sanpham_fk, npp_fk, kbh_fk, (-1)*tongxuat,  (-1)*tongxuat, 0.0, 0.0);
						if(this.msg.length()>0)
						{
							db.getConnection().rollback();
							return false;
