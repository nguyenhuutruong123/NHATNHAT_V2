...
public class catalogUpdateSvl extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (request.getQueryString().indexOf("display") >= 0) {
			nextJSP = request.getContextPath() + "/pages/Center/catalogDisplay.jsp";
		} else {
			nextJSP = request.getContextPath() + "/pages/Center/catalogUpdate.jsp";
		}
			...
			if (action.equals("save")) {
				if(id.trim().length() <= 0)
				{
					...
					else
					{
						...
						session.setAttribute("userId", userId);
						String nextJSP = request.getContextPath() + "/pages/Center/catalog.jsp";
						response.sendRedirect(nextJSP);
					}
				}
				else
				{
					if(!nspBean.updateNsp())
					{
						...
						session.setAttribute("nspBean", nspBean);
						session.setAttribute("userId", userId);
						String nextJSP = request.getContextPath() + "/pages/Center/catalogUpdate.jsp";
						response.sendRedirect(nextJSP);
					}
					else
					{
