...
<% INopTienNPPList obj = (INopTienNPPList)s.getAttribute("obj"); %>
...
<%
	int[] quyen = new  int[6];
	quyen = Util.Getquyen("NopTienNPPSvl","",userId);
	String userTen = (String) session.getAttribute("userTen");
	String url = Util.getUrl("NopTienNPPSvl","");
...
<SCRIPT language="javascript" type="text/javascript">
 ...
 {   
	...
	document.forms['dhForm'].sohoadon.value ='';
	document.forms['dhForm'].doituong.value ='';
...
<form name="dhForm" method="post" action="../../NopTienNPPSvl">
							...
							<TABLE width="100%" cellpadding="6" cellspacing="0">
								...
								<TR>
									...
									<TD class="plainlabel">
										<select name="trangthai" onChange="submitform();">
											<option value=""></option>
											<option value="0" <%= obj.getTrangthai().equals("0") ? "selected":""  %>    > Chưa chốt </option>
											<option value="1" <%= obj.getTrangthai().equals("1") ? "selected":""  %>    > Đã chốt </option>
											<option value="2" <%= obj.getTrangthai().equals("2") ? "selected":""  %>    > Đã xóa </option>
									    </select>
									</TD>
									...
									<td class="plainlabel">
			                            <input type="text" 
			                                    id="sohoadon" name="sohoadon" value="<%= obj.getSohoadon() %>"   />
			                    	</td>
								...
								<TR>
									...
									<TD class="plainlabel" colspan="5">
										<select  id="doituong"  name="doituong" onChange="submitform();">
											<option value=""></option>
											<option value="0" <%= obj.getBarcode().equals("0") ? "selected":""  %>    > Nhà phân phối</option>
											<option value="1" <%= obj.getBarcode().equals("1") ? "selected":""  %>    > Khách hàng </option>
						...
						<TR>
							...
							<TABLE width="100%" border="0" cellspacing="1px" cellpadding="4px">
								...
								<TR class="tbheader">
									...
									<th width="7%" align="center">Ngày chứng từ</th>
								...
								if(dhlist != null)
								{
									...
									<%} else {%>
										...
										<%}%>
												...
												<TD align="center"><%= dhlist.getString("doituong").equals("1") ? "Khách hàng" : "Nhà phân phối" %></TD>
												...
												<TABLE border = 0 cellpadding="0" cellspacing="0" >
													...
													<%if(trangthai.equals("0")  ){ %>
														...
														<%if(quyen[Utility.SUA]!=0){ %>
															<A href = "../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NopTienNPPUpdateSvl?userId="+userId+"&id="+ dhlist.getString("dhId")+"&action="+ Util.SUA+"")%>"><img src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" longdesc="Cap nhat" border = 0></A>&nbsp;
														<%} %>
															 <%if(quyen[Utility.CHOT]!=0){ %>
																	<A href="../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NopTienNPPSvl?userId="+userId+"&id="+ dhlist.getString("dhId") +"&action="+ Util.CHOT+"")%> " >
																		<img  src="../images/Chot.png" alt="Duyệt đơn hàng" width="20" height="20"  border='0' title="Duyệt"	 >
																	</A>&nbsp;
															 <%} %>
															 	
														<%if(quyen[Utility.XOA]!=0){ %>
															<A href="../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NopTienNPPSvl?userId="+userId+"&id="+ dhlist.getString("dhId") +"&action="+ Util.XOA+"")%> " >
													...
													<% } %>
														...
														<%if(quyen[Utility.XEM]!=0){ %>
															<A href = "../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NopTienNPPUpdateSvl?userId="+userId+"&id="+ dhlist.getString("dhId") +"&action="+ Util.XEM+"")%>"><img src="../images/Display20.png" alt="Hien thi" width="20" height="20" longdesc="Hien thi" border = 0></A>&nbsp;
														<%} %>
