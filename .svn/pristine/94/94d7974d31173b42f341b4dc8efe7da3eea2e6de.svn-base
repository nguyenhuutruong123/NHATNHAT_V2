		...
		try
		{
			...
			if(rs != null)
			{
				...
				if(rs.next())
				{
					...
					if(count > 0)
					{
						this.msg = "Scheme đã tồn tại, vui lòng nhập lại";
						rs.close();
						return false;
					}
				}
			...
			{		
				this.msg = "Scheme khuyến mãi/tích lũy đã tồn tại, vui lòng nhập lại";
				return false;
			}
			 ...
			 * this.msg = "Đến ngày đăng ký không được nhỏ hơn ngày hiện tại!";
			...
			if(checkRs.getInt("sl")>0) {
				this.msg = "Đến ngày đăng ký không được nhỏ hơn từ ngày đăng ký!";
				db.getConnection().rollback();
				return false;
			}
			checkRs.close();
