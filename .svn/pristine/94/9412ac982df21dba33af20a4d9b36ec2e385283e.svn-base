...
public class ChiTieuNhanvien implements IChiTieuNhanvien {
		...
		if(this.loaiUpload.equals("2"))
		{
			...
			{	
				...
				{	
					...
					query ="\n select nv.TEN as N'GSBH',t.DIENGIAI as N'Diễn giải',isnull(nsp.TEN,'') "+hienthiSanPham+" as N'Nhóm sản phẩm',a.chitieu as N'Chỉ tiêu', isnull(a.chitieuUpload,0) [Chỉ tiêu upload]  from chitieunhanvien_gsbh a   " + 
	...
	public void setListChiTieu(String sql) {
		...
		sql_getdata="SELECT   C.PK_SEQ,c.trangthai, C.THANG, C.NAM, C.DIENGIAI, C.NGAYTAO, C.NGAYSUA,NT.TEN AS NGUOITAO, "+ 
				...
				" where 1=1 order by  C.NAM desc, C.THANG desc ";
	...
	public boolean CreateChiTieuLuongThuong(String values,String sqlTrongSo) 
	{
		try
		{
			if(!KiemTraHopLe()){
			...
			if(rs_check.next()){//co du lieu
				this.setMessage("Chỉ tiêu trong tháng đã được thiết lập. Vui lòng kiểm tra lại.");
				rs_check.close();
				return false;
			}
			boolean pass = checkSQL(values,sqlTrongSo);
			if(!pass)
			{
			...
			pass = InsertSR(db,this.Id,values,sqlTrongSo);
	...
	public boolean UpdateChiTieuLuongThuong(String values,String valuesNhom) 
	{
		try
		{
			...
			String qr = "select sum(trongso),nhanvien_fk as manv from "+table+" where CTNV_FK = "+this.Id+" "
					+ " group by nhanvien_fk"
					+ " having sum(trongso) > 100 ";
			System.out.println("CheckTS "+qr);
			String manv = "";
			ResultSet rscheck = db.get(qr);
			...
			if(rscheck != null)
			{
					...
					while(rscheck.next())
					{
						manv += rscheck.getString("manv");
					}
			
			}
			if(manv.length() > 0)
			{
				this.Message="Trọng số các Mã nhân viên sau vượt quá 100: "+manv;		
				pass = false;
			}
	...
	public boolean InsertSR(dbutils db, String id,String values,String valuesNhom)
	{
		...
		String sql = "";
		String chitieu = "CHITIEU";
		
		String table ="ChiTieuNhanVien_DDKD";
		if(this.loai.equals("2"))
		{
			table ="ChiTieuNhanVien_GSBH";
			chitieu = "chitieuUpload";
		}
		else if(this.loai.equals("3"))
		{
			chitieu = "chitieuUpload";
		...
		sql = "\n update ct set  NSP_FK = t.nhomsp_fk,  TIEUCHI = t.tieuchi , "+chitieu+" = isnull( data.chitieu,0)   " +
			  "\n from "+table+" ct " +
			  "\n left join ("+values+")data on  ct.TCTCT_FK = data.TCTCT_FK and ct.NHANVIEN_FK = data.manv  " +
			  "\n inner join tieuchithuong_chitiet t on data.TCTCT_FK = t.pk_Seq " +
			  "\n where ct.ctnv_fk = "+id; 
		if(db.updateReturnInt(sql) < 0)
		{
		...
		"\n where not exists ( select 1 from "+table+" x where x.CTNV_FK = "+id+" and x.TCTCT_FK = data.TCTCT_FK and x.NHANVIEN_FK = data.manv)   ";
		...
		if(this.loai.equals("1"))// sum lên cho cập trên
		{
			
			
			sql="\n update x set chitieu = ud.chitieu, x.NSP_FK = ud.NSP_FK ,x.tieuchi =ud.TieuChi " +
				"\n from ChiTieuNhanVien_GSBH x " +
				"\n outer apply " +
				"\n ( " +
				"\n 	select a.CTNV_FK,gs.GSBH_FK,a.TCTCT_FK,a.tieuchi,a.NSP_FK" +
				"\n			, case when a.TieuChi in (4,6) then round(dbo.PhepChia( sum(a.chitieu), count(distinct a.nhanvien_fk)),0) else  sum(a.chitieu)end chitieu " +
				"\n 	from ChiTieuNhanVien_DDKD a " +
				"\n 	inner join DDKD_GSBH gs on gs.DDKD_FK =a.nhanvien_fk  " +
				"\n 	where  x.CTNV_FK  =  a.CTNV_FK and x.nhanvien_fk = gs.GSBH_FK and a.TCTCT_FK = x.TCTCT_FK " +
				"\n 	group by a.CTNV_FK,gs.GSBH_FK,a.TCTCT_FK,a.tieuchi,a.NSP_FK " +
				"\n )ud" +
				"\n where x.CTNV_FK = " + id ;
			if(db.updateReturnInt(sql) < 0)
			{
			...
			sql="\n insert ChiTieuNhanVien_GSBH(CTNV_FK,nhanvien_fk,TCTCT_FK,tieuchi,NSP_FK,chitieu,chitieuUpload) " +
				"\n select a.CTNV_FK,gs.GSBH_FK,a.TCTCT_FK,a.tieuchi,a.NSP_FK" +
				"\n		, case when a.TieuChi in (4,6) then round(dbo.PhepChia( sum(a.chitieu), count(distinct a.nhanvien_fk)),0) else  sum(a.chitieu)end chitieu , 0 " +
				"\n from ChiTieuNhanVien_DDKD a " +
				"\n inner join DDKD_GSBH gs on gs.DDKD_FK =a.nhanvien_fk  " +
				"\n where  a.CTNV_FK = " + id + " and not exists ( select 1  from ChiTieuNhanVien_GSBH x where x.CTNV_FK =  a.CTNV_FK and x.nhanvien_fk = gs.GSBH_FK and a.TCTCT_FK = x.TCTCT_FK) " +
				"\n group by a.CTNV_FK,gs.GSBH_FK,a.TCTCT_FK,a.tieuchi,a.NSP_FK " ;
			if(db.updateReturnInt(sql) < 0)
			{
				System.out.println("save ChiTieuNhanVien_SR: " +sql);
			...
			sql="\n update x set chitieu = ud.chitieu, x.NSP_FK = ud.NSP_FK ,x.tieuchi =ud.TieuChi  " +
				"\n from ChiTieuNhanVien_ASM x  " +
				"\n outer apply  " +
				"\n (  " +
				"\n 	select a.CTNV_FK,gs.asm_fk,a.TCTCT_FK,a.tieuchi,a.NSP_FK" +
				"\n			, case when a.TieuChi in (4,6) then round(dbo.PhepChia( sum(a.chitieu), count(distinct a.nhanvien_fk)),0) else  sum(a.chitieu)end chitieu  " +
				"\n 	from ChiTieuNhanVien_GSBH a  " +
				"\n 	inner join GIAMSATBANHANG gs on gs.pk_seq =a.nhanvien_fk   " +
				"\n 	where  x.CTNV_FK  =  a.CTNV_FK and x.nhanvien_fk = gs.asm_fk and a.TCTCT_FK = x.TCTCT_FK  " +
				"\n 	group by a.CTNV_FK,gs.asm_fk,a.TCTCT_FK,a.tieuchi,a.NSP_FK  " +
				"\n )ud " +
				"\n where x.CTNV_FK  = " + id ;
			if(db.updateReturnInt(sql) < 0)
			{
			...
			sql="\n insert ChiTieuNhanVien_ASM(CTNV_FK,nhanvien_fk,TCTCT_FK,tieuchi,NSP_FK,chitieu,chitieuUpload)  " +
				"\n select a.CTNV_FK,gs.asm_fk,a.TCTCT_FK,a.tieuchi,a.NSP_FK" +
				"\n		, case when a.TieuChi in (4,6) then round(dbo.PhepChia( sum(a.chitieu), count(distinct a.nhanvien_fk)),0) else  sum(a.chitieu)end chitieu, 0  " +
				"\n from ChiTieuNhanVien_GSBH a  " +
				"\n inner join GIAMSATBANHANG gs on gs.pk_seq =a.nhanvien_fk   " +
				"\n where  a.CTNV_FK = " + id + " and not exists ( select 1  from ChiTieuNhanVien_ASM x where x.CTNV_FK =  a.CTNV_FK and x.nhanvien_fk = gs.asm_fk and a.TCTCT_FK = x.TCTCT_FK)  " +
				"\n group by a.CTNV_FK,gs.asm_fk,a.TCTCT_FK,a.tieuchi,a.NSP_FK" ;
			if(db.updateReturnInt(sql) < 0)
			{
				System.out.println("save ChiTieuNhanVien_SR: " +sql);
