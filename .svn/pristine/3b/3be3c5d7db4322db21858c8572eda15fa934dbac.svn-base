...
public class BoBundleSvl extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(!cutil.check(userId, userTen, sum)){
			...
			userId = request.getParameter("userId");
			System.out.println();
			System.out.println("BoBundleSvl user :" + userId + "  ,sessionId: " + session.getId()) ;
			...
			{
				IBoBundle dhBean = (IBoBundle) new BoBundle("");
			...
			else
			{
				...
				if(action.equals("search"))
				{
					...
					response.sendRedirect(request.getContextPath() + "/pages/Center/BoBundle.jsp");	    		    	
				}
	...
	private  String XoaChungTu(String dhId, String lydoxoa) 
	{
		...
		try 
		{
			...
			"\n where dh.trangthai = 0 and dhsp.BUNDLE_fk =  " + dhId +
			...
			while (rssp.next()){
				...
				String msg1=util_kho.Update_NPP_Kho_Sp("", "bundle revert  tổng", db, _khoid, _spid, _nppid ,_kbhid, 0, (-1)*_soluong, _soluong, 0);// giảm booked,avai cộng
				if(msg1.length() >0)
				{
			...
			"\n WHERE DH_CT.BUNDLE_fk=" + dhId +"  AND DH.TRANGTHAI=0 ";
			...
			query = "update ERP_BUNDLE set trangthai = 2 where pk_seq = '" + dhId + "' and TrangThai = 0 ";
	...
	private  String ChotChungTu(String dhId) 
	{
		...
		try 
		{
			...
			query = "update ERP_BUNDLE set trangthai = 1 where pk_seq = '" + dhId + "' and TrangThai = 0 ";
			System.out.println("--CAP NHAT TRANG THAI: " + query);
			if(db.updateReturnInt(query)!=1)
			{
			...
			"\n from ERP_BUNDLE dh   " + 
			"\n inner join ERP_BUNDLE_SANPHAM dhsp on dh.PK_SEQ = dhsp.BUNDLE_fk  " + 
			...
			while ( rs.next())
			{
				...
				String msg1=util_kho.Update_NPP_Kho_Sp(NgayChungTu, "Chốt bundle BoBundleSvl 393 ", db, khoId, spId, nppId, kbhId,  soluong*(-1), soluong*(-1), 0, 0);
				if(msg1.length() >0){
					msg = msg1;
					db.getConnection().rollback();
					return msg;
				...
				while (rssp.next())
				{
					...
					msg1 =util_kho.Update_NPP_Kho_Sp_Chitiet(NgayChungTu, "Chốt bundle BoBundleSvl 423 ",   db, _KHO_FK, _spid, _NPP_FK, _KBH_FK, _SOLO, _NGAYHETHAN, _NGAYNHAPKHO,
				...
				{
					msg = "437.BoBundleSvl.Lỗi hệ thống ( SỐ LƯỢNG LÔ TỔNG VÀ LÔ CHI TIẾT KHÔNG KHỚP ). Vui lòng liên hệ trung tâm để được hỗ trợ xử lý.";
					db.getConnection().rollback();
					return msg;
			...
			query =" select KHO_FK,KBH_FK,NPP_FK,SANPHAM_FK,SOLO,NgayHetHan,NgayChungTu,SOLUONG from ERP_BUNDLE where PK_SEQ = "+ dhId;
			rs = db.get(query);
			if(rs.next())
			{
				String _spid=rs.getString("SANPHAM_FK");
				String _KHO_FK=rs.getString("KHO_FK");
				String _KBH_FK=rs.getString("KBH_FK");
				String _NPP_FK=rs.getString("NPP_FK");
				String _SOLO=rs.getString("SOLO");
				String _NGAYNHAPKHO=rs.getString("NgayChungTu");
				String _NGAYHETHAN=rs.getString("NgayHetHan");
				double soluong=rs.getDouble("SOLUONG");
				String msg1=util_kho.Update_NPP_Kho_Sp(_NGAYNHAPKHO, "(Tăng kho bundle)Chốt bundle t BoBundleSvl 462 ", db, _KHO_FK, _spid
						, _NPP_FK, _KBH_FK,  soluong,0, soluong, 0);
				if(msg1.length() >0){
					msg = msg1;
					db.getConnection().rollback();
					return msg;
				}
				msg1 =util_kho.Update_NPP_Kho_Sp_Chitiet(_NGAYNHAPKHO, "(Tăng kho bundleChốt bundle ct BoBundleSvl 469 ",   db, _KHO_FK, _spid, _NPP_FK, _KBH_FK, _SOLO, _NGAYHETHAN, _NGAYNHAPKHO,
						soluong, 0, soluong, 0, 0);
	...
	private String getSearchQuery(HttpServletRequest request,IBoBundleList obj) 
	{
		...
		String query =  "\n select a.pk_seq, a.ngaychungtu, d.ten as spTen, a.soluong, a.trangthai, b.ten as nguoitao, a.ngaytao, c.ten as nguoisua, a.ngaysua    " +
		"\n from Erp_BUNDLE a inner join NhanVien b on a.nguoitao = b.pk_seq    " +
