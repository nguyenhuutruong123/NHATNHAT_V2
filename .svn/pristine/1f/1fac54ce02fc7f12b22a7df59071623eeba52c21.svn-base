...
public class TrakhuyenmaiNpp implements ITrakhuyenmaiNpp 
{
	...
	private void createSpList() 
	{
		...
		if(this.trangthai.length() == 0)
		{
			...
			query = "select f.MA as spMa, f.PK_SEQ as spId, f.TEN as spTen, e.dongia,  g.AVAILABLE as hienhuu from Trakhuyenmai_sanpham a inner join " +
					"(select distinct c.SANPHAM_FK, c.GIAMUANPP as dongia from NHAPHANPHOI a inner join BANGGIAMUANPP_NPP b on a.PK_SEQ = b.NPP_FK " +
					"inner join BGMUANPP_SANPHAM c on b.BANGGIAMUANPP_FK = c.BGMUANPP_FK where a.PK_SEQ = '" + this.nppId + "' and c.GIAMUANPP > 0) e on a.sanpham_fk = e.SANPHAM_FK " +
					"inner join SANPHAM f on a.SANPHAM_FK = f.PK_SEQ inner join (select SANPHAM_FK, AVAILABLE from NHAPP_KHO where NPP_FK = '" + this.nppId + "' " +
