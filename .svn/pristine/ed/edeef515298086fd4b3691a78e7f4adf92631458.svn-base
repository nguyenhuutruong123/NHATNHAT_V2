 ...
 public class BanggiamuanppUpdateSvl extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String nextJSP = request.getContextPath() + "/pages/Center/BangGiaMuaNppUpdate.jsp";
        String copy = request.getParameter("copy");
        if(copy != null)
		{			
        	bgBean.setTungay("");
        	bgBean.setDenngay("");
			nextJSP = request.getContextPath() + "/pages/Center/BangGiaMuaNppNew.jsp";
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		{
			bgBean.setMessage("Vui lòng nhập ngày bắt đầu !");
			error = true;
		}
		
		/*if (denngay.trim().length()== 0)
		{
			bgBean.setMessage("Vui lòng nhập ngày kết thúc !");
			error = true;
		}*/
		...
		if(action.equals("save") & !error)
		{
			...
			if (id.length()==0){			
				...
				nextJSP = request.getContextPath() + "/pages/Center/BangGiaMuaNppUpdate.jsp";   						
			}
			response.sendRedirect(nextJSP);
	...
	private void CreateStaticHeader(Workbook workbook, String UserName) 
	{
	    ...
	    cell.setValue("Bảng giá mua NPP");
	    ...
	    cell.setValue("Gía bán cho NPP");
	    cell = cells.getCell("F8");
	    cell.setValue("Đơn vị");
	    
	    worksheet.setName("Bảng giá mua NPP");
	}
