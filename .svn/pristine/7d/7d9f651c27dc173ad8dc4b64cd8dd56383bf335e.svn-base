...
<script language="javascript" type="text/javascript">
	...
	function DinhDangDonGia(num) 
	{
	 ...
	 function TinhTien()
	 {
		...
		var slDonViChuan = document.getElementsByName("slDonViChuan");
		...
		for(var i = 0; i < spMa.length; i++)
		{
			if(spMa.item(i).value != "" && dongia.item(i).value != "" && soluong.item(i).value != "" )
			{
				...
				console.log('sl='+soluong.item(i).value.replace(/,/g,"")) ;
				console.log('qudoi='+spQuyDoi.item(i).value.replace(/,/g,"") );
				var slChuan = parseFloat(soluong.item(i).value.replace(/,/g,""))  *  parseFloat(spQuyDoi.item(i).value.replace(/,/g,""))
				console.log('slChuan='+slChuan);
				if(hdId.trim().length!=0)
					{
						slDonViChuan.item(i).value = DinhDangTien(slChuan);	
					
					}
				
				var tt = Math.floor( parseFloat(dongia.item(i).value.replace(/,/g,"")) * parseFloat(soluong.item(i).value.replace(/,/g,"")) - parseFloat(_ck));
	 ...
	 { 
		...
		var mahd = document.getElementById("mahopdong").value;
		var ddhId = document.getElementById("ddhId").value;
		...
		(
			{
		    url: "../../ErpDonhangNppETCSvl?type=GetDonGia&spMa=" + spMa.item(pos).value + "&dvdlId=" + dvdlId.item(pos).value + "&nppId=" + nppId+"&kbhId="+kbhId+"&dvkdId="+dvkdId+"&mahopdong=" + mahd + "&ddhId=" + ddhId,
		    type : 'GET',
		    dataType: 'json',
		    success: function( data )
		    {
		        var npp = data ;
		        console.log((spSoluongtongoc.item(pos).value));
		        console.log(npp.quycach);
		        console.log(parseFloat(spSoluongtongoc.item(pos).value.replace(/,/g, ""))/parseFloat(npp.quycach));
		        console.log(data);
		        ...
		        if(npp.dongia!='0' && npp.quycach!='0'){
		        	$( spSoluongton.item(pos)         ).val(DinhDangTien(parseFloat(roundNumber(parseFloat(spSoluongtongoc.item(pos).value.replace(/,/g, ""))/parseFloat(npp.quycach), 0))));
		        }
		        if(npp.dongia!='0' && npp.quycach=='0'){
		        	$( spSoluongton.item(pos)         ).val(DinhDangTien(parseFloat(spSoluongtongoc.item(pos).value)));
		        }
		        if(npp.dongia=='0' && npp.quycach=='0')
		        	$( spSoluongton.item(pos)         ).val('0');
		     }
		}); 
...
<input type="hidden" name="id" id="ddhId" value='<%= lsxBean.getId() %>'>
<input type="hidden" name="nppId" id="nppId" value='<%= lsxBean.getNppId() %>'>
<input type="hidden" name="hdId" id="hdId" value='<%= lsxBean.getMahopdong() %>'>


<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý bán hàng > Bán hàng ETC/Đối tác > Bán hàng cho ETC > Cập nhật
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
    	<A href= "../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"ErpDonhangNppETCSvl?userId="+ userId+"") %>" >
        	<img src="../images/Back30.png" alt="Quay ve"  title="Quay ve" border="1px" longdesc="Quay ve" style="border-style:outset" onclick="goBack()"></A>
        <span id="btnSave">
    ...
    <fieldset>
    	<legend class="legendtitle">Đơn hàng ETC </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0">	
            	<TR <%= lsxBean.getMahopdong().trim().length() > 0 ? "" : " style='display: none;' " %>   >
                    <TD class="plainlabel" >Mã hợp đồng </TD>
                    <TD class="plainlabel"  colspan="3" >
                    	<input type="text" name="mahopdong" id="mahopdong" value="<%= lsxBean.getMahopdong() %>" readonly="readonly" />
                    </TD>
                </TR>						
                <TR>
                    <TD width="150px" class="plainlabel" >Ngày đơn hàng </TD>
                    <TD width="300px"  class="plainlabel"  >
                    	<input type="text" class="days" readonly="readonly" onchange="submitform();"  id="hopdong_tungay" name="hopdong_tungay" value="<%= lsxBean.getTungay() %>"/>
                    </TD>
                ...
                <TD class="plainlabel" >Tổng thành tiền </TD>
                    ...
                    <TD class="plainlabel" style="display: none;" >
                    	<input type="text"  value="0" id="txtPTChietKhau" style="text-align: right;" name="ptChietkhau" readonly="readonly" />
                    </TD>
			...
			<hr />
				<table cellpadding="0px" cellspacing="1px" width="100%">
					<tr class="tbheader">
						...
						<th align="center" width="17%" rowspan="2" >Tên sản phẩm</th>
						<th align="center" width="10%" rowspan="2" >Đơn vị</th>
						<th align="center" width="8%" rowspan="2" >Số lượng</th>
						<%if(lsxBean.getMahopdong().trim().length()>0){ %>
							<th align="center" width="8%" rowspan="2" >Số lượng quy đổi</th> 
						<%} %>
					 	<th align="center" width="8%" rowspan="2" >Số lượng tồn</th>
					 	<th align="center" width="8%" rowspan="2" >Đơn giá</th>
						<th align="center" width="8%" rowspan="2" style="display: none;" >Chiết khấu</th>
					...
					<%
						...
						if(spMa != null)
						{
							...
							<tr>
								...
								<% } else { %>
									<input type="text" name="spMa" value="<%= spMa[i] %>" style="width: 100%"   AUTOCOMPLETE="off"  > 
								<% } %>	
								...
								<td> 
									<input type="text" name="soluong" value="<%= spSoluong[i].equals("0") ? "" : spSoluong[i] %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > 
								</td>
								<%if(lsxBean.getMahopdong().trim().length()>0){ %>
									<td><input readonly type="text" name="slDonViChuan" value="<%=Utility.parseDouble(spSoluong[i].replace(",","")) * Utility.parseDouble(spQuyDoi[i].replace(",",""))%>" 
								...
								<%} %>
							  	...
							  	<td> 
									<input type="text" name="soluongton" value="<%=spSoluonton[i]==null ? "": spSoluonton[i]%>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" readonly > 
								</td>
							<input  style="display:none;" type="text"  name="soluongtongoc" value="<%=spSoluonton[i]==null ? "": spSoluonton[i]  %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" >	
				 			  	<td> <input type="text" name="dongia" value="<%= spGianhap[i] %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getMahopdong().trim().length() > 0 ? " readonly='readonly' " : "  " %>  > </td>
					...
					<% for(int i = count; ( i < 20 && lsxBean.getMahopdong().trim().length() <= 0 ); i++) { %>
						
						<tr>
							<td>
								<input type="text" name="spMa" value="" style="width: 100%"  onkeyup="ajax_showOptions(this,'nhapkho',event)" AUTOCOMPLETE="off" <%if(lsxBean.getMahopdong().trim().length()>0){ %> readonly="readonly" <%} %>  > 
								<input type="hidden" name="spTrongLuong" value="" > 
								<input type="hidden" name="spTheTich" value="" > 
								<input type="hidden" name="spQuyDoi" value=""  >
								<input type="hidden" name="spStt" value="99"  >
							</td>
							...
							<%if(lsxBean.getMahopdong().trim().length()>0){ %>
									<td><input readonly type="text" name="slDonViChuan" value="" 
											style="width: 100%; text-align: right;" > 
									</td> 
								<%} %>
							<td> <input type="text" name="soluongton" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > </td>
						 <input style="display:none;" type="text" name="soluongtongoc" value="0" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" >
					 		<td> <input type="text" name="dongia" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getMahopdong().trim().length() > 0 ? " readonly='readonly' " : "  " %> > </td>
