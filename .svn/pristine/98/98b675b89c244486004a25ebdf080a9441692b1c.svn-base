...
public class PhieuthuhoiPdfSvl extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
	    ...
	    pthBean.init();
	    List<ISanphamthuhoi> spList = pthBean.getSanphamList();
	    List<ISanphamthuhoi> spkmList = pthBean.getSpkmList();
	...
	private void CreatePth(Document document, ServletOutputStream outstream, String id, List<ISanphamthuhoi> spList, List<ISanphamthuhoi> spKMList, IPhieuthuhoi pthBean) throws IOException
	{		
		try 
		{
			...
			float[] withs = {10.0f, 15.0f, 30.0f, 7.0f, 10.0f, 10.0f};
	        ...
	        if(spList.size() > 0)
	        {
				...
				String[] th = new String[]{"STT", "Mã sản phẩm", "Tên sản phẩm", "Số lượng", "Diễn giải","Số lô"};
				...
				{ 
					ISanphamthuhoi sanpham = (ISanphamthuhoi)spList.get(i);
					String[] arr = new String[]{Integer.toString(i + 1), sanpham.getMasanpham(), sanpham.getTensanpham(), sanpham.getSoluong(), "",sanpham.getSOLo()};
			...
			if(spKMList.size() > 0)
			{
				...
				PdfPTable tableKM = new PdfPTable(7);
				tableKM.setWidthPercentage(100);
				tableKM.setHorizontalAlignment(Element.ALIGN_LEFT);
				float[] withsKM = {10.0f, 15.0f, 45.0f, 10.0f, 20.0f, 20.0f,20.0f};
		        tableKM.setWidths(withsKM);
		        
				String[] thKM = new String[]{"STT", "Mã sản phẩm", "Tên sản phẩm", "Số lượng", "Kho", "Kênh bán hàng","Số lô"};
				PdfPCell[] cellKM = new PdfPCell[7];
				for(int i=0; i < 7 ; i++)
				{
				...
				for(int i = 0; i < spKMList.size(); i++)
				{
					...
					ISanphamthuhoi sanpham = (ISanphamthuhoi)spKMList.get(i);
					String[] arr = new String[]{Integer.toString(i + 1), sanpham.getMasanpham(), sanpham.getTensanpham(), sanpham.getSoluong(), sanpham.getKhoTen(), sanpham.getKenhTen(),sanpham.getSOLo()};
					Font font4 = new Font(bf, 8);
					PdfPCell cells = new PdfPCell();
					for(int j=0; j <7; j++)
					{
