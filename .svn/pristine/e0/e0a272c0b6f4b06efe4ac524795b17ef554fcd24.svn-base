...
<head>
    ...
    function replaces()
	{
		...
		function saveform()
		{
			...
			if(ngayTds > ngayKtTds)
			{
				alert('Lỗi. Bạn phải chọn ngày kết thúc sau ngày bắt đầu của ngày tính doanh số...');
				return;
			}
			...
			if(ngayTb > ngayKtTds)
			{
				alert('Lỗi. Bạn phải chọn ngày kết thúc sau ngày bắt đầu của ngày trưng bày...');
				return;
			}
			...
			if(checkNpp() == false)
			{
				alert('Lỗi. Không có NPP nào được chọn. Vui lòng chọn NPP...');
				return;
			}
