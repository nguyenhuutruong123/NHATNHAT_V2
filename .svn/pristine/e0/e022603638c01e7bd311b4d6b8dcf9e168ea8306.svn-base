...
public class Nhomthuong implements INhomthuong, Serializable
{
	...
	public boolean saveNewNkm()
	{
		...
		command = "insert into NHOMSANPHAMCHITIEU(tungay,denngay,diengiai,nsp_parent_fk,loaithanhvien,ngaytao,ngaysua,nguoitao,nguoisua,trangthai,ten,type,loainhom,NHOMSPTT) " +
				"	values( '"+this.tungay+"', '"+this.denngay+"',N'" + this.diengiai + "', '0', '2', '" + this.ngaytao + "', '" + this.ngaysua +"', " +
				"	'" + this.nguoitao + "', '" + this.nguoisua + "', '0', N'" + this.ten + "','" + this.type + "','0','" + this.NspTT + "')";
		...
		command = "select SCOPE_IDENTITY()as nkmId";		
		ResultSet rs = this.db.get(command);
			...
			if(this.sanpham != null){
				...
				while(m < size){
					command = "insert into NHOMSANPHAMCHITIEU_sanpham(sp_fk, nsp_fk) values('" + sanphamList[m] + "', '"+ this.id + "')";
				...
				if(sqlcheck.next())
				{
					...
					{
						command = "update a set a.sanpham_fk = c.PK_SEQ,a.spma = c.MA,a.spten = c.TEN from NHOMSANPHAMCHITIEU a inner join NHOMSANPHAMCHITIEU_SANPHAM b on a.PK_SEQ = b.NSP_FK inner join SANPHAM c on c.PK_SEQ = b.SP_FK and a.PK_SEQ =" +this.id;
	...
	public boolean updateNkm(){
		...
		command ="update NHOMSANPHAMCHITIEU set sanpham_fk = null, spma= null,spten = null , tungay ='"+ this.tungay +"' ,denngay = '"+ this.denngay +"', ten =N'"+ this.ten +"', diengiai = N'"+ this.diengiai +"',ngaysua ='"+ this.ngaysua +"' , nguoisua ='"+this.nguoisua+"', NHOMSPTT = '"+this.NspTT+"' " +
	...
	private ResultSet createDvkdRS(){  	
		...
		ResultSet dvkdRS =  this.db.get("select a.pk_seq, a.diengiai from donvikinhdoanh a,Nhacungcap_dvkd b where a.pk_seq = b.dvkd_fk and a.trangthai='1' ");
