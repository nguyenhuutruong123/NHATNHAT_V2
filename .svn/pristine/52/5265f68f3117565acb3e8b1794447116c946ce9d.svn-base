...
<%@ page import="geso.dms.center.beans.nhomhang.INhomHangList"%>
...
<%
	...
	else
	{
		...
		INhomHangList obj = (INhomHangList) session.getAttribute("obj");
		ResultSet npplist = (ResultSet) obj.getNhomRs();
	
		obj.setNextSplittings();
		int[] quyen = new  int[6];
		quyen = util.Getquyen("NhomHangSvl","",userId);
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="nppForm" method="post" action="../../NhomHangSvl">
										...
										<TD width="100%" align="center">
											<FIELDSET>
												...
												<TABLE width="100%" cellpadding="6" cellspacing="0">
													<tr>
														<TD width="22%" class="plainlabel"><%=Utility.GLanguage("Mã nhóm",session,jedis) %></TD>
														<TD colspan="3" class="plainlabel">
														<INPUT name="ma" id="ma" type="text" value="<%=obj.getMa()%>" onChange="submitform();"></TD>
													</TR>

													<tr>
														<TD width="22%" class="plainlabel"><%=Utility.GLanguage("Tên nhóm",session,jedis) %></TD>
														<TD colspan="3" class="plainlabel">
															<INPUT name="ten" id="ten" type="text" value="<%=obj.getTen()%>" onChange="submitform();">
														</TD>
													</TR>
													<TR>
							...
							<TD width="100%">
								<FIELDSET>
									<LEGEND class="legendtitle">
										<%=Utility.GLanguage("Nhóm sản phẩm",session,jedis) %>
									...
									<TABLE class="" width="100%">
										<TR>
												...
												<TABLE width="100%" id="table" class="tabledetail sortable" border="0" cellspacing="1" cellpadding="4">
													<thead>
														<TR class="tbheader">
															...
															<TH width="13%"><%=Utility.GLanguage("Mã nhóm",session,jedis) %></TH>
															<TH width="13%"><%=Utility.GLanguage("Tên nhóm",session,jedis) %></TH>
																	...
																	<TR>
																		<TD>
																			<%if(quyen[Utility.SUA]!=0) {%>
																			 <A href="../../RouterSvl?task=<%= Util.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NhomHangUpdateSvl?userId="+userId+"&update="+npplist.getString("id")+"")%>"><img src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" longdesc="Cap nhat" border=0></A> <%
 																			}%>
																		</TD>
																		<TD>
																			<%
																			if(quyen[Utility.XOA]!=0)
																							{
																			%> <A href="../../RouterSvl?task=<%= Util.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NhomHangSvl?userId="+userId+"&delete="+npplist.getString("id")+"")%>"><img src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0
																		...
																		{%>
																			 <A href="../../RouterSvl?task=<%= Util.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"NhomHangUpdateSvl?userId="+userId+"&display="+npplist.getString("id")+"")%>"><img src="../images/Display20.png" alt="Hien thi" width="20" height="20" longdesc="Hien thi" border=0></A>
																		<%}%>
																		</TD>
										 ...
										 <TR>
											<TD height="26" colspan="8" align="center" class="tbfooter">&nbsp;	</TD>
										</TR>
									</TABLE>
