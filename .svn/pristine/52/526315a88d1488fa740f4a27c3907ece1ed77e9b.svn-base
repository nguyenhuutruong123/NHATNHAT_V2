...
public class ErpHoadontaichinhNPPList extends Phan_Trang implements IErpHoadontaichinhNPPList
{
	...
	public void init(String search)
	{
		...
		else
		{
			...
			{
			query = "select isnull(a.loaihd,0) loaihd ,a.PK_SEQ, a.trangthai, a.ngayxuatHD,  RTRIM(LTRIM (a.sohoadon)) as sohoadon, a.kyhieu as kyhieu, NV.TEN as nguoitao, a.tongtienavat as tongtien ,isnull(kb.ten,'') as kenhbanhang,  " +
					...
					" left join kenhbanhang kb on kb.pk_seq=a.kbh_fk "+
					 "where  isnull(loaihd,0)<>1 and    dh.isKM <>1 and a.pk_seq > 0 and a.npp_fk ='"+ this.nppId +"' ";
			}
			else
			{
				query = "select isnull(a.loaihd,0) loaihd, a.PK_SEQ, a.trangthai, a.ngayxuatHD, RTRIM(LTRIM (a.sohoadon)) as sohoadon, a.kyhieu as kyhieu, NV.TEN as nguoitao, a.tongtienavat as tongtien ,isnull(kb.ten,'') as kenhbanhang,  " +
						...
						"inner join NHANVIEN nv2 on a.NGUOISUA = nv2.PK_SEQ "+
						" left join kenhbanhang kb on kb.pk_seq=a.kbh_fk "+
						 "where (isnull(loaihd,0)=1 or    dh.isKM =1 ) and a.pk_seq > 0 and a.npp_fk ='"+ this.nppId +"'  ";
		...
		System.out.println(" qr danh sach khach hang" + query);
		...
		query = " select a.PK_SEQ as PK_SEQ, a.mahopdong as sohopdong "+    
				 " from ERP_HOPDONGNPP a  left join KHACHHANG c on a.khachhang_FK = c.pk_seq  "+    
				 " where a.npp_fk = " +this.nppId;
		System.out.println(" qr danh sach hop dong: " + query);
