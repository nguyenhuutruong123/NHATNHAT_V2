...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <fieldset  >
    ...
    <% if(xlkm.getDieuchinh() == false && xlkm.getDungDieuKien() == true){ %>
    	<tr><td><input type="checkbox" name="ShowAll" checked="checked" value='1' onchange="submitform()">Hiện tất cả các xuất khuyến mãi ( tối đa ) có thể đạt được của đơn hàng cua don hang</td></tr>
    <%}else{ %>
...
<fieldset>
    ...
    <table class="tabledetail" id="content" width="100%" border="0" cellspacing="1" cellpadding="2">
    	<tr class="tbheader">
            ...
            <th width="17%" align="center"><%=Utility.GLanguage("Sản phẩm",session,jedis) %></th>
					...
					for(int k = 0; k < spList.size(); k++)
					{
						...
						if(k == 0)
						{
					       ...
					       <% if(dkkm.getTongtien() > 0 || dkkm.getTongluong() > 0){ %>
       							...
       							%>	           								
           								<fieldset>
           								<legend><a class="addspeech" href="" title="<%= trakm2.getDiengiai()%>" onclick="return false;"><%= trakm2.getId()%> <b> - Tổng lượng KM <%= trakm2.getTongluong() * ctkm.getSoxuatKM() %></b></a></legend>
	           							...
	           							<%}%>
           									<input type="checkbox" id="<%= ctkm.getscheme() + "." + trakm2.getId() + ".auto"%>" onchange="Auto('<%= ctkm.getscheme() + "." + trakm2.getId() + ".auto" %>')"><i style="font-size:0.7em"> Tự động chọn</i>
