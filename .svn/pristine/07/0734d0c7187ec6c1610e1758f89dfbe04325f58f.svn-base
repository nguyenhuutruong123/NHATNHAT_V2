...
public class ThongbaoList extends Phan_Trang implements IThongbaoList
{
		...
		String sql="";
		 if(query.trim().length() <= 0)
		 {
			 sql= "select  ct.pk_seq,ct.trangthai, ct.tinhtrang, ct.noidung,ct.tieude,ct.ngaybatdau,ct.filename, case when len(ct.ngayketthuc) <= 0 then N'Vô thời hạn' else ct.ngayketthuc end as ngayketthuc," +
			 	  ...
			 	  "from thongbao ct inner join NHANVIEN nv on ct.nguoitao = nv.PK_SEQ inner join NHANVIEN nv2 on ct.NGUOISUA = nv2.PK_SEQ where 1=1 ";
			 
				/*
				 * if(this.loaithongbao.trim().length() > 0) sql += " and ct.loaithongbao = '" +
				 * this.loaithongbao + "' "; else sql += " and ct.loaithongbao != '5' "; //chi
				 * lay Van ban, khong lay thong bao
		 ...
		 System.out.println("Init Thong bao: " + sql);
	...
	public void initNhanvien(String query) 
	{
		 ...
		 if(query == "")
		 {
			 sql = "select a.pk_seq,a.tieude,a.filename,a.noidung,b.trangthai,a.ngaybatdau,a.ngayketthuc,a.ngaytao,a.nguoitao,a.ngaysua,a.nguoisua, a.loaithongbao, c.Ten as tennv " +
			 	   "from   thongbao a inner join thongbao_nhanvien b on a.pk_seq = b.thongbao_fk " +
			 	   "	   inner join nhanvien c on a.nguoitao = c.pk_seq	"+
			 	   "where b.trangthai != '2' and nhanvien_fk = '" + this.NHANVIEN + "' ";
			 /*if(this.viewMode.equals("0"))
		 ...
		 System.out.println("lay thong bao nhan vieb"+sql);
