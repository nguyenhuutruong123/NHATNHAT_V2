...
<% 
   ...
   ResultSet nganhhang = obj.getNganhHangRs();
  ResultSet gsbh = obj.getgsbh();
  String url = util.getChuyenNguUrl("SecondarySalesPIRTT", "",session);
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="frm" method="post" action="../../SecondarySalesPIRTT">
	...
	<input type="hidden" name="userId" value='<%=userId%>'>
		<div id="main" style="width: 99%; padding-left: 2px">
			...
			<div align="left"
				...
				<fieldset>
					<legend class="legendtitle"><%=Utility.GLanguage("Tiêu chí lấy báo cáo",session,jedis) %></legend>
						...
						<div style="width: 100%; float: none" align="left"
							...
							<TABLE width="70%" cellpadding="6" cellspacing="0">
								...
								<TD class="plainlabel"><%=Utility.GLanguage("Kênh bán hàng",session,jedis) %></TD>
									...
									<TD class="plainlabel"><select name="dvkdId" id="dvkdId" onchange="seach();" style="width:202px">
											<option value="" selected>All</option>
											<% if(dvkd != null)
											   while(dvkd.next()){
									...
									<TD class="plainlabel">
										<select name="vungId" id="vungId" onchange="seach();" style="width:202px">
											...
											<% if(vung != null)
											   while(vung.next()){
											   if(vung.getString("pk_seq").trim().equals(obj.getvungId()))
											   { %>
									...
									<select name="khuvucId" id="khuvucId" onchange="seach();" style="width:202px">
											<option value="" selected>All<%=Utility.GLanguage("",session,jedis) %></option>
											<% if(khuvuc != null)
											   while(khuvuc.next()){
                             ...
                             <TD class="plainlabel"><%=Utility.GLanguage("Nhà phân phối",session,jedis) %></TD>
									...
									<TD class="plainlabel"><select name="nganhhangId" id="nganhhangId"  style="width:200px;" >
											<option value="" selected>All</option>
												<% if(nganhhang != null)
											   while(nganhhang.next()){
											   if(nganhhang.getString("pk_seq").equals(obj.getNganhHangId()))
											   { %>
