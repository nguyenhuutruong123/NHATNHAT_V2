...
public class TieuchithuongTLUpdateSvl extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		{
			tctskuBean.init();
			nextJSP = request.getContextPath() + "/pages/Center/TieuChiThuongTLDisplay.jsp";
		}
		else if(querystring.indexOf("copy") >= 0)
		{
			...
			nextJSP = request.getContextPath() + "/pages/Center/TieuChiThuongTLNew.jsp";
		}
		else
		{
			tctskuBean.init();
			nextJSP = request.getContextPath() + "/pages/Center/TieuChiThuongTLUpdate.jsp";
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try
		{
			...
			String[] slspMua = request.getParameterValues("slspTra");
			if(maspMua != null)
			for(int i = 0; i < maspMua.length ;i ++)
			{
				if(maspMua[i] != null && maspMua[i].trim().length() > 0)
				{
					...
					try
					{
						sl = "" + Double.parseDouble( slspMua[i].replace(",","")  );
					}
					catch (Exception e) {
						
					}
					tctskuBean.getSpMuaList().add(maspMua[i] +"#_#_#"+sl);
				}
		...
		{    
			if (id == null )
			{
				if (!tctskuBean.createTctSKU())
				{
					...
					String nextJSP = request.getContextPath() + "/pages/Center/TieuChiThuongTLNew.jsp";
					response.sendRedirect(nextJSP);
				}
				else
				{
			...
			else
			{
				...
				{			
					...
					String nextJSP = request.getContextPath() + "/pages/Center/TieuChiThuongTLUpdate.jsp";
					response.sendRedirect(nextJSP);
				}
				else
				{
		...
		if (filename.length() > 0)
		{
			...
			try
			{
						...
						if(npp.trim().length() >0)
							for(int z= 0; z < count; z ++)
							{
								...
								if(values.trim().length() > 0)
									values += "\n  union all select "+npp+" as nppId, "+muc+" as muc, "+sosuat+" as [sosuat] ";
								else
									values = "\n  select "+npp+" as nppId, "+muc+" as muc, "+sosuat+" as [sosuat] ";
				...
				if(values.trim().length() <= 0)
				{
					...
					Redirect(tctskuBean ,request.getContextPath() + "/pages/Center/TieuChiThuongTLUpdate.jsp",  response, session);
					return;
				}
