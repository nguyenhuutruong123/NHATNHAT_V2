...
<% 
	...
	Hashtable<String, List<ICtkm>> ctkm = lsxBean.getCtkm();
...
<script language="javascript" type="text/javascript">
	...
	function replaces()
	{
	 ...
	 function layctkm()
	 {   
		document.forms["hctmhForm"].action.value = "laykm";
	    document.forms["hctmhForm"].submit();
	 }
...
<form name="hctmhForm" method="post" action="../../DontrahangTTUpdateSvl">
    ...
    <fieldset>
    	<legend class="legendtitle">Trả hàng </legend>
			...
			<table cellpadding="0px" cellspacing="1px" width="100%">
				<tr class="tbheader">
					<% if(lsxBean.getKhoXuatId().equals("100001")){	 %>  
					...
					<th align="center" width="15%"  >Chương trình khuyến mãi</th>
					...
					if(spMa != null )
					{
						...
						<tr>
							...
							<td > <input type="text" name="thanhtien" value="" style="width: 100%; text-align: right;" readonly="readonly" > </td>
							 <%
							if(lsxBean.getKhoXuatId().equals("100001")){	
								List<ICtkm> scheme = ctkm.get(spMa[i]);
							%> 
							<td > <!-- <input type="text" name="scheme" value="" style="width: 97%; text-align: right;" readonly="readonly" > -->
									<select name="scheme" style="width: 100%;"  >
										...
										{ 
											...
											{
												System.out.println("........ "+scheme.get(j).getId());
											if(spSCheme[i].equals(scheme.get(j).getId()))
												{%>
													<option value="<%= scheme.get(j).getId() %>" selected="selected" ><%= scheme.get(j).getScheme() %></option>
												<% } else { %>
													<option value="<%= scheme.get(j).getId() %>" ><%= scheme.get(j).getScheme() %></option>
											   <% } }
										} %>
								 	</select>
