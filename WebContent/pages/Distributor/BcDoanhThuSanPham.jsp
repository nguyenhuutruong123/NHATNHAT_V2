...
String url = util.getChuyenNguUrl("BcDoanhThuSanPhamSvl", "&View=TT",session);
%>
...
<form name="ckParkForm" method="post" action="../../BcDoanhThuSanPhamSvl">
...
<div class="box" style="border: 1px solid #dddbdb; background-color: #C5E8CD;">
	...
	<div class="box-header">
		<div class="form-group col-md-6">
		  <h3 class="box-title"><%=Utility.GLanguage("Doanh Số theo sản phẩm",session,jedis) %> </h3>
		</div> 
	</div>
	...
	<div class="box-body">
        ...
        String[] arrXem = {"Khách hàng", "Nhân viên bán hàng", "Chi nhánh/NPP"};
        
        %>
         <div class="col-md-4"><label><%=Utility.GLanguage("Xem theo",session,jedis) %></label></div> 
          		<select class="select2" name="laytheo"  id="laytheo" style="width:250px"  onchange="submitform();">
          			<% for(int x = 0; x < arrXem.length ; x ++) { %>
          				<option value="<%=x%>" <%=(obj.getLaytheo().equals( (x+"") ) ? "selected" :"")  %>  ><%=arrXem[x] %></option>
...
<table id="example2" class="table table-bordered table-hover table-striped">
  <thead class="colorheader">
     <tr>
       ...
       <th><%=Utility.GLanguage("CHI NHÁNH ĐỐI TÁC",session,jedis) %></th>
       <%if(!obj.getLaytheo().equals("2")){ %>
       ...
       <th><%=Utility.GLanguage("TÊN KH",session,jedis) %></th>
       <th><%=Utility.GLanguage("ĐỊA CHỈ KHÁCH HÀNG",session,jedis) %></th> 
       <%} %>
...
function readyFunc()
{
	if ( $.fn.DataTable.isDataTable('#example2') ) 
	{
     ...
     $('#example2').DataTable({
    	"sAjaxSource": "../../AjaxDistributionTT?userId="+nhanvien+"&laytheo="+laytheo+"&tungay="+ tungay +"&denngay="+ denngay +"&vungId="+ vungId +"&kenhId="+ kenhId +"&spId="+ spId +"&nppId="+ nppId +"&nhomId="+ nhomId +"&action=DoanhSoTheoSP",
    	"dom": 'C<"clear">lfrtip',
    	"serverSide": true,
	    "responsive": true,
	    "searching": false,
