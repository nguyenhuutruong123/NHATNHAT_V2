	...
	function thue_loaikhac_new(){
		...
		var count = document.getElementById("count").value;
		 ...
		 for(var i = 0; i < count; i++)
		 {
			 ...
			 if(tienthue != ''){
				 tienthue = parseFloat(tienthue);	
				}else{
					tienthue = 0;
				}	
				 
			 tienthueVATct = parseFloat(tienthue);
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý công nợ > Công nợ phải trả > Phiếu chi  > Hiển thị
        </div>
    ...
    <div align="left" style="width:100%; float:none; clear:left">
    <fieldset>
    	<legend class="legendtitle"> Phiếu chi  </legend>
	        ...
	        <%if(tthdBean.getDoiTuongChiPhiKhac().equals("1")){ %>
	         ...
	         <%}else if(tthdBean.getDoiTuongChiPhiKhac().equals("0")){ // LOAITT: KHÁCH HÀNG TRẢ TRƯỚC >> LOAD NV %>
						<TR>
						  <TD width="20%" class="plainlabel" valign="top">Tên nhân viên </TD>					        
			...
			<%} else if(tthdBean.getDoiTuongChiPhiKhac().equals("2") || tthdBean.getDoiTuongChiPhiKhac().equals("5")){ // LOAITT: KHÁCH HÀNG TRẢ TRƯỚC >> LOAD KH %>
						<TR>
							<TD width="20%" class="plainlabel" valign="top">Tên khách hàng </TD>
							<TD class="plainlabel" colspan="4">
            ...
            <% if(!tthdBean.getTienteId().equals("100000")){ %>
            	<TR>
                   <%if(tthdBean.getDoiTuongChiPhiKhac().equals("3")){  %>
                    ...
                    <TD class="plainlabel">                   
                       <input type="text" style="text-align: right;" name="sotienHDNT" id = "sotienHDNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getSotienHDNT().replaceAll(",",""))) %>" onchange="ThanhToan(100)" onKeyPress = "return keypress(event);" readonly>
	...
	<%}else{ %>
			...
			<%} else  {%>
					...
					<TD align = "left" class="plainlabel" colspan = 3>
	                   ...
	                   <DIV id="subcontentVAT" style="position:absolute; visibility: hidden; border: 2px solid #80CB9B; background-color: white; width: 900px; padding: 4px; max-height: 300px; overflow: auto; ">
                         ...
                         <input type="text" style="text-align: right;width:181px" name="vatVND" id = "vatVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getThueVAT().replaceAll(",",""))) %>" onchange="ThanhToan(100)" onKeyPress = "return keypress(event);" readonly="readonly" >
	        ...
	        <% if(tthdBean.getTienteId().equals("100000")){ // Don vi tien la VND %>
                <TR class="tbheader"> 
                	...
                	<TH align="center" width="8%">Mã đối tượng</TH>
                	<TH align="center" width="8%">Ký hiệu HĐ</TH>
                	<TH align="center" width="8%">Số HĐ</TH>
                	<TH align="center" width="8%">Ngày HĐ</TH>
                	<TH align="center" width="10%">Số tiền gốc HĐ</TH>
			  ...
			  <!-- KÊNH BÁN HÀNG  -->
				...
				<TD>
					<%if(Tienthuehd[i]!=null && Tienthuehd[i].trim().length()>0) {%>	
						<input type="text" name= "tienthue_VAT_<%=i %>"  id= "tienthue_VAT_<%=i %>" value = "<%= formatter2.format(Double.parseDouble(Tienthuehd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" onkeyup = "javascript:thue_loaikhac_new();">
					<%} else { %>
						<input type="text" name= "tienthue_VAT_<%=i %>" id= "tienthue_VAT_<%=i %>"  value = "<%= Tienthuehd[i] %>" style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" onkeyup = "javascript:thue_loaikhac_new();">
					<%} %>
				</TD>  		                    
			</TR>
     <% }
		...
		<TR>
        	...
        	<%  } %> 
				...
				<TD>
					...
					<%} else { %> --%>
						<input type="text" name= "tienthue_VAT_<%=i %>" id= "tienthue_VAT_<%=i %>" value = '' style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" onkeyup = "javascript:thue_loaikhac_new();">
					<%-- <%} %> --%>
				</TD>  		                         		
	</TR>
	<%
