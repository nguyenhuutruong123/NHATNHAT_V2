...
<%@ page import="geso.dms.distributor.beans.hoadonphelieu.IErpHoadonphelieuList"%>
                         		...
                         		{ 
                         			while(csxRs.next())
                         			{
        		                        ...
        		                        <%} %>
                                                ...
                                                <% if( tt.equals("0") ) { %>
                                                  <% if(quyen[2] == 1) { %>
							                   		<A href = "../../RouterSvl?task=<%= Utility.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"ErpHoadonphelieuUpdateSvl?userId="+userId+"&update="+csxRs.getString("pk_seq")) %>"><img src="../images/Edit20.png" alt="Cập nhật" width="20" height="20" longdesc="Cập nhật" border = 0></A> &nbsp;
							                   		<%} %>
							                   		<% if(quyen[4] == 1) { %>
							                   		<A href = "../../RouterSvl?task=<%= Utility.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"ErpHoadonphelieuSvl?userId="+userId+"&chot="+csxRs.getString("pk_seq")) %>"><img src="../images/Chot.png" alt="Duyệt" width="20" height="20" longdesc="Duyệt" border=0 onclick="if(!confirm('Bạn Muốn Duyệt Hóa Đơn Này?')) return false;"></A>&nbsp;
							                   		<%} %>
							                   		<% if(quyen[1] == 1) { %>
													<A href = "../../RouterSvl?task=<%= Utility.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"ErpHoadonphelieuSvl?userId="+userId+"&delete="+csxRs.getString("pk_seq")) %>"><img src="../images/Delete20.png" alt="Xóa" width="20" height="20" longdesc="Xóa" border=0 onclick="if(!confirm('Bạn Muốn Xóa Hóa Đơn Này?')) return false;"></A>
													<%} %>
													
												<% } else { %>
													<A href = "../../RouterSvl?task=<%= Utility.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"ErpHoadonphelieuUpdateSvl?userId="+userId+"&display="+csxRs.getString("pk_seq")) %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>&nbsp;
												<% }  %>												
