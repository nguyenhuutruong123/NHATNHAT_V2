...
function TinhTien()
{
	...
	for(var i = 0; i < thanhtien.length; i++)
	{
		...
		if( don_gia != '' && so_luong != '' )
		{
			var tienkm = 0;
			if( $('#km_'+ _ma).val() )
				tienkm =  parseFloat($('#km_'+ _ma).val());				
			tongkm = tongkm + tienkm;
			
			
			var tt =  parseFloat(don_gia) * parseFloat(so_luong) - parseFloat(tienkm) ;
			var _bvat = Math.round(tt) ;
			var _tienvat = Math.round (  _bvat  * (  parseFloat(_ptvat)/100.0   ) )
				
			var _avat = _tienvat + _bvat ;
	...
	//document.getElementById("SoTienCKKM").value = DinhDangTien( tongkm );
	$('#SoTienCKKM').val( DinhDangTien( tongkm ))
	...
	var tonggiatri =  tongtien  - parseFloat(tiengiamtru) ; 
 ...
 congDonSPCungMa();
		...
		if(khoId.value == "")
		{
		 ...
		 document.forms['dhForm'].action.value='khongapkhuyenmai';
...
<input type="hidden" name="dieuchinh" value='<%= dhBean.getDieuchinh() %>'>
		...
		<input type="hidden" name="ngaykhoaso" id="ngaykhoaso"
			...
			value='<%= dhBean.getTrangthai() %>'> <INPUT type="hidden"
			name="khoTen" value='<%= dhBean.getKhoId() %>'> <input
...
<%
ResultSet km = dhBean.getPhanboKM();
if(km!= null)
while(km.next())
{
	%>
	<input type="hidden"  id="km_<%=km.getString("ma") %>" value='<%=km.getString("tienkhuyenmai") %>'>
	<%
}
										...
										<TR>
											<TD align="left">
												<TABLE width="100%" border="0" cellpadding="0"
													cellspacing="0">
													<TR height="22">
														<TD align="left" class="tbnavigation">Quản lý bán hàng > Bán hàng OTC > Bán hàng cho khách hàng > Cập nhật
														</TD>
									...
									<TABLE width="100%" border="0" cellpadding="1" cellspacing="0">
										<TR>
											<TD>
										   	...
										   	<A href="javascript:khongapkhuyenmai()" ><img src="../images/Save30.png" alt="Luu lai"  title="Luu lai" border="1" longdesc="Luu lai" style="border-style:outset"></A>
														...
														<TR class='tblightrow'>
															...
															<%} %>
															  ...
															  <%  //System.out.println(" Da vao +.+ "); %>
									    						...
									    						<TD align="center"><input readonly="readonly" type="text" name="dongiagoc" onkeyup="FormatTien(this);" value="<%=formatter2.format(Double.parseDouble( sanpham.getDongiagoc()))%>" style="width: 100%; text-align: right; color: black;">
																		<input type="hidden" name="dongia1" value="<%= sanpham.getDongiagoc() %>" style="width: 100%; text-align: right">
																		<input type="hidden" name="dongia"  value="<%= sanpham.getDongiagoc() %>" style="width: 100%; text-align: right">
																</TD>
