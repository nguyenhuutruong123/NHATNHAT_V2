...
<%
	IThayDoiKhuyenMai tdkmBean = (IThayDoiKhuyenMai) session
			.getAttribute("tdkmBean");
%>
...
%>
<%
	String url = util.getChuyenNguUrl("ThayDoiKhuyenMaiUpdateSvl", "",
			session);
%>
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="trakmForm" method="post"
		action="../../ThayDoiKhuyenMaiUpdateSvl">
		<%
		...
		<div id="main" style="width: 99%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 55%; padding: 5px; float: left"
					class="tbnavigation">
					Quản lý khuyến mãi > Khai báo > Thêm sản phẩm CTKM
					>
			...
			<div align="left" id="button"
				...
				<A href="javascript:saveform()"> <IMG src="../images/Save30.png" title="Luu lai" alt="Luu lai" border="1px" style="border-style: outset"></A>
			</div>
			...
			<div align="left" style="width: 100%; float: none; clear: left">
				<fieldset>
					...
					<textarea name="dataerror"
						style="width: 100%; color: #F00; font-weight: bold" rows="1"
						style="width: 100%" readonly="readonly" readonly="readonly"><%=tdkmBean.getMsg()%></textarea>
					<%
						tdkmBean.setMsg("");
					%>
				</fieldset>
			...
			<div align="left" style="width: 100%; float: none; clear: left">
				<fieldset>
					<legend class="legendtitle">
						<%=Utility.GLanguage("Thêm sản phẩm CTKM", session, jedis)%>
					</legend>
					<div style="float: none; width: 100%" align="left">
						<TABLE width="100%" cellpadding="6" cellspacing="0">
							<TR>
								<TD class="plainlabel" width="15%"><%=Utility.GLanguage("Loại", session, jedis)%></TD>
								<TD class="plainlabel">
								<select name="loai" id="loai" style="width: 250px;" onChange="submitform()" class="select2">
									<option value=""></option>
									<% if (tdkmBean.getLoai().equals("1")) { %>
										<option value="1" selected><%=Utility.GLanguage("Điều kiện KM", session, jedis)%></option>
										<option value="2"><%=Utility.GLanguage("Trả KM", session, jedis)%></option>
									<% } else { if (tdkmBean.getLoai().equals("2")) { %>
										<option value="1"><%=Utility.GLanguage("Điều kiện KM",session,jedis) %></option>
										<option value="2" selected><%=Utility.GLanguage("Trả KM", session, jedis)%></option>
									<% } else { %>
										<option value="1"><%=Utility.GLanguage("Điều kiện KM", session, jedis)%></option>
										<option value="2"><%=Utility.GLanguage("Trả KM", session, jedis)%></option>
									<% } } %>
							...
							<TR>
								<TD class="plainlabel"><%=Utility.GLanguage("Chương trình khuyến mại", session,
					jedis)%>
								</TD>
								<TD class="plainlabel"><select name="ctkmId"
									onChange="submitform()" id="ctkmId" style="width: 250px;"
									class="select2">
							...
							%>
							<TR>
								<TD class="plainlabel"><%=Utility.GLanguage("Điều kiện KM", session, jedis)%>
								</TD>
								<TD class="plainlabel"><select name="dkkmId" id="dkkmId"
									style="width: 350px;" class="select2" onChange="submitform()">
										<option value="" selected>All</option>
										<%
											if (dkkmRs != null)
													while (dkkmRs.next()) {
														if (dkkmRs.getString("pk_seq").equals(
																tdkmBean.getDkkmId())) {
										%>
							...
							<TR>
								...
								<TD class="plainlabel"><select name="type" id="type">
										<%
											if (tdkmBean.getType().equals("1")) {
										%>
										<option value="1" selected><%=Utility.GLanguage(
							"Bắt buộc nhập số lượng từng sản phẩm", session,
							jedis)%></option>
										...
										<option value="1"><%=Utility.GLanguage(
							"Bắt buộc nhập số lượng từng sản phẩm", session,
							jedis)%></option>
										<option value=""></option>
										<%
							...
							<TR>
								...
								<TD class="plainlabel" valign="middle">
									...
									%> <input type="radio"
									name="option" value="1" id="tonglg" onChange="submitform();"><%=Utility.GLanguage("Nhập tổng lượng", session,
							jedis)%>
									&nbsp; <input type="radio" name="option" value="2" id="tongtn"
									onChange="submitform();" checked><%=Utility.GLanguage("Nhập tổng tiền", session,
							jedis)%>
									&nbsp; <%
 	} else {
 %> <input type="radio" name="option" value="1"
									id="tonglg" onChange="submitform();" checked><%=Utility.GLanguage("Nhập tổng lượng", session,
							jedis)%>
									&nbsp; <input type="radio" name="option" value="2" id="tongtn"
									onChange="submitform();"><%=Utility.GLanguage("Nhập tổng tiền", session,
							jedis)%>
									&nbsp; <%
 	}
 %>
							...
							%>
							<TR>
								<TD class="plainlabel"><%=Utility.GLanguage("Tổng tiền", session, jedis)%>
								</TD>
								<%
									if (tdkmBean.getType().equals("1")) {
								%>
								<TD class="plainlabel"><input type="text" name="tongtien"
									id="tongtien" value="<%=tdkmBean.getTongtien()%>"
									style="text-align: right" size="15" readonly> VND</TD>
								<%
									} else {
								%>
								<TD class="plainlabel"><input type="text" name="tongtien"
									...
									onkeypress="return keypress(event);"> VND</TD>
								<%
									}
								%>
							</TR>
							...
							%>
							<TR>
								<TD class="plainlabel"><%=Utility.GLanguage("Tổng lượng", session, jedis)%>
								</TD>
								<%
									if (tdkmBean.getType().equals("1")) {
								%>
								<TD class="plainlabel"><input type="text" name="tongluong"
									id="tongluong" value="<%=tdkmBean.getTongluong()%>"
									style="text-align: right" size="15" readonly>
								</TD>
								<%
									} else {
								%>
							...
							%>
							<TR>
								<TD class="plainlabel"><%=Utility.GLanguage("Trả KM", session, jedis)%>
								</TD>
								<TD class="plainlabel"><select name="trakmId" id="trakmId"
									style="width: 350px;" class="select2" onChange="submitform()">
										<option value="" selected>All</option>
										<%
											if (trakmRs != null)
													while (trakmRs.next()) {
														if (trakmRs.getString("pk_seq").equals(
																tdkmBean.getTrakmId())) {
										%>
							...
							<TR>
								<TD class="plainlabel"><%=Utility.GLanguage("Hình thứ", session, jedis)%>c
								</TD>
								<TD class="plainlabel"><select name="hinhthuc"
									id="hinhthuc">
										<%
							...
							jedis)%></option>
										...
										<%
													...
													if (tdkmBean.getHinhthuc().equals("2")) {
										%>
										<option value="2" selected><%=Utility.GLanguage("Bất kỳ trong", session,
								jedis)%></option>
										<option value="1"><%=Utility.GLanguage(
								"Bắt buộc nhập số lượng từng sản phẩm",
								session, jedis)%></option>
										<option value=""></option>
										<%
											} else {
										%>
								...
								"Bắt buộc nhập số lượng từng sản phẩm",
								session, jedis)%></option>
										<option value="2"><%=Utility.GLanguage("Bất kỳ trong", session,
								jedis)%></option>
										<%
											}
												}
							...
							<TR>
								...
								<TD class="plainlabel">
									<%
										if (tdkmBean.getHinhthuc().equals("1")) {
									%> <input type="text"
									name="tongluong" id="tongluong"
									value="<%=tdkmBean.getTongluong()%>"
									style="text-align: right" style="width:95%" readonly> <%
 	} else {
 %>
									<input type="text" name="tongluong" id="tongluong"
									value="<%=tdkmBean.getTongluong()%>"
									style="text-align: right" style="width:95%"
									onkeypress="return keypress(event);"> <%
 	}
 %>
								</TD>
							...
							%>
							<%
								if (tdkmBean.getTrakmId().length() > 0
										|| tdkmBean.getDkkmId().length() > 0) {
							%>
							<TR>
								<TD class="plainlabel">Chương trình khuyến mại sử dụng<%=Utility.GLanguage("", session, jedis)%>
								</TD>
								<TD class="plainlabel"><select name="ctkmId"
									disabled="disabled" id="ctkmId" style="width: 250px;"
									multiple="multiple">
										<%
											if (ctkmRsSuDung != null)
													while (ctkmRsSuDung.next()) {
														if (ctkmRsSuDung.getString("pk_seq").equals(
																tdkmBean.getCtkmId())) {
										%>
				...
				<div align="left" style="width: 100%; float: none; clear: none;">
					<fieldset>
						<legend class="legendtitle">
							<%=Utility.GLanguage("Sản phẩm hiện tại", session, jedis)%>
						</legend>
						...
						<TABLE class="tabledetail" width="100%" border="0" cellpadding="1"
							...
							<TR class='tbdarkrow'>
								...
								<td align="center"><input type="text"
									value="<%=sp.getMa()%>" name="spMaOld"
									onkeyup="ajax_showOptions(this,'abc',event)" AUTOCOMPLETE="off">
									<input type="hidden" value="<%=sp.getId()%>" name="spIdOld">
								</td>
								<td align="left"><input type="text"
									value="<%=sp.getTen()%>" name="spTenOld" readonly>
								</td>
								<td align="right"><input type="text" name="soluongOld"
									value="<%=sp.getSoluong()%>" readonly="readonly"
									style="text-align: right;" onkeypress="return keypress(event);">
								</td>
							</TR>
							<%
								}
				...
				<div align="left" style="width: 100%; float: none; clear: none;">
					...
					<fieldset>
						<legend class="legendtitle">
							<%=Utility.GLanguage("Sản phẩm mới", session, jedis)%>
						</legend>
						<TABLE class="tabledetail" width="100%" border="0" cellpadding="1"
							cellspacing="1">
							<TR class="tbheader">
								<TH align="center" width="15%"><%=Utility.GLanguage("Mã sản phẩm", session, jedis)%></TH>
								<TH align="left" width="73%"><%=Utility.GLanguage("Tên sản phẩm", session, jedis)%></TH>
								<TH align="center" width="10%"><%=Utility.GLanguage("Số lượng /Số tiền", session, jedis)%></TH>
							</TR>
							<%
								String readonly = "";
							...
							<TR class='tbdarkrow'>
								<td align="center"><input type="text"
									value="<%=sp.getMa()%>" name="masp"
									onkeyup="ajax_showOptions(this,'abc',event)" AUTOCOMPLETE="off">
								</td>
								<td align="left"><input type="text"
									value="<%=sp.getTen()%>" name="tensp" readonly>
								</td>
							...
							%>
							<TR class='tbdarkrow'>
								<td align="center"><input type="text" value="" name="masp"
									onkeyup="ajax_showOptions(this,'abc',event)" AUTOCOMPLETE="off">
								</td>
								<td align="left"><input type="text" value="" name="tensp"
									readonly>
								</td>
								<td align="right"><input type="text" <%=readonly%>
									name="soluong" style="text-align: right;"
									onkeypress="return keypress(event);">
								</td>
							</TR>
							<%
