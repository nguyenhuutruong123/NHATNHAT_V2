...
Hashtable dvdlList = new Hashtable<String, String>();
if(dvtRs!=null)
{	
	 while( dvtRs.next())
	 {
		 dvdlList.put(dvtRs.getString("DONVI"),dvtRs.getString("DONVI"));
	 }
}

	String[] isNppDat = lsxBean.getIsNppDat(); 
...
var DonViDoLuong = function(id, ten) {
	this.id = id;
	this.ten = ten;
};

var Kho = function(id, ten) {
	this.id = id;
	this.ten = ten;
};
...
if(dvdlList!=null)
{
	...
	while(keys.hasMoreElements())
	{
		...
		%>
		dvdlArr.push(new DonViDoLuong('<%= key %>', '<%= dvdlList.get(key) %>'));
	    <%
	}
}
%>
function getDvdlSelect(selectedId, ngay) {		
	var html = '<option value=""></option>', qh;
	for(var i = 0; i < dvdlArr.length; i++) {
		npp = dvdlArr[i];
	...
	return "<select name='donvi' onchange='CapNhatGia("+ngay+");' style='width: 100%;' >" + html + "</select>";
...
function addTR(ngay) {
	...
	var spMa = document.getElementsByName("spMa");
	var ngay=parseFloat( spMa.length +1);
...
'<td> <input type="text" name="spGiagoc" value="" style="width: 100%; text-align: right;" readonly="readonly" > </td> '+
...
'	<a class="spGHICHU'+ngay+'" href="#"> '+
'   <img style="top: -4px;" src="../images/vitriluu.png" title="Ghi chú"></a> '+
'<div style=\'display:none\'> '+
'	<div id=\'spGHICHU'+ngay+'\' style=\'padding:0px 5px; background:#fff;\'> '+
...
'</tr> ';	
	...
	function replaces()
	{
		...
		for(i = 0; i < spMa.length; i++)
		{
			if(spMa.item(i).value != "")
			{
				...
				if(pos > 0)
				{
					...
					spDongiaBVAT.item(i).value = DinhDangTien(parseFloat(dongia.item(i).value.replace(/,/g,"")) );
					// mặc định số lượng = 0, khi trung tâm thêm hay sửa sản phẩm
					soluong.item(i).value = "0"; 
				}
	 ...
	 function TinhTien()
	 {
		...
		{
			var dongiabvat = DinhDangTien(parseFloat(dongia.item(i).value.replace(/,/g,"")) );

			spDongiaBVAT.item(i).value = dongiabvat; 
			if(spMa.item(i).value != "" && dongia.item(i).value != "" && soluong.item(i).value != "" )
			{
				vatTong = parseFloat(thueVAT.item(i).value.replace(/,/g,"")) ;
				...
				tongtien += parseFloat(parseFloat(tt));
		...
		document.getElementById("txtTongSauCK").value = DinhDangDonGia(TongTienTruocVat.toFixed(0));
		...
		ocument.getElementById("txtThanhToan").value = DinhDangDonGia(parseFloat(tongtienThanhToan.toFixed(0)));
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý kho trung tâm > Đơn đặt hàng > Cập nhật
        </div>
    ...
    <fieldset>
    	<legend class="legendtitle">Đơn đặt hàng </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0">	
            	<tr>
					...
					<TD class="plainlabel">
						...
						%> 
							<input disabled name="pokhuyenmai" id="pokhuyenmai" type="checkbox" value="1" onchange="submitform();" checked> 
							<input type="hidden" name="pokhuyenmai" id="pokhuyenmai"   value='1'>
						<%
						} else {
						%> <input disabled name="pokhuyenmai" id="pokhuyenmai" type="checkbox" value="1" onchange="submitform();">
						<% } %>
                ...
                <TR>
                    ...
                    <TD class="plainlabel" valign="top"  >
                    	<input type="text" readonly="readonly"  value="<%= lsxBean.getChietkhau() %>" name="ptChietKhau" id="ptChietKhau" style="text-align: right;" />
                    </TD>	
                ...
                <TR>
                    ...
                    <TD class="plainlabel"  >
                    	<input type="text" name="txtCongNo" id="txtCongNo" value='<%= lsxBean.getCongNo() %>' readonly="readonly" onkeyup="Dinhdang(this);" style="text-align: right;" />
                    </TD>
			...
			<div>
				...
				<table  id="spTable" cellpadding="0px" cellspacing="1px" width="100%">
					<tr class="tbheader">
						...
						if(spMa != null)
						{
							...
							<tr  onMouseover="ddrivetip('<%=spGhichu[i] %>', 250)"; onMouseout='hideddrivetip()'>
								...
								if(spSCheme[i].trim().length() <= 0 ) { 
									...
									String readonly = "";
									String onkeyup ="ajax_showOptions(this,'nhapkho',event)";
									if(isNppDat[i]!= null & isNppDat[i].equals("1"))
									{
										readonly= "readonly";
										onkeyup = "";
									}
								...
								%>
									...
									<input type="hidden" name="isNppDat" value='<%= isNppDat[i] %>'>
									 </td>
									<td>
										<input <%= readonly %> type="text" name="spMa" value="<%= spMa[i] %>" style="width: 96%"  onkeyup="<%=onkeyup %>" AUTOCOMPLETE="off"  > 
									</td>
								...
								<% } else { %>
									...
									<td> <input type="text" name="soluongtt" value="<%=spSoluongtt[i] %>" style="width: 97%; color: red; text-align: right;"   > </td>
									<td style="display:none"> <input type="text" name="spvat" value="<%= spVat[i] %>" style="width: 100%; text-align: right;" readonly="readonly"  > </td>
					...
					<% for(int i = count; i < 50; i++) { %>
						...
						<tr>
							<td style="text-align: center;" > <%= i + 1 %>
							<input type="hidden" name="isNppDat" value='0'>
							 </td>
							<td>
								<input type="text" name="spMa" value="" style="width: 100%"  onkeyup="ajax_showOptions(this,'nhapkho',event)" AUTOCOMPLETE="off"  > 
							</td>
					...
					<% } %>	
							  		...
							  		&nbsp;&nbsp;&nbsp;&nbsp;	<a class="button2" href="javascript:addTR(0)">
