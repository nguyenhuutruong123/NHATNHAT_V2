...
%>
<% IRaBundle dhBean = (IRaBundle)s.getAttribute("dhBean");
%>
<% List<ISanpham> splist = (List<ISanpham>)dhBean.getSpList(); %>
<% List<ISanpham> soloList = (List<ISanpham>)dhBean.getSoLoList(); %>
...
<script type="text/javascript" src="../scripts/rabundle-ajax-list.js"></script>
...
<script language="javascript" type="text/javascript">
	...
	function replaces()
	{
		...
		var ngayhethan = document.getElementsByName("ngayhethan");
		...
		for(i=0; i < masp.length; i++)
		{
			if(masp.item(i).value != "")
			{
				...
				if(pos > 0)
				{
					...
					if( ngayhethan.item(i).value == "")
					{
						 ngayhethan.item(i).value  = "<%=Utility.getNgayHienTai() %>";
					}
	...
	{    
	 ...
	 {
		 document.getElementById("khoTen").removeAttribute('disabled');
		 document.getElementById("nppId").removeAttribute('disabled');
		...
		for(ii = 0; ii < (masp.length - 1) ; ii++)
		{
			for( j = ii + 1; j < masp.length; j++)
			{
				if(masp.item(ii).value != "" && masp.item(ii).value == masp.item(j).value  
							&& solo.item(ii).value == solo.item(j).value  && ngayhethan.item(ii).value == ngayhethan.item(j).value  )
				{		
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0" 
	onload="replaces()"
 >
<form name="dhForm" method="post" action="../../RaBundleUpdateSvl">
						...
						<TD align="left" >
								...
								<TR>
									<TD align="left">
									   <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
										   <TR height="22">
			 								   <TD align="left"  class="tbnavigation">Quản lý bán hàng > Bán hàng Nhà thuốc > Bán hàng cho khách hàng > Tạo mới </TD>								   
							...
							<TABLE border="0" width="100%" cellpadding = "0" cellspacing = "0">
								...
								<TR>
									<TD align="left">						
										<FIELDSET>
										<LEGEND class="legendtitle">&nbsp;Thông tin chung</LEGEND>
										<TABLE cellpadding = "3" cellspacing = "0" width = "100%" border = 0>
											...
											<TR>
														...
														<TD  class="plainlabel" >
															<SELECT class="select2"   name="nppId" id ="nppId" onchange="submitform()">
																<option value=""></option>
																<%  if(nppRs != null)
																	{
																		try
						                ...
						                <TR class="tblightrow">
					                		...
					                		<TD class="plainlabel"  colspan="7"  >
					                			<select name="spId"   class="select2"  onchange="submitform()" >
					                            	<option value="" ></option>
					                            	<% if(spRs != null) { 
							   ...
							   <TR>
							   		<TD>
							   		<FIELDSET>
										<LEGEND class="legendtitle">&nbsp;Gói sản phẩm</LEGEND>
							...
							for ( int k = 0; k <  soloList.size(); k++){
								sanpham = soloList.get(k); 
								%>
									<TR class= 'tblightrow' >
									
									
										<TD align="left" >
											<input type="hidden" name="ra_spId"  value="<%=sanpham.getId()%>" >
											<input type="hidden" name="ra_nppId"  value="<%=sanpham.getNppId()%>" >
											<input type="hidden" name="ra_kbhId"  value="<%=sanpham.getKbhId()%>" >
											<input type="hidden" name="ra_khoId"  value="<%=sanpham.getKhoId()%>" >
											
											<input name="ra_solo" type="text" value="<%=sanpham.getSOLo()%>"  style="width:100%"  readonly>
										</TD>
										<TD align="left" >
											<input name="ra_ngayhethan" type="text" value="<%=sanpham.getNgayHetHan()%>"  style="width:100%"  readonly>
										</TD>
										
								        <TD align="left" >
											<input name="ra_ngaynhapkho" type="text" value="<%=sanpham.getNgaynhapkho()%>"  style="width:100%"  readonly>
										</TD>
										<TD align="left" >
											<input name="ra_tonhientai" type="text"value='<%=sanpham.getTonhientai() == null ? "0" : formatter.format(Utility.parseDouble(sanpham.getTonhientai().replaceAll(",", "")))%>'  style="width:100%"  readonly>
										</TD>
								        <TD align="center">
								        	<input style="text-align: right; width: 100%;" type="text" name="ra_soluong" 
												value='<%=sanpham.getSoluong() == null ? "0" : formatter.format(Utility.parseDouble(sanpham.getSoluong().replaceAll(",", "")))%>'onkeypress="return keypress(event);"onChange="Dinhdang(this);" />
										</TD>
							   ...
							   <TR>
							   		<TD>
							   		<FIELDSET>
										<LEGEND class="legendtitle">&nbsp;Nguyên liệu rã</LEGEND>
										<TABLE width = "100%" border="0" cellpadding="0" cellspacing="1" id="tbSanPham">
										<tbody id="san_pham">
												<TR class="tbheader" >
							...
							while (m < size){
								...
								%>
								    ...
								    <TD align="left" >
										<input  name="solo" id="solo_<%= m %>" type="text" value="<%= sanpham.getSOLo() %>" style="width:100%;text-align:left"  onchange="UpperText(this)"   >
									</TD>
									<TD align="left" >
										<input class="days" size="11" readonly name="ngayhethan" id="ngayhethan_<%= m %>" type="text" value="<%= sanpham.getNgayHetHan().trim().length() > 0 ? sanpham.getNgayHetHan() : Utility.getNgayHienTai()  %>" style="width:100%;text-align:left" >
									</TD>
						        	<TD align = "center" >
								...
								<TR class= 'tblightrow'>
								    ...
								    <TD align="left" >
										<input  name="solo" id="solo_<%= i %>" type="text" value="" style="width:100%;text-align:left" onchange="UpperText(this)" >
									</TD>
									<TD align="left" >
										<input class="days" size="11" readonly name="ngayhethan" id="ngayhethan_<%= i %>" type="text" value="" style="width:100%;text-align:left" >
									</TD>
								    <TD align = "center" >
