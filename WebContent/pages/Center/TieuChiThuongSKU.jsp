...
<%	
	...
	int[] quyen = new  int[5];
	quyen = util.Getquyen("TieuchithuongSKUSvl","",userId);
...
<form name="tcsku" method="post" action="../../TieuchithuongSKUSvl">
            ...
            <TR>
                <TD align="left" class="tbnavigation">
                    <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
                        <TR height="22">
                            <TD align="left" colspan="2" class="tbnavigation">&nbsp; Dữ liệu nền > Kinh doanh > Tiêu chí thưởng SKU tập trung </TD>  
        ...
        <TABLE width="100%" cellpadding="0" cellspacing="1">
            <TR>
                ...
                <TR>
                    <TD width="100%">
                        <FIELDSET>
                            <LEGEND class="legendtitle">&nbsp;Tiêu chí thưởng SKU tập trung &nbsp;&nbsp;	   
                              <%if(quyen[0]!=0){ %>                      
                                ...
                                <% 
                                    ...
                                    while ( tieuchiSKU.next()){
                                        ...
                                        <%}%>
                                                ...
                                                <% if( tieuchiSKU.getString("trangthai").trim().equals("0") ) { %>
                                                	<%if(quyen[2]!=0) {%>	
					                                <A href = "../../TieuchithuongSKUUpdateSvl?userId=<%=userId%>&update=<%= tieuchiSKU.getString("pk_seq") %>"><IMG src="../images/Edit20.png" alt="Cap nhat" title="Cap nhat" border=0></A>&nbsp;
					                               <%} %>
					                               	<%if(quyen[4]!=0){ %>
					                                <A href = "../../TieuchithuongSKUSvl?userId=<%=userId%>&duyet=<%= tieuchiSKU.getString("pk_seq") %>"><img src="../images/Chot.png" alt="Duyet" width="20" height="20" longdesc="Duyet" border=0 onclick="if(!confirm('Bạn có muốn duyệt chỉ tiêu này?')) return false;"></A>&nbsp;
					                                <%} %>
					                                <%if(quyen[1]!=0) {%>
					                                <A href = "../../TieuchithuongSKUSvl?userId=<%=userId%>&delete=<%= tieuchiSKU.getString("pk_seq") %>"><img src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn có muốn xóa chỉ tiêu này?')) return false;"></A>									
							                    	<%} %>
							                    <%} else { %>
							                    	<%if(quyen[3]!=0){ %>
							                    	<A href = "../../TieuchithuongSKUUpdateSvl?userId=<%=userId%>&display=<%= tieuchiSKU.getString("pk_seq") %>"><IMG src="../images/Display20.png" alt="Hien thi" title="Hien thi" border=0></A>&nbsp;
							                    	<%} %>
							                    <%} %>
							                    </TD>
                                            </TR>
