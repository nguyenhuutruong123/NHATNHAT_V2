												...
												<TABLE border = 0 cellpadding="0" cellspacing="0" >
													...
													<%if(trangthai.equals("0")){ %>
														...
														<%if(quyen[Utility.SUA]!=0){ %>
															<A href = "../../RaBundleUpdateSvl?userId=<%=userId%>&update=<%= dhlist.getString("pk_seq") %>"><img src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" title="Cập nhật đơn hàng" longdesc="Cap nhat" border = 0></A>&nbsp;
														<%} %>
													...
													<%} else{ if(!trangthai.equals("2")) { %>
														...
														<%if(quyen[Utility.XEM]!=0){ %>
															<A href = "../../RaBundleUpdateSvl?userId=<%=userId%>&display=<%= dhlist.getString("pk_seq") %>"><img src="../images/Display20.png" alt="Hien thi" width="20" height="20" title="Xem đơn hàng" longdesc="Hien thi" border = 0></A>&nbsp;
														<%} %>
														<%if(quyen[Utility.THEM]!=0){ %>
															<A id='<%="copypk_seq" + dhlist.getString("pk_seq") %>' href="">
																<img  src="../images/copy20.png" alt="Copy đơn hàng" width="20" height="20"  border=0	onclick="if(!confirm('Bạn có muốn copy đơn hàng ( <%= dhlist.getString("pk_seq") %> ) này?')) {return false ;}else{ processing('<%="copypk_seq" + dhlist.getString("pk_seq") %>' , '../../RaBundleUpdateSvl?userId=<%=userId%>&copy=<%= dhlist.getString("pk_seq") %>&nppId=<%= obj.getNppId() %>');}" >
															</A>
														<%} %>	 
													<% } else { %> 
														<%if(quyen[Utility.XEM]!=0){ %>
