...
<%
	ITieuchithuongTL obj = (ITieuchithuongTL) session
				.getAttribute("tctskuBean");

		ArrayList<ITichLuyItem> tichluyItemList = obj
				.getTichluyItemList();
...
<%
	String url = util.getChuyenNguUrl("TieuchithuongTLSvl",
				"", session);
%>
			...
			<TABLE width="100%" border="0" cellpadding="0"  cellspacing="1" >
				...
				<TR>
				  <TD height="100%" width="100%">
						<FIELDSET >
						<LEGEND class="legendtitle" style="color:black"><%=Utility.GLanguage("Thông tin khuyến mãi", session, jedis)%></LEGEND>
						<TABLE border="0" width="100%" cellpadding="6" cellspacing="0">
						   <TR>
								<TD width="120px" class="plainlabel" ><%=Utility.GLanguage("Ngày đăng ký", session, jedis)%> <br />&nbsp;&nbsp;&nbsp;&nbsp; <%=Utility.GLanguage("Từ ngày", session, jedis)%><FONT class="erroralert"> *</FONT></TD>
						  ...
						  <TR>
								<TD width="120px" class="plainlabel" ><%=Utility.GLanguage("Ngày tính DS", session, jedis)%> <br />&nbsp;&nbsp;&nbsp;&nbsp; <%=Utility.GLanguage("Từ ngày", session, jedis)%><FONT class="erroralert"> *</FONT></TD>
								<TD width="250px" class="plainlabel" >
						  ...
						  <TR>
						  	  	<TD class="plainlabel"><%=Utility.GLanguage("Scheme",session,jedis) %> <FONT class="erroralert"> *</FONT></TD>
						  	  	<TD class="plainlabel" >
						  ...
						  <tr>
		                		<td class="plainlabel" ><%=Utility.GLanguage("Kho trả", session, jedis)%> <FONT class="erroralert"> *</FONT> </td>
		                     ...
		                     <Tr>
		                    	<TD class="plainlabel"> <%=Utility.GLanguage("Tính theo", session, jedis)%> </TD>
		                    	<TD class="plainlabel" colspan="4" >
		                			<select name="tinhtheosl" style="width: 200px;"   >
										
										<%
																					String[] data = { "Doanh số", "Số lượng", "Điểm" };

																						for (int x = 0; x < data.length; x++) {
																							System.out.println("x =   " + x + ", getTinhtheoSl = "
																									+ obj.getTinhtheoSl());
						  	  ...
						  	  <td colspan="3">
						  	  	<INPUT type="file" name="filename" size="40" value=''> 
						  	  		 <a class="button2" href="javascript:upload()">
											<img style="top: -4px;" src="../images/button.png" alt="">Phân bổ<%=Utility.GLanguage("", session, jedis)%></a>
						  	  </td>
						...
						: ""%>><a href="#tab1"><%=Utility.GLanguage("Tiêu chí tích lũy", session, jedis)%></a></li>
							<li <%=obj.getActiveTab().equals("1") ? "class='current'"
						: ""%>><a href="#tab2"><%=Utility.GLanguage("Sản phẩm", session, jedis)%> mua<%=Utility.GLanguage("", session, jedis)%></a></li>			
						</ul>
				                    ...
				                    <TR >
				                        <TH align="left" width="25%" colspan="6" >
				                        	<a class="button3" href="javascript:newline()">
                           						<img style="top: -4px;" src="../images/New.png" alt=""><%=Utility.GLanguage("Thêm", session, jedis)%></a> 
				                        </TH>
										
				                    </TR>
				                    
				                    <TR class="tbheader">
				                        <TH align="center" width="2%" rowspan="2" ><%=Utility.GLanguage("Xóa", session, jedis)%></TH>
				                        <TH align="center" width="28%" rowspan="2" ><%=Utility.GLanguage("Mức tích lũy", session, jedis)%></TH>
										<TH align="center" width="30%" colspan="2" ><%=Utility.GLanguage("Doanh số phát sinh", session, jedis)%></TH>
				                        <TH align="center" width="15%" rowspan="2" ><%=Utility.GLanguage("Trả tích lũy", session, jedis)%></TH>
				                        <TH align="center" width="15%" rowspan="2" ><%=Utility.GLanguage("Đơn vị", session, jedis)%></TH>
				                    </TR>
			                    
				                    <TR class="tbheader">
					                  	<TH align="center" width="15%" ><%=Utility.GLanguage("Từ mức", session, jedis)%></TH>
				                        <TH align="center" width="15%" ><%=Utility.GLanguage("Đến mức", session, jedis)%></TH>
														...
														<a class="sanphamTRA<%=i%>" href="#">
											                ...
											                <div style='display:none; '>
										                        <div id='sanphamTRA<%=i%>' style='padding:0px 5px; background:#fff;'>
										                        	<h4 align="left"><%=Utility.GLanguage("Sản phẩm", session, jedis)%> trả mức ( <%=i + 1%> ) </h4>
																	<table cellpadding="4px" cellspacing="2px" width="100%" align="center">
										                            <tr>
									                                	<td width="150px" valign="top" align="left"><span style="font-size: 12px"><%=Utility.GLanguage("Hình thức", session, jedis)%></span>
									                                	</td>
									                                    <td valign="top" align="left">
										                                    <select name="hinhthuc" id="hinhthuctra" style="width: 200px;" >
													                			<option value="0" <%=item.getHinhthuc() == 0 ? "selected" : ""%>  ><%=Utility.GLanguage("Bất kỳ trong", session, jedis)%></option>
													                			<option value="1" <%=item.getHinhthuc() == 1 ? "selected" : ""%>  >N<%=Utility.GLanguage("hập số lượng sản phẩm", session, jedis)%></option>			
																			</select>
									                                    </td>
									                                </tr>
										                            ...
										                            <tr>
										                                	<td colspan="2">
										                                		<table align="left" cellpadding="0px" cellspacing="1px">
											                                		<tr class="tbheader">
											                                			<td width="100px" align="center" ><span style="font-size: 0.9em"><%=Utility.GLanguage("Mã sản phẩm", session, jedis)%></span></td>
											                                			<td width="300px" align="center" ><span style="font-size: 0.9em"><%=Utility.GLanguage("Tên sản phẩm", session, jedis)%></span></td>		                                			
											                                			<td width="100px" align="center" ><span style="font-size: 0.9em"><%=Utility.GLanguage("Số lượng", session, jedis)%></span></td>
											                                		</tr>
											                                	</table>
											                                												                                				...
											                                												                                				if (item.getSanPhamRs() != null) {
											                                												                                					while (item.getSanPhamRs().next()) {
											                                												                                						...
											                                												                                						for (int xx = 0; xx < item.getSpList().size(); xx++) {
											                                												                                							if (item.getSpList().get(xx)[0].toString()
											                                												                                									.equals(item.getSanPhamRs().getString(
											                                												                                											"ma"))) {
											                                												                                								soluong = item.getSpList().get(xx)[1]
											                                												                                										.toString().trim().length() > 0 ? item
											                                												                                										.getSpList().get(xx)[1].toString()
											                                												                                										: "";
											                                												                                								break;
											                                												                                							}
											                                												                                						}
										                                ...
										                                <tr>
										                                	<td valign="top" align="left" colspan="2">
										        								...
										        								<img style="top: -4px;" src="../images/button.png" alt=""> <%=Utility.GLanguage("Đóng lại", session, jedis)%> </a>
										                                	</td>
										                                </tr>
										                            </table>
																</div>
						...
						<div id="tab2" class="tab_content">
								...
								<TABLE class="tabledetail" width="100%" border="0" cellspacing="1px" cellpadding="0px">
				                    ...
				                    <tr>
				                		<td style="font-size: 12px; padding: 8px; " colspan="2" ><%=Utility.GLanguage("Nhóm sản phẩm", session, jedis)%> &nbsp;&nbsp;   
				                			...
				                			<select name="nspId" style="width: 200px;" onchange="loadSPTheoNhom();"  >
												...
												<%
													if (nhomSpRs != null) {
															while (nhomSpRs.next()) {
												%>
														<option value="<%=nhomSpRs.getString("pk_seq")%>" <%=obj.getNhomsanphamIds().contains(
								nhomSpRs.getString("pk_seq")) ? "selected" : ""%>   ><%=nhomSpRs.getString("ten")%></option>
															<%
																}
																		nhomSpRs.close();
																	}
												...
												<TR class="tbheader">
													<td align="center" width="20%"> <%=Utility.GLanguage("Mã sản phẩm", session, jedis)%></td>
													<td align="center" width="35%"> <%=Utility.GLanguage("Tên sản phẩm", session, jedis)%></td>
													<td align="center" width="10%"> <%=Utility.GLanguage("Điểm thưởng", session, jedis)%></td>
													<%
														int count = 0;
												...
												<td>
					                    			<input type="text" name="slspTra" style="width: 100%"  value="<%=obj.getSanphamMuaRs().getString("sl")%>"  onkeyup="FormatNumber(this);"  onkeypress="return keypress(event);" >
					                    		</td>
