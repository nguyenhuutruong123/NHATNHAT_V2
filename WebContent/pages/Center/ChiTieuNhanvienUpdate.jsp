	...
	<TR>
						  ...
						  <tr height="22">
						   <%if(!objbean.getIsDisplay().equals("1") )
						    { 	if (objbean.getID().equals("")){%>
							 	<TD align="left" colspan="2" class="tbnavigation"><%=url %>  > <%=Utility.GLanguage("Tạo mới",session,jedis) %>
							 	<%}
						    	else  {%>
		...
		<TABLE width="100%" border="0" cellpadding="0" cellspacing="1">
			...
			<TR >
							...
							<TR>
								<TD width="20%" class="plainlabel" ><%=Utility.GLanguage("Tháng",session,jedis) %> <FONT class="erroralert"> </FONT></TD>
								<TD class="plainlabel">
							...
							<TR>
						  	...
						  	<TR  >
						  	  	  <TD class="plainlabel"><%=Utility.GLanguage("Loại Upload",session,jedis) %></TD>
							  	  <TD class="plainlabel" >
							  	  		<select name="loaiUpload"  style="width :200px">
							  	  		<option value="1" selected ><%=Utility.GLanguage("Chia chỉ tiêu bình thường",session,jedis) %> </option>  
							  	  		<%-- <%if(objbean.getLoaiUpload().equals("2")){ %>
										<option value="1" >Chia chỉ tiêu bình thường </option>  
										<option value="2" selected>Chia chỉ tiêu đến mức SP </option>
										<option value="3" >  Thiết lập tiến độ </option>
										<%} else if(objbean.getLoaiUpload().equals("1")){ %>
										<option value="1" selected>Chia chỉ tiêu bình thường </option>  
										<option value="2" >Chia chỉ tiêu đến mức SP </option>
										<option value="3" >  Thiết lập tiến độ </option>
										<%} else if(objbean.getLoaiUpload().equals("3")){ %>
										<option value="1" >Chia chỉ tiêu bình thường </option>  
										<option value="2" >Chia chỉ tiêu đến mức SP </option>
										<option value="3" selected>  Thiết lập tiến độ </option>
										<%} %> --%>
									</select> 
							  	  </TD>
						  	</TR>	
