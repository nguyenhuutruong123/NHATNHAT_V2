...
%>
<%
	String url = util.getChuyenNguUrl("DuyettrakhuyenmaiSvl", "",
				session);
%>
	...
	<TR>
		...
		<TABLE width="100%" border="0" cellpadding="0" cellspacing="1">
			<TR ><TD >
					<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
						<TR class = "tbdarkrow">
							<TD width="30" align="left"><A href= "../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"DuyettrakhuyenmaiSvl?userId="+userId+"")%>"><img src="../images/Back30.png" alt="Quay ve"  title="Quay ve" border="1" longdesc="Quay ve" style="border-style:outset"></A></TD>
				...
				<TR>
				  <TD height="100%" width="100%">
						...
						<TABLE border="0" width="100%" cellpadding="6" cellspacing="0">
							<TR>
											<TD width="120px" class="plainlabel"><%=Utility.GLanguage("Ngày duyệt", session, jedis)%> <FONT
										...
										<TR>
											<TD width="120px" class="plainlabel"><%=Utility.GLanguage("Từ ngày", session, jedis)%> tính Doanh số <FONT
												class="erroralert"> *</FONT></TD>
											<TD class="plainlabel"><input
											...
											<TD width="120px" class="plainlabel"><%=Utility.GLanguage("Đến ngày", session, jedis)%> tính Doanh số<FONT
												class="erroralert"> *</FONT></TD>
											<TD class="plainlabel"><input
						...
						<TABLE class="chitieutable">
											<TR class="tbheader">
												<TH align="center" width="20%"><%=Utility.GLanguage("Mã", session, jedis)%></TH>
												<TH align="center" width="50%"><%=Utility.GLanguage("Diễn giải", session, jedis)%></TH>
												<TH align="center" width="30%"><%=Utility.GLanguage("Ngày đăng ký", session, jedis)%></TH>
												<TH align="center" width="10%"><%=Utility.GLanguage("Chọn", session, jedis)%></TH>
											</TR>
											...
											<TR >
												...
												<TD >
													<input type="text" value="<%=obj.getDangkyTichluyRs()
								.getString("DienGiai")%>" style="width: 100%; text-align: center;<%=cl%>" readonly="readonly">
												</TD>
												<TD >
													<input type="text" value="<%=obj.getDangkyTichluyRs().getString(
								"NgayDangKy")%>" style="width: 100%; text-align: center;<%=cl%>" readonly="readonly">
												</TD>											
												<TD >
													<input   value="<%=obj.getDangkyTichluyRs().getString("pk_seq")%>"  type="checkbox" name="dangkyIds"  <%=obj.getDangkyIds().contains(
								obj.getDangkyTichluyRs().getString("pk_seq")) ? "checked"
								: ""%>     >
												</TD>
						...
						<TABLE class = "chitieutable">
							...
							<TR class="tbheader">
								...
								<TH align="center" width="10%" ><%=Utility.GLanguage("Mã KH", session, jedis)%></TH>
		                        <TH align="center" width="20%" ><%=Utility.GLanguage("Khách hàng", session, jedis)%></TH>
		                        <TH align="center" width="15%" ><%=Utility.GLanguage("Nhà phân phối", session, jedis)%></TH>
		                        <TH align="center" width="10%" ><%=Utility.GLanguage("Doanh số", session, jedis)%></TH>
		                        <TH align="center" width="7%" ><%=Utility.GLanguage("SL mua", session, jedis)%></TH>
		                        <TH align="center" width="7%" ><%=Utility.GLanguage("Số điểm", session, jedis)%></TH>
		                        <TH align="center" width="7%" ><%=Utility.GLanguage("Mức đk", session, jedis)%></TH>
		                        <TH align="center" width="7%" ><%=Utility.GLanguage("Mức đạt", session, jedis)%></TH>
		                        <TH align="center" width="7%" ><%=Utility.GLanguage("Mức duyệt", session, jedis)%></TH>
		                        <TH align="center" width="10%" ><%=Utility.GLanguage("Tổng thưởng/Tổng lượng", session, jedis)%></TH>
		                    			...
		                    			<TR>
		                    				...
		                    				<TD>
		                    					<input type="text" name="doanhso" style="width:100%; border:none; background: transparent;" 
		                    							value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"DOANHSO"))%>" style="width: 100%; text-align: right;" readonly="readonly" >
		                    				</TD>
		                    			
		                    				<TD>
		                    					<input type="text" name="soluongmua" style="width:100%; border:none; background: transparent;" 
		                    						value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"SoLuongMua"))%>" style="width: 100%; text-align: right;" readonly="readonly" >
		                    				</TD>
		                    				<TD>
		                    					<input type="text" name="soluongmua" style="width:100%; border:none; background: transparent;" 
		                    						value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"sodiem"))%>" style="width: 100%; text-align: right;" readonly="readonly" >
		                    				</TD>
		                    				<TD>
		                    					<input type="hidden" name="mucdk"  value="<%=mucdk[i]%>">
		                    					<input type="text" name="mucthuong" style="width:100%; border:none; background: transparent;" value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"MUCDANGKY"))%>" style="width: 100%; text-align: center;" readonly="readonly" >
		                    				 
		                    				</TD>
		                    				<TD>
		                    					<input type="text" name="mucdat" style="width:100%; border:none; background: transparent;" value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"MUCDAT"))%>" style="width: 100%; text-align: center;" readonly="readonly" >
		                    				</TD>
		                    				<TD>
		                    					<input type="text" name="mucduyet" style="width:100%; border:none; background: transparent;" value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"MUCDUYET"))%>" style="width: 100%; text-align: center;" readonly="readonly" >
		                    				</TD>
		                    				<TD>
		                    					<input type="text" name="tongthuong" style="width:100%; border:none; background: transparent;" value="<%=format.format(obj.getKhDuyetRs().getDouble(
								"TONGTIEN"))%>" style="width: 100%; text-align: right;" readonly="readonly" >
		                    				</TD>
		                    		...
		                    		<%
		                    					                    			if (obj.getKhDuyetRs().getString("jsonSP") != null) {
		                    					                    							System.out.println("   [=]"
		                    					                    									+ obj.getKhDuyetRs().getString("jsonSP"));
		                    					                    							System.out.println("Json string: "
		                    					                    									+ obj.getKhDuyetRs().getString("jsonSP"));
		                    					                    							JsonArray jsonArray = (JsonArray) new JsonParser()
		                    					                    									.parse(obj.getKhDuyetRs().getString(
		                    					                    											"jsonSP"));
		                    					                    							for (int xx = 0; xx < jsonArray.size(); xx++) {
		                    					                    								...
		                    					                    								Double soluong = rec.get("SoLuong")
		                    					                    										.getAsDouble();
		                    					                    		%>
										<TR>
											<td colspan="3"><%=Utility.GLanguage("Sản phẩm", session, jedis)%>
											</td>
											<td colspan="1"><%=spMa%></td>
											<td colspan="5"><%=spTen%></td>
											<td colspan="5"><%=soluong > 0 ? format.format(soluong)
										: ""%></td>
										</TR>
