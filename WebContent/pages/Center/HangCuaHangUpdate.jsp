...
<%
	IHangcuahang hchBean = (IHangcuahang) session
				.getAttribute("hchBean");
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name='hchForm' method="post" action="../../HangcuahangUpdateSvl">
		...
		<input type="hidden" name="<%=csdr.get_tokenName()%>"
			...
			value='<%=hchBean.getId()%>'> <INPUT name="userId"
			type="hidden" value='<%=userId%>' size="30">
								...
								<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr height="22">
										<TD align="left" colspan="2" class="tbnavigation"><%=url%>
											> <%=Utility.GLanguage("Cập nhật", session, jedis)%></TD>
								...
								<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
									<TR class="tbdarkrow">
										<TD width="30" align="left"><A
											href="../../RouterSvl?task=<%= Utilback.dongMa(getServletContext().getInitParameter("RedirectNoScript") +"HangcuahangSvl?userId="+userId+"")%>"><img
					...
					<TABLE width=100% cellpadding="3" cellspacing="0" border="0">
						...
						.GLanguage("Báo lỗi nhập liệu", session, jedis)%></LEGEND>
									...
									<textarea name="dataerror"
										...
										style="width: 100%" readonly="readonly" rows="1"><%=hchBean.getMessage()%></textarea>
								</FIELDSET></TD>
						</TR>
							...
							<TD align="left" colspan="4" class="legendtitle">
								<FIELDSET>
									<LEGEND class="legendtitle">
										<%=Utility.GLanguage("Thông tin hạng cửa hàng", session,
						jedis)%>
									</LEGEND>
									<TABLE class="tblight" width="100%" cellspacing="0"
										cellpadding="6">
										<TR>
											<TD width="15%" class="plainlabel">Hạng khách hàng <%=Utility.GLanguage("", session, jedis)%>
												<FONT class="erroralert">*</FONT>
											</TD>
											<TD class="plainlabel"><INPUT name="hangcuahang"
												type="text" value='<%=hchBean.getHangcuahang()%>'
												style="width: 300px">
											</TD>
										</TR>
										<TR>
											<TD class="plainlabel"><%=Utility.GLanguage("Diễn giải", session, jedis)%><FONT
												class="erroralert">*</FONT></TD>
											<TD class="plainlabel"><INPUT name="diengiai"
												type="text" value='<%=hchBean.getDiengiai()%>'
												style="width: 300px">
											</TD>
										</TR>

										<TR>
											<TD class="plainlabel"><%=Utility
						.GLanguage("Hạn mức doanh thu", session, jedis)%><FONT
												class="erroralert">*</FONT></TD>
											<TD class="plainlabel"><input type="text" name="DT_Tu"
												style="text-align: left; width: 133px"
												onkeypress="return keypress(event)"
												value="<%=hchBean.getDoanhThu_Tu()%>"> &le; Doanh
												Thu &lt; <input type="text" name="DT_Den"
												style="text-align: left; width: 133px"
												onkeypress="return keypress(event)"
												value="<%=hchBean.getDoanhThu_Den()%>"></TD>
										</TR>
