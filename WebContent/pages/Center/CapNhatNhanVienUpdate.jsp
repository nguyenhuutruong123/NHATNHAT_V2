...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
		...
		<div id="main" style="width: 100%">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 70%; padding: 5px; float: left"
					...
					<%=Utility.GLanguage("Cập nhật",session,jedis) %></div>
			...
			<div align="left"
				...
				<fieldset>
					<legend class="legendtitle"><%=Utility.GLanguage("Cập nhật nhân viên",session,jedis) %> </legend>
								...
								<td>
									...
									<ul class="tabs">
										<li
											...
											<a href="#tabNv"><%=Utility.GLanguage("Cập nhật nhân viên",session,jedis) %></a>
										</li>
										<%if(obj.getphanloai().equals("2")){ %>
										<li
											<%=obj.getActiveTab().equals("1") ? "class='current'" : "" %>><a
											href="#tabMenu"><%=Utility.GLanguage("Menu",session,jedis) %></a></li>
										<li
											<%=obj.getActiveTab().equals("2") ? "class='current'" : "" %>><a
											href="#tabKenh"><%=Utility.GLanguage("Kênh",session,jedis) %></a></li>
										<li
											<%=obj.getActiveTab().equals("3") ? "class='current'" : "" %>><a
											href="#tabNpp"><%=Utility.GLanguage("Nhà phân phối",session,jedis) %></a></li>
										<li
											<%=obj.getActiveTab().equals("4") ? "class='current'" : "" %>><a
											href="#tabSp"><%=Utility.GLanguage("Sản phẩm",session,jedis) %></a></li>
										<li
											<%=obj.getActiveTab().equals("5") ? "class='current'" : "" %>><a
											href="#tabKho"><%=Utility.GLanguage("Kho",session,jedis) %></a></li>
										<%} %>
									</ul>
													...
													<TD width="100%" align="left">
														<FIELDSET>
															...
															<TABLE class="tblight" width="100%" cellpadding="6"
																cellspacing="0">
																<TR>
																	<TD width="15%" class="plainlabel"><%=Utility.GLanguage("Họ tên",session,jedis) %></TD>
																...
																<TR>
																	<TD class="plainlabel"><%=Utility.GLanguage("Tên đăng nhập",session,jedis) %></TD>
																...
																<TR>
																	<TD class="plainlabel"><%=Utility.GLanguage("Mật khẩu khởi tạo",session,jedis) %></TD>
 ...
 %>
																	...
																	<TD class="plainlabel"><select name="nppId">
																			...
																			<%
																				if (nhaphanphoi != null)
																					...
																					System.out.println("nha phan phoi la "+ obj.getnppId());
																					System.out.println("nha phan phoi la "+ nhaphanphoi.getString("convsitecode"));
																								if (nhaphanphoi.getString("convsitecode").trim()
																										.equals(obj.getnppId().trim())) {
																			%>
																			<option
																...
																<TR>
																	<TD class="plainlabel"><%=Utility.GLanguage("Danh mục quyền",session,jedis) %> </TD>
																...
																%>
																<TR>
																	<TD class="plainlabel"><%=Utility.GLanguage("Loại NV",session,jedis) %></TD>
																	<TD class="plainlabel"><select name="loai"
																		id="loai" onchange="submitform();">
																			<option value="0"
																				<%=obj.getLoai().equals("0") ? " selected " : ""%>></option>
																			<option value="4"
																				<%=obj.getLoai().equals("4") ? " selected " : ""%>><%=Utility.GLanguage("NHÂN VIÊN BÁN HÀNG",session,jedis) %></option>
																			<option value="5"
																				<%=obj.getLoai().equals("5") ? " selected " : ""%>><%=Utility.GLanguage("Ban Giám Đốc",session,jedis) %></option>
																			<option value="1"
																				<%=obj.getLoai().equals("1") ? " selected " : ""%>><%=Utility.GLanguage("Giám Đốc Miền",session,jedis) %></option>
																			<option value="2"
																				<%=obj.getLoai().equals("2") ? " selected " : ""%>><%=Utility.GLanguage("Phụ trách Vùng",session,jedis) %></option>
																			<option value="3"
																				<%=obj.getLoai().equals("3") ? " selected " : ""%>><%=Utility.GLanguage("Phụ Trách Tỉnh/ GĐCN Cấp 2",session,jedis) %></option>																			
																	</select></TD>
																</TR>
																<%
																	if (obj.getLoai().equals("1") || obj.getLoai().equals("2")|| obj.getLoai().equals("3")&& obj.getLoaiRs() != null) {
																				ResultSet loaiRs = obj.getLoaiRs();
																				String title = obj.getLoai().equals("1") ? "Giám đốc miền" : obj.getLoai().equals("2") ? "Phụ trách vùng" : obj.getLoai().equals("3") ? "Phụ trách tỉnh/GĐCN cấp 2" : "NV";
																%>
																<TR>
										...
										<%if(obj.getphanloai().equals("1")){ %>										
											...
											<legend><%=Utility.GLanguage("Tỉnh thành",session,jedis) %> &nbsp;</legend> 
											...
											<legend> <%=Utility.GLanguage("Quận Huyện",session,jedis) %>&nbsp;</legend> 
											<select name="qhId" id="qhId" multiple="multiple" style="width: 100%; height: 200px" >
											       ...
											       <TR class="plainlabel" valign="bottom">
														...
														<legend><%=Utility.GLanguage("Kho",session,jedis) %> &nbsp;</legend> 
												<TABLE width="100%" border="0" cellspacing="1px"
												cellpadding="3px">
												<TR class="tbheader">
													<!-- <TH width="20%">Mã kho</TH> -->
													<TH width="20%"><%=Utility.GLanguage("Tên kho",session,jedis) %></TH>
													<TH width="20%"><%=Utility.GLanguage("Diễn giải",session,jedis) %></TH>
													<TH width="10%"><%=Utility.GLanguage("Chọn tất cả",session,jedis) %><input type="checkbox"
														name="allkho" value='1'
									 ...
									 { if (k % 2 != 0) {%>
										...
										<div id="tabMenu" class="tab_content">
											<TABLE width="100%" border="0" cellspacing="1px"
												cellpadding="3px">
												<TR class="tbheader">
													<TH width="20%"><%=Utility.GLanguage("Tên quyền",session,jedis) %></TH>
													<TH width="10%"><%=Utility.GLanguage("Chọn tất cả",session,jedis) %><input type="checkbox"
									 ...
									 while(quyen.next())
									 {
										...
										<div id="tabKenh" class="tab_content" <%=style%>>
											...
											<TABLE width="100%" border="0" cellspacing="1px"
												...
												<TR class="tbheader">
													<TH width="20%"><%=Utility.GLanguage("Tên kênh",session,jedis) %></TH>
													<TH width="10%"><%=Utility.GLanguage("Chọn tất cả",session,jedis) %><input type="checkbox"
												...
												<TR class="plainlabel" valign="bottom">
													<th colspan="3">
														...
														<fieldset style="width: 30%; float: left;">
															...
															<select name="kbhId" id="kenhId" multiple="multiple"
																onchange="AjaxNpp();">
																<option value=""></option>
																<% if(kenhRs != null) {
		                         while(kenhRs.next()) 
		                         {
		                           if(obj.getKenhId().indexOf(kenhRs.getString("pk_seq")) >= 0 ){ %>
														...
														<fieldset style="width: 30%; float: left;">
															...
															<select name="vung" id="vungId" multiple="multiple"
																onchange="AjaxNpp()">
                         ...
                         while(vung.next()) 
                         {
                           if(obj.getvungId().indexOf(vung.getString("pk_seq")) >= 0 ){ %>
														...
														<fieldset style="width: 30%; float: left;">
															...
															<select name="khuvuc" multiple="multiple" id="khuvucId"
																onchange="AjaxNpp()">
																<option value=""><%=Utility.GLanguage("Chọn hết",session,jedis) %></option>
																<% if(khuvuc != null) 
			            {
			            	while(khuvuc.next())
	                          {
	                            ...
	                            { %>
											...
											<TABLE id="nppTable" width="100%" border="0"
												...
												<TR class="tbheader">
													<TH width="20%"><%=Utility.GLanguage("Mã",session,jedis) %></TH>
													<TH width="20%"><%=Utility.GLanguage("Tên",session,jedis) %></TH>
													<TH width="10%"><%=Utility.GLanguage("Chọn tất cả",session,jedis) %><input type="checkbox"
														name="allnpp" value='1'
									 ...
									 while(npp.next())
									 {
										...
										<div id="tabSp" class="tab_content" <%=style%>>
											<TABLE width="100%" border="0" cellspacing="1px"
												...
												<TR class="tbheader">
													<TH width="20%"><%=Utility.GLanguage("Tên Sản phẩm",session,jedis) %></TH>
													<TH width="10%"><%=Utility.GLanguage("Chọn tất cả",session,jedis) %><input type="checkbox"
												...
												<tr>
													<td align="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span
														style="font-weight: bold; font-size: 0.9em"><%=Utility.GLanguage("Nhãn hàng",session,jedis) %>
															 </span>&nbsp;&nbsp; <select name="nhanhangId"
														onchange="submitform();">
															<option value="" selected></option>
															<% if(nhanhang!=null)
											while(nhanhang.next())
										   	...
										   	{%>
															...
															<%} }%>
													</select> <span style="font-weight: bold; font-size: 0.9em"><%=Utility.GLanguage("Chủng loại",session,jedis) %>
															 </span>&nbsp;&nbsp;<select name="chungloaiId"
														onchange="submitform();">
															<option value="" selected></option>
															<% if(chungloai!=null)
