...
public class ErpYeucauxuatkhoNpp implements IErpYeucauxuatkhoNpp
{
	...
	public void createRs() 
	{
		...
		System.out.print(" n qr khach hang rs: "+query +"\n");
		if(this.khId.trim().length() > 0 && this.khoNhanId.trim().length() > 0 )
		{
			...
			query = "select PK_SEQ, sohoadon + KYHIEU + ' / ' + NGAYXUATHD as ten " +
					"from ERP_HOADONNPP where TRANGTHAI in (2, 4) and hoantat = 0 and NPP_FK = '" + this.nppId + "' and KHO_FK = '" + this.khoNhanId + "'  ";
			...
			if(this.id.trim().length() > 0)
			{
				query += " union  " +
						 ...
						 "from ERP_HOADONNPP where pk_seq in ( select hoadon_fk from ERP_YCXUATKHONPP_DDH where ycxk_fk = '" + this.id + "'  )   ";
			}
		...
		if(this.ddhId.trim().length() > 0 && this.khoNhanId.trim().length() > 0 )
		{
			...
			query = " select ddh.*, ISNULL(xuat.soluongXUAT, 0) as xuat,   " +
					...
					" 	ISNULL( ( select AVAILABLE from NHAPP_KHO where kho_fk = '" + this.khoNhanId + "' and sanpham_fk = ddh.PK_SEQ and NPP_FK = '" + this.nppId + "' "
							+ " and KBH_FK = ( select top(1) kbh_fk from ERP_HOADONNPP where pk_seq in (" + this.ddhId + ") ) ), 0) + ISNULL(xuat.soluongXUAT, 0) as tonkho    " +
					" from   " +
					" (   " +
					...
					"		union all	" +
					"  		select a.sanpham_fk, b.DVDL_FK as dvCHUAN,    " + 
					"  			a.soluong as soluong, 1 as loai, a.ctkm as scheme   " + 
					...
					" 		and a.PK_SEQ in ( select ycxk_fk from  ERP_YCXUATKHONPP_DDH where hoadon_fk in (  " + this.ddhId + "  ) )  " +
	...
	public void init() 
	{
		...
		if(rs != null)
		{
			try 
			{
				...
				//INIT DDH
				query = "select hoadon_fk from ERP_YCXUATKHONPP_DDH where ycxk_fk = '" + this.id + "' ";
				rs = db.get(query);
				String ddhID = "";
				while(rs.next())
				{
	...
	public ResultSet getSoloTheoSp(String spIds, String tongluong)
	{
		...
		String query = "select AVAILABLE + ISNULL( ( select sum(soluong) from ERP_YCXUATKHO_SANPHAM_CHITIET where ycxk_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  ), 0 ) as AVAILABLE, " + 
	...
	public boolean create() 
	{
		...
		try
		{
			...
			String query = " insert ERP_YCXUATKHONPP(NgayYeuCau, ghichu, trangthai, npp_fk, kho_fk, xuatcho, NPP_DAT_FK, KHACHHANG_FK, KBH_FK, ngaytao, nguoitao, ngaysua, nguoisua) " +
						   " select top(1) '" + this.ngayyeucau + "', N'" + this.ghichu + "', '0', '" + this.nppId + "', " + this.khoNhanId + ", '" + this.xuatcho + "', " + npp_dat_fk + ", " + khachhang_fk + ", kbh_fk, " +
						   		" '" + getDateTime() + "', '" + this.userId + "', '" + getDateTime() + "', '" + this.userId + "' " +
						   " from ERP_HOADONNPP where pk_seq = '" + this.ddhId + "' ";
			...
			query = "Insert ERP_YCXUATKHONPP_DDH(ycxk_fk, hoadon_fk,ddh_fk) " +
					"select IDENT_CURRENT('ERP_YCXUATKHONPP'), HOADONNPP_FK,DDH_FK from ERP_HOADONNPP_DDH  a inner join ERP_HOADONNPP b on a.HOADONNPP_FK=b.pk_seq  where a.HOADONNPP_FK in ( " + this.ddhId + " )  and b.trangthai not in (3,5) ";
					...
					if(this.sanpham_soluong != null)
					{
						...
						while(keys.hasMoreElements())
						{
							...
							if(key.startsWith( spId[i] + "__" + spLoai[i] + "__" + spSCheme[i].trim() ))
							{
								...
								{
									query = "insert ERP_YCXUATKHONPP_SANPHAM_CHITIET( ycxk_fk, SANPHAM_FK, solo, ngayhethan, ngaynhapkho, soluong, loai, scheme, kho_fk ) " +
											"select IDENT_CURRENT('ERP_YCXUATKHONPP'), pk_seq, N'" + _sp[3] + "', '" + _sp[4] + "', '" + _sp[5] + "', '" + _soluongCT.replaceAll(",", "") + "', '" + spLoai[i] + "', '" + spSCheme[i] + "', '" + this.khoNhanId + "' " +
	...
	public boolean update() 
	{
		...
		try
		{
			...
			if(!Utility.KiemTraNgayChungTuSoNet(db,this.userId,this.nppId ,"BanHang_SoChungTu",this.ngayyeucau, this.id,"ERP_YCXUATKHONPP" ))
			{
			...
			String query = " Update ERP_YCXUATKHONPP set NgayYeuCau = '" + this.ngayyeucau + "', ghichu = N'" + this.ghichu + "', npp_fk = '" + this.nppId + "', kho_fk = '" + this.khoNhanId + "', " +
						   "	xuatcho = '" + this.xuatcho + "', khachhang_fk = " + khachhang_fk + ", kbh_fk = ( select top(1) KBH_FK from ERP_HOADONNPP where pk_seq = '" + this.ddhId + "' ), npp_dat_fk = " + npp_dat_fk + ", " +
			...
			query = "Insert ERP_YCXUATKHONPP_DDH(ycxk_fk, hoadon_fk) " +
