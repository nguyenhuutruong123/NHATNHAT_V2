			...
			if(spId != null)
			{
				...
				while(m < spId.length)
				{
					if(spGiasieuthi[m].length() > 0) //chi them nhung san pham co gia
					{
						...
						String sql = "insert into banggiast_sanpham(bgst_fk, sanpham_fk, giasieuthi) values('"+ this.id +"','" + spId[m] + "','" + spGiasieuthi[m].replaceAll(",", "") + "')";
						if(!db.update(sql))
						{
							db.update("rollback");
		...
		if(spId != null)
		{
			...
			while(m < spId.length)
			{
				if(spGiasieuthi[m].length() > 0) //chi them nhung san pham co gia
				{
					...
					String sql = "insert into banggiast_sanpham(bgst_fk, sanpham_fk, giasieuthi) values('" + this.id + "','" + spId[m].replaceAll(",", "") + "','" + spGiasieuthi[m].replaceAll(",", "") + "')";
		...
		try{
			...
			if(num > 0){ //Buoc 1; xoa cac nha phan phoi khong duoc chon trong form dieu chuyen bang gia cho npp
				...
				//	System.out.println("chbox" + rs.getString("nppId"));
					...
					if (request.getParameter("chbox" + rs.getString("khachhang_fk")) == null ){
//						System.out.println("chbox" + rs.getString("nppId"));
				...
				if (request.getParameter("chbox" + rs.getString("pk_seq")) != null ){
					...
					//System.out.println("Get khachhang:  "+query);
						...
						if(!cn.update(sql_insert)){
							...
							//System.out.println("insert into BANGGIAMUANPP_NPP values('" + this.id + "', '" + request.getParameter("chbox" + rs.getString("nppId")) + "')");
