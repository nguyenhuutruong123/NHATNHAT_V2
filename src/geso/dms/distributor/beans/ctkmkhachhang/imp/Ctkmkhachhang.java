...
public class Ctkmkhachhang implements ICtkmkhachhang, Serializable 
{
	...
	public void init() 
	{
		...
		sql = "select scheme + ' - ' + diengiai as ten, tungay, denngay from CTKHUYENMAI where PK_SEQ = '" + this.Id + "'";
		rs = db.get(sql);
		if(rs != null)
		{
			try
	...
	private void createKhachHang()
	{
		...
		{
			String query = "select pk_seq, ten, isnull(diachi, '') as diachi, isnull(dienthoai, '') as dienthoai, '1' as chon from khachhang where npp_fk ='" + this.nppId + "' and pk_seq in (select khachhang_fk from CTKM_KHACHHANG where nhomkhnpp_fk ='"+ this.Id +"')";
			query = query + " union select pk_seq, ten, isnull(diachi, '') as diachi, isnull(dienthoai, '') as dienthoai, '0' as chon from khachhang where npp_fk ='" + this.nppId + "' and pk_seq not in (select khachhang_fk from CTKM_KHACHHANG where nhomkhnpp_fk ='"+ this.Id +"')";
