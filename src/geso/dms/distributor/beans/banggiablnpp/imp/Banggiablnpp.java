...
public class Banggiablnpp implements IBanggiablnpp
{
	...
	public boolean CreateBgbl(String[] spId, String[] spGiabanlechuan, String[] spGiabanlenpp) 
	{
		try{
			...
			String query = "select count(pk_seq) as num from banggiabanlenpp where dvkd_fk='" + this.dvkdId + "' and npp_fk ='"+ this.nppId +"'";
			...
			if(rs.getString("num").equals("0")){
				...
				if(!this.db.update(query))
				{
					...
					this.msg = "Khong the tao moi 'Bang Gia Ban Le Nha Phan Phoi ' , " + this.nppTen + "";
					return false; 
				}
	...
	{	
		...
		{
			cn.update("rollback");
			this.msg = "Khong the cap nhat 'Bang gia ban le' , " + query;
			return false; 
		}
		...
		cn.getConnection().setAutoCommit(true);
			...
			cn.update("rollback");
			er.printStackTrace();
			this.msg="Loi Khi Cap Nhat Lại Bang Gia,Vui Long Bao Voi Admin. Error : 352 "+ er.toString();
			return false;
		}
		return true;
	}
