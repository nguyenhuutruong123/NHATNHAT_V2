...
//									 
									...
									this.msg=util.Update_NPP_Kho_Sp_Chitiet(this.ngayyeucau, "Xuất kho khác", db, this.khoXuatId, spId, this.nppId, this.kbhId,
...
//										return false;
//									}
									this.msg=util.Update_NPP_Kho_Sp(this.ngayyeucau, "Xuất kho khác", db, this.khoXuatId, spId, this.nppId, this.kbhId , 0.0, Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), 0.0);
									if(this.msg.length()>0)
									{
										db.getConnection().rollback();
										return false;
		...
		//TANG KHO NGUOC LAI
			...
			ResultSet nkRs= db.get(query);
		 	while (nkRs.next())
		 	{
				...
				msg=util.Update_NPP_Kho_Sp_Chitiet(ngaychuyen, "Đơn trả hàng về NCC-TT duyệt", db, kho_fk, sanpham_fk, npp_fk, kbh_fk, solo, NgayHetHan, ngaynhapkho, 0.0, (-1)*tongxuat, tongxuat, tongxuat, 0.0);
				if(msg.length()>0)
				{
					db.getConnection().rollback();
				...
				msg=util.Update_NPP_Kho_Sp(ngaychuyen, "Đơn trả hàng về NCC-UPDATE", db, kho_fk, sanpham_fk, npp_fk, kbh_fk, 0.0,  (-1)*tongxuat, tongxuat, 0.0);
				if(msg.length()>0)
				{
					db.getConnection().rollback();
					return false;
...
//									}
									...
									this.msg=util.Update_NPP_Kho_Sp_Chitiet(this.ngayyeucau, "Xuất chuyển kho khác - UPDATE", db, this.khoXuatId , spId, this.nppId, this.kbhId,
											_sp[1], ngayhethan, ngaynhapkho,0.0 , Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), (-1)*Double.parseDouble(_soluongCT.replaceAll(",", "")), 0.0);
									if(this.msg.length()>0)
									{
										db.getConnection().rollback();
...
//										return false;
//									}
									this.msg=util.Update_NPP_Kho_Sp(this.ngayyeucau, "Xuất kho khác -UPDATE", db, this.khoXuatId , 
