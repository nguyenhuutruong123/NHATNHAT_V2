...
import geso.dms.distributor.beans.hoadonphelieu.imp.ErpHoaDonPL_SP;
import geso.dms.center.db.sql.dbutils;
import geso.dms.distributor.beans.hoadonphelieu.IErpHoaDonPL_SP;
import geso.dms.distributor.beans.hoadonphelieu.IErpHoadonphelieu;
...
public class ErpHoadonphelieu implements IErpHoadonphelieu
{
	...
	public void init() 
	{
		...
		String query =  " select a.ngayghinhan, a.ngayhoadon, a.kyhieuhoadon, a.sohoadon, " +
						...
						" from erp_hoadonphelieu a inner join khachhang b on a.khachhang_fk = b.pk_seq " +
	...
	{	
		...
		try 
		{
			...
			String sql="select count(*) as sodong from erp_hoadonphelieu where sohoadon='"+this.sohieuhoadon+"' and kyhieuhoadon ='"+this.kyhieuhoadon+"' and trangthai != 2 and npp_fk = "+ this.nppId +" ";
			...
			db.getConnection().setAutoCommit(false);
			 ...
			 query = " insert erp_hoadonphelieu(ngayghinhan, ngayhoadon, kyhieuhoadon, sohoadon, bvat, vat, avat, khachhang_fk, diengiai,  trangthai, nguoitao, ngaytao, nguoisua, ngaysua, npp_fk)  " +
							"values('" + this.ngayghinhan + "', '" + this.ngayhoadon + "', '" + this.kyhieuhoadon + "', '" + this.sohieuhoadon + "', '" + this.Bvat.replaceAll(",", "") + "', '" + this.vat.replaceAll(",", "") + "', '" + this.Avat.replaceAll(",", "") + "', '" + this.khId + "',  N'" + this.diengiai + "',  '0'," +
									" '" + this.userId + "', '" + getDateTime() + "', '" + this.userId + "', '" + getDateTime() + "', '"+ this.nppId +"') ";
			...
			while(count < this.sanphamlist.size() )
			{
				...
				{
					query = "insert ERP_HoaDonPheLieu_SanPham(hoadonphelieu_fk, diengiai, donvitinh, soluong, dongia, thuevat, vat, thanhtien, ghichu)  " +
		...
		try 
		{
			...
			// check so hoa don && ky hieu hoa don 
				...
				String sql=" select count(*) as sodong"
						 ...
						 + " where sohoadon='"+this.sohieuhoadon+"' and kyhieuhoadon ='"+this.kyhieuhoadon+"'"
						 + "       and trangthai != 2 and npp_fk = "+ this.nppId +" and pk_seq != "+ this.id +" ";
				ResultSet checkHD = db.get(sql);
				while(checkHD.next())
				{
					sodong= checkHD.getInt("sodong");
			...
			String query = "update erp_hoadonphelieu set ngayghinhan = '" + this.ngayghinhan + "', ngayhoadon = '" + this.ngayhoadon + "', kyhieuhoadon = '" + this.kyhieuhoadon + "', sohoadon = '" + this.sohieuhoadon + "', " +
							...
							"khachhang_fk = '" + this.khId + "', diengiai = N'" + this.diengiai + "', " +
