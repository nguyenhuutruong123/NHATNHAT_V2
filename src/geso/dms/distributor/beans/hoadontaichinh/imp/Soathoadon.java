...
import geso.dms.distributor.beans.hoadontaichinh.ISoathoadon;
...
public class Soathoadon implements ISoathoadon
{
	...
	public void init()
	{
		...
		if(this.tungay.trim().length() > 0 || this.denngay.trim().length() > 0 || this.sohoadontu.trim().length() > 0 || this.sohoadonden.trim().length() > 0 )
		{
			...
			{
			 query =  "select a.PK_SEQ, a.sohoadon, cast(a.sohoadon as numeric(18, 0)) as sohoadonSORT, a.kyhieu, a.trangthai, a.ngayxuatHD, KH.MAFAST as MAFAST, KH.TEN as khTEN,(case when a.LOAIHOADON=1 and isnull(d.donquatang,0)=0 and isnull(d.DONHANGKHAC,0)=0 then 0 else 1 end)* isnull(a.tongtienBVATNK,a.tongtienBVAT) - isnull(isnull(a.chietkhauNK,a.chietkhau), 0) as tongtienBVAT,(case when a.LOAIHOADON=1 and isnull(d.donquatang,0)=0 and isnull(d.DONHANGKHAC,0)=0 then 0 else 1 end)* isnull(a.VATNK,a.VAT) as VAT,(case when a.LOAIHOADON=1 and isnull(d.donquatang,0)=0 and isnull(d.DONHANGKHAC,0)=0 then 0 else 1 end)* isnull(a.tongtienAVATNK,a.tongtienAVAT) as tongtienAVAT , " +
							...
							"\n inner join HOADON_DDH c on c.HOADON_FK=a.PK_SEQ "+
							 "\n inner join DONHANG d on d.PK_SEQ=c.DDH_FK "+
							"\n inner join KHACHHANG KH on a.KHACHHANG_FK = KH.PK_SEQ  " +
							"\n where    ISNULL(inchung1,0)=0  and a.SoHoaDon!='NA' and ISNUMERIC( a.SOHOADON)= 1  and  a.NPP_FK = '" + this.nppId + "' and a.trangthai not in ( 3, 5 ) and a.kho_fk in "+util.quyen_kho(this.userId) ;
			...
			String queryETC =   "select a.PK_SEQ, a.sohoadon, cast(a.sohoadon as numeric(18, 0)) as sohoadonSORT, a.kyhieu, a.trangthai, a.ngayxuatHD,   " +
			...
			String queryHDKhac = 
				" SELECT a.pk_seq, a.sohoadon,  cast(a.sohoadon as numeric(18, 0)) as sohoadonSORT, a.kyhieuhoadon kyhieu, a.trangthai, a.ngayhoadon ngayxuatHD,\n "+
...
/*			if(this.nvbhId.trim().length() > 0)
	...
	public void initETC()
	{
		...
		if(this.tungay.trim().length() > 0 || this.denngay.trim().length() > 0 || this.sohoadontu.trim().length() > 0 || this.sohoadonden.trim().length() > 0 )
		{
			String query =  "select a.PK_SEQ, a.sohoadon, cast(sohoadon as numeric(18, 0)) as sohoadonSORT, a.kyhieu, a.trangthai, a.ngayxuatHD, KH.MAFAST as MAFAST, KH.TEN as khTEN, isnull(a.tongtienBVATNK,a.tongtienBVAT) as tongtienBVAT,isnull (a.VATNK,a.VAT) as VAT, isnull(a.tongtienAVATNK,a.tongtienAVAT) as tongtienAVAT, " +
							"	case a.loaihoadon when 0 then N'HĐ hàng bán' else N'HĐ hàng khuyến mại' end as loaiHD, 'OTC' as type   " +
							"from HOADON a  " +
							"inner join KHACHHANG KH on a.KHACHHANG_FK = KH.PK_SEQ  " +
							"where   ISNULL(inchung1,0)=0 and a.sohoadon != 'NA' and ISNUMERIC( a.SOHOADON)= 1  and a.NPP_FK = '" + this.nppId + "' and a.trangthai in ( 2, 4 )  and a.kho_fk in "+util.quyen_kho(this.userId)  ;
...
/*			if(this.nvbhId.trim().length() > 0)
		...
		try 
		{
			...
			if(kq==1)
			{
				...
				if(mahoadon != null )
				{
					...
					"	where  trangthai != 3 and sohoadon != 'NA' and isnull(inchung1,0)=0 and npp_fk = " +this.nppId+ 
					"union ALL " +
			...
			if(mahoadon != null )
			{
				for(int i = 0; i < mahoadon.length; i++)
				{
					if(type[i].equals("OTC"))
					{
						...
						query="	select ( select count(*) from DONHANG_CTKM_TRAKM where donhangID = (select a.DDH_FK from HOADON_DDH a where a.HOADON_FK='"+mahoadon[i]+"') and spMA is not null "+
						...
						if(flag_inchung==0)
						{
							...
							query="select a.DDH_FK,b.PK_SEQ,b.LOAIHOADON from HOADON_DDH a inner join HOADON b on a.HOADON_FK=b.PK_SEQ  \n"+
								  "	where b.LOAIHOADON not in (3,5) and a.DDH_FK=(select a.DDH_FK from HOADON_DDH a where a.HOADON_FK='"+mahoadon[i]+"') \n"+ 
								  "	and b.LOAIHOADON=1 \n";

							ResultSet rskm=db.get(query);
							if(rskm.next())
							{
								query = "Update HOADON set sohoadon = '" + sohoadon[i] + "', kyhieu = '" + kyhieu[i] + "' , ngayxuathd = '"+ ngayhoadon[i] +"' where pk_seq = '" + rskm.getString("pk_seq") + "' ";
								System.out.println("--SOAT HOA DON: " + query);
	...
	public void createRs() 
	{
		...
		this.khRs = db.get("select  PK_SEQ, MAFAST + ', ' + TEN AS TEN from KHACHHANG where TRANGTHAI = '1' and NPP_FK = '" + this.nppId + "' ");
		this.KyhieuHDRs = db.get("select distinct ISNULL(KYHIEU,'') as KYHIEU from HOADON where TRANGTHAI <> 3 and NPP_FK = '" + this.nppId + "' ");
		System.out.println("KÝ HIỆU: select distinct ISNULL(KYHIEU,'') as KYHIEU from HOADON where TRANGTHAI <> 3 and NPP_FK = '" + this.nppId + "' ");
	}
			...
			if(mahoadon != null )
			{
				for(int i = 0; i < mahoadon.length; i++)
				{
					if(sohoadonMOI[i].trim().length() > 0)
					{
						...
						if(type[i].equals("OTC"))
						{
							...
							if(loaiHD[i].trim().equals("HĐ hàng khuyến mại"))  //HOA DON KHUYEN MAI
							{
								...
								if(db.updateReturnInt(query) <= 0 )
								{
									...
									query ="insert HOADON_CTKM_TRAKM_CHITIET(HoaDonId,hoadon_fk, sanpham_fk, soluong, scheme, solo, ngayhethan, kbh_fk, kho_fk, dongia,ngaynhapkho )"+
											" select '" + hdId + "','" + hdId + "', sanpham_fk, soluong, scheme, solo, ngayhethan, kbh_fk, kho_fk, dongia,ngaynhapkho   " +
							...
							else
							{
								...
								query = "insert HOADON_SP_CHITIET( hoadon_fk, MA, TEN, DONVI, DONGIA, SOLO, NGAYHETHAN, THUEVAT, SOLUONG, CHIETKHAU,ngaynhapkho ) " +
										"select '" + hdId + "', MA, TEN, DONVI, DONGIA, SOLO, NGAYHETHAN, THUEVAT, SOLUONG, CHIETKHAU,ngaynhapkho " +
						...
						{
							//COPY HOA DON MOI
							query = "insert ERP_HOADONNPP(FROM_HOADON_GOC, ngayxuatHD, trangthai, ngaytao, nguoitao, ngaysua, nguoisua, dondathangnpp_fk, sohoadon, kyhieu, chietkhau, tongtienbvat, tongtienavat, vat, hinhthuctt, ghichu, loaixuatHD, NPP_FK, KHACHHANG_FK, hopdong, nguoimua,TENKHACHHANG,DIACHI,MASOTHUE ) " +
									"select '" + mahoadon[i] + "', ngayxuatHD, trangthai, ngaytao, nguoitao, ngaysua, nguoisua, dondathangnpp_fk, sohoadon, kyhieu, chietkhau, tongtienbvat, tongtienavat, vat, hinhthuctt, ghichu, loaixuatHD, NPP_FK, KHACHHANG_FK, hopdong, nguoimua ,TENKHACHHANG,DIACHI,MASOTHUE " +
									"from ERP_HOADONNPP where pk_seq = '" + mahoadon[i] + "'";
							...
							query = "insert ERP_HOADONNPP_DDH(hoadonnpp_fk, ddh_fk) " +
									"select '" + hdId + "', ddh_fk from ERP_HOADONNPP_DDH where hoadonnpp_fk = '" + mahoadon[i] + "'";
							if(db.updateReturnInt(query) <= 0 )
							{
							...
							query = "Update ERP_HOADONNPP set trangthai = '5' where pk_seq = '" + mahoadon[i] + "' ";
							//System.out.println("--SOAT HOA DON: " + query);
							...
							System.out.println("SoHD_ETC_new" + SoHD_ETC_new);
						}
						else{
							//COPY HOA DON MOI
				...
				/*query = "select sohoadon  " +
					...
					"( " +
					"	select sohoadon, kyhieu, MAUHOADON from HOADON  " +
					"	where  trangthai != 3 and isnull(inchung1,0)=0 and sohoadon != 'NA' and npp_fk = " +this.nppId+
					"union ALL " +
					"	select sohoadon, kyhieu, MAUHOADON from ERP_HOADONNPP  " +
					"	where  trangthai != 3 and sohoadon != 'NA' and npp_fk = " +this.nppId+
					/*"union ALL " +
