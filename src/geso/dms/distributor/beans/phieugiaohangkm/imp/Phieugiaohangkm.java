...
public class Phieugiaohangkm implements IPhieugiaohangkm
{
	...
	public Phieugiaohangkm(String id)
	{
		this.id = id;
		this.ngayxuatHD = getDateTime();
		this.ngayghinhanCN = getDateTime();
					...
					else
					{
						...
						// KIEM TRA SOHOADON DA DUOC DUNG CHUA : 
							...
							query =" select count(pk_seq) as dem" +
								   ...
								   "      and trangthai != '3' and nguoisua= "+ this.userId +"  ";
			  ...
			  try 
			  {
				  ...
				  {				    	
					  while(rsLaySP.next())
					  {
							...
							tienthue = (rsLaySP.getDouble("SOLUONG") * rsLaySP.getDouble("DONGIA") - rsLaySP.getDouble("chietkhau"))*rsLaySP.getDouble("vat")/100;
							spTienThue += tienthue + "__";
					  }
					  rsLaySP.close();
	...
	public boolean create() 
	{
		...
		try
		{
			...
			if(!loainpp.equals("4") && !loainpp.equals("5"))	// KHONG PHAI LA DOI TAC VA CHI NHANH CUA DOI TAC
			{
			 ...
			 query = " insert HOADON(ngayghinhanCN, khachhang_fk, ngayxuatHD, trangthai, ngaytao, nguoitao, ngaysua, nguoisua, kyhieu, sohoadon, hinhthuctt ," +
					       " ghichu, npp_fk, loaihoadon, nguoimua, innguoimua) " +
						   " values( '"+ this.ngayghinhanCN +"', "+ this.khId +",'" + this.ngayxuatHD + "', '1','" + getDateTime() + "', '" + this.userId + "', '" + getDateTime() + "', '" + this.userId + "', " +
						   "       RTRIM(LTRIM('" + this.kyhieuhoadon + "')), RTRIM(LTRIM('" + this.sohoadon + "')) , N'"+ this.hinhthuctt +"', N'"+ this.ghichu +"', "+ this.nppId +" , '0', N'"+ this.nguoimua +"', "+ this.innguoimua +")";
			...
			query = "Update PHIEUXUATKHO set NGAYLAPPHIEU = '" + this.ngayxuatHD + "' " +
					"where PK_SEQ in ( select pxk_fk from PHIEUXUATKHO_DONHANG where donhang_fk in (  select ddh_fk from HOADON_DDH where hoadon_fk = '" + hdId + "' ) )  ";
				...
				if(spId[i].trim().length() > 0 && Double.parseDouble(spSoluong[i].trim().replaceAll(",", "")) > 0 )
				{
					query = "insert HOADON_SP( hoadon_fk, sanpham_fk, donvitinh, soluong, dongia, thanhtien, chietkhau, scheme, vat, dongiaGOC ) " +
							" select "+ hdId +" , '" + spId[i] + "', N'"+ spDonvi[i] +"', '" + spSoluong[i].replaceAll(",", "") + "', '" + spDongia[i].replaceAll(",", "") + "'," +
									...
									"		[dbo].[GiaBanLeNppSanPham]("+nppId+","+spId[i]+",(select  ngaynhap from DONHANG where pk_seq in ( " + this.ddhId + " ) ) )  ";
			...
			query = "insert HOADON_CHIETKHAU(hoadon_fk, donhang_fk, diengiai, chietkhau, thueVAT, STT, tichluyQUY, HIENTHI) " +
					...
					"	where thueVAT = '5' and donhang_fk in ( select DDH_FK from HOADON_DDH where HOADON_FK = '" + hdId + "' ) and chietkhau > 0 " +
					...
					"	from DONHANG_SANPHAM   " +
					"	where thueVAT = '10' and donhang_fk in ( select DDH_FK from HOADON_DDH where HOADON_FK = '" + hdId + "' ) and chietkhau > 0 " +
					"	group by donhang_fk, thueVAT " +
					"union  " +
					...
					"	where a.thanhtoan > 0 and donhang_fk in ( select DDH_FK from HOADON_DDH where HOADON_FK = '" + hdId + "' )  " +
					"union  " +
					...
					"	where a.chietkhau > 0 and donhang_fk in ( select DDH_FK from HOADON_DDH where HOADON_FK = '" + hdId + "' )  ";
			if( !db.update(query)   )
			{
		...
		try
		{
			...
			else
			{
				query =    " update HOADON set ngayghinhanCN =  '"+ this.ngayghinhanCN +"', khachhang_fk= "+ this.khId +" , ngayxuatHD = '" + this.ngayxuatHD + "' , ngaysua = '" + getDateTime() + "' , nguoisua ='" + this.userId + "'," +
					       ...
					       " ghichu= N'"+ this.ghichu +"', NPP_FK= "+ this.nppId +" , loaihoadon = '0', nguoimua = N'"+ this.nguoimua +"', innguoimua = "+ this.innguoimua +" " +
					       " where pk_seq = '"+ this.id +"'" ;
				if(!db.update(query))
				{
				...
				for(int i = 0; i < spId.length; i++)
				{
					if(spId[i].trim().length() > 0 && Double.parseDouble(spSoluong[i].trim().replaceAll(",", "")) > 0 )
					{
						query = "insert HOADON_SP( hoadon_fk, sanpham_fk, donvitinh, soluong, dongia, thanhtien, chietkhau, scheme, vat, dongiaGOC ) " +
								...
								" (" + spSoluong[i].replaceAll(",", "")+ " * "+ spDongia[i].replaceAll(",", "") +"), '"+ spChietkhau[i].replaceAll(",", "") +"', N'"+ spSCheme[i].replaceAll(",", "") +"', '" + spVat[i].replaceAll(",", "") + "', " +
								"		[dbo].[GiaBanLeNppSanPham]("+nppId+","+spId[i]+",(select  ngaynhap from DONHANG where pk_seq in ( " + this.ddhId + " ) ) ) as dongiaGOC ";
			...
			query = "update kho set kho.SOLUONG = kho.SOLUONG + xuat.soluong, " +
					...
					"( " +
					"	select kho_fk, kbh_fk, NPP_FK, SANPHAM_FK, SOLO, SUM(soluong) as soluong " +
					"   from (	" +
					"		select b.kho_fk, b.kbh_fk, a.NPP_FK, b.SANPHAM_FK, b.SOLO, SUM(b.soluong) as soluong " +
					"		from PHIEUXUATKHO a inner join PHIEUXUATKHO_SANPHAM_CHITIET b on a.PK_SEQ = b.PXK_FK " +
					"		where a.PK_SEQ = '" + pxkID + "' " +
					"		group by b.kho_fk, b.kbh_fk, a.NPP_FK, b.SANPHAM_FK, b.SOLO " +
					"	)" +
					"	TX group by kho_fk, kbh_fk, NPP_FK, SANPHAM_FK, SOLO " +
					") " +
					"xuat on kho.SANPHAM_FK = xuat.SANPHAM_FK and kho.NPP_FK = xuat.NPP_FK and kho.KBH_FK = xuat.kbh_fk and kho.KHO_FK = xuat.kho_fk and kho.SOLO = xuat.SOLO ";
	...
	public boolean chot(String hdId, boolean updateBEFORE) 
	{
		...
		try
		{
			...
			String query = "select top(1) NAM as namMax, THANGKS as thangMax, " +
							...
							"	( select count(*) as soDONG from HOADON where sohoadon = ( select sohoadon from HOADON where pk_seq = '" + hdId + "' ) and sohoadon != 'NA' and npp_fk = ( select npp_fk from HOADON where pk_seq = '" + hdId + "' ) and trangthai != '3' ) as soDONG_SOHOADON " +
			...
			query = "insert HOADON_SP_CHITIET( hoadon_fk, MA, TEN, DONVI, DONGIA, dongiaGOC, SOLO, NGAYHETHAN, THUEVAT, SOLUONG, CHIETKHAU,NGAYNHAPKHO )  " +
					...
					"	SUM( f.soluong) as soluong, '0' as chietkhau ,F.NGAYNHAPKHO  " +
					"from  DONHANG dh inner join DONHANG_SANPHAM dhsp on dhsp.donhang_fk=dh.PK_SEQ    " +
					"				 inner join SANPHAM c on dhsp.sanpham_fk = c.PK_SEQ                 " +
					"				 left join DONVIDOLUONG d on d.PK_SEQ = c.dvdl_fk   " +
					"				 left join PHIEUXUATKHO_DONHANG e on dh.pk_seq = e.donhang_fk  " +
			...
			query =	" SELECT hd.npp_fk, round( isnull(hd_ck.TienCK,0), 0) TienCK, " +
			 ...
			 if(Double.parseDouble(TienCK) > 0 && denghitraCK.equals("1")  )
			 {
				 ...
				 else
				 {
					 query = "INSERT CANTRUCONGNO(NGAYCHUNGTU, TRANGTHAI, NGAYTAO, NGAYSUA, NGUOITAO, NGUOISUA, NPP_FK, GHICHU) " +
							 "values('" + getDateTime() + "', 1, '" + getDateTime() + "', '" + getDateTime() + "', '" + this.userId + "', '" + this.userId + "','" + npp + "',N'Phiếu khấu trừ chiết khấu tháng tự động HD " + id + "')";
					 ...
					 query = "insert CANTRUCONGNO_HOADON(CANTRUCONGNO_FK, HOADON_FK, KHACHHANG_FK, SOTIENCANTRU)" +
			         		 " select IDENT_CURRENT('CANTRUCONGNO'), '" + id + "', khachhang_fk, '" + TienCK + "' from HOADON where pk_seq = '" + id + "'";
   ...
   public String[] getTichLuy_Scheme() {
	...
	public void init_Display() 
	{
		...
		String query =  " select dondathang_fk, ngayxuatHD, ISNULL(ghichu, '') as ghichu, kyhieu, sohoadon, hinhthuctt ,khachhang_fk, npp_fk, trangthai, isnull(nguoimua,'') as nguoimua, isnull(innguoimua,1) as innguoimua,  " +
				        ...
				        "        isnull(vat, 0) as vat, isnull(tongtienavat, 0) as tongtienavat, isnull(tongtienkm, 0) as tongtienkm  " +
						" from HOADON" +
						" where pk_seq = '" + this.id + "'";
		System.out.println("____INIT HOADON: " + query);
		ResultSet rs = db.get(query);
	...
	public ResultSet getSoloTheoSp(String spMa, String donvi, String tongluong)
	{
		...
		String query = "select  AVAILABLE, NGAYHETHAN, SOLO " +
