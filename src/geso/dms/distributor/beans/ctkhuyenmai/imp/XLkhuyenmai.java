...
public class XLkhuyenmai 
{
	...
	{   
		...
		//NẾU CTKM KHAI BÁO PHỤ THUỘC NHAU. THÌ TỪ ĐƠN HÀNG SAU, NẾU ĐÃ CHỌN KM NÀO THÌ CHỈ ĐƯỢC CHỌN KM ĐÓ
...
/*					*/
					...
					if(str.length >=3)
					{
						System.out.println("2222222.Thu tu tổng điểm theo chương trình:" + str[2] );
		...
		else
		{
			...
			ctkhuyenmai.setTong_diem_theo_dkkm(tong_diem_theo_dk);// truong hop tra tiền theo số điểm;
			...
			while(keyList.hasMoreElements())
			{
				...
				if(dkkm.getType() == 2)
				{
					...
					if(hashSanpham.get(key) != null)
					{
						...
						else if(dkkm.getIsThung().equals("2")) // điểm
						{
				...
				if(sanpham != null)
				{
					for(int i=0; i < sanpham.length; i++)
					{
						...
						if(sp_sl.containsKey(key))
						{
							...
							if(ctkm.getTrakhuyenmai().get(0).getType()==4)// tính tiền theo điểm
							{
								System.out.println("-----------------CTKM nay la điểm-------------------------------");
								slgconlai = 0;
								soluongsudung= sp_sl.get(key);
							}
	...
	private Sanpham[] InitSanPham(Dieukienkhuyenmai dkkm, Hashtable<String, Double> sp_sl2, boolean flag)
	{
		...
		while(keyList.hasMoreElements())
		{
			...
			if(sp_sl2.get(key) != null)
			{
				...
				else if(dkkm.getIsThung().equals("2")) // theo điểm
				{
