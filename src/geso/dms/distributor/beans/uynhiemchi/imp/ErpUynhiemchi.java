...
public class ErpUynhiemchi implements IErpUynhiemchi{
		...
		try {
			...
			if (nhomNCCCNId.length() > 0) {
				String query = "Insert ERP_THANHTOANHOADON"
						...
						+ " NGAYTAO, NGUOITAO, NGAYSUA, NGUOISUA, LOAITHANHTOAN, TIENTE_FK, TIGIA, THANHTOANTUTIENVAY, NPP_FK,PREFIX  "
				...
				String query = "Insert ERP_THANHTOANHOADON "
						...
						+ " PHINGANHANG, PHINGANHANGNT, VAT, VATNT, CHENHLECHVND, "
						+ " TRICHPHI, SOTAIKHOAN_TP, NGANHANG_TP_FK, CHINHANH_TP_FK, "
						+ " NGAYTAO, NGUOITAO, NGAYSUA, NGUOISUA, LOAITHANHTOAN, TIENTE_FK, TIGIA , THANHTOANTUTIENVAY, KHACHHANG_FK, NPP_FK, PREFIX, isNPP, isTICHLUY "
						+ " ) " + "values(" + (this.bophanId.length() == 0 ? null : this.bophanId) + ", '"
						...
						+ this.userId + "', '" + ngaytao + "', '" + this.userId + "','" + this.LoaiThanhToan + "', "
						+ this.tienteId + ", " + this.tigia.replaceAll(",", "") + ", '" + this.checkThanhtoantuTV
						+ "', " + (this.khachhangId.length() == 0 ? null : this.khachhangId) + ", "
				...
				if (this.DoiTuongChiPhiKhac.equals("3")) {
					query = "Insert ERP_THANHTOANHOADON(NGAYGHINHAN, HTTT_FK, NGANHANG_FK, CHINHANH_FK, SOTAIKHOAN, NOIDUNGTT,  SOTIENTT, SOTIENTTNT, SOTIENHD, SOTIENHDNT,"
							...
							+ " TIENTE_FK, TIGIA, THANHTOANTUTIENVAY, CONGTY_FK, NPP_FK, PREFIX ) " + "values('"
							...
							+ this.tigia.replaceAll(",", "") + " ,'" + this.checkThanhtoantuTV
							+ ", " + this.nppdangnhap + ", 'DNBN')";
			...
			if (this.bophanId.trim().length() > 0) // THANH TOÁN THEO BỘ PHẬN
			{
				...
				for (int i = 0; i < this.hoadonList.size(); i++) {
					...
					if (thanhtoan.length() > 0) {
						if (Float.parseFloat(thanhtoan) != 0) {
							if (this.tienteId.equals("100000")) {
								query = "Insert ERP_THANHTOANHOADON_HOADON(THANHTOANHD_FK, HOADON_FK, SOTIENTT, SOTIENAVAT, SOTIENNT, CONLAI, LOAIHD, SOHOADON, TIENTE_FK , TIGIA) "
										+ "values('" + tthdCurrent + "', '" + hoadon.getId() + "', '"
										+ thanhtoan.trim().replaceAll(",", "") + "', '" + avat.replaceAll(",", "")
										+ "'," + " 0, '" + conlai.trim().replaceAll(",", "") + "', '" + loaihd + "', '"
										+ sohoadon + "', "+tientehd+", "+tigiahd+")";
								if (loaihd.equals("4")) {
									query = "Insert ERP_THANHTOANHOADON_HOADON(THANHTOANHD_FK, HOADON_FK, SOTIENTT, SOTIENAVAT, SOTIENNT, CONLAI, LOAIHD, LOAITHUE, SOHOADON, TIENTE_FK, TIGIA) "
											+ "values('" + tthdCurrent + "', '" + hoadon.getId() + "', '"
											+ thanhtoan.trim().replaceAll(",", "") + "', '" + avat.replaceAll(",", "")
											+ "'," + " 0, '" + conlai.trim().replaceAll(",", "") + "', '" + loaihd
											+ "', N'" + hoadon.getKyhieu() + "', '" + sohoadon + "', "+tientehd+", "+tigiahd+")";
								}
								...
								query = "Insert ERP_THANHTOANHOADON_HOADON(THANHTOANHD_FK, HOADON_FK, SOTIENTT, SOTIENAVAT, SOTIENNT, CONLAI, LOAIHD, SOHOADON, TIENTE_FK, TIGIA) "
										...
										+ thanhtoan.trim().replaceAll(",", "") + "', '" + avat.replaceAll(",", "")
										+ "', " + sotienNT.replaceAll(",", "") + "," + " '"
										+ conlai.trim().replaceAll(",", "") + "', '" + loaihd + "', '" + sohoadon+ "', "+tientehd+", "+tigiahd+")";
								if (loaihd.equals("4")) {
									query = "Insert ERP_THANHTOANHOADON_HOADON(THANHTOANHD_FK, HOADON_FK, SOTIENTT, SOTIENAVAT, SOTIENNT, CONLAI, LOAIHD, LOAITHUE, SOHOADON, TIENTE_FK, TIGIA ) "
											+ "values('" + tthdCurrent + "', '" + hoadon.getId() + "', '"
											+ thanhtoan.trim().replaceAll(",", "") + "', '" + avat.replaceAll(",", "")
											...
											+ hoadon.getKyhieu() + "', '" + sohoadon + "', "+tientehd+", "+tigiahd+")";
	...
	public boolean updateTTHD() {
		...
		try {
			...
			query = "update ERP_THANHTOANHOADON " + "set NGAYGHINHAN = '" + this.ngayghinhan + "', " + "DVTH_FK="
					...
					+ "SOTAIKHOAN = '" + this.sotaikhoan + "'," + " NOIDUNGTT = N'" + this.noidungtt + "', "
					...
					+ ", PREFIX ='DNBN', isNPP = '" + this.isNpp + "'  " + ", isTICHLUY = " +isTICHLUY+ " where PK_SEQ = '" + this.id + "'";
			...
			if (this.DoiTuongDinhKhoan.equals("3")) {
				query = "update ERP_THANHTOANHOADON " + "set  NGAYGHINHAN = '" + this.ngayghinhan + "', "
						...
						+ this.sotaikhoan + "'," + " NOIDUNGTT = N'" + this.noidungtt + "', " + "SOTIENTT = "
						...
						+ this.tigia.replaceAll(",", "") + ", THANHTOANTUTIENVAY = '" + this.checkThanhtoantuTV
						+ "', PREFIX ='DNBN'  " + " where PK_SEQ = '" + this.id + "'";
			}
			...
			if (Double.parseDouble(this.checkThanhtoantuTV) == 1) {
				query = "update ERP_THANHTOANHOADON " + "set SOTIENTT = " + ("" + this.sotientt).replaceAll(",", "")
						...
						+ this.userId + "', PREFIX ='DNBN'  " + " where PK_SEQ = '" + this.id + "'";
			}
	...
	public boolean chotTTHD(String userId) {
		...
		try {
					...
					while (ktRs.next()) {
						...
						if (Double.parseDouble(this.checkThanhtoantuTV) <= 0) {
							...
							if(tienhang > 0)
							{
								...
								nganhang_fk = ktRs.getString("nganhang_fk") == null ? "": ktRs.getString("nganhang_fk");
								...
								this.msg = util.Update_TaiKhoan_Vat_DienGiai_CHIKHAC( db, thang, nam, ktRs.getString("ngayghinhan"), ktRs.getString("ngayghinhan"), "Trả khác", this.id, taikhoanNO, taikhoanCO, "", 
										Double.toString(tienhang), Double.toString(tienhang), loaidoituongno, madoiduongno, loaidoituongco, madoiduongco, "0", "", "", tiente_fk, "", tigia, Double.toString(tienhang), Double.toString(tienhang), "Tiền hàng", Double.toString(tienthue) , diengiai , machungtu, isNPP,
							...
							if(tienthue > 0)
							{
								...
								this.msg = util.Update_TaiKhoan_Vat_DienGiai_CHIKHAC( db, thang, nam, ktRs.getString("ngayghinhan"), ktRs.getString("ngayghinhan"), "Trả khác", this.id, taikhoanNO, taikhoanCO, "", 
								Double.toString(tienthue), Double.toString(tienthue), loaidoituongno, madoiduongno, loaidoituongco, madoiduongco, "0", "", "", tiente_fk, "", tigia, Double.toString(tienthue), Double.toString(tienthue), "Tiền thuế", Double.toString(tienthue), diengiai , machungtu, isNPP,
								MAHOADON, MAUHOADON, KYHIEU, SOHOADON,  NGAYHOADON,  TENNCC, MST,  TIENHANG,  THUESUAT, MAFAST_DT,  TEN_DT,  TEN_PB,
								TEN_KBH,  TEN_VV,  TEN_DIABAN,  TEN_TINHTHANH,  TEN_BENHVIEN,  TEN_SANPHAM, DIENGIAI_CT	);
			...
			{ 
				{
				// 1. TIỀN VAT, PHÍ NGÂN HÀNG GIỐNG NHAU MỌI TRƯỜNG HỢP
				query =   " SELECT  TTHD.CHENHLECHVND, ISNULL(TTHD.PHINGANHANG, 0) AS PHINGANHANG , TTHD.TRICHPHI, \n"+
						  ...
						  " LEFT JOIN KHACHHANG KH ON KH.PK_SEQ = TTHD.KHACHHANG_FK \n"+
						  " WHERE TTHD.PK_SEQ = " + this.id + "  ";
				System.out.println("Câu lấy tiền vat/phinganhang: " + query);
						...
						if (!tiente_fk.equals("100000") && trichphi_fk.equals("1")) {
							...
							if (phinganhang > 0) {
								...
								this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, RsRs.getString("NGAYGHINHAN"), RsRs.getString("NGAYGHINHAN"), "Thanh toán hóa đơn", this.id, taikhoanNO, taikhoanCO, "", 
										Double.toString(phinganhang), Double.toString(phinganhang), "BANK CHARGES", "", "Ngân hàng", nganhangTP_fk, "0", "", "", tiente_fk, "", tigia, Double.toString(phinganhang), Double.toString(phinganhang), "PHÍ NGÂN HÀNG", "0" , diengiai , machungtu, isNPP);
							...
							// GHI NHAN VAT
							if (vat > 0) {
								taikhoanNO = RsRs.getString("taikhoan_THUE");
								String doituong = "";
								String madoituong = "";
				...
				if (Double.parseDouble(this.checkThanhtoantuTV) <= 0) {
						...
						if (this.DoiTuongChiPhiKhac.equals("1")) // NCC
						{
										...
										if(rsTk!= null)
										{
											while(rsTk.next())
											{
												...
												if(totalDS > 0)
												{
													...
													this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, ngayghinhan, ngayghinhan, "Chi phí khác", this.id, taikhoanNO_DS, taikhoanCO_NH, "", 
												...
												if(totalVAT > 0)
												{
													...
													this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, ngayghinhan, ngayghinhan, "Chi phí khác", this.id, taikhoanNO_VAT, taikhoanCO_NH, "", 
							...
							{										
								...
								query = " SELECT  TTHD.NGANHANG_FK , NCC.TAIKHOAN_FK as taikhoan_NCC,  TTHD.NGAYGHINHAN , \n"+
										...
										" FROM   ERP_THANHTOANHOADON TTHD \n"+
										" inner join TraphacoERP.dbo.ERP_NHACUNGCAP NCC on TTHD.NCC_FK= NCC.PK_SEQ \n"+
										" inner join ERP_THANHTOANHOADON_HOADON TTHD_HD ON TTHD.pk_seq = TTHD_HD.THANHTOANHD_FK \n"+
										" WHERE TTHD.PK_SEQ =  " + this.id + " ";
								...
								if (psktRs != null) {
									while (psktRs.next()) {
										...
										if (Double.parseDouble(this.checkThanhtoantuTV) <= 0) {
											...
											if(TIENCHENHLECH > 0)
											{
												if(hinhthuctt.equals("100001") ||hinhthuctt.equals("100003")) // NGÂN HÀNG
												{
								...
								query = "select (select SOCHUNGTU from ERP_THUENHAPKHAU where PK_SEQ = (select top 1 THUENHAPKHAU_FK from ERP_THANHTOANHOADON_CHIPHIKHAC  where THANHTOANHD_FK= "+ this.id + " )) as sotokhai, \n"+
										...
										"from ERP_THANHTOANHOADON a  left join TraphacoERP.dbo.ERP_NHACUNGCAP d on a.ncc_fk = d.pk_seq \n"+
										"where a.PK_SEQ = '" + this.id + "'";
								...
								while(rs.next())
								{
									...
									if(loaihd.equals("6"))// ĐỀ NGHỊ THANH TOÁN
									{
										...
										query= "  SELECT N'CHI PHÍ' AS LOAIDOITUONGNO, E.PK_SEQ AS MADOITUONGNO,    " +  
											   ...
											   "  CASE  WHEN A.NHACUNGCAP_FK  IS NOT NULL THEN NCC.TAIKHOAN_FK WHEN A.NHANVIEN_FK  IS NOT NULL THEN NV.TAIKHOAN_FK ELSE KH.TAIKHOAN_FK  END AS TAIKHOANCO_VAT,   " +  
											   "  (SELECT PK_SEQ FROM ERP_TAIKHOANKT WHERE SOHIEUTAIKHOAN = E.TAIKHOAN_FK AND NPP_FK = A.NPP_FK) AS TAIKHOANNO_DS,   " +  
											   "  ( SELECT PK_SEQ FROM ERP_TAIKHOANKT WHERE SOHIEUTAIKHOAN = '13311000' AND NPP_FK = "+this.nppdangnhap+"  ) AS TAIKHOANNO_VAT   " +  
											   "  FROM ERP_MUAHANG A  " +  
											   "  LEFT JOIN ERP_NHANVIEN NV ON NV.PK_SEQ = A.NHANVIEN_FK    " +  
											   "  LEFT JOIN TraphacoERP.dbo.ERP_NHACUNGCAP NCC ON A.NHACUNGCAP_FK = NCC.PK_SEQ   " + 
											   "  LEFT JOIN KHACHHANG KH ON A.KHACHHANG_FK = KH.PK_SEQ   " +  
										...
										if(rsTk!= null)
										{
											while(rsTk.next())
											{
												...
												if(totalDS > 0)
												{
													...
													this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, ngayghinhan, ngayghinhan, "Chi phí khác", this.id, taikhoanNO_DS, taikhoanCO_NH, "", 
															Double.toString(totalDS), Double.toString(totalDS), loaidoituongNO, madoituongNO, loaidoituongCO, madoituongCO, "0", "", "", tiente_fk, "", "1", Double.toString(totalDS), Double.toString(totalDS),  "Doanh số" , "0" , diengiai , machungtu, isNPP);
									...
									{										
										...
										query = " SELECT  TTHD.NGANHANG_FK , NV.TAIKHOAN_FK as taikhoanNO_NV,  TTHD.NGAYGHINHAN , \n"+
												"        ( SELECT TaiKhoan_fk from ERP_NGANHANG_CONGTY where  SoTaiKhoan = TTHD.SOTAIKHOAN AND NPP_FK = " + this.nppdangnhap + " ) as taikhoanCO_NH , \n"+
												"        ( SELECT pk_seq from ERP_TAIKHOANKT where SOHIEUTAIKHOAN = '11110000' AND NPP_FK = "+ this.nppdangnhap + ") as taikhoan_TIENVND, \n"+
												"         case when TTHD.TIENTE_FK = 100000 then (select SUM(SOTIENTT) from ERP_THANHTOANHOADON_HOADON where THANHTOANHD_FK = "+ this.id + " and LOAIHD in (1,5) ) \n"+
												"              else (select SUM(SOTIENTT) from ERP_THANHTOANHOADON_HOADON where THANHTOANHD_FK = "+ this.id + " and LOAIHD in (1,5,8) ) * TTHD.TIGIA \n"+
												"              end as tonggiatri, TTHD.NOIDUNGTT, TTHD.SOCHUNGTU MACHUNGTU, TTHD.ISNPP \n"+
												" from   ERP_THANHTOANHOADON TTHD  inner join ERP_NHANVIEN NV on TTHD.NHANVIEN_FK = NV.PK_SEQ \n"+
												" where TTHD.PK_SEQ = " + this.id + " ";
										System.out.println("Câu query NV1 " + query);
										ResultSet psktRs = db.get(query);
										if (psktRs != null) {
											while (psktRs.next()) {
												...
												if (tonggiatri > 0) {
													...
													this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, psktRs.getString("NGAYGHINHAN"), psktRs.getString("NGAYGHINHAN"), "Thanh toán hóa đơn", this.id, taikhoanNO, taikhoanCO, "", 
													Double.toString(tonggiatri), Double.toString(tonggiatri), "Nhân viên", nhanvien_fk, loaidoituongco, madoiduongco, "0", "", "", tiente_fk, "", "1", Double.toString(tonggiatri), Double.toString(tonggiatri),  "" , "0" , diengiai , machungtu, isNPP);
						...
						} else if (this.khachhangId.trim().length() > 0) // Khách	// hàng		// (LOAIHD:	// KHÁCH// HÀNG// TRẢ// TRƯỚC)	// //
						{
	
							query = " SELECT TTHD.NGANHANG_FK , TTHD_HD.SOTIENTT, TTHD_HD.LOAIHD, TTHD_HD.HOADON_FK, TTHD.NGAYGHINHAN, TTHD.TIENTE_FK,   \n" +
									" ( SELECT TaiKhoan_fk FROM ERP_NGANHANG_CONGTY where  SoTaiKhoan = TTHD.SOTAIKHOAN AND NPP_FK = "+ this.nppdangnhap + " ) as taikhoan_NH, "+
							        " ( SELECT pk_seq FROM ERP_TAIKHOANKT where SOHIEUTAIKHOAN = '11110000' AND NPP_FK = "+ this.nppdangnhap + ") as taikhoan_TIENVND, isnull(TTHD.NOIDUNGTT,'') NOIDUNGTT, "+
							        " TTHD.SOCHUNGTU MACHUNGTU , ISNULL(TTHD.ISNPP, '') ISNPP "+
									" FROM ERP_THANHTOANHOADON TTHD  \n" +
									" INNER JOIN ERP_THANHTOANHOADON_HOADON TTHD_HD ON TTHD.PK_SEQ = TTHD_HD.THANHTOANHD_FK \n"+
									" WHERE TTHD.PK_SEQ = "+this.id;
							...
							if(rs!=null)
							{
								while(rs.next())
								{
									...
									String taikhoanCO = rs.getString("taikhoan_NH")== null ? "": rs.getString("taikhoan_NH") ;
										...
										if(rsTk!= null)
										{
											...
											{												
												...
												if(totalDS > 0)
												{
													...
													msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, ngayghinhan, ngayghinhan, "Chi phí khác", this.id, taikhoanNO_DS, taikhoanCO, "", 
													Double.toString(totalDS), Double.toString(totalDS), loaidoituongNO, madoituongNO, 
													loaidoituongCO, madoituongCO, "0", "", "", tiente_fk, "", "1", Double.toString(totalDS), Double.toString(totalDS), "Doanh số", Double.toString(totalVAT) , diengiai, machungtu, isNPP );
												...
												if(totalVAT > 0)
												{
													...
													msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, ngayghinhan, ngayghinhan, "Chi phí khác", this.id, taikhoanNO_DS, taikhoanCO, "", 
															Double.toString(totalVAT), Double.toString(totalVAT), loaidoituongNO, madoituongNO, 
															loaidoituongCO, madoituongCO, "0", "", "", tiente_fk, "", "1", Double.toString(totalVAT), Double.toString(totalVAT), "VAT", "0" , diengiai, machungtu, isNPP );
							...
							{
								// NHỮNG LOẠI HĐ CÓ KHACH HANG > 0
								query = " select  TTHD.NGANHANG_FK , KH.TAIKHOAN_FK as taikhoanNO_KH,  TTHD.NGAYGHINHAN , \n"+
										"        ( select TaiKhoan_fk from ERP_NGANHANG_CONGTY where  SoTaiKhoan = TTHD.SOTAIKHOAN AND NPP_FK = "+ this.nppdangnhap + " ) as taikhoanCO_NH , \n"+
										"        (select pk_seq from ERP_TAIKHOANKT where SOHIEUTAIKHOAN = '11110000' AND NPP_FK = "	+ this.nppdangnhap + ") as taikhoan_TIENVND, "+
										"         case when TTHD.TIENTE_FK = 100000 then (select SUM(SOTIENTT) from ERP_THANHTOANHOADON_HOADON where THANHTOANHD_FK = "+ this.id + " and LOAIHD in (7,8)  ) "+
								...
								if (psktRs != null) {
									while (psktRs.next()) {
										...
										if (tonggiatri > 0) {
											...
											this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, psktRs.getString("NGAYGHINHAN"), psktRs.getString("NGAYGHINHAN"), "Thanh toán hóa đơn", this.id, taikhoanNO, taikhoanCO, "", 
											Double.toString(tonggiatri), Double.toString(tonggiatri), "Khách hàng", khachhang_fk, loaidoituongco, madoiduongco, "0", "", "", tiente_fk, "", tigia, Double.toString(tonggiatri), Double.toString(tonggiatri),  "" , "0" , diengiai , machungtu, isNPP);
						...
						} else if (this.bophanId.trim().length() > 0) // Bộ phận
						{
							query = " select distinct TTHD.NGANHANG_FK , NV.PK_SEQ as NVID, NV.TAIKHOAN_FK as taikhoanNO_NV,  TTHD.NGAYGHINHAN , \n"+
									"        ( select TAIKHOAN_FK from ERP_NGANHANG_CONGTY where  SoTaiKhoan = TTHD.SOTAIKHOAN AND NPP_FK = "+ this.nppdangnhap + " ) as taikhoanCO_NH , \n"+
									"        ( select PK_SEQ from ERP_TAIKHOANKT where SOHIEUTAIKHOAN = '11110000' AND NPP_FK = "+ this.nppdangnhap + " ) as taikhoan_TIENVND, "+
									"         case when TTHD.TIENTE_FK = 100000 then (select SUM(SOTIENTT) from ERP_THANHTOANHOADON_HOADONBOPHAN where THANHTOANHD_FK = "+ this.id + 
									" 		  and NHANVIEN_FK = NV.PK_SEQ and LOAIHD in (6)  ) "+
									"              else (select SUM(SOTIENTT) from ERP_THANHTOANHOADON_HOADONBOPHAN where THANHTOANHD_FK = "+ this.id + 
									" and NHANVIEN_FK = NV.PK_SEQ  and LOAIHD in (6) ) * TTHD.TIGIA "+
									"              end as tonggiatri, TTHD.NOIDUNGTT, TTHD.SOCHUNGTU MACHUNGTU, ISNULL(TTHD.ISNPP, '') ISNPP \n"+
			...
			this.msg = "Khong the cap nhat TAIKHOAN ABC: " + e.getMessage();
			return false;
		}
		...
		if (rs != null) {
			try {
				while (rs.next()) {
					...
					if (this.bophanId.trim().length() <= 0 && this.nccId.trim().length() <= 0
							&& this.NhanvienId.trim().length() <= 0 && this.khachhangId.trim().length() <= 0) // tra// Khac
					{
	...
	public void createDoiTuongDinhKhoan() {
		...
		if (this.DoiTuongDinhKhoan.equals("1")) {
			...
			query = "select cast(PK_SEQ as varchar) + ' -- ' + MA+ ',' + TEN  as maten from TraphacoERP.dbo.ERP_NHACUNGCAP where PK_SEQ = "
					+ this.doituongdinhkhoanId+ " AND NPP_FK = 1";
	...
	public void initDisplay() {
		...
		this.khachhangRs = db.get("select pk_seq, ma + ', ' + ten as TEN, '0' isNPP from KhachHang where trangthai = '1' AND NPP_FK ="+ this.nppdangnhap + " "
								+ "union all select pk_seq, ma + ', ' + ten as TEN, '1' isNPP from NHAPHANPHOI WHERE  TRANGTHAI=1 AND TRUCTHUOC_FK IN (SELECT PK_SEQ FROM NHAPHANPHOI WHERE PK_SEQ = " + this.nppdangnhap + ") \n");

		
		this.htttRs = db.get("select pk_seq, ma, ten from ERP_HINHTHUCTHANHTOAN where trangthai = '1' and pk_seq IN ( '100001' )");
		
		
		this.nganhangRs = db.get("select pk_seq, ma + ', ' + ten as nhTen from erp_nganhang where PK_SEQ in ( select NganHang_FK from ERP_NGANHANG_CONGTY where NPP_fk = '"	+ this.nppdangnhap + "' ) ");
...
*/
	...
	public void createRs() {
		...
		this.nccRs = db.get("select pk_seq, ma + ', ' + ten as TEN from TraphacoERP.dbo.erp_nhacungcap where trangthai = '1' and duyet= '1' and NPP_FK = 1 ");

		this.NhanvienRs = db.get("SELECT PK_SEQ ,MA+','+TEN AS TEN  FROM ERP_NHANVIEN WHERE TRANGTHAI=1 and NPP_FK  = "+ this.nppdangnhap + " ");
		this.nhomNCCRs = db.get("SELECT PK_SEQ, DIENGIAI AS TEN FROM TraphacoERP.dbo.NHOMNHACUNGCAPCN where TRANGTHAI = 1 ");

		this.khachhangRs = db.get("select pk_seq, maFAST + ', ' + ten as TEN, '0' isNPP from KhachHang where trangthai = '1' AND NPP_FK ="+ this.nppdangnhap + " ");
		...
		if(this.id.length() >0){
			String sql =	" SELECT COUNT(*) as count"+
							" FROM  ERP_THANHTOANHOADON_PHINGANHANG PC "+
							" WHERE PC.THANHTOANHD_FK = '"+this.id+"'";
...
*/
	...
	public static void main(String[] args) {
		ErpUynhiemchi erpThanhtoanhoadon = new ErpUynhiemchi();
...
public void setPhongBanIds(String[] PhongBanIds) {
	...
	public ResultSet getBenhVienRs() {
		...
		String query = "SELECT CONVERT(VARCHAR, PK_SEQ) AS PK_SEQ, MAFAST + ' - ' + TEN AS TEN FROM KHACHHANG WHERE  TRANGTHAI = 1 AND LCH_FK IN (100011) AND NPP_FK = "+this.nppdangnhap;
