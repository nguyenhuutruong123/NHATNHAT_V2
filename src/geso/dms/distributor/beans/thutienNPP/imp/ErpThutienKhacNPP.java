...
public class ErpThutienKhacNPP implements IErpThutienKhacNPP 
{
	...
	public ErpThutienKhacNPP(String Id)
	{
		this.Id = Id;
		this.NgayButToan = "";
		this.DienGiai = "";
	...
	public void createRs()
	{
		...
		String tknoCondition = "'1111'";
		if(!htttId.equals("100000"))
			tknoCondition = "'1121'";
			
			
		this.nghiepvuketoanRs = KeToan.getNghiepvuketoanRs(db,this.UserId,this.nppId,tknoCondition,"");
	...
	public boolean Save()
	{
		...
		try
		{
			...
			String tenHTTT= "(select TEN from HINHTHUCTHANHTOAN where PK_SEQ = " +this.htttId+ ")";
			 ...
			 query=	"\n INSERT INTO ERP_THUTIENKHAC(donviTen,DiaChi,DonVi,NGAYCHUNGTU,TRANGTHAI,NPP_FK,TENNGUOINOP,SOTIENTT,NGAYTAO,NGUOITAO,NGAYSUA,NGUOISUA,GhiChu,HINHTHUCTT,noidungtt_fk,HTTT_FK,loaiMenu,MaNghiepVu) " +
							"\n select N'"+donviTen+"',N'"+donviDiaChi+"',N'"+this.donvi+"','"+this.NgayButToan+"',0,'" + nppInsert + "',N'"+this.NguoiNopTien+"',0,'"+ngaytao+"','"+this.UserId+"','"+ngaytao+"','"+this.UserId+"' ,N'"+this.DienGiai+"',"+tenHTTT+",100002,"+this.htttId+",'1',N'"+this.nghiepvuketoanId+"' ";
		    ...
		    if( this.nghiepvuketoanId != null & this.nghiepvuketoanId.length > 0)
			{
				for(int i = 0 ;  i < this.nghiepvuketoanId.length ; i ++)
				{
					...
					if(tien > 0)
					{
						query = "\n insert ERP_THUTIENKHAC_CHITIET ([THUTIENKHAC_FK],[MaNPP],[MaNghiepVu],[Noidung],[tkno],[dtno],[ytno],[tkco],[dtco],[ytco],[tiennt])  " +
				...
				query = "\n update ERP_THUTIENKHAC set " +
				"\n		SOTIENTT = (select sum (tiennt) from ERP_THUTIENKHAC_CHITIET where THUTIENKHAC_FK =  "+ this.Id + ") " +
				"\n		,  SOTIENTHU =  (select sum (tiennt) from ERP_THUTIENKHAC_CHITIET where THUTIENKHAC_FK =  "+ this.Id + ") " +
	...
	public boolean Edit()
	{
		...
		try
		{
			...
			if(!Utility.KiemTraNgayChungTuSoNet( db,this.UserId ,nppInsert,"PhieuThu_SoChungTu",this.NgayButToan, this.Id,"ERP_THUTIENKHAC" ))
			{
				this.db.getConnection().rollback();
			...
			query=	"\n update ERP_THUTIENKHAC set donviTen = N'"+donviTen+"' , diachi = N'"+donviDiaChi+"' ,DonVi = N'"+this.donvi+"' , NGAYCHUNGTU = '"+this.NgayButToan+"'" +
							"\n		,TENNGUOINOP = N'"+this.NguoiNopTien+"',NGAYSUA = '"+ngaytao+"',NGUOISUA = '"+this.UserId+"'" +
							"\n		,GhiChu = N'"+this.DienGiai+"',HINHTHUCTT = "+tenHTTT+",HTTT_FK = "+this.htttId+",MaNghiepVu = N'"+this.nghiepvuketoanId+"' " +
			...
			query = " delete ERP_THUTIENKHAC_CHITIET where  THUTIENKHAC_FK =   "+  this.Id;
	...
	public void Init()
	{
		...
		ResultSet rs;
		if(this.Id .length() >0){
			  query=" SELECT isnull(donvi,'')donvi, isnull(diachi,'')diachi,tennguoinop,PK_SEQ,ngaychungtu,GhiChu,HTTT_FK FROM ERP_THUTIENKHAC WHERE PK_SEQ='"+this.Id+"'";
			...
			if(rs!=null)
			{
				try
				{
					while(rs.next())
					{
						...
						this.NguoiNopTien=rs.getString("tennguoinop");
		...
		if(this.Id.length() >0){
			try
			{
				query =" select count(*) a from ERP_THUTIENKHAC_CHITIET where THUTIENKHAC_FK ='"+this.Id+"'";
				rs =this.db.get(query);
				rs.next();
				...
				query=	" select MaNghiepVu,tkno,tkco,Noidung,dtno,ytno,dtco,ytco,tiennt from ERP_THUTIENKHAC_CHITIET where THUTIENKHAC_FK ='"+this.Id+"' order by id ";
	...
	public String Chot()
	{
		...
		try
		{
			...
			String query = "UPDATE ERP_THUTIENKHAC SET ngaychungtu = '"+this.NgayButToan+"' , TRANGTHAI =1,NGUOISUA='"+this.UserId+"' WHERE PK_SEQ='"+this.Id+"'";
			if(!this.db.update(query))
			{
