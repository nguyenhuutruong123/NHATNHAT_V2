package geso.dms.distributor.servlets.inchietkhautruckthang;
...
public class InKhauTruCKThangSvl extends HttpServlet 
{
	private static final long serialVersionUID = 1L;
	
	public InKhauTruCKThangSvl() {
		super();
	}
	...
	float CONVERT = 28.346457f;  // =1cm
      ...
      protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
			...
			private void CreateBienBanBuTru(Document document, ServletOutputStream outstream, IInKhauTruCKThang obj, String nppId, String[] khIds, String dsKh ) 
			{
				...
				int songayTRONGTHANG = InKhauTruCKThangSvl.Songaytrongthang(thangTRUOC, namTRUOC);
			...
			private void CreateBienBanBuTru_HoaDon(Document document, ServletOutputStream outstream, String id, String nppId) 
			{
				...
				{	
					...
					String query = 
							"		select npp.HanMucDoanhThu,tt.VUNG_FK,(select top(1) DDH_FK from HoaDon_DDH dd where dd.hoadon_fk=hd.pk_Seq) as dhId,khachhang_fk, ngayxuatHD, sohoadon,		ISNULL(  ( select MAHD from KHACHHANG where pk_seq = hd.khachhang_fk ), '') as MAHD  "+ 
									"		,isnull(npp.TENKYHD,'') nppTEN , npp.DIACHI as nppDiaChi, npp.MASOTHUE as nppMASOTHUE,  "+
									"		npp.DIENTHOAI as nppDIENTHOAI, isnull(npp.TENNGUOIDAIDIEN, '') as nppTENNGUOIDAIDIEN   "+
									" , ( "+
									"		select top(1) ngaynhap from DONHANG a inner join HOADON_DDH b on b.DDH_FK=a.PK_SEQ "+
									"		where b.HOADON_FK=hd.pk_Seq  "+
									"	)as ngayDH "+
									"	from HOADON hd inner join NHAPHANPHOI npp on npp.PK_SEQ=hd.NPP_FK " +
									" inner join TINHTHANH tt on tt.PK_SEQ=npp.TINHTHANH_FK               																													"+
									"	where hd.PK_SEQ='"+id+"' ";
					System.out.println("cau query in khau tru:"+query);
					...
					if(rs.next())
					{
						...
						tennpp = rs.getString("nppTEN");
						diachinpp = rs.getString("nppDIACHI");
						masothuenpp = rs.getString("nppMASOTHUE");
						dienthoainpp = rs.getString("nppDIENTHOAI");
						tennguoiddNpp = rs.getString("nppTENNGUOIDAIDIEN");
						VUNG_FK = rs.getString("VUNG_FK");
						if(VUNG_FK.equals("100002"))
						{
							tableNAME = "NHOMSANPHAM_MIENNAM";
						}else  if(VUNG_FK.equals("100003"))
						{
							tableNAME = "NHOMSANPHAM_MIENTRUNG";
						}
						HanMucDoanhThu = rs.getDouble("HanMucDoanhThu");
					}
					rs.close();
					
					query = " select maFAST, kh.PK_SEQ khId, ISNULL(kh.TEN,'') TEN, isnull(TENKyHd, '') as chucuahieu, kh.DIACHI, kh.MASOTHUE, kh.DIENTHOAI " +
							" from KHACHHANG kh where kh.PK_SEQ = '" + khId + "'";
					System.out.println("--THONG TIN KH: " + query);
					String tenkh="";
					String chucuahieu = "";
					String diachikh ="";
					...
					if(rs!= null)
					{
						while(rs.next())
						{
							...
							masothuekh=rs.getString("MASOTHUE");
							dienthoaikh=rs.getString("DIENTHOAI");
							chucuahieu = rs.getString("chucuahieu");
					...
					/*************************************************************************************************************/
						query=
								"	INSERT INTO  "+ 
										"	[BienBanBuTruCongNo]([NPP_FK],[KhachHang_FK],[HoaDon_FK],[DonHang_FK],[NgayXuatHD]  "+
										"	,[NgayDonHang],[SoHoaDon],[KyHieuHD],[Thang],[Nam],[Ten_NPP],[DiaChi_NPP] "+
										"	,[MaSoThue_NPP],[DienThoai_NPP],[DaiDien_NPP],[Ten_KH],[DiaChi_KH]  "+ 
										"	,[MaSoThue_KH],[DienThoai_KH],[DaiDien_KH],[MaFast_KH],[MaHD])  " +
										" select '"+nppId+"','"+khId+"','"+id+"','"+dhId+"','"+ngayxuatHD+"','"+ngayDonHang+"','"+sohoadon+"','"+kyhieuHD+"','"+Thang+"','' as Nam,N'"+tennpp+"',N'"+diachinpp+"'," +
										" N'"+masothuenpp+"',N'"+dienthoainpp+"',N'"+tennguoiddNpp+"',N'"+tenkh+"',N'"+diachikh+"',N'"+masothuekh+"',N'"+dienthoaikh+"',N'"+chucuahieu+"','"+maFAST+"','"+mahopdong+"'     ";									
						db.update(query);
						query =" select scope_identity() as bbId ";
						String bbId=""; 
						rs=db.get(query);
						while(rs.next())
						{
					...
					if(khId.trim().length() > 0 )
					{
						...
						double totalThuVe=0;
						double totalCK=0;
						String sql="select count(*) as sl from  DUYETTRAKHUYENMAI_DONHANG_LOG  where donhang_fk='"+dhId+"' and tichluyQUY=0   ";

						System.out.println("sql--------"+sql);
						ResultSet rscount=db.get(sql);
						int slcount=0;
						if(rscount.next())
						{
							slcount=rscount.getInt("sl");
						}
						rscount.close();
						
						String sqlischot="SELECT COUNT(*) FROM HOADON_DDH a inner join HOADON b on a.HOADON_FK=b.PK_SEQ \n"+
									 	 "	WHERE a.DDH_FK="+dhId+" and b.LOAIHOADON=0 and TRANGTHAI not in(1,3,5)";
						
						System.out.println("da chot sql"+sqlischot);
						ResultSet rsischot=db.get(sql);
						int ischot=0;
						if(rsischot.next())
						{
							ischot=rsischot.getInt("sl");
						}
						rsischot.close();
						
					/*	if(!nppId.equals("100002") && slcount<1 && ischot>0)
						{
							System.out.println("da chot vao day");
							query=
							...
							while(rs.next())
							{
								...
								NgayThang=NgayDonHang.substring(0, 7);
								...
								String cuoithangHT = NgayDonHang.split("-")[0] + "-" + NgayDonHang.split("-")[1] + "-" + this.getngayCUOITHANG(NgayDonHang);
								
								*//****************************************************************************************************************
								 ********************* Dư nợ tháng nào trả trong tháng đó.DUYETTRAKHUYENMAI_DUNO(Tháng hưởng ck )****************
								 ********************** DUYETTRAKHUYENMAI_DUNO_DONHANG_DATRA-->CK sẽ trả trong đơn hàng tháng sau. **************//*
								...
								else
								{
									...
									query = 
											"\n         select "+sql_HHBG+"  , count(*) as soDONG, isnull( PT_CHIETKHAU, 0) as PT_CHIETKHAU, isnull(kh.thanhtoan, 0) as thanhtoan, ( select loaiNPP from NHAPHANPHOI where pk_seq = '" + nppId + "' ) as loaiNPP,  " +
													...
													"\n        	ISNULL( ( select sum( tiencothue + bosung ) from DUYETTRAKHUYENMAI_DUNO where khachhang_fk = '" + khId  + "' and tichluyQUY = '0' and Thang='"+thangTRUOC+"' and Nam='"+namTRUOC+"'), 0 ) " +
													"\n        	-  ISNULL( ( select sum( thanhtoan ) from DUYETTRAKHUYENMAI_DUNO_DONHANG_DATRA where donhang_Fk!='"+dhId+"' and khachhang_fk = '" + khId  + "' and tichluyQUY = '0' and ngaythang =  '" + NgayThang + "'     ), 0 ) as duno_dauky ,  " +
													...
													"\n        	   or (  ROUND (	ISNULL( ( select sum( tiencothue + bosung ) from DUYETTRAKHUYENMAI_DUNO where khachhang_fk = '" + khId + "' and tichluyQUY = '0' and Thang='"+thangTRUOC+"' and Nam='"+namTRUOC+"'  ), 0 ) " +
													"\n        						- ISNULL( ( select sum( thanhtoan ) from DUYETTRAKHUYENMAI_DUNO_DONHANG_DATRA where khachhang_fk = '" +khId + "' and tichluyQUY = '0' and DonHang_FK in (select pk_Seq from DonHang where trangthai!=2 and NgayNhap>='"+dauthangHT+"' and NgayNhap<='"+cuoithangHT+"' )  ), 0 ) , 0 ) != 0  ) )			 " ;
										if(donhang_01_02_2015>=0)
													query+=
								...
								while(rsKQ.next())
								{
									...
									totalThuVe+=tongTHUVEThangtruoc;
								...
								*//**
								...
								if(donhang_01_02_2015>=0 && tongDSThangtruoc>0 )
								{
									...
									query="select count(*) as sl from DUYETTRAKHUYENMAI_DONHANG_CHEN_NGANG where khachhang_fk="+khId +" and npp_fk="+nppId +" and donhang_fk="+dhId;
									System.out.println("query --------"+query);
								...
								else
								{
									...
									query = 
											"\n        select tct.pk_seq, tct.scheme, '" + khId + "' as khId,sum("+cotLAY+") as pt_chietkhau,  " +
													"\n        	sum( " + tongTHUVEThangtruoc + " * tc." + cotLAY + " / 100 )  " +
													"\n        	- isnull(( select SUM( thanhtoan / ( 1  ) ) from DUYETTRAKHUYENMAI_DONHANG where donhang_fk!='"+dhId+"' and  isnull(NgayThang,(select  SUBSTRING(ngaynhap,0,8) from DONHANG where PK_SEQ = donhang_fk)) ='"+NgayThang+"' and  khachhang_fk = '" + khId + "' and duyetkm_fk = tct.pk_seq and donhang_fk in ( select pk_seq from DONHANG where ngaynhap >= '" + dauthangHT + "'  and trangthai != '2' and khachhang_fk = '" + khId + "' )  ), 0)  " +
													"\n        	- ISNULL( ( select sum( thanhtoan  ) from DUYETTRAKHUYENMAI_DUNO_DONHANG_DATRA where  donhang_fk!='"+dhId+"' and  isnull(NgayThang,(select  SUBSTRING(ngaynhap,0,8) from DONHANG where PK_SEQ = donhang_fk)) ='"+NgayThang+"'  and khachhang_fk = '" + khId + "' and tichluyQUY = '0' and donhang_fk in ( select pk_seq from DONHANG where ngaynhap >= '" + dauthangHT + "'  and trangthai != '2' and khachhang_fk = '" + khId + "' )  ), 0 )  as conLAI  " +
													"\n        from " +
													"\n        ( " +
													"\n        	select pk_seq as NSP_FK, 100 as tongGiatri, 50  as phantram " +
													"\n        	from NHOMSANPHAM " +
								...
								totalCK +=Math.round(pt_chietkhau * tongTHUVEThangtruoc / 100) + duno_dauky;
								
								if(dhId.equals("602246")||dhId.equals("602576"))
								{
									tongTHUVEThangtruoc=(TienCK/6.0)*100;
									totalCK=TienCK;
								}
								System.out.println("tong chiey khau ne daika:"+totalCK);
								
								*//****************************************************************************************************************************************************//*
								query=
								"	INSERT INTO  "+
								"	[BienBanBuTruCongNo_ChiTiet]([BBBTCN_FK],[HoaDon_FK],[KhachHang_Fk],[Npp_FK],[CotLay],[LoaiThanhToan],[DuNoDauKy],[DoanhSo],[DoanhThu],[DoanhSoBOGA], "+
								"			[DonHang_FK],[PtChietKhau],[HanMucDoanhThu],[BOGA],[DaDuyet],VUNG_FK,[NGAYTHANG],[NgayXuatHD],[NgayDonHang],[TienCK]) " +
								" select '"+bbId+"','"+id+"','"+khId+"','"+nppId+"','"+cotLAY+"','"+thanhTOAN+"','"+duno_dauky+"','"+tongDSThangtruoc+"','"+tongTHUVEThangtruoc+"','"+dsHHBG+"','"+dhId+"','"+pt_chietkhau+"'," +
								...
								"									from NHOMSANPHAM a      "+
								"									WHERE A.PK_SEQ=100003  "+")  ,'"+daDuyet+"','"+VUNG_FK+"','"+NgayThang+"','"+ngayxuatHD+"','"+NgayDonHang+"','"+tongTHUVEThangtruoc*pt_chietkhau/100+"' ";
								db.update(query);
								...
								*//****************************************************************************************************************************************************//*
								...
								if(tongTHUVEThangtruoc>0)
								{
									...
									cell25.addElement(td);
									tablethongtinC.addCell(cell25);
									System.out.println("Tong cong ne daika:"+tongcong);
									tongcong+=pt_chietkhau * tongTHUVEThangtruoc / 100;
									System.out.println("Tong cong cong don daika:"+tongcong);
									double tienin=Math.round(pt_chietkhau * tongTHUVEThangtruoc / 100 );
									PdfPCell cell26 =  new PdfPCell();
									td = new Paragraph(formatter.format(tienin),  new Font(bf, 12, Font.NORMAL));			
									td.setAlignment(Element.ALIGN_CENTER);
									cell26.addElement(td);
									tablethongtinC.addCell(cell26);
								}
					...
					*/	{
							...
							int i=0;
							query=
									"select thanhtoan as ck,isnull(tienthuve,round((thanhtoan/6.0)*100,0)) as DoanhThu,6 as pt_chietkhau,DATEPART(MONTH,DATEADD(month,-1,isnull(NgayThang+'-01','"+ngayxuatHD+"'))) as Thang  " +
											"	from DUYETTRAKHUYENMAI_DONHANG where donhang_fk='"+dhId+"' and tichluyQUY=0 and ROUND(thanhtoan,0)>0 order by NgayThang asc ";
							rs=db.get(query);
							String query1="select isnull(c.inhoadon,0) inhoadon,ISNULL(d.hoadon_fk,'0') ckt from hoadon a inner join hoadon_ddh b on a.pk_seq=b.hoadon_fk inner join CANTRUCONGNO_HOADON c on c.hoadon_fk=a.pk_seq left join (select hoadon_fk from HOADON_CHIETKHAU where diengiai='CT5' and hienthi = '0' and  "+
									"round(chietkhau, 0) > 0) d on a.PK_SEQ = d.hoadon_fk where b.ddh_fk="+dhId +" and a.trangthai not in(3,5)";
								System.out.println("query----"+query1);
								ResultSet rs2=db.get(query1);
								rs2.next();
								int dainpgh=rs2.getInt("inhoadon");
								String ckt=rs2.getString("ckt");
								if(!ckt.equals("0") && dainpgh==0)
								{
									query1="update hoadon set dain=1 where pk_seq =(select a.pk_seq from hoadon a inner join hoadon_ddh b on a.pk_seq=b.hoadon_fk and b.ddh_fk="+dhId+" and a.trangthai not in (3,5) and a.loaihoadon=0) ";
									db.update(query1);
								}
							 query1="select isnull(dain,0) as dain from hoadon a inner join hoadon_ddh b on a.pk_seq=b.hoadon_fk and b.ddh_fk="+dhId+" and a.trangthai not in (3,5) and a.loaihoadon=0";
					
							System.out.println("query----"+query1);
							ResultSet rs1=db.get(query1);
							rs1.next();
							 int dainpgh1=rs1.getInt("dain");
							
							System.out.println("query ne daika :"+query +"-----"+dainpgh);
							while(rs.next())
							{
								pt_chietkhau = rs.getFloat("pt_chietkhau");
								double thanht=0;
								if(dainpgh1==0 )
									 thanht=Math.round(rs.getDouble("ck"));
								else
								 thanht=Math.round(rs.getFloat("ck"));
								System.out.println("dainpgh:::::::"+dainpgh1);
								
								totalCK =totalCK+thanht;
								tongTHUVEThangtruoc = rs.getFloat("DoanhThu");
								Thang +=rs.getString("Thang") +",";
						...
						if(duno_dauky != 0)
						{
							...
							tongcong+=duno_dauky;
							System.out.println("tong cong la:"+tongcong);
						...
						/*PdfPCell cellslogo = new PdfPCell(); 
					cellslogo.setFixedHeight(1.0f*CONVERT);
					cellslogo.setBorder(0);
					tableheader.addCell(cellslogo);*/
						...
						PdfPCell cell100 =  new PdfPCell();
						if(nppId.equals("106210")||nppId.equals("106275"))
						...
						System.out.println("chiet khau la "+totalCK +"-------"+Math.round(totalCK));
					...
					query="update CANTRUCONGNO_HOADON set Inhoadon=1 where hoadon_fk="+id;
					db.update(query);
			...
			private void CreateStaticData(Workbook workbook, IInKhauTruCKThang obj ) 
			{
				...
				String query = "select d.maFAST, d.TEN as khTEN, d.CHUCUAHIEU, a.SOHOADON, a.NGAYXUATHD, isnull(a.tongtienavatNK,a.tongtienavat) as tongtienavat , isnull(b.SOTIENCANTRU, 0) as SOTIENCANTRU,   " +
						"	(	select top(1) aa.TEN from DAIDIENKINHDOANH aa inner join TUYENBANHANG bb on bb.DDKD_FK = aa.PK_SEQ       " +
						"				inner join KHACHHANG_TUYENBH cc on cc.TBH_FK = bb.PK_SEQ      " +
						"		where cc.KHACHHANG_FK = d.PK_SEQ  " +
						"	) as ddkdTEN  " +
			...
			private void CreateStaticData_TheoBangKe(Workbook workbook, IInKhauTruCKThang obj ) 
			{
				...
				String query = "select d.maFAST, d.TEN as khTEN, d.CHUCUAHIEU, a.SOHOADON, a.NGAYXUATHD, isnull((case when a.tongtienavat=0 then null else a.tongtienavat end),a.tongtienAVATNK) as tongtienavat, isnull(b.SOTIENCANTRU, 0) as SOTIENCANTRU,   \n " +
						"	(	select top(1) aa.TEN from DAIDIENKINHDOANH aa inner join TUYENBANHANG bb on bb.DDKD_FK = aa.PK_SEQ     \n  " +
						"				inner join KHACHHANG_TUYENBH cc on cc.TBH_FK = bb.PK_SEQ     \n " +
						"		where cc.KHACHHANG_FK = d.PK_SEQ  \n " +
						"	) as ddkdTEN \n  " +
