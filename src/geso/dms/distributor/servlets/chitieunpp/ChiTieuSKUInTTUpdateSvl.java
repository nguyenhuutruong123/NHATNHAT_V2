...
public class ChiTieuSKUInTTUpdateSvl extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
        ...
        String nextJSP = request.getContextPath() + "/pages/Center/ChiTieuSKUInUpdate.jsp";
        if(querystring.indexOf("display") >= 0)
        	nextJSP = request.getContextPath() + "/pages/Center/ChiTieuSKUInDisplay.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nspId = util.antiSQLInspection(request.getParameter("nspId"));
		if (nspId == null)
			nspId = "";
 		...
 		if(action.equals("save"))
 		{
    		if (id == null )
    		{
    			if (!ctskuBean.createChiTieuThang())
    			{
    				...
    				String nextJSP = request.getContextPath() + "/pages/Center/ChiTieuSKUInNew.jsp";
    				response.sendRedirect(nextJSP);
    			}
    			else
    			{
    			    ...
    			    String nextJSP = request.getContextPath() + "/pages/Center/ChiTieuSKUIn.jsp";
    				response.sendRedirect(nextJSP);	
    			}		
    		}
    		else{
		...
		{		
			...
			else
			{
				...
				if (id == null){
					nextJSP = request.getContextPath() + "/pages/Center/ChiTieuSKUInNew.jsp";
				}
				else{
					nextJSP = request.getContextPath() + "/pages/Center/ChiTieuSKUInUpdate.jsp";					
				}
				response.sendRedirect(nextJSP);
			}
		}		
					...
					"left join " +
						"( " +
							...
							"from nhaphang a inner join nhaphang_sp b on a.pk_seq = b.nhaphang_fk " +
											"inner join sanpham c on b.sanpham_fk = c.ma inner join quycach d on c.pk_seq = d.sanpham_fk and d.dvdl2_fk=100018 " +
