...
//		    PrintWriter out = response.getWriter();
			...
			userId = util.getUserId(querystring);
				...
				String nextJSP = request.getContextPath() + "/pages/Distributor/DatHangNpp.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//	String nppId = request.getParameter("nppId");
			...
			String tongchuavat = util.ValidateParam(util.antiSQLInspection(request.getParameter("tongchuavat")));
		    if(tongchuavat.length()>0)
		    	  obj.setTongtienbVAT(tongchuavat);
			String vat = util.ValidateParam(util.antiSQLInspection(request.getParameter("vat")));
			if(vat.length()>0)
		    obj.setVat(vat);
		    String tongcovat = util.ValidateParam(util.antiSQLInspection(request.getParameter("tongcovat")));
