...
import geso.dms.distributor.beans.bcchietkhauconlai.imp.BcChietKhauConLai;
...
@WebServlet("/BcChietKhauConLaiSvl")
public class BcChietKhauConLaiSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;
	
	public BcChietKhauConLaiSvl()
	{
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		nextJSP = request.getContextPath() + "/pages/Distributor/BcChietKhauConLai.jsp";
    ...
    IBcChietKhauConLai obj = new BcChietKhauConLai();
    obj.setUserId(userId);
    ...
    if (action.equals("excel")  )
    {
	    ...
	    { 
				...
				response.setHeader("Content-Disposition", "attachment; filename=BcChietKhauConLai.xlsm");
	      FileInputStream fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\BcChietKhauConLai.xlsm");
    	...
    	session.setAttribute("obj", obj);
    	response.sendRedirect(request.getContextPath() + "/pages/Distributor/BcChietKhauConLai.jsp");
    }
	...
	private boolean FillData(Workbook workbook, IBcChietKhauConLai obj, String query) throws Exception
  {
		...
		if(hdRs != null)
  	{
  		try {
			...
			Style style ;
  			...
  			while (hdRs.next()) 
  			{    		
				String tenkh = hdRs.getString("ten");
  				String makh = hdRs.getString("mafast");
	...
	private void CreateStaticHeader(Workbook workbook, IBcChietKhauConLai obj)
  {
			Worksheets worksheets = workbook.getWorksheets();
			Worksheet worksheet = worksheets.getSheet(0);
