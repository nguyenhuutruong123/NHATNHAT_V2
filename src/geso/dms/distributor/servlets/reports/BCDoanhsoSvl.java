...
public class BCDoanhsoSvl extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(view.length()>0  )
		{
			 npp=  util.antiSQLInspection(request.getParameter("npp"));
  		 if(npp==null)
  			 npp="";
		}
		else 
		...
		obj.setNppId(npp);
		 ...
		 String cn1 = util.antiSQLInspection(request.getParameter("cn1"));
		 if(cn1==null)
			 cn1="";
		 obj.setCn(cn1);
			...
			String search_ETC_HO = getSearchQuery_ETC_HO(request, (IBCHoadonbanra) obj, view);
		...
		System.out.println("__ETC_Total__CN "+search_ETC);
		System.out.println("__ETC_Total__"+search_ETC_HO);
	    ...
	    if (action.equals("excel") && ( tungay.trim().length() > 0 && denngay.trim().length() > 0 ) )
	    {
		    ...
		    { 
					...
					response.setHeader("Content-Disposition", "attachment; filename=BaoCaoDoanhSo.xlsm");
					FileInputStream fstream ;
					if(obj.getMuclay().equals("0"))
					{
						 fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\BCDoanhSo.xlsm");
					}else 
					{
						 fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\BCDoanhSo_Detail.xlsm");
					}
					...
					if(obj.getMuclay().equals("0"))
					{
					  ...
					  CreateStaticData_ETC_HO(workbook, obj, view,search_ETC_HO);
    		...
    		obj.searchQuery_ETCHO(search_ETC_HO);
	...
	private void CreateStaticData_KM_Detail(Workbook workbook, IBCHoadonbanra obj, String view, String search_KM)
  {
	...
	private String getSearchQuery_ETC_HO(HttpServletRequest request, IBCHoadonbanra obj, String view)
	{
		...
		String sql = 
				    ...
				    "	where 1=1  and c.SOLUONG > 0  and dbo.trim(a.nguoitao)=(select nv.pk_seq from NHANVIEN nv where nv.PK_SEQ= a.NGUOITAO and nv.MAUHOADON2=2 and len(dbo.Trim(nv.KYHIEU2))>0 )   and a.trangthai not in ( 1 , 3, 5 )  and a.ngayxuatHD >= '" + obj.getTungay() + "'  " +
		...
		if(obj.getMuclay().equals("1"))
		{
			 sql = 
					...
					"				inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq      "+
					   "		inner join khachhang kh on kh.pk_seq=a.khachhang_fk"+
					"			where 1=1  and c.SOLUONG > 0 and dbo.trim(a.nguoitao)=(select nv.pk_Seq from NHANVIEN nv where nv.PK_SEQ= a.NGUOITAO and nv.MAUHOADON2=2 and len(dbo.Trim(nv.KYHIEU2))>0 )  and a.trangthai not in ( 1 , 3, 5 )  and a.ngayxuatHD >= '"+tungay+"'  "+    
			 ...
			 if(obj.getCn().equals("1"))
			 {
				 sql =      "  union all "+	
							...
							"				inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq      "+
							   "		inner join khachhang kh on kh.pk_seq=a.khachhang_fk"+
							"			where 1=1  and c.SOLUONG > 0 and dbo.trim(a.nguoitao)=(select pk_seq from NHANVIEN nv where nv.PK_SEQ= a.NGUOITAO and nv.MAUHOADON2=2 and len(dbo.Trim(nv.KYHIEU2))>0 )   and a.trangthai not in ( 1 , 3, 5 )  and a.ngayxuatHD >= '"+tungay+"'  "+    
	...
	private String getSearchQuery_ETC(HttpServletRequest request, IBCHoadonbanra obj, String view)
	{
		...
		String sql = 
				    ...
				    "		inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq   " +
				    "		inner join khachhang kh on kh.pk_seq=a.khachhang_fk"+
				    "	where 1=1  and c.SOLUONG > 0 and dbo.trim(a.nguoitao)=(select nv.pk_seq from NHANVIEN nv where nv.PK_SEQ= a.NGUOITAO and nv.MAUHOADON=1 and len(dbo.Trim(nv.KYHIEU))>0 )  and a.trangthai not in ( 1 , 3, 5 )  and a.ngayxuatHD >= '" + obj.getTungay() + "'  " +
				    		"and a.ngayxuatHD <= '" + obj.getDenngay() + "' " + condition +
				    ") " +
				    "ETC group by MA, TEN, DONVI  ";
		...
		if(obj.getMuclay().equals("1"))
		{
			 sql = 
					...
					"				inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq      "+
					   "		inner join khachhang kh on kh.pk_seq=a.khachhang_fk"+
					"			where 1=1  and c.SOLUONG > 0 and dbo.trim(a.nguoitao)=(select nv.pk_seq from NHANVIEN nv where nv.PK_SEQ= a.NGUOITAO and nv.MAUHOADON=1 and len(dbo.Trim(nv.KYHIEU))>0 )  and a.trangthai not in ( 1 , 3, 5 )  and a.ngayxuatHD >= '"+tungay+"'  "+    
					"			and a.ngayxuatHD <=  '"+denngay+"'   " + condition +   ""+
					"			)   "+
					"			ETC group by MA, TEN, DONVI,ddkdTen ";
			 ...
			 if(obj.getCn().equals("1"))
			 {
				 sql =      "  union all "+	
							...
							"				inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq      "+
							   "		inner join khachhang kh on kh.pk_seq=a.khachhang_fk"+
							"			where 1=1  and c.SOLUONG > 0 and dbo.trim(a.nguoitao)=(select pk_Seq from NHANVIEN nv where nv.PK_SEQ= a.NGUOITAO and nv.MAUHOADON=1 and len(dbo.Trim(nv.KYHIEU))>0 )   and a.trangthai not in ( 1 , 3, 5 )  and a.ngayxuatHD >= '"+tungay+"'  "+    
							"			and a.ngayxuatHD <=  '"+denngay+"'   " + condition1 +   ""+
							"			)   "+
							"			ETC group by MA, TEN, DONVI,ddkdTen ";
	...
	sql=	"	select SUM(VAT) as VAT,SUM(BVAT) as BVAT,SUM(AVAT) as AVAT  from   ("+
		  ...
		  " 		sum(ROUND(( ROUND (soluong * dongia , 0 ) ) *( thuexuat/100.0),0 ))- sum(ROUND(( ROUND (ck , 0 ) ) *( thuexuat/100.0),0 ))as VAT, \n"+    		
			...
			"				"+condition+"   "+condition_ETC+"   \n"+ 
			"		) ETC  \n"+
			"		union all   \n"+
			"		select	" +
			"		sum(c.soluong) as soluong,   "+  
			"		cast(avg(c.DonGia) as numeric(18,4)) as dongia, sum(ROUND(c.soluong * c.DonGia,0)) as BVAT,     "+
			"		sum(ROUND( ROUND( c.soluong * c.DonGia,0 )*c.VAT/100,0 ) ) as VAT,sum( ROUND( ROUND( c.soluong * c.DonGia,0) *(1+c.vat/100.0)  ,0)  )  as AVAT  "+    			
			"		from HOADON a  \n"+
			"			inner join HOADON_SP c on a.pk_seq = c.hoadon_fk  \n"+
			"			inner join SANPHAM d on c.sanpham_fk = d.pk_seq   \n"+
			"			inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq   \n"+
			"		where c.SoLuong>0 and a.trangthai not in ( 1 , 3, 5 ) and isnull(a.loaihoadon, '0') = '0'  and a.NgayXuatHD>='"+tungay+"' and a.NgayXuatHD<='"+denngay+"'  \n"+
			"				"+condition+"  "+condition_OTC+"   "+
			//HD KHAC
			"		union all   \n"+
			"		select	" +
			"			c.soluong as soluong,   "+  
			"			c.DonGia as dongia, ROUND(c.soluong * c.DonGia,0) as BVAT,     "+
			"			c.vat as VAT,c.thanhtien  as AVAT  "+    			
			"		from ERP_HOADONPHELIEU a  \n"+
			"			inner join ERP_HOADONPHELIEU_sanpham c on a.pk_seq = c.hoadonphelieu_fk  \n"+
			"		where c.SoLuong>0 and a.trangthai = 1 and a.ngayhoadon>='"+tungay+"' and a.ngayhoadon<='"+denngay+"'  \n"+
			"				  "+condition_HDKhac+"   "; 
				{
					sql+=" UNION ALL	select 0 as SoLuong,0 as DonGia,-1*sum(ROUND( ck.ChietKhau,0) ) as BVAT,-1*sum(ROUND(ck.ChietKhau*ck.ThueVat/100,0)) as VAT,-1*sum( ROUND( ROUND(ck.ChietKhau,0) *(1+ck.ThueVat/100) , 0) ) as AVAT    \n"+
							"	from hoadon_chietkhau ck left join loaick loai on loai.ma=ck.diengiai \n"+
							"   where isnull(HienThi,0)=1 and  hoadon_fk in  (select pk_seq from hoadon  a where  trangthai not in ( 1 , 3, 5 ) and isnull(loaihoadon, '0') = '0' and a.NgayXuatHD>='"+tungay+"' and a.NgayXuatHD<='"+denngay+"'    "+condition+"   "+condition_OTC+"   )" ;
				}
	sql+="	)as total "; 
	...
	if(obj.getCn().equals("1"))
	{
		sql+=	"\n  union all \n "+
			  ...
			  " 		sum(ROUND(( ROUND (soluong * dongia , 0 ) ) *( thuexuat/100.0),0 ))- sum(ROUND(( ROUND (ck , 0 ) ) *( thuexuat/100.0),0 ))as VAT, \n"+    		
				...
				"			inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq   \n"+
				"		where c.SoLuong>0 and a.trangthai not in ( 1 , 3, 5 ) and isnull(a.loaihoadon, '0') = '0'  and a.NgayXuatHD>='"+tungay+"' and a.NgayXuatHD<='"+denngay+"'  \n"+
				"				"+condition1+"  "+condition_OTC1+"   "; 	
					{
						sql+=" UNION ALL	select 0 as SoLuong,0 as DonGia,-1*sum(ROUND( ck.ChietKhau,0) ) as BVAT,-1*sum(ROUND(ck.ChietKhau*ck.ThueVat/100,0)) as VAT,-1*sum( ROUND( ROUND(ck.ChietKhau,0) *(1+ck.ThueVat/100) , 0) ) as AVAT    \n"+
								"	from hoadon_chietkhau ck left join loaick loai on loai.ma=ck.diengiai \n"+
								"   where isnull(HienThi,0)=1 and  hoadon_fk in  (select pk_seq from hoadon  a where  trangthai not in ( 1 , 3, 5 ) and isnull(loaihoadon, '0') = '0' and a.NgayXuatHD>='"+tungay+"' and a.NgayXuatHD<='"+denngay+"'    "+condition1+"   "+condition_OTC1+"   )" ;
					}
		sql+="	)as total  "; 
	...
	private String getSearchQuery_KM(HttpServletRequest request, IBCHoadonbanra obj, String view)
	{
		    ...
		    String conditionE ="";
		  	if(obj.getVung().trim().length() > 0)
		  	{
		  		condition += "and a.npp_fk in (Select pk_Seq  from nhaphanphoi where  khuvuc_Fk in (Select pk_Seq from khuvuc where vung_Fk='"+obj.getVung()+"' ) ) ";
		  		condition1 += "and a.npp_fk in (Select pk_Seq  from nhaphanphoi where  khuvuc_Fk in (Select pk_Seq from khuvuc where vung_Fk='"+obj.getVung()+"' ) ) ";
		  		conditionE += "and a.npp_fk in (Select pk_Seq  from nhaphanphoi where  khuvuc_Fk in (Select pk_Seq from khuvuc where vung_Fk='"+obj.getVung()+"' ) ) ";
	 			...
	 			if(obj.getKhuvuc().trim().length() > 0)
	 			{
	 				condition += " and a.npp_fk in (Select pk_Seq from nhaphanphoi where khuvuc_Fk='"+obj.getKhuvuc()+"')";	
	 				condition1 += " and a.npp_fk in (Select pk_Seq from nhaphanphoi where khuvuc_Fk='"+obj.getKhuvuc()+"')";
					conditionE += " and a.npp_fk in (Select pk_Seq from nhaphanphoi where khuvuc_Fk='"+obj.getKhuvuc()+"')";	
				...
				{
	 				condition += " and a.npp_Fk in "+util.quyen_npp(obj.getUserId() ) +" ";
	 				conditionE += " and a.npp_Fk in "+util.quyen_npp(obj.getUserId() ) +" ";
	 				condition1 += " and a.npp_Fk in "+util.quyen_npp(obj.getUserId() ) +" ";
				}
	 			...
	 			{
	 				condition += " and a.pk_seq  in (select hoadon_Fk from hoadon_ddh where ddh_Fk  in (select pk_seq from donhang  where ddkd_fk='"+obj.getTdvId()+"')   ";
	 				conditionE += " and a.pk_seq  in (select hoadon_Fk from hoadon_ddh where ddh_Fk  in (select pk_seq from donhang  where ddkd_fk='"+obj.getTdvId()+"')   ";
					condition1 += " and a.pk_seq  in (select hoadon_Fk from hoadon_ddh where ddh_Fk  in (select pk_seq from donhang  where ddkd_fk='"+obj.getTdvId()+"')   ";
				}
		    ...
		    "	where 1=1  and c.SOLUONG > 0  and a.trangthai not in ( 1 , 3, 5 ) and isnull(a.LoaiHoaDon,0) =1  and a.ngayxuatHD >= '" + obj.getTungay() + "'  " +
		    		"and a.ngayxuatHD <= '" + obj.getDenngay() + "' " + conditionE +
		    ") " +
		    "ETC group by MA, TEN, DONVI  ";
		    }
		    ...
		    if(obj.getMuclay().equals("1"))
		    {
		    	 ...
		    	 "			select	ddkdTen,MA, TEN, DONVI, sum(soluong) as soluong, cast( ( sum(round( soluong * dongia,0) ) / sum(soluong) ) as numeric(18, 4) ) as dongia,    "+  
						...
						"			and a.ngayxuatHD <=  '"+denngay+"'   " + conditionE +   ""+
						"			)   "+
						"			ETC group by MA, TEN, DONVI,ddkdTen ";
	...
	private void CreateStaticHeader(Workbook workbook, IBCHoadonbanra obj) 
	{
	    ...
	    worksheet2.setName("ETCCN");
	    
	    /***************************END SHEET 2 - ETC ***********************************/
	    
	    /***************************SHEET 1 - ETC ***********************************/
	    Worksheet worksheet6 = worksheets.getSheet(2);
	    cells = worksheet6.getCells();
	    ...
	    /***************************SHEET 3 - OTC ***********************************/
	    Worksheet worksheet3 = worksheets.getSheet(3);
	    cells = worksheet3.getCells();
	...
	{
		Worksheets worksheets = workbook.getWorksheets();
	    Worksheet worksheet = worksheets.getSheet("KM");
	    Cells cells = worksheet.getCells();
	...
	{
		Worksheets worksheets = workbook.getWorksheets();
	    Worksheet worksheet = worksheets.getSheet("OTC");
	    Cells cells = worksheet.getCells();
	    String query=search_OTC;
	...
	{
		Worksheets worksheets = workbook.getWorksheets();
	    Worksheet worksheet = worksheets.getSheet("HDKhac");
	    Cells cells = worksheet.getCells();
	    String query=search_HDKhac;
	...
	{
		Worksheets worksheets = workbook.getWorksheets();
	    Worksheet worksheet = worksheets.getSheet("ETCCN");
	    Cells cells = worksheet.getCells();
			...
			try 
			{
				...
				if(obj.getMucCN_DT().equals("1")&&obj.getMuc_KhachHang().equals("0"))
				{
				 ...
				 query=
					...
					"	from ERP_HOADONNPP  a inner join ERP_HOADONNPP_SP b on b.HOADON_FK=a.PK_SEQ  "+
					"	where  (select isnull(mauho,0) as mauho from khachhang where pk_seq=a.khachhang_fk)=0  and  a.trangthai not in (1,3,5) "+condition+""+
					"  having sum(b.CHIETKHAU)>0    ";
				 if(obj.getCn().equals("1"))
				 {
					 ...
					 query=		" union all "+
								...
								"		 sum(ROUND(( ROUND (B.CHIETKHAU , 0 ) ) *( b.VAT/100.0),0 )) as VAT,sum(ROUND(( ROUND (b.CHIETKHAU , 0 ) ) *(1+ b.VAT/100.0),0 )) as BVAT "+
								"	from ERP_HOADONNPP  a inner join ERP_HOADONNPP_SP b on b.HOADON_FK=a.PK_SEQ  "+
								"	where (select isnull(mauho,0) as mauho from khachhang where pk_seq=a.khachhang_fk)=0 and  a.trangthai not in (1,3,5) "+condition1+""+
								"  having sum(b.CHIETKHAU)>0    ";
				 }
	...
	private void CreateStaticData_ETC_HO(Workbook workbook, IBCHoadonbanra obj, String view, String search_ETC ) 
	{
		Worksheets worksheets = workbook.getWorksheets();
	    Worksheet worksheet = worksheets.getSheet("ETCHO");
	    Cells cells = worksheet.getCells();
			...
			try 
			{
				...
				if(obj.getMucCN_DT().equals("1")&&obj.getMuc_KhachHang().equals("0"))
				{
				 ...
				 query=
					...
					"		 sum(ROUND(( ROUND (B.CHIETKHAU , 0 ) ) *( b.VAT/100.0),0 )) as VAT,sum(ROUND(( ROUND (b.CHIETKHAU , 0 ) ) *(1+ b.VAT/100.0),0 )) as BVAT "+
					"	from ERP_HOADONNPP  a inner join ERP_HOADONNPP_SP b on b.HOADON_FK=a.PK_SEQ  "+
					"	where  (select isnull(mauho,0) as mauho from khachhang where pk_seq=a.khachhang_fk)=1  and  a.trangthai not in (1,3,5) "+condition+""+
					"  having sum(b.CHIETKHAU)>0    ";
				 if(obj.getCn().equals("1"))
				 {
					 ...
					 query=		" union all "+
								...
								"		 sum(ROUND(( ROUND (B.CHIETKHAU , 0 ) ) *( b.VAT/100.0),0 )) as VAT,sum(ROUND(( ROUND (b.CHIETKHAU , 0 ) ) *(1+ b.VAT/100.0),0 )) as BVAT "+
								"	from ERP_HOADONNPP  a inner join ERP_HOADONNPP_SP b on b.HOADON_FK=a.PK_SEQ  "+
								"	where (select isnull(mauho,0) as mauho from khachhang where pk_seq=a.khachhang_fk)=1 and  a.trangthai not in (1,3,5) "+condition1+""+
								"  having sum(b.CHIETKHAU)>0    ";
				 }
