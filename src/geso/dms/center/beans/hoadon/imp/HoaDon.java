...
public class HoaDon implements IHoaDon
{
	...
	public void init()
	{
		...
		try
		{
			...
			rs = this.db.get("select count(nhaphang_fk) as num from nhaphang_sp where nhaphang_fk='" + this.id + "'");
			rs.next();
			...
			this.SLHOADON = new String[i];
			this.QUYCACHSP = new String[i];
			this.SoLo = new String[i];
			this.Ngayhethan = new String[i];

			query = "select  (select ten from nhaphanphoi where pk_Seq=npp_fk ) as nppTen,npp_fk as nppId,ncc_fk, dvkd_fk, kbh_fk, chungtu, ngaychungtu, ngaynhan, sotienbvat, vat, sotienavat, kho_fk,trangthai from nhaphang where pk_seq='" + this.id + "' order by ngaychungtu,trangthai";
			...
			query = " select  a.solo,a.ngayhethan ,a.sanpham_fk as ma, b.ten as tensp,isnull(a.SLHOADON,0) as SLHOADON " + " ,isnull( a.QUYCACHSP,0) as QUYCACHSP, a.soluong as sl, a.donvi, a.gianet, "
					+ "  a.tienbvat, a.vat, a.tienavat,a.DonViTinh from nhaphang_sp  a " + " inner join   sanpham b on  a.sanpham_fk = b.ma " + " where  a.nhaphang_fk='" + this.id + "' " + " order by a.sanpham_fk";
			...
			while (rs.next())
			{
				...
				this.donvi[m] = rs.getString("DonViTinh");
				...
				this.SLHOADON[m] = rs.getString("SLHOADON");
				this.QUYCACHSP[m] = rs.getString("QUYCACHSP");
				this.SoLo[m] = rs.getString("solo");
				this.Ngayhethan[m] = rs.getString("ngayhethan");
				m++;
			}
			this.size = "" + m;
			...
			query = "select nhaphang_fk from nhaphang_sp where nhaphang_fk='" + this.id + "' and scheme like '%DG%'";
			ResultSet rscheck = this.db.get(query);
			if (rscheck != null)
			{
				if (rscheck.next())
				{
					query = "select distinct a.kho_fk as khoId, b.ten as kho from nhapp_kho a, kho b where npp_fk='" + this.nppId + "' and a.kho_fk=b.pk_seq and a.kbh_fk='" + this.kbhId + "' and b.ten='DG001'";
				} else
				{
					query = "select distinct a.kho_fk as khoId, b.ten as kho from nhapp_kho a, kho b where npp_fk='" + this.nppId + "' and a.kho_fk=b.pk_seq and a.kbh_fk='" + this.kbhId + "' and  b.ten <> 'DG001'";
				}
			...
			query = "select pk_seq,ten from giamsatbanhang inner join nhapp_giamsatbh a on a.gsbh_fk=pk_seq  where trangthai=1  and npp_fk=" + this.nppId + " and a.ngayketthuc >='" + this.getDateTime() + "'";
	...
	@Override
	public String[] getSLHOADON()
	{

		return this.SLHOADON;
	}

	@Override
	public void setSLHOADON(String[] _SLHOADON)
	{

		this.SLHOADON = _SLHOADON;
	}

	@Override
	public String[] getQUYCACHSP()
	{

		return this.QUYCACHSP;
	}

	@Override
	public void setQUYCACHSP(String[] _QUYCACHSP)
	{

		this.QUYCACHSP = _QUYCACHSP;
	}
