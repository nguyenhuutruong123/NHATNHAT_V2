...
public class Danhmucquyen implements IDanhmucquyen
{
		...
		try
		{
			...
			{				
				for (int i = 0; i < ungdungIds.length; i++)
				{
					...
					" values('" + this.Id + "','" + ungdung + "','" + them + "','" + xoa + "','" + sua + "','" + xem + "','" + chot + "','" + huychot + "','"+hienthi+"')";
	...
	public boolean save(HttpServletRequest request)
	{
		try
		{
			...
			sql = "SELECT SCOPE_IDENTITY() as spId";
			...
			sql="delete from NhanVien_UngDung where nhanvien_Fk in ( select nhanvien_fk from PHANQUYEN where dmq_Fk='"+this.Id+"' ) " ;
			if (!db.update(sql))
			{
				this.Msg = sql;
				db.getConnection().rollback();
...
String ungdungIds;
  ...
  public void createRs()
  {
		String	query = 
				...
				"where a.level = '3' and b.level = '2'  and a.loaiMenu='"+this.loaiMenu+"' and a.TrangThai=1  and b.TrangThai = 1 and c.trangthai = 1 "+          
				"union all           "+
				...
				"where a.level = '3' and c.level = '1'  and a.loaiMenu='"+this.loaiMenu+"'  and a.TrangThai=1 and c.trangthai = 1  "+           
