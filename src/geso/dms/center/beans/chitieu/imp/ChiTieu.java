...
public class ChiTieu implements IChiTieu 
{
	...
	public boolean save()
	{
		try
		{
			...
			String	query =
					...
					" select '"+this.tructhuocId+"','CN/DT',"+(this.thang.length()<=0?"NULL":this.thang)+","+(this.nam.length()<=0?"NULL":this.nam)+","+(this.quy.length()<=0?"NULL":this.quy)+",N'"+this.scheme+"',N'"+this.diengiai+"','"+this.userId+"','"+this.userId+"','"+this.getDateTime()+"','"+this.getDateTime()+"',0,'"+this.apdung+"','"+this.loaichitieu+"' ";
			...
			sql="";
			 keys = this.tdv_chitieu.keys();
			while(keys.hasMoreElements())
			{
				String key = keys.nextElement();
				String value=this.tdv_chitieu.get(key);
				String donvi=this.tdv_donvi_chitieu.get(key);
				sql +="select '"+key+"' as nppId,'"+value+"' as ChiTieu,'"+donvi+"' as DonVi union "; 
				System.out.println("KEY____"+key);
			}
			
			if (this.tdvIds.trim().length() > 0)
			{
				sql = sql.substring(0, sql.length() - 6);
				query = 
					"Insert ChiTieu_DoiTuong(ChiTieu_fk, ddkd_fk,ChiTieu,DonViTinh_ChiTieu) " + 
					"select '"+this.id+"' as tctId, tieuchi.* from (" + sql + ") tieuchi  ";
				System.out.println("4.Insert: " + query);
				if (!db.update(query))
				{
					this.msg = "3.Khong the tao moi ChiTieu_DoiTuong: " + query;
					db.getConnection().rollback();
					return false;
				}
			}
	...
	public void init()
	{
		String query = 
				...
				" STUFF  "+
				"	((select DISTINCT TOP 100 PERCENT ' , ' +cast( tbh.npp_fk as nvarchar(18) ) from chitieu_doituong tbh where npp_fk is not null and  chitieu_fk='"+this.id+"'  "+
				"		ORDER BY ' , ' +cast( tbh.npp_fk as nvarchar(18) ) FOR XML PATH('') ), 1, 2, +' ') as nppId  ,"+
				" STUFF  "+
				"	((select DISTINCT TOP 100 PERCENT ' , ' +cast( tbh.ddkd_fk as nvarchar(18) ) from chitieu_doituong tbh where ddkd_fk is not null and chitieu_fk='"+this.id+"'  "+
				"		ORDER BY ' , ' +cast( tbh.ddkd_fk as nvarchar(18) ) FOR XML PATH('') ), 1, 2, +' ') as ddkdID  "+
		...
		if (rs != null)
		{
			try
			{
				...
				while (rs.next())
				{
					...
					this.spIds=rs.getString("spId")==null?"":rs.getString("spId");
					this.nppIds=rs.getString("nppId")==null?"":rs.getString("nppId");
					this.tdvIds=rs.getString("ddkdiD")==null?"":rs.getString("ddkdiD");
				}
				rs.close();
				
				query = "select npp_fk,chitieu_fk,DonViTinh_ChiTieu,chitieu " +
						" from ChiTieu_DoiTuong where chitieu_fk = '" + this.id + "' and npp_fk is not null ";
				rs = db.get(query);
				if(rs != null)
				{
				...
				query = "select ddkd_fk,chitieu_fk,DonViTinh_ChiTieu,chitieu " +
						" from ChiTieu_DoiTuong where chitieu_fk = '" + this.id + "' and ddkd_fk is not null ";
				rs = db.get(query);
				if(rs != null)
				{
					this.tdv_chitieu = new Hashtable<String, String>();
					this.tdv_donvi_chitieu = new Hashtable<String, String>();
					
					while(rs.next())
					{
						this.tdv_chitieu.put(rs.getString("ddkd_fk"), format.format(rs.getDouble("chitieu")));
						this.tdv_donvi_chitieu.put(rs.getString("ddkd_fk"),     rs.getString("DonViTinh_ChiTieu"));
					}
					rs.close();
				}	
		...
		try
		{
			...
			if (rs != null)
			{
				if (rs.next())
				{
					...
					{
						this.tructhuocId = Ult.getIdNhapp(this.userId);
					}
					else
						this.tructhuocId ="1";
					rs.close();
	...
	public void createRs()
	{
		...
		{
			query += " union select PK_SEQ, MA, TEN, TRANGTHAI, NHANHANG_FK, CHUNGLOAI_FK from SanPham where pk_seq in ( select sanpham_fk from ChiTieu_SANPHAM where chitieu_fk = '" + this.id + "' ) ";
		}
		...
		query = "select PK_SEQ, MA, TEN  from NhaPhanPhoi where trangthai = '1' and isKhachHang=0 and tructhuoc_fk='"+this.tructhuocId+"'";
		...
		query="select pk_seq,ten,manhanvien as ma from daidienkinhdoanh where trangthai=1 and pk_Seq in (select ddkd_fk from daidienkinhdoanh_npp where npp_fk='"+this.tructhuocId+"' ) order by ten";
		System.out.println("___NVBH__"+query);
