...
public class NganhHang implements INganhHang
{
	...
	public ResultSet createDvkdList(){//chi cho load don vi kinh doanh nao co checked=1
		...
		return db.get("select distinct a.pk_seq, a.donvikinhdoanh from donvikinhdoanh a,nhacungcap_dvkd b where a.pk_seq = b.DVKD_fk and a.trangthai ='1' order by a.donvikinhdoanh");
	}
	...
	public boolean create()
	{
		try{
			...
			//"+ this.thuexuat+ "thuexuat,
			String command = "insert into nganhhang(ten, diengiai, trangthai, dvkd_fk, ngaytao, ngaysua, nguoitao, nguoisua)" +
					" values( N'" + this.ten + "',N'" + this.diengiai + "'," + this.trangthai + ",'" + this.dvkdId + "','" + this.ngaytao + "','" + this.ngaytao + "','" + this.userId + "','" + this.userId + "')"; 
	...
	public boolean update() 
	{
		...
		//thuexuat="+this.thuexuat+",
		String command ="update nganhhang set  ten = N'" + this.ten + "', diengiai = N'" + this.diengiai + "', trangthai='" + this.trangthai + "', ngaysua = '" + this.ngaysua + "', nguoisua = '" + this.userId + "' where pk_seq = '" + this.id + "'";
