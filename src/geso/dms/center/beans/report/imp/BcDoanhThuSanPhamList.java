...
public class BcDoanhThuSanPhamList  extends Phan_Trang implements IBcDoanhThuSanPhamList, Serializable
{
	private int num;
	...
	public void xuatexcelNPP(String search)
	{
		...
		{
			query = 
					"select v.TEN as vTen,tt.TEN as ttTEN,npp.MaFAST as nppMa,npp.TEN as nppTEN, \n" +  
					"    kh.maFAST as khMa,KH.TEN AS khTEN,kh.CHUCUAHIEU as khChuCH,loai.ten as khLOAI,KH.DIACHI as khDIACHI,kh.MaHD as khMaHD, \n" +   
					"    HD.spMa, HD.spTen,SUM(SoLuong)as SoLuong,AVG(DonGia) as DonGia,SUM(AVAT)as AVAT,SUM(BVAT)as BVAT,SUM(VAT)as VAT,sum(tonggiamtru) tonggiamtru, \n" + 
					"    HD.SOHOADON, HD.sonetId, HD.NGAYLAPPHIEU, hd.SOLO, hd.LoaiHD, hd.DONVI, hd.ghichu, hd.chietkhaungay \n" +  
					" 	,isnull(( \n" +	
					"		select top(1) c.ten \n" +	
					...
					"from \n" +  
					"( \n" +  
					"	select sonetId,NGAYLAPPHIEU,SOHOADON,NPP_FK,KHACHHANG_FK,'OTC' as LoaiHD,hdOTC.spMa, hdOTC.spTen,hdOTC.SOLO,hdOTC.DONVI, \n" + 
					"	hdOTC.SoLuong,hdOTC.DonGia,hdOTC.BVAT, hdOTC.VAT ,hdOTC.AVAT, hdOTC.tonggiamtru, hdOTC.ghichu, hdOTC.chietkhaungay \n" +  
					"	from \n" +  
					"	( \n" +  
					"  		select d.sonetId, d.NGAYLAPPHIEU, a.SOHOADON, a.KHACHHANG_FK,a.NPP_FK, \n" +  
					"		sp.MA as spMa, sp.TEN as spTen, b.SOLO, dv.DONVI, SUM(b.SoLuong)as SoLuong,AVG(b.DonGia) as DonGia, \n" +
					"		sum(round(b.SoLuong*b.DONGIA,0)) as BVAT, \n" +
					"  		sum( round( round( b.SoLuong*b.DONGIA,0) *(1+ b.THUEVAT/100) ,0)) as AVAT, \n" +  
					"		sum( round( round( b.SoLuong*b.DONGIA,0) *(b.THUEVAT/100.0) ,0)) as VAT, SUM(round(b.tonggiamtru,0)) tonggiamtru, '' ghichu, b.chietkhaungay \n" +
					"		from HOADON a inner join HOADON_SP_CHITIET b on b.HOADON_FK=a.PK_SEQ \n" + 
					"		inner join PHIEUXUATKHO_DONHANG c on a.PK_SEQ = c.hoadon_fk \n" +
					"		inner join PHIEUXUATKHO d on c.PXK_FK = d.PK_SEQ \n" + 
					"		left join SANPHAM sp on sp.ma=b.ma \n" +
					"		left join DONVIDOLUONG dv on sp.DVDL_FK = dv.PK_SEQ \n" +	
					"		where a.LOAIHOADON =0 and a.TRANGTHAI not in (1,3,5) " + condition + conditionNK + " \n" + 
					"		group by a.NPP_FK ,A.KHACHHANG_FK,sp.MA,sp.TEN, a.SOHOADON,d.sonetId, d.NGAYLAPPHIEU, b.SOLO, dv.DONVI, b.chietkhaungay \n\n" +
					"	)as hdOTC \n" +
					"	UNION ALL \n" +  
					"	select sonetId, NgayYeuCau,SOHOADON,npp_fk,KHACHHANG_FK, \n" +   
					"	'ETC' as LoaiHD,ETC.spMa, ETC.spTen, ETC.solo, ETC.spDonVi,SUM(ETC.SoLuong)as SoLuong,AVG(ETC.DonGia) as DonGia, \n" +   
					"	sum( ROUND( soluong * dongia,0 ) - ck )  as BVAT, \n" +   		
					"	sum( ROUND( ROUND ( soluong * dongia- ck,0 )* thuexuat/100.0,0 ) ) as VAT, \n" +  		
					"	sum( ROUND( ( ROUND (soluong * dongia , 0 ) - ck )   * (1+ thuexuat/100.0),0 ) ) as AVAT, SUM(round(tonggiamtru,0)) tonggiamtru, etc.ghichu, 0 chietkhaungay \n" +     		
					"	from \n" +  
					"	( \n" +  
					"		select g.sonetId, g.NgayYeuCau, a.SOHOADON, a.npp_fk, a.KHACHHANG_FK, \n" +   
					"		case when c.donvi = e.donvi then c.soluong \n" +  
					"		else c.soluong * ( select SOLUONG1 / SOLUONG2 from QUYCACH where sanpham_fk = d.PK_SEQ and DVDL2_FK = \n" + 
					"		( select pk_seq from DONVIDOLUONG where donvi = c.donvi ) and DVDL1_FK = d.DVDL_FK ) end as soluong, \n" +  
					"		case when c.donvi = e.donvi then c.dongia \n" +  
					"		else c.dongia * ( select SOLUONG2 / SOLUONG1 from QUYCACH where sanpham_fk = d.PK_SEQ and DVDL2_FK = \n" + 
					"		( select pk_seq from DONVIDOLUONG where donvi = c.donvi ) and DVDL1_FK = d.DVDL_FK ) end as dongia, \n" +  
					"		c.THUEVAT as thuexuat , round(isnull(c.CHIETKHAU,0),0) as ck ,d.MA as spMa,d.TEN as spTen, c.SOLO, \n" +
					"		e.DONVI as spDonVi, isnull(c.tonggiamtru, 0) tonggiamtru, c.ghichu \n" +  
					"		from ERP_HOADONNPP a \n" +  
					"		inner join ERP_HOADONNPP_SP_CHITIET c on a.pk_seq = c.hoadon_fk \n" +  
					"		inner join ERP_YCXUATKHONPP_DDH f on a.PK_SEQ = f.hoadon_fk \n" +
					"		inner join ERP_YCXUATKHONPP g on f.ycxk_fk = g.PK_SEQ \n" +
					"		inner join SANPHAM d on c.ma = d.ma \n" +  
					"		inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq \n" +   
					"		where 1=1 and a.trangthai not in ( 1 , 3, 5 ) and a.KHACHHANG_FK is not null \n" + conditionETC + " \n" +  
					"	) ETC \n" +  
					"	where soluong>0 \n" +   
					"	GROUP BY SOHOADON, NPP_FK,KHACHHANG_FK,ETC.spMa,ETC.spTen,sonetId, NgayYeuCau, SOLO, spDonVi, ghichu \n\n" +  
					") as HD \n" +  
					...
					"group by v.TEN,tt.TEN,npp.MaFAST,npp.TEN, kh.maFAST,KH.TEN,KH.DIACHI,loai.ten,kh.CHUCUAHIEU,kh.MaHD, HD.spMa, HD.spTen, kh.PK_SEQ, \n" + 
					"hd.SOHOADON, hd.sonetId, HD.NGAYLAPPHIEU, hd.SOLO, hd.LoaiHD, hd.DONVI, hd.ghichu, hd.chietkhaungay \n" + 
					"order by hd.sonetId asc, kh.maFAST asc, HD.spMa desc ";
			this.queryHd=query;
	...
	public void xuatexcelTT(String search)
	{
		...
		if(this.action.length()>0)
		{
			query = "select v.TEN as vTen,tt.TEN as ttTEN,npp.MaFAST as nppMa,npp.TEN as nppTEN, \n"+ 
					"    npp.maFAST as khMa,npp.TEN AS khTEN,npp.CHUCUAHIEU as khChuCH,npp.DIACHI as khDIACHI,npp.MaHD as khMaHD, \n"+ 
					"    HD.spMa, HD.spTen,SUM(SoLuong)as SoLuong,AVG(DonGia) as DonGia,SUM(AVAT)as AVAT,SUM(BVAT)as BVAT,SUM(VAT)as VAT, 0 as tonggiamtru, \n"+ 
					"    HD.SOHOADON, HD.sonetId, HD.NGAYLAPPHIEU, hd.SOLO, hd.LoaiHD, hd.DONVI, hd.ghichu, 0 as chietkhaungay \n"+ 	 
					"from \n"+ 
					"( \n"+ 
					"	select sonetId, NgayYeuCau as NGAYLAPPHIEU,SOHOADON,isnull(npp_fk,KHACHHANG_FK) khachhang_fk, \n"+ 
					"	ETC.loaihd as LoaiHD,ETC.spMa, ETC.spTen, ETC.solo, ETC.spDonVi as DONVI,SUM(ETC.SoLuong)as SoLuong,AVG(ETC.DonGia) as DonGia, \n"+ 
					"	sum( ROUND( soluong * dongia,0 ) - ck )  as BVAT, \n"+ 
					"	sum( ROUND( ROUND ( soluong * dongia- ck,0 )* thuexuat/100.0,0 ) ) as VAT, \n"+ 
					"	sum( ROUND( ( ROUND (soluong * dongia , 0 ) - ck )   * (1+ thuexuat/100.0),0 ) ) as AVAT, etc.ghichu \n"+ 
					"	from \n"+ 
					"	( \n"+ 
					"		select g.sonetId, g.NgayYeuCau, a.SOHOADON, a.npp_fk, a.KHACHHANG_FK, \n"+ 
					"		case when k.DONVITINH = e.donvi then c.soluong \n"+ 
					"		else c.soluong * ( select SOLUONG1 / SOLUONG2 from QUYCACH where sanpham_fk = d.PK_SEQ and DVDL2_FK = \n"+ 
					"		( select pk_seq from DONVIDOLUONG where donvi = k.DONVITINH ) and DVDL1_FK = d.DVDL_FK ) end as soluong, \n"+ 
					"		case when k.DONVITINH = e.donvi then c.dongia \n"+ 
					"		else c.dongia * ( select SOLUONG2 / SOLUONG1 from QUYCACH where sanpham_fk = d.PK_SEQ and DVDL2_FK = \n"+ 
					"		( select pk_seq from DONVIDOLUONG where donvi = k.DONVITINH ) and DVDL1_FK = d.DVDL_FK ) end as dongia, \n"+ 
					"		c.THUEVAT as thuexuat , round(isnull(c.CHIETKHAU,0),0) as ck ,d.MA as spMa,d.TEN as spTen, c.SOLO, \n"+ 
					"		e.DONVI as spDonVi, '' ghichu, i.ten loaihd \n"+ 
					"		from ERP_HOADON a \n"+ 
					"		inner join ERP_HOADON_SP_CHITIET c on a.pk_seq = c.hoadon_fk \n"+ 
					"		inner join SANPHAM d on c.ma = d.ma \n"+ 
					"		inner join ERP_HOADON_SP k on a.pk_seq = k.hoadon_fk and k.SANPHAM_FK = d.pk_seq \n"+
					"		inner join ERP_HOADON_DDH j on a.pk_seq = j.hoadon_fk \n"+ 
					"		inner join ERP_YCXUATKHO_DDH f on a.PK_SEQ = f.hoadon_fk \n"+ 
					"		inner join ERP_YCXUATKHO g on f.ycxk_fk = g.PK_SEQ \n"+ 
					"		inner join ERP_DONDATHANG h on j.ddh_fk = h.PK_SEQ \n"+ 
					"		inner join KENHBANHANG i on h.kbh_fk = i.PK_SEQ \n"+ 
					"		inner join DONVIDOLUONG e on d.DVDL_FK = e.pk_seq \n"+ 
					"		where 1=1 and a.trangthai not in ( 1 , 3, 5 ) and (a.KHACHHANG_FK is not null or a.NPP_FK is not null) \n" + conditionETC + " \n"+ 
					"	) ETC \n"+ 
					"	where soluong>0 \n"+ 
					"	GROUP BY SOHOADON, isnull(NPP_FK,KHACHHANG_FK),ETC.spMa,ETC.spTen,sonetId, NgayYeuCau, SOLO, spDonVi, ghichu,loaihd \n"+
					") as HD \n"+ 
					"inner join NHAPHANPHOI npp on npp.PK_SEQ=HD.khachhang_fk \n"+
					"left join KHUVUC kv on kv.PK_SEQ=npp.KHUVUC_FK \n"+ 
					"left join VUNG v on v.PK_SEQ=kv.VUNG_FK \n"+ 
					"left join TINHTHANH tt on tt.PK_SEQ=npp.TINHTHANH_FK \n"+ 
					"group by v.TEN,tt.TEN,npp.MaFAST,npp.TEN, npp.DIACHI,npp.CHUCUAHIEU,npp.MaHD, HD.spMa, HD.spTen, npp.PK_SEQ, \n"+ 
					"hd.SOHOADON, hd.sonetId, HD.NGAYLAPPHIEU, hd.SOLO, hd.LoaiHD, hd.DONVI, hd.ghichu \n"+ 
					"order by hd.sonetId asc, npp.maFAST asc, HD.spMa desc ";
			System.out.println("::: BCTT: "+query);
			this.queryHd=query;
			setTotal_Query(query);
		}
		//this.hoadonRs= super.createSplittingData(super.getItems(), super.getSplittings(), " khMa ", query); 
