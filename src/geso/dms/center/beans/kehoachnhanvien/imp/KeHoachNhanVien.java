...
public class KeHoachNhanVien implements IKeHoachNhanVien
{
	...
	public boolean update() 
	{
		try {
			...
			query = " DELETE KEHOACHNV_TBH WHERE KEHOACHNV_FK = " + this.id + queryNgay;
			if (!db.update(query)) {
				this.msg = "Không thể cập nhật kế hoạch tháng " + thang + " năm " + nam + " của nhân viên " + this.tennhanvien + "(" + query + ")";
				db.getConnection().rollback();
				return false;
			...
			for(int i = 0; i < this.ngayList.length; i++) {
				...
				else if(nam == namHienTai)
				{
					...
					{
						if(Integer.parseInt(ngay.getNgay()) >= ngayHienTai )
							condition =true;
					}
				...
				System.out.println("condition= "+ condition + ",---" +thang + ", " + thangHienTai + ", " + nam + ", " + namHienTai + ", " + ngay.getNgay() + ", " + ngay.getNgay() + ", " + (Integer.parseInt(ngay.getNgay()) >= ngayHienTai));
				if(ngay != null && condition) {
					//Insert nhà phân phối
					nppList = ngay.getNppList();
					
					System.out.println(" == nppList == : "+ nppList);
					...
					if(tbhList != null){
						for(int j = 0;j<tbhList.size(); j++){
							...
							if (!db.update(query)) {
								...
								this.msg = "Vui lòng chọn đầy đủ thông tin cho kế hoạch ngày.";
								db.getConnection().rollback();
								return false;
							}
						}
		...
		if(id != null && id.length() > 0) 
		{
	        ...
	        try
	        {
	        	...
	        	query = " SELECT KHNPP.KEHOACHNV_FK, KHNPP.NGAY, KHNPP.NPP_FK, ISNULL(KHNPP.LAT, '') AS LAT, ISNULL(KHNPP.LONG, '') AS LONG, ISNULL(KHNPP.GHICHU, '') AS GHICHU, ISNULL(KHNPP.GHICHU2, '') AS GHICHU2, NPP.TEN AS NPP_TEN, " +
						...
						" ORDER BY NGAY ";
	        	System.out.println(" ke hoach npp: " +   query);
	        	rs =  db.get(query);
	        	try 
        		{
        			while(rs.next()) {
	...
	public void createRs() {
		...
		query = "select a.PK_SEQ, a.TEN, b.NPP_FK  from DAIDIENKINHDOANH a inner join daidienkinhdoanh_npp b on a.pk_seq = b.DDKD_FK where  a.TRANGTHAI = '1' and b.NPP_FK in "+ util.quyen_npp(this.userId);
		System.out.println("\nGET DDKD = " + query);
