...
public class ErpYeucauxuatkho implements IErpYeucauxuatkho
{
	...
	public boolean create() 
	{
		...
		try
		{
			...
			query = "Insert ERP_YCXUATKHO_DDH(ycxk_fk, hoadon_fk) " +
					"select "+ycxkCurrentId+", pk_seq from ERP_HOADON where pk_seq in ( " + this.ddhId + " )  ";
			...
			for(int i = 0; i < spId.length; i++)
			{
						...
						&& Double.parseDouble(spSoluong[i].trim().replaceAll(",", "")) > 0 )
				{
					query = "insert ERP_YCXUATKHO_SANPHAM( ycxk_fk, sanpham_fk, soluongDAT, tonkho, daxuat, soluongXUAT, LOAI, SCHEME ) " +
							"select "+ycxkCurrentId+", '" + spId[i] + "', '" + spSoluongDat[i].replaceAll(",", "") + "', '" + spTonkho[i].replaceAll(",", "") + "', '" + spDaxuat[i].replaceAll(",", "") + "', '" + spSoluong[i].replaceAll(",", "") + "', '" + spLoai[i] + "', N'" + spSCheme[i] + "' ";
							...
							if(key.startsWith( spId[i] + "__" + spLoai[i] + "__" + spSCheme[i].trim() ))
							{
								...
								if( !_soluongCT.equals("0") )
								{
									query = "insert ERP_YCXUATKHO_SANPHAM_CHITIET( ycxk_fk, SANPHAM_FK, solo, ngayhethan, ngaynhapkho, soluong, loai, scheme, kho_fk ) " +
											"select "+ycxkCurrentId+", pk_seq, N'" + _sp[3] + "', '" + _sp[4] + "', '" + _sp[5] + "', '" + _soluongCT.replaceAll(",", "") + "', '" + spLoai[i] + "', '" + spSCheme[i] + "', '" + this.khoNhanId + "' " +
	...
	public boolean update() 
	{
		...
		try
		{
			...
			if(!Utility.KiemTraNgayChungTuSoNet(db,this.userId,"1" ,"BanHang_SoChungTu",this.ngayyeucau, this.id,"ERP_YCXUATKHO" ))
			{
