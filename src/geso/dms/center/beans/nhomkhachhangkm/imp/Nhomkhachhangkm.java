...
public class Nhomkhachhangkm implements INhomkhachhangkm {
	...
	{ 
	 ...
	 System.out.println("select * from NHOMKHACHHANGNPP where pk_seq ='"+ Id +"'");
	  try 
	  {
		  ...
		  String query=
		  "	select b.PK_SEQ ,b.SMARTID as khMa,b.TEN,b.DIACHI,b.DIENTHOAI,c.MA as nppMa,c.TEN as nppTen " +
	...
	public boolean Save() 
	{
		...
		try
		{
			while(rsCheck.next())
			{
				...
				{
					this.Msg="Tên nhóm đã tồn tại,vui lòng nhập lại";
					return false;
				}
			}
		...
		try
		{
			...
			if(this.action.equals("upload"))
			{
				...
				query=
					 " select * from ("+this.khMaUpload+") up where not exists (select 1 from khachhang where mafast = up.makh )	";
				...
				query=
					...
					"  FROM KHACHHANG WHERE mafast IN ("+this.khMaUpload+") ";
	...
	public boolean Update()
	{
		...
		try
		{
			...
			System.out.print("[Edit]"+query);
			/*query="delete from CTKM_KHACHHANG where NHOMKHNPP_FK='"+this.Id+"' ";
			if(!this.db.update(query))
			{
				Utility.rollback_and_shutdown(db); 
				this.Msg="Lỗi hệ thống";
			...
			"  SELECT '"+this.Id+"' ,PK_SEQ "+  
			"  FROM KHACHHANG WHERE PK_SEQ IN ("+this.khId+") "+
			" 	and not exists ( select 1 from CTKM_KHACHHANG x where x.NHOMKHNPP_FK =  '"+this.Id+"' and x.KHACHHANG_FK = KHACHHANG.pk_SEQ)   ";
			if(this.action.equals("upload"))
				query=
					"	INSERT INTO CTKM_KHACHHANG(NHOMKHNPP_FK, KHACHHANG_FK) "+
					"  SELECT '"+this.Id+"' ,PK_SEQ "+  
	...
	{
		String	query="select PK_SEQ,MA,TEN  from NHAPHANPHOI npp where 1=1 and npp.pk_Seq<>1 ";
	...
	{
		String query=
		"select   ddkd.PK_SEQ,ddkd.TEN , ddkd.DIACHI , ddkd.MANHANVIEN  as Ma,npp.MA as nppMa,npp.TEN  as nppTen "+
