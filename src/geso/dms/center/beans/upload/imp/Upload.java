...
/***  PHẦN THÊM BẰNG CHƯƠNG TRÌNH   ***/	
	...
	{
		String query=
		"select PK_SEQ as nppId,MA as nppMa,mafast madms,Ten as nppTen from NHAPHANPHOI "+
		"where PK_SEQ "+  
		"in(  select NPP_FK from NHAPP_KHO )";
		...
		query=
		"select PK_SEQ as nppId,ma as nppMa,mafast madms,TEN as nppTen from NHAPHANPHOI "+ 
		"where PK_SEQ in (select npp_fk from NHAPP_KBH where KBH_FK=100025)";
		...
		System.out.println("initImportRs   "+query);
	...
	private String Delete_TonKhoDauKy (String nppId)
	{
	
		String query="select count(*) as sodong from donhang where npp_fk="+nppId+" AND TRANGTHAI!=2 ";
		System.out.println("\n lay don hang  : "+query);
		...
		try
		{
			...
			query="delete from tonkhothang where npp_fk='"+nppId+"'";
			System.out.println("queyr : "+query);
			if(!db.update(query))
			{
				db.getConnection().rollback();
			...
			query="delete from tonkhothang_chitiet where npp_fk='"+nppId+"'";
			if(!db.update(query))
			{
				db.getConnection().rollback();
				return "Lỗi hệ thống "+query;
	 ...
	 * Nhap Hang
	 * (non-Javadoc)
	 * @see geso.dms.center.beans.upload.IUpload#importTonKho()
	 */
	@Override
	public void importTonKho()
	{ 
		String query=" select distinct khoId from tonkho_tmp where khoid not in (select pk_seq from KHO where trangthai=1) ";
		...
		try
		{
			if (rs.next())
			{
				this.msg+=" Vui lòng kiểm tra lại file ,bạn đang để kiểu chữ ,hoặc bạn đang để rỗng (ô trống) ở cột thứ 7 s .\n Vui lòng sửa theo định dạng: \n  Kho chính :100000 \n Kho khuyến mãi :100001   \n";
				return;
			}
			if(rs!=null)rs.close();
		} catch (SQLException e)
		{
		 ...
		 query=
		" select nppMa,spMa,KhoId ,KENHBANHANG  "+
		"	from TonKho_Tmp "+
		"	group by nppMa,spMa,KhoId, KENHBANHANG   "+
		"	having COUNT(*)>1 ";
		...
		System.out.println("Ma nhapp: "+ this.nppMa);
		rs=this.db.get(query);
		try
		{
			while(rs.next())
		...
		"	select nppMa "+
		"	from TonKho_Tmp  "+
		"	where nppMa  not in ( select  mafast from NHAPHANPHOI) ";
...
/*		query=	
		"	select nppMa "+
		"	from TonKho_Tmp a inner join nhaphanphoi b on a.nppMa=b.ma  "+
		"	where b.pk_seq  in ( select  NPP_FK from DONHANG where trangthai not in (0,2) )" ;
		...
		try
		{
			while(rs.next())
			{
				this.msg+="Nhà phân phối đã có đơn hàng \n";
				return;
			}
			if(rs!=null)rs.close();
		} catch (SQLException e)
		...
		/*query=	
			"	select nppMa "+
			"	from TonKho_Tmp a inner join nhaphanphoi b on a.nppMa=b.ma  "+
			"	where b.pk_seq  in ( select  NPP_FK from NHAPHANG where trangthai=1)" ;
			...
			try
			{
				while(rs.next())
				{
					this.msg+="Nhà phân phối đã có Nhap Hang \n";
					return;
				}
				if(rs!=null)rs.close();
			} catch (SQLException e)
		...
		*/
		...
		String errr= this.Delete_TonKhoDauKy(this.nppId);
	...
	if(errr.length() >0)
		...
		String ngaythangnam = this.nam + "-"+ ( this.thang.trim().length() >1 ? this.thang : "0"+ this.thang ) +"-01";
		
		query=
		"	    select tk.khoId,npp.PK_SEQ as nppId,sp.PK_SEQ as spId,tk.TonDau*qc.soluong1/qc.soluong2+ isnull(tk.soluongle,0) as soluong,\n"+
		"		0 as booked,tk.KENHBANHANG, kbh.PK_SEQ as kenhId,sp.MA as spMa,sp.TEN as spTen \n"+
		"	    from \n" +
		"		TonKho_Tmp tk inner join SANPHAM sp on sp.MA=tk.spMa \n"+
		"		left join QUYCACH qc on qc.SANPHAM_FK=sp.PK_SEQ and qc.DVDL2_FK=100018 and qc.dvdl1_fk=sp.dvdl_fk \n" +
		"		inner join NHAPHANPHOI npp on npp.mafast=tk.nppMa \n"  +
		"       inner join KENHBANHANG kbh on tk.KENHBANHANG=kbh.DIENGIAI " ;

		System.out.println("[Query Insert TonKho 3 : ]"+query);
		rs=this.db.get(query);			
		try
		{
			db.getConnection().setAutoCommit(false);
			while(rs.next())
			{
				...
				" select '"+khoId+"','"+nppId+"','"+spId+"','"+soluong+"',0,"+soluong+",null,'"+kenhId+"' ";
				if(!db.update(query))
				{
					this.db.getConnection().rollback();
					this.msg+="Lỗi hệ thống "+query;
					System.out.println("[Error nhapp_kho 3: ]"+query);
					return;
				}					
				query=
				"insert into NHAPP_KHO_CHITIET(KHO_FK,NPP_FK,SANPHAM_FK,SOLUONG,BOOKED,AVAILABLE,GIAMUA,KBH_FK,SOLO,NGAYHETHAN,ngaynhapkho)"+
				" select '"+khoId+"','"+nppId+"','"+spId+"','"+soluong+"',0,"+soluong+",null,'"+kenhId+"','NA','2030-12-31',convert( varchar(10), DATEADD(month,1,'"+ngaythangnam+"'),120) ";
				if(!db.update(query))
				{
					this.db.getConnection().rollback();
					this.msg+="Lỗi hệ thống "+query;
					
					System.out.println("[Error NHAPP_KHO_CHITIET 3:]"+query);
					return;
				}
				//-----  TỒN KHO THÁNG -----//		
				
				System.out.println(" tháng :" + this.thang + " &  năm: " + this.nam);
				 
				query= " insert into TONKHOTHANG(KHO_FK, NPP_FK,KBH_FK,SANPHAM_FK,THANG,NAM,SOLUONG,booked,available)" +
			    "select '"+khoId+"','"+nppId+"','"+kenhId+"','"+spId+"','"+this.thang+"' ,'"+this.nam+"', '"+soluong+"' ,0, '"+soluong+"'  ";
			
				if(!db.update(query))
				{
					this.db.getConnection().rollback();
					this.msg+="Lỗi hệ thống "+query;
					System.out.println("[Error TON KHO THANG]"+query);
					return;
				}
				
			/*	query=
				"INSERT INTO TONKHONGAY_CHITIET(KHO_FK,NPP_FK,SANPHAM_FK,NGAY,SOLUONG,KBH_FK,SOLO,NGAYSANXUAT)"+
				" select '"+khoId+"','"+nppId+"','"+spId+"','"+this.Ngaykhoaso+"','"+soluong+"','"+kenhId+"','DMSVIFON201308','2014-12-01' ";
				*/
				//-----  TỒN KHO THÁNG - CHI TIẾT -----//	
				
				
				
				query ="insert into TONKHOTHANG_CHITIET(KHO_FK, NPP_FK,KBH_FK,SANPHAM_FK,THANG,NAM,SOLUONG,booked,available,SOLO, NGAYHETHAN,ngaynhapkho)"+
						"select '"+khoId+"','"+nppId+"','"+kenhId+"','"+spId+"','"+thang+"' ,'"+nam+"', '"+soluong+"' ,0, '"+soluong+"', 'NA', '2030-12-31',convert( varchar(10), DATEADD(month,1,'"+ngaythangnam+"'),120)  ";
				if(!db.update(query))
				{
					this.db.getConnection().rollback();
					this.msg+="Lỗi hệ thống "+query;
					System.out.println("[Error TON KHO THANG_ chi tiet]"+query);
					return;
				}
			}
			...
			query= "insert into KHOASOTHANG(NPP_FK, THANGKSGANNHAT, THANGKS, NAM, NGAYTAO,NGUOITAO,ngaythangnam) "+
					"select '"+this.nppId+"','', '"+thang+"', '"+nam+"', '"+getDateTime()+"', '"+this.userId+"','"+ngaythangnam+"'  ";
			
			if(!db.update(query))
			{
				this.db.getConnection().rollback();
				this.msg+="Lỗi hệ thống "+query;
				System.out.println("[Error khoa so THANG]"+query);
				return;
			}
	...
	public void NewImport()
	{
		...
		query="select pk_Seq from nhaphanphoi where mafast='"+this.nppMa+"'";
		rs=this.db.get(query);
		try
		{
			while(rs.next())
		...
		query=
		"	    select tk.khoId,npp.PK_SEQ as nppId,sp.PK_SEQ as spId,tk.TonDau*qc.soluong1/qc.soluong2+ isnull(tk.soluongle,0) as soluong,\n"+
		"		0 as booked,  isnull(bg.GIABANLECHUAN,0) as giamua,tk.KENHBANHANG, kbh.PK_SEQ as kenhId ,sp.MA as spMa,sp.TEN as spTen \n"+
		"	    from \n" +
		"		TonKho_Tmp tk inner join SANPHAM sp on sp.MA=tk.spMa \n"+
		"		left join QUYCACH qc on qc.SANPHAM_FK=sp.PK_SEQ and qc.DVDL2_FK=100018 \n" +
		...
		System.out.println("[Query Insert TonKho 2: ]"+query);
		rs=this.db.get(query);			
		try
		{
			db.getConnection().setAutoCommit(false);
			while(rs.next())
			{
				...
				System.out.println("Chuyen kho temp vao kho nhapp: " + query);
				int sodong=db.updateReturnInt(query);
				System.out.println("ket qua 1 : " + sodong);
				if(sodong==0)
				{
					query=
					"insert into NHAPP_KHO(KHO_FK,NPP_FK,SANPHAM_FK,SOLUONG,BOOKED,AVAILABLE,GIAMUA,KBH_FK)"+
					...
					if(!db.update(query))
					{
						this.db.getConnection().rollback();
						this.msg+="Lỗi hệ thống "+query;
						System.out.println("[Error NHAPP_KHO 1 :]"+query);
						return;
					}
				}	
	...
	{	
		...
		try
		{
			...
			"		inner join NHAPHANPHOI npp on npp.ma=tk.nppMa \n"  +
			"		left join BANGGIABLC_SANPHAM bg on bg.SANPHAM_FK=sp.PK_SEQ ";
			System.out.println("[Query Insert TonKho 1: ]"+query);
			rs=this.db.get(query);			
			while(rs.next())
			{
				String khoId=rs.getString("khoId");
 ...
 */
	...
	public static void main(String[] arg)
	{
		...
		String query=" delete from DanhSach_CKQuy  ";
		db.update(query);
		
		 query=
		  "select distinct   KHACHHANG_FK,NPP_FK ,c.vung_Fk  "+
			"	from HOADON a  inner join nhaphanphoi b on b.pk_Seq=a.npp_fk "+
			"		inner join tinhthanh c on c.pk_Seq=b.tinhthanh_fk  "+
			"	where month( a.ngayxuatHD ) >= '10'  and month( a.ngayxuatHD ) <= '12'   "+
			"				and year( a.ngayxuatHD ) = '2014' and a.trangthai in ( 2, 4 ) and a.loaihoadon = '0'  and b.loaiNPP in (0,1) ";
		 
		 //query=" select 101807 as KHACHHANG_FK,100002 as npp_fk,100001 as vung_Fk  ";
		...
		try
    {
	    ...
	    while(rs.next())
	    {
      	...
      	}else  if(rs.getString("Vung_fk").equals("100003"))
      	{
	    	...
	    	query=
	    	"declare @khId numeric(18,0),@nppId numeric(18,0),@tienTV FLOAT,@tienBUTRU FLOAT,@tienDS float "+
	      "   set @khId='"+rs.getString("KHACHHANG_FK")+"'   \n "+      
	      "   set @nppId='"+rs.getString("NPP_FK")+"'    \n "+      
	      "      \n "+      
	      "   SET @tienTV=   \n "+      
	      "   (    \n "+      
	      "   	select sum(TV.toTAL) as tongTHUVE     \n "+      
	      "   	from    \n "+      
	      "   	(    \n "+      
	      "   		select isnull( sum( tongtienAVAT ), 0)  as toTAL     \n "+      
	      "   		from HOADON a     \n "+      
	      "   		where month( a.ngayxuatHD ) >= '10'  and month( a.ngayxuatHD ) <= '12'    \n "+      
	      "   		and year( a.ngayxuatHD ) = '2014' and a.trangthai in ( 2, 4 ) and a.loaihoadon = '0'      \n "+      
	      "   		and a.khachhang_fk = @khId     \n "+      
	      "   		and a.khachhang_fk in (    \n "+      
	      "   		select khachhang_fk from DUYETBANDUNGGIA db    \n "+      
	      "   		inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk     \n "+      
	      "   		where khachhang_fk = @khId and trangthai = '1' and loaict = '0'    \n "+      
	      "   		and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId	)     \n "+      
	      "   			and cast( month(a.ngayxuatHD) as varchar(10) ) in      \n "+      
	      "   		(    \n "+      
	      "   			select thang from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk     \n "+      
	      "   			where khachhang_fk = @khId and trangthai = '1' and loaict = '0' and cast(thang as int) >= '10'    \n "+      
	      "   			and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId   \n "+      
	      "   		)     \n "+      
	      "   		union ALL    \n "+      
	      "   		select isnull( sum( tongtienAVAT ), 0)  as toTAL     \n "+      
	      "   		from HOADON a       \n "+      
	      ...
	      "   		(    \n "+      
	      "   			select thang from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk     \n "+      
	      "   			where khachhang_fk = @khId and trangthai = '1'   \n "+      
	      "   			and loaict = '0' and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId	     \n "+      
	      "   		)     \n "+      
	      "   		and cast( month(a.ngayxuatHD) as varchar(10) ) in      \n "+      
	      "   		(    \n "+      
	      "   			select thang from DUYETBANDUNGGIA db      \n "+      
	      "   			where trangthai = '1' and loaict = '0' and cast(thang as int) >= '10'   \n "+      
	      "   			and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId and  khongtinhtienthuve = '0'    \n "+      
	      "   		)    \n "+      
	      "   	)    \n "+      
	      "   	TV   \n "+      
	      "   )    \n "+      
	      "   SET @tienBUTRU=   \n "+      
	      "   (   \n "+      
	      "   select sum(TV.toTAL) as tongBUTRU     \n "+      
	      "   from    \n "+      
	      "   (    \n "+      
	      "   	select SUM(thanhtoan) as toTAL from DUYETTRAKHUYENMAI_DONHANG a    \n "+      
	      "   		inner join DonHang b on b.pk_seq=a.DonHang_fk     \n "+      
	      "   	where tichluyQUY=0 and a.thanhtoan>0 and b.khachhang_fk=@khId   \n "+      
	      "   		and b.trangthai!=2 and b.NgayNhap >='2015-01-01' and b.NgayNhap<='2015-01-31'       \n "+      
	      "   	union all 		   \n "+      
	      "   	select isnull( sum( SOTIENCANTRU ), 0)  as toTAL     \n "+      
	      "   	from	HOADON a  inner join CANTRUCONGNO_HOADON b on a.PK_SEQ=b.HOADON_FK     \n "+      
	      "   		inner join CANTRUCONGNO c on c.PK_SEQ=b.CANTRUCONGNO_FK    \n "+      
	      "   	where month( a.ngayxuatHD ) >= '10'  and month( a.ngayxuatHD ) <= '12' and year( a.ngayxuatHD ) = '2014' and a.trangthai in ( 2, 4 )   \n "+      
	      "   		and a.loaihoadon = '0'      \n "+      
	      "   		and a.khachhang_fk = @khId  and c.TRANGTHAI=1     \n "+      
	      "   		and a.khachhang_fk in (    \n "+      
	      "   	select khachhang_fk from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk     \n "+      
	      "   	where khachhang_fk = @khId and trangthai = '1' and loaict = '0' and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and	db.NPP_FK = @nppId	     \n "+      
	      "   )     \n "+      
	      "   	and cast( month(a.ngayxuatHD) as varchar(10) ) in      \n "+      
	      "   	( select thang from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk     \n "+      
	      "   	where khachhang_fk = @khId and trangthai = '1' and loaict = '0' and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId)     \n "+      
	      ...
	      "   	from HOADON a  inner join CANTRUCONGNO_HOADON b on a.PK_SEQ=b.HOADON_FK     \n "+      
	      "   	inner join CANTRUCONGNO c on c.PK_SEQ=b.CANTRUCONGNO_FK     \n "+      
	      "   	where month( a.ngayxuatHD ) >= '10'  and month( a.ngayxuatHD ) <= '12' and year( a.ngayxuatHD ) = '2014' and a.trangthai in ( 2, 4 ) and a.loaihoadon = '0'      \n "+      
	      "   	and a.khachhang_fk = @khId   and c.TRANGTHAI=1     \n "+      
	      "   	and cast( month(a.ngayxuatHD) as varchar(10) ) not in      \n "+      
	      "   	(    \n "+      
	      "   		select thang from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk     \n "+      
	      ...
	      "      \n "+      
					"set @tienDS=   \n "+      
					"   (    \n "+      
					"   	select sum(DS.toTAL) as tongDOANHSO     \n "+      
					"   	from    \n "+      
					"   	(     \n "+      
					"   		select isnull( cast( sum( ( cast( ( soluong * dongia ) as numeric(18, 0) )  ) * ( 1 + b.VAT / 100 ) ) as numeric(18, 0) ), 0)  as toTAL     \n "+      
					...
					"   			and cast( month(a.ngayxuatHD) as varchar(10) ) in      \n "+      
					"   					( select thang from DUYETBANDUNGGIA db      \n "+      
					"   					  where trangthai = '1' and loaict = '0' and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId and  khongtinhdoanhso = '0' )    \n "+      
					"   	)    \n "+      
					"   	DS )   \n "+
				"	select c.ten as nppTEN,b.mafast,b.ten as khTEN,a.TienCK,a.DaTra,a.ConLai  "+  
	      "   from   \n "+      
	      "   (   \n "+      
	      "   	select TL.pk_seq, TL.scheme, TL.NSP_FK, TL.khId,    \n "+      
	      "   		TL.thanhtoan  as TienCK,   \n "+      
	      "   	isnull(( select SUM(round( thanhtoan,0  ))       \n "+      
	      "   	from DUYETTRAKHUYENMAI_DONHANG where khachhang_fk = @khId    \n "+      
	      "   			and duyetkm_fk = TL.pk_seq and DIENGIAI = TL.maNHOM and donhang_fk in    \n "+      
	      "   		( select pk_seq from DONHANG where month( ngaynhap ) >= '1' and month( ngaynhap ) <= '3'   \n "+      
	      "   		and trangthai != '2' and khachhang_fk = @khId ) ), 0) as DaTra,   \n "+      
	      "   		TL.thanhtoan - isnull(( select SUM( thanhtoan  )    \n "+      
	      "   	from DUYETTRAKHUYENMAI_DONHANG where khachhang_fk = @khId    \n "+      
	      "   			and duyetkm_fk = TL.pk_seq and DIENGIAI = TL.maNHOM and donhang_fk in    \n "+      
	      "   		( select pk_seq from DONHANG where month( ngaynhap ) >= '1' and month( ngaynhap ) <= '3'   \n "+      
	      "   		and trangthai != '2' and khachhang_fk = @khId ) ), 0) as conLAI    \n "+      
	      "   	from    \n "+      
	      "   	(    \n "+      
	      "   		select tct.pk_seq, tct.scheme, NSP_FK, maNHOM, @khId as khId,  tc.chietkhau,     \n "+      
	      "   			sum(  ( (ISNULL(@tienTV,0)-ISNULL(@tienBUTRU,0)) * tc.chietkhau / 100 ) )  as thanhtoan      \n "+      
	      "   	from     \n "+      
	      "   	(     \n "+      
	      "   		select NSP_FK, maNHOM, sum(DS.toTAL) as tongGiatri,    \n "+      
	      "   		round ( cast ( sum(DS.toTAL) * 100 / @tienDS  as numeric(18, 1) ) - 0.5 + 0.05, 0 )  as phantram        \n "+      
	      "   		from     \n "+      
	      "   		(     \n "+      
	      "   			select  c.NSP_FK, case c.NSP_FK when 100002 then 'CQX5' else 'CQB5' end as maNHOM,     \n "+      
	      "   				isnull( cast( sum( ( cast( ( soluong * dongia ) as numeric(18, 0) )  ) * ( 1 + b.VAT / 100 ) ) as numeric(18, 0) ), 0)  as toTAL      \n "+      
	      "   			from HOADON a inner join HOADON_SP b on a.pk_seq = b.hoadon_fk       \n "+      
	      "   				inner join "+tableNAME+"_SANPHAM c on b.sanpham_fk = c.SP_FK      \n "+      
	      "   			where month( a.ngayxuatHD ) >= '10'  and month( a.ngayxuatHD ) <= '12' and year( a.ngayxuatHD ) = '2014'    \n "+      
	      "   				and a.trangthai in ( 2, 4 ) and a.loaihoadon = '0'       \n "+      
	      "   				and a.khachhang_fk = @khId  and a.khachhang_fk in (  							   \n "+      
	      "   			select khachhang_fk    \n "+      
	      "   			from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk   									   \n "+      
	      "   			where khachhang_fk = @khId and trangthai = '1' and loaict = '0'    \n "+      
	      "   			and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId 	   							  )   			and cast( month(a.ngayxuatHD) as varchar(10) ) in    				   \n "+      
	      "   			( select thang from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk   				     \n "+      
	      "      \n "+      
	      "   			where khachhang_fk =	@khId and trangthai = '1' and loaict = '0'    \n "+      
	      "   				and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId )  	   \n "+      
	      ...
	      "   			and cast( month(a.ngayxuatHD) as varchar(10) ) not in  (  		   \n "+      
	      "   			select thang from DUYETBANDUNGGIA db inner join DUYETBANDUNGGIA_KHACHHANG db_kh on db.pk_seq = db_kh.duyet_fk   									" +
	      "						where khachhang_fk = @khId and trangthai = '1' and loaict = '0' and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId 	  )   and cast( month(a.ngayxuatHD) as varchar(10) ) in    	   \n "+      
	      "   			( select thang from DUYETBANDUNGGIA db  where trangthai = '1' and loaict = '0' and cast(thang as int) >= '10' and cast(thang as int) <= '12' and nam = '2014' and db.NPP_FK = @nppId and  khongtinhdoanhso = '0' )  	   \n "+      
	      "   			group by c.NSP_FK )  DS where NSP_FK in ( 100002, 100003  )    \n "+      
	      "   			group by NSP_FK, maNHOM 	)  	PT_NHOM inner join TIEUCHITHUONGTL_TIEUCHI tc on PT_NHOM.NSP_FK = tc.nhomsanpham_fk  		   \n "+      
	      "   			and tc.tumuc <= PT_NHOM.phantram and PT_NHOM.phantram < tc.denmuc  			   \n "+      
	      "   			inner join TIEUCHITHUONGTL tct on tc.thuongtl_fk = tct.pk_seq  	   \n "+      
	      "   			where tct.nam = '2014' and tct.trangthai = '1' and tct.loaict = '1'  			   \n "+      
	      "   			 and tct.pk_seq in ( select thuongtl_fk from TIEUCHITHUONGTL_LOAINPP where loainpp = ( select loaiNPP from NHAPHANPHOI where pk_seq = @nppId ) )   	group by tct.pk_seq, tct.scheme, NSP_FK, tc.chietkhau, tct.pk_seq, maNHOM  ) TL    \n "+      
	      "   		 where round(TL.thanhtoan,0) -  	isnull(( select SUM( round( thanhtoan,0)  )    \n "+      
	      "   	from DUYETTRAKHUYENMAI_DONHANG where khachhang_fk = @khId    \n "+      
	      "   			and duyetkm_fk = TL.pk_seq and DIENGIAI = TL.maNHOM and donhang_fk in    \n "+      
	      "   		( select pk_seq from DONHANG where month( ngaynhap ) >= '1' and month( ngaynhap ) <= '3'   \n "+      
	      "   		and trangthai != '2' and khachhang_fk = @khId ) ), 0) > 0    \n "+      
	      "   )as  a inner join khachhang b on b.pk_seq=a.khId   \n "+      
	      "   inner join nhaphanphoi c on c.pk_Seq=b.npp_fk ";
	    	...
	    	while(kq.next())
	    	{
	    		query=	"	insert into DanhSach_CKQuy(nppTen,maFast,khTen,tienckQuy,TienCK_DaTra,TienCKQuy_ConLai)  " +
	    						" select N'"+kq.getString("nppTen")+"','"+kq.getString("mafast")+"',N'"+kq.getString("khTen")+"','"+kq.getDouble("TienCK")+"','"+kq.getDouble("DaTra")+"','"+kq.getDouble("ConLai")+"'		";
	    						if(!db.update(query))
	    						{
	    							System.out.println("____"+query);						
	    						}
