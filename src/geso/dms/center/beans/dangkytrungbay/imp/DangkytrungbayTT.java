...
public class DangkytrungbayTT implements IDangkytrungbayTT, Serializable
{
	...
	public void init() 
	{
		if( this.Id.length() > 0 )
		{
			...
			String sql ="\n select ddkd.DDKDTEN, ddkd.DDKDMA,a.khachhang_fk, k.PK_SEQ, k.SMARTID as MAFAST, k.TEN, isnull(SOXUAT,0) as SOXUAT, isnull(DANGKY,0) as DANGKY, isnull(SUATDUYETDK, 0) as SUATDUYETDK, " +
						...
						"\n outer apply" +
						"\n (" +
						"\n		select top 1 ddkd.ten DDKDTEN , ddkd.manhanvien DDKDMA" +
	...
	public boolean save() 
	{
		try
		{
			...
			if( this.db.updateReturnInt(query)!=1 )
			{
			 ...
			 rs.next();
			 String idTmp = rs.getString("id");
	
				
			query = "insert dktrungbay_khachhang(dktrungbay_fk, khachhang_fk) " +
							"select "+idTmp+", pk_seq from KhachHang where pk_seq in (" + this.khId + ") " +
			...
			while(keyList.hasMoreElements())
			{
				...
				if(suatduyet.get(key) != null){
					query  += " update dktrungbay_khachhang set DANGKY ="+this.suatdk.get(key).toString()+", SUATDUYETDK = " + this.suatduyet.get(key).toString() +
							 " where dktrungbay_fk = "+idTmp+" and KHACHHANG_FK = " + key + " \n";
	...
	public boolean edit() 
	{
		try{
			...
			query = "select count(*) as sl from khachhang_anhchup where cttb_fk in (select cttrungbay_fk from dangkytrungbay where pk_seq="+this.Id+")";
			ResultSet rsCheckPhatSinh = this.db.get(query);
			int sl=0;
			System.out.println("____check phat sinh: "+query);
			if(rsCheckPhatSinh != null)
			{
				if(rsCheckPhatSinh.next())
				{
					sl=rsCheckPhatSinh.getInt("sl");
					if(sl >0)
					{
						// ko delete khách hàng của cttb đã phát sinh
					}
					else{
	...
	public boolean UnChot() 
	{
		...
		try{
			...
			String query = "";
			query="select count(*) as dem from DENGHITRATRUNGBAY where trangthai <> 2 and CTTRUNGBAY_FK in (select CTTRUNGBAY_FK from DANGKYTRUNGBAY where pk_seq='"+this.Id+"') ";
			...
			if(dem>0){
			    this.Msg="CTTB đã phát sinh duyệt trả, không thể mở chốt đăng ký! ";
			    geso.dms.center.util.Utility.rollback_throw_exception(db);
			    return false;
			}
			rs.close();
	...
	public boolean Delete() {
		...
		try{
			...
			String sql = " select COUNT(distinct a.KHACHHANG_FK) as kt from " 
					 + " \n DKTRUNGBAY_KHACHHANG a inner join DANGKYTRUNGBAY " 
					 + " \n b on a.DKTRUNGBAY_FK = b.PK_SEQ" 
					 + " \n inner join DENGHITRATRUNGBAY c" 
					 + " \n on b.CTTRUNGBAY_FK = c.CTTRUNGBAY_FK" 
					 + " \n inner join DENGHITRATB_KHACHHANG " 
					 + " \n d on c.PK_SEQ = d.DENGHITRATB_FK and d.KHACHHANG_FK=a.KHACHHANG_FK " 
					 + " \n where b.PK_SEQ = "+ this.Id +" ";	
			System.out.println("Check delete "+sql);
			...
			if(rs.next())
			{
				...
				if(sl > 0)
				{
					this.Msg="Trong đăng ký trưng bày đã có khách hàng tham gia duyệt trả trưng bày ! Không được phép xóa";
					return false;
				}
			}
			rs.close();
	...
	public void getId_Khachhang(String maKh, Hashtable<String, Integer> sosuatdk, Hashtable<String, Integer> sosuatduyet) {
		...
		try{
			...
			if(khcheck.length() > 0)
				this.Msg += "\nKhông có khách hàng nào được chọn. " + khcheck;
