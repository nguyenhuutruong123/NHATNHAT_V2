...
public class ThayDoiKhuyenMaiSvl extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		} else
		{
			...
			String nextJSP = request.getContextPath() + "/pages/Center/ThayDoiKhuyenMai.jsp";
	...
	private String Chot(String ddhId, String userId)
	{
		...
		try
		{
			...
			query=" UPDATE THAYDOIKM SET TRANGTHAI=1,DATEDUYET=GETDATE(),NGUOIDUYET='"+userId+"',NgayDuyet='"+getDateTime()+"' WHERE PK_SEQ='"+ddhId+"' AND TRANGTHAI=0 ";
			System.out.println("2.THAYDOIKM_CTKM"+query);
			if(db.updateReturnInt(query)!=1)
			{
				db.getConnection().rollback();
