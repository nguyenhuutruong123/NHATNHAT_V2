...
//		Utility util = new Utility();
		...
		else
		{
			...
			System.out.println("---LOAI DON HANG: " + loaidonhang+ " action : "+action);
			...
			else
			{
				...
				//GIU NGUYEN TIM KIEM		   		   
				String tungaytk = util.antiSQLInspection(request.getParameter("tungay"));
				System.out.println("tungay : "+tungaytk);
				if(tungaytk == null)
					tungaytk = "";	    	   	    
				obj.setTungay(tungaytk);

				String denngaytk = util.antiSQLInspection(request.getParameter("denngay"));

				if(denngaytk == null)
					denngaytk = "";	    	   	    
				obj.setDenngay(denngaytk);

				String vungtk = util.antiSQLInspection(request.getParameter("vungId"));
				if(vungtk == null)
					vungtk = "";	    	   	    
				obj.setVungId(vungtk);

				String khuvuctk = util.antiSQLInspection(request.getParameter("khuvucId"));
				if(khuvuctk == null)
					khuvuctk = "";	    	   	    
				obj.setKhuvucId(khuvuctk);

				String npptk = util.antiSQLInspection(request.getParameter("nppId"));
				System.out.println("nppId : "+npptk);
				if(npptk == null)
					npptk = "";	    	   	    
				obj.setNppTen(npptk);

				String kbhtk = util.antiSQLInspection(request.getParameter("kenhId"));
				if(kbhtk == null)
					kbhtk = "";	    	   	    
				obj.setKbhId(kbhtk);		    		   

				String trangthaitk = util.antiSQLInspection(request.getParameter("trangthai"));		    
				if(trangthaitk == null)
					trangthaitk = "";	    	   	    
			...
			System.out.println("GET LENH SQL :"+search);
			obj.init(search);		   		   
...
*/
	private String Chot(String lsxId,String userId) 
	{
		...
		try
		{
			...
			query = "select a.npp_fk, b.loainpp, b.nppc1, a.kbh_fk from erp_dondathang a  "
					+ "inner join nhaphanphoi b on a.npp_fk = b.pk_seq where a.pk_seq = " + lsxId;
			ResultSet	rs = db.get(query);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		else
		{
			...
			/*else if(action.equals("DMS_ChietKhauThanhToan"))
				{
					String msg = this.DMS_ChietKhauThanhToan();
					obj.setMsg(msg);
	...
	private String getSearchQuery(HttpServletRequest request, IErpDondathangList obj)
	{
		...
		String query = 
				...
				"   	) as DaXuatKho "
				+ " ,a.chietkhau  ckThanhToan, '' soso,isnull(a.sotienthu,0) as avat  "
				+ ",  (select ten from vung where pk_seq in (select vung_fk from khuvuc where pk_seq = c.khuvuc_fk)) as vungTen "+					
				" from ERP_Dondathang a left join KHOTT b on a.kho_fk = b.pk_seq "
				...
				+ "  and a.NPP_DACHOT = '1' and a.kbh_fk in "+util.quyen_kenh(obj.getUserId());
		...
		String kbhId = util.antiSQLInspection(request.getParameter("kenhId"));
		obj.setKbhId(kbhId);
