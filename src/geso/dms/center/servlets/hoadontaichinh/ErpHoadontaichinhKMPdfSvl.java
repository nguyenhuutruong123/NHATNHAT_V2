...
public class ErpHoadontaichinhKMPdfSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public ErpHoadontaichinhKMPdfSvl()
	{
		super();
	}
	...
	private void CreatePxk(Document document, ServletOutputStream outstream, IErpHoadontaichinh pxkBean) throws IOException
	{
		try
		{
			...
			/// ===================ham in
			/*form_hoadon1(document, outstream, pxkBean, 
					ctyTen, cty_MST, cty_Diachi, cty_Sotaikhoan, cty_Dienthoai, 
					cty_Fax, Donvi, kh_MST, hinhthucTT, ngayxuathd, nguoimuahang, kh_sotaikhoan,kh_nganhang, kh_Diachi, db);*/
				...
				if(spMa != null)
				{
					for(int i = 0; i < spMa.length; i++)
					{
						...
						if(spRs != null)
	            		{
	            			while(spRs.next())
	            			{
	            				solo=spRs.getString("SOLO")== null ?"" :spRs.getString("solo").trim();;
	            			}
	            		}
						System.out.println("\n so lo : "+solo +"\n");
