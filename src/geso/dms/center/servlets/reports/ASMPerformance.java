...
public class ASMPerformance extends HttpServlet
{
	private static final long serialVersionUID = 1L;
	public ASMPerformance()
	{
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nextJSP = request.getContextPath() + "/pages/Center/ASMPerformance.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (action.equals("Taomoi"))
		{
			try
			{
				...
				response.setHeader("Content-Disposition", "attachment; filename=ThucHienChiTieu_ASM_" + util.setTieuDe(obj) + ".xlsm");
	...
	private String setQuery(IStockintransit obj)
	{
		...
		String sql=
			...
			"    from  "+   
			"   (       "+   
			"   	select	ct.kenh_fk  as kbh_fk,ct.dvkd_fk, asm.pk_seq as  asm_fk , asm_kv.khuvuc_fk , sum(ctnpp.chitieu) as chitieu	     "+   
			"   	from chitieu ct						       "+   
			"   		inner join chitieu_asm ctnpp on ct.pk_seq = ctnpp.chitieu_fk        "+   
			"   		inner join asm  on asm.pk_seq = ctnpp.asm_fk      "+   
			"   		inner join asm_khuvuc asm_kv on asm_kv.asm_fk=ctnpp.asm_fk      "+   
			...
			"   	group by    ct.kenh_fk ,ct.dvkd_fk, asm.pk_seq, asm_kv.khuvuc_fk      "+   
			"   ) ctpri       "+   
			"   left join       "+   
			"   (       "+   
			"   	select nh.kbh_fk, sp2.dvkd_fk as dvkd_fk, asm.pk_seq as asm_fk ,      "+   
			"   		npp.khuvuc_fk as khuvuc_fk, sum(nh_sp.soluong * nh_sp.DONGIA ) as 'ds_pri'      "+   
			...
			"   		inner join asm_khuvuc asm_kv on asm_kv.khuvuc_fk = npp.khuvuc_fk       "+   
			"   		inner join asm asm on asm.pk_seq = asm_kv.asm_fk 	      "+   
			"   	where   "+   
			" 	 SUBSTRING( nh.NgayDonHang,1,7)   >= '"+obj.getFromYear()+"-"+obj.getFromMonth()+"'  "+   
			"   	and   SUBSTRING( nh.NgayDonHang,1,7)  <= '"+obj.getFromYear()+"-"+obj.getToMonth()+"'  and nh.trangthai in (2,4)       "+   
			"   	and  SUBSTRING( asm_kv.NGAYKETTHUC,1,7) >='"+obj.getFromYear()+"-"+obj.getToMonth()+"'       "+   
			"   	and nh.NgayDonHang>=asm_kv.ngaybatdau and nh.NgayDonHang <=asm_kv.ngayketthuc   and nh.LoaiDonHang=0     "+   
			"   	and npp.PK_SEQ not in   "+   
			"   	(  "+   
			"   		select NPP_FK from NHAPP_GIAMSATBH  "+   
			"   		where GSBH_FK in (select PK_SEQ from GIAMSATBANHANG where BM_FK is not null)  "+   
			"   	)      "+   
			"   	group by nh.kbh_fk, sp2.dvkd_fk,asm.pk_seq , npp.khuvuc_fk  "+   
			"   ) as dspri on dspri.dvkd_fk=ctpri.dvkd_fk    "+   
			"   and dspri.asm_fk=  ctpri.asm_fk and dspri.kbh_fk=ctpri.kbh_fk   "+   
			"   and ctpri.khuvuc_fk= dspri.khuvuc_fk        "+   
			"   left join DONVIKINHDOANH dvkd on dvkd.PK_SEQ=ctpri.DVKD_FK  "+   
			"   left join KENHBANHANG kbh on kbh.PK_SEQ=ctpri.kbh_fk  "+   
			"   left join khuvuc kv on kv.pk_seq=ctpri.khuvuc_fk    "+   
			"   left join asm  on asm.pk_seq=ctpri.asm_fk   "+   
			"   where 1=1  ";
		if (obj.getkenhId().length() > 0)
			query += " and ctpri.kbh_fk ='" + obj.getkenhId() + "'";
		if (obj.getvungId().length() > 0)
			query += " and kv.vung_fk = '" + obj.getvungId() + "'";
		if (obj.getdvkdId().length() > 0)
			query += " and ctpri.DVKD_FK = '" + obj.getdvkdId() + "'";
		if (obj.getkhuvucId().length() > 0)
			query += " and kv.pk_seq = '" + obj.getkhuvucId() + "'";
		if (obj.getASMId().length() > 0)
			query += " and asm.pk_seq = '" + obj.getASMId() + "'";
	...
	private void CreatePivotTable(OutputStream out, IStockintransit obj, String query) throws Exception
	{
		try
		{
			String chuoi = getServletContext().getInitParameter("path") + "\\ASM_ThucHienChiTieuTT.xlsm";
