...
public class DailyKHStockTTSvl extends HttpServlet 
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
		...
		String nextJSP = request.getContextPath() + "/pages/Center/rp_DailyKHStock_center.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		{	
			...
			response.setHeader("Content-Disposition", "Attachment; filename=TonKhoHangNgayKH_"+util.setTieuDe(obj)+".xlsm");

	        CreatePivotTable(out, response, request, obj, bfasle);
			}
			else
			{
				response.sendRedirect(request.getContextPath() + "/pages/Center/rp_DailyKHStock_center.jsp");
			}
	    }
	    catch (Exception ex)
	    {
	...
	private void CreateStaticHeader(Workbook workbook, String tungay, String denngay, String UserName,IStockintransit obj) 
	{
	    ...
	    cell.setValue("TỒN KHO HÀNG NGÀY CỦA KHÁCH HÀNG");   	
	    ...
	    cell = cells.getCell("EM1"); cell.setValue("SoLuongLe");	//13  
	    cell = cells.getCell("EN1"); cell.setValue("SoTien");
	    cell = cells.getCell("EO1"); cell.setValue("Date");
	    cell = cells.getCell("EP1"); cell.setValue("SanLuong(Kg)"); //16
	...
	private void CreateStaticData(Workbook workbook, IStockintransit obj, boolean bfasle) 
	{
	    ...
	    String sql = " select isnull(d.ten, 'Chua xac dinh') as Chanel, h.ten as Region," +
	    		" f.ten as Area, e.ten as Distributor, e.sitecode as Distcode, kh.ten as KhachHang, "+
		...
		if(rs != null)
		{
			...
			{// se do rong cho cac cot se dung
				...
				{ 
					...
					cell = cells.getCell("EM" + Integer.toString(i)); cell.setValue(Piece); //11
					cell = cells.getCell("EN" + Integer.toString(i)); cell.setValue(Amount); //12
					cell = cells.getCell("EO" + Integer.toString(i)); cell.setValue(Date);	//13
					cell = cells.getCell("EP" + Integer.toString(i)); cell.setValue(rs.getFloat("sanluong"));	//13
					i++;
				}
