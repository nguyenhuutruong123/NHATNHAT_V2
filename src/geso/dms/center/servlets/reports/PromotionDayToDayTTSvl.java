...
{	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String nextJSP = request.getContextPath() + "/pages/Center/PromotionDayToDayTT.jsp";
	...
	/*	obj.setYear(util.antiSQLInspection(request.getParameter("year")) != null ? 
		...
		obj.settungay(request.getParameter("tungay"));
		obj.setdenngay(request.getParameter("denngay"));
		...
		if (action.equals("Taomoi")) 
		{
			try 
			{
				if (action.equals("Taomoi"))
				{
					...
					response.setHeader("Content-Disposition", "attachment; filename=TraKhuyenMaiNgay"+util.setTieuDe(obj)+".xlsm");
	...
	private void CreateStaticHeader(Workbook workbook, IStockintransit obj)throws Exception 
	{
		try
		{
		    ...
		    cell.setValue("BÁO CÁO TRẢ KHUYẾN MÃI NGÀY");  getCellStyle(workbook,"A1",Color.RED,true,14);	  	
			...
			*/
			...
			cell = cells.getCell("AW1");		cell.setValue("mafast");
    ...
    " ISNULL(GSBH.MANHANVIEN,0) AS SALESSUPID,   ISNULL(GSBH.TEN, 'Khong xac dinh') AS SALESSUP, NPP.MA AS DISTRIBUTORID,  NPP.TEN AS DISTRIBUTOR,  "+
    " DDKD.MANHANVIEN AS SALEREPID, isnull(DDKD.TEN, 'Chưa khai báo') AS SALESREP,  CAST(KH.mafast as nvarchar(20)) as mafast,  CAST(KH.PK_SEQ as nvarchar(8)) as khId,KH.ten +'__' +isnull(kh.diachi,'') +'__' + isnull(kh.dienthoai,'') AS CUSTOMER, DH.NGAYNHAP "+ 
		...
		if (rs != null) 
		{
			try 
			{
				...
				while (rs.next())
				{
					...
					cell = cells.getCell("AW" + Integer.toString(i)); cell.setValue(rs.getString("mafast"));
					i++;
