...
public class XuatNhapTonKhachHangTTSvl extends HttpServlet 
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
		...
		String nextJSP = request.getContextPath() + "/pages/Center/XuatNhapTonKhachHang_center.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		{	
			...
			if(action.equals("taomoi"))
			{
				...
				response.setHeader("Content-Disposition", "Attachment; filename=XuatNhapTonKH" + util.setTieuDe(obj) + ".xlsm");
	
		        CreatePivotTable(out, response, request, obj, bfasle);
			}
			else
			{
				response.sendRedirect(request.getContextPath() + "/pages/Center/XuatNhapTonKhachHang_center.jsp");
			}
	    }
	    catch (Exception ex)
	    {
	...
	private void CreateStaticHeader(Workbook workbook, String tungay, String denngay, String UserName,IStockintransit obj ) 
	{
	    ...
	    cell = cells.getCell("EM1"); cell.setValue("Nhap(Le)");	//13
	    cell = cells.getCell("EN1"); cell.setValue("Nhap(Thung)");	//13
	    cell = cells.getCell("EO1"); cell.setValue("Ban(Le)");
	    cell = cells.getCell("EP1"); cell.setValue("Ban(Thung)");
	    cell = cells.getCell("EQ1"); cell.setValue("TonCuoi(Le)");
	    cell = cells.getCell("ER1"); cell.setValue("TonCuoi(Thung)");
	...
	private void CreateStaticData(Workbook workbook, IStockintransit obj, boolean bfasle) 
	{
	    ...
	    String sql =" select kbh.TEN as Chanel, dvkd.DIENGIAI as DVKD, vung.TEN as Vung, kv.TEN as KhuVuc, npp.TEN as Distributor, npp.ma as Distcode, kh.TEN as KhachHang, cl.TEN as ChungLoai, nh.TEN as NhanHang, sp.PK_SEQ as SKUcode, sp.TEN as SKU, kho.TEN as Warehouse, kh.pk_seq as OutletCode, kh.TEN as OutletName, soluong as nhap, SOLUONG-ton as ban, ton as toncuoi, isnull(qc.soluong1,1) as qc1,isnull(qc.soluong2,1) as qc2  from " +
				    " (" +
		...
		if(rs != null)
		{
			...
			{// se do rong cho cac cot se dung
				...
				while(rs.next())
				{ 
					double nhap = rs.getDouble("nhap");
					double ban = rs.getDouble("ban");
					double toncuoi = rs.getDouble("toncuoi");
					int qc1=rs.getInt("qc1");
					int qc2=rs.getInt("qc2");
					double nhapThung = nhap/qc1/qc2;
					double banThung = ban/qc1/qc2;
					double toncuoiThung = toncuoi/qc1/qc2;
					...
					cell = cells.getCell("EN" + Integer.toString(i)); cell.setValue(nhapThung); //12
					cell = cells.getCell("EO" + Integer.toString(i)); cell.setValue(Double.parseDouble(Ban)); //13
					cell = cells.getCell("EP" + Integer.toString(i)); cell.setValue(banThung); //14
					cell = cells.getCell("EQ" + Integer.toString(i)); cell.setValue(Double.parseDouble(TonCuoi));	//15
					cell = cells.getCell("ER" + Integer.toString(i)); cell.setValue(toncuoiThung);	//16
					i++;
				}
