...
public class ErpInphieuxuatchuyennoiboSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public ErpInphieuxuatchuyennoiboSvl()
	{
		super();
	}
	...
	private void form_phieuxuat(Document document,ServletOutputStream outstream,IErpChuyenkhoTT pxkBean, dbutils db)
	{
		...
		//tim ma so thue ben xuat dcl ho
		String qr_npp=" SELECT TEN, DIACHI, MASOTHUE, taikhoan, nganhang, DIENTHOAI, FAX FROM NHACUNGCAP WHERE PK_SEQ=100001 ";		
	...
	//lay kho nhan, lay tu donhang_fk cua e r p chuyen kho
		...
		try {
			...
			document.setMargins(1.0f*CONVERT, 1.5f*CONVERT, 0.0f*CONVERT, 0.0f*CONVERT); // L,R,T,B
			...
			cell = new PdfPCell();
			cell.setBorder(0);
			cell.setPaddingTop(4.9f*CONVERT);
			...
			cell_nho.setPaddingTop(-0.1f * CONVERT);
			tbl_veviec.addCell(cell_nho);

			cell_nho = new PdfPCell(); //ad ngay thang nam
			cell_nho.setBorder(0);
			cell_nho.setPaddingLeft(0.3f * CONVERT);
			cell_nho.setVerticalAlignment(Element.ALIGN_LEFT);
	...
	//== thong tin danh sach san pham
			...
			/*//lay gia tu gia ban le cua HO
				...
				float[] tbl_withd = { 6.0f, 20f ,73f , 30.0f,10f, 22f, 22.0f, 30.0f };

				PdfPTable tbl_sanpham = new PdfPTable(tbl_withd.length);
				//tbl_sanpham.setSpacingBefore(2.0f*CONVERT );
				tbl_sanpham.setSpacingBefore(2.5f*CONVERT );
				tbl_sanpham.setWidthPercentage(102);
				tbl_sanpham.setHorizontalAlignment(Element.ALIGN_RIGHT);
									...
									for (int j = 0; j < tbl_withd.length; j++)
									{
										...
										if(j==2)
										{
											...
											else
											{
												...
												{
													chuoicd = chuoi1.substring(0, 40);
													chuoi2 = chuoi2.substring(40 + 1,arr[2].length());
													cells = new PdfPCell(new Paragraph(chuoicd, new Font(bf, 10, Font.BOLD)));  
												}
									...
									if(arr[2].length() > 40)
									{
										...
										while(dodaichuoi > 0)
										{
											...
											if(boiso > 0)
											{
												...
												{
													chuoicd = chuoi1.substring(0, 40);
													chuoi2 = chuoi2.substring(40 + 1,dodaichuoi );
												}
											}
											else
											{
				...
				//	tbl_tongtien.setSpacingBefore(2.5f*CONVERT ); //add dong trong
					tbl_tongtien.setSpacingBefore(0.5f*CONVERT );
					...
					cells.setPaddingTop(3.2f*CONVERT);
					cells.setHorizontalAlignment(Element.ALIGN_CENTER);
					cells.setBorder(0);
					tbl_tongtien.addCell(cells);
