		...
		session.setAttribute("rs_dondathang", rs_dondathang);
         ...
         String sql_getdvkd="select pk_seq,donvikinhdoanh as ten from donvikinhdoanh where trangthai='1'";//lay nhung don dat hang da chot,va chua chuyen het hang sang kho khac
			...
			if(action.equals("update")){
				String 	nextJSP = request.getContextPath() + "/pages/Center/HoaDonGTGTUpdate.jsp";
				response.sendRedirect(nextJSP);
			}else if(action.equals("chot")){
				session.setAttribute("type", "0");
				String 	nextJSP = request.getContextPath() + "/pages/Center/HoaDonGTGTDisplay.jsp";
				response.sendRedirect(nextJSP);
				...
				obj.setTrangthai("0");
				  ...
				  String  nextJSP = request.getContextPath() + "/pages/Center/HoaDonGTGT.jsp";
				   response.sendRedirect(nextJSP);
			}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String action = request.getParameter("action");		
 		...
 		if(tenform.equals("newform")){
 			nextJSP = request.getContextPath() + "/pages/Center/HoaDonGTGTNew.jsp";
 		}else{
 		   nextJSP = request.getContextPath() + "/pages/Center/HoaDonGTGTUpdate.jsp";
 		}
		...
		hoadongtgt.setVat(util.antiSQLInspection(request.getParameter("VAT")));
         ...
         session.setAttribute("rs_khott", rs_khott);
			...
			if(action.equals("loadddh")){
			  ...
			  if(hoadongtgt.SaveHoaDonGTGT() ){
			     ...
			     nextJSP = request.getContextPath() + "/pages/Center/HoaDonGTGT.jsp";
			    	response.sendRedirect(nextJSP);
			  }else{
				  session.setAttribute("hoadon",hoadongtgt );
					session.setAttribute("obj",hdBean);
